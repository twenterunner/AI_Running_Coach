AI Running Coach v14.1.0 · build 40100
...
- Cache/version identifiers aligned at build 40100.
...
AI Running Coach v13.8.2 — authoritative shoe optimiser rebuild — build 30802
...
AI Running Coach v13.8.2 — purchase/first-use/retirement consistency — build 30802

- Purchase date and first use are now one authoritative lifecycle point for every future physical pair.
- A future shoe cannot enter the rotation, receive mileage, appear in Plan/Rehab assignments, or generate a graph event before it has been purchased.
- Pair-entry and handover events are regenerated from the final purchase plan after all optimisation and Race Day reallocations.
- A final post-Race-Day sanitation pass removes any assignment beyond a physical pair's retirement boundary and then rebuilds the complete graph/events.
- Retirement remains final: no later workout can be assigned to a retired physical pair.
- Purchase events are now shown as prominent full-height vertical lines on the shoe graph with BUY number and date.
- Numbered graph explanations use the same final pair-entry/handover/retirement dates as the purchase plan and graph.
- Adds validation for use-before-purchase.
- Existing training, prediction, recovery, rehabilitation progression, pathway, scoring and import calculations are unchanged.
- Schema remains 10400.

AI Running Coach v13.8.2 — retirement only at final use — build 30802

- A successor entering the rotation is now a handover, not a retirement.
- A physical pair is labelled retired only on its actual final positive-mileage assignment and only when it has reached its lifecycle boundary.
- Retirement events are rebuilt after training, rehab, handover and Race Day reallocations are complete.
- A shoe can therefore never be shown as retired and then appear in later Race Day-preparation or training assignments.
- Graph-event explanations distinguish Shoe handover from final Shoe lifecycle retirement.
- Validation flags any retirement marker that occurs before the pair's final assigned use.
- Existing training, prediction, recovery, rehabilitation progression, pathway, scoring and import calculations are unchanged.
- Schema remains 10400.

AI Running Coach v13.8.2 — retirement-clipped graph + compact rehab indication — build 30802

- Retired shoes are excluded from the shoe graph.
- Every projected physical-pair curve is hard-clipped at its projected retirement / final planned-use date; no horizontal continuation is drawn afterwards.
- The larger Active Rehab Forecast tile above the graph has been removed.
- Active rehabilitation remains visible directly on the graph as the shaded rehab period and in the graph key.
- Rehab mileage continues to feed physical-pair mileage, wear, retirement timing and purchase timing; only the redundant explanatory tile was removed.
- Graph validation now rejects any projected point after the physical pair's final allowed graph date.
- Existing training, prediction, recovery, rehabilitation progression, pathway, scoring and import calculations are unchanged.
- Schema remains 10400.

AI Running Coach v13.8.2 — visible rehab forecast + strict retirement boundary — build 30802

- Active rehabilitation is now visibly shown immediately above the shoe graph with forecast date range, estimated total shoe exposure and the physical pair(s) receiving that rehab mileage.
- The graph now renders a shaded 'Active rehab forecast' time band; v13.8.2 computed the band but did not insert it into the SVG.
- Rehab forecast mileage continues to feed physical-pair mileage, wear, retirement timing and replacement timing.
- Physical-pair curves now stop at the last positive-mileage assignment before/at the planned retirement boundary.
- Zero-distance assignments no longer create horizontal graph extensions.
- Any assignment occurring after a physical pair reaches its retirement boundary is moved to another suitable active pair or a successor before final graph generation.
- Validation now explicitly checks that no plotted point occurs after projected retirement.
- Existing training, prediction, recovery, rehabilitation progression, pathway, scoring and import calculations are unchanged.
- Schema remains 10400.

AI Running Coach v13.8.2 — unified shoe-plan assignments + rehab-visible lifecycle — build 30802

- Plan-tab shoe labels now come directly from the physical-pair lifecycle engine. They no longer run an independent generic shoe recommender that could show the same owned pair for every workout.
- Today uses the same lifecycle assignment as Plan.
- Future proposed physical pairs can be displayed on future Plan sessions before they are purchased; manual user shoe choices still override automatic planning.
- Rehab shoe recommendations now use the same lifecycle engine and retain explicit user-selected rehab shoes.
- The lifecycle cache now includes active rehab/check-in state, so rehab changes invalidate and recalculate the shoe proposal automatically.
- Rehab forecast is projected through the active rehab model window rather than only the visible 3-back/today/4-ahead calendar.
- Expected rehab walking/running mileage is included in physical-pair mileage curves and appears as an 'Active rehab forecast' band on the shoe graph.
- Rehab mileage remains excluded from the 25% weekly running-volume share rule; that rule applies to the running programme.
- Two active non-race training pairs is now a continuity minimum target whenever future running exists. Three is allowed during handover and in specialist/performance use.
- If an existing future pair is scheduled too late and the rotation would otherwise drop to one pair, its purchase/first use is brought forward into a short real-use handover rather than creating an extended one-pair period.
- Validation now flags any future running week with fewer than two active non-race training pairs.
- Existing training-plan generation, rehabilitation progression, prediction, pathway, scoring and import calculations are unchanged.
- Schema remains 10400.

AI Running Coach v13.8.2 — expert shoe lifecycle continuity + event-cause fix — build 30802

- Rehab progress rail now uses the semantic recovery status colour: ahead/met green, on-track teal, behind/not-met red, unknown grey.
- Purchase-avoided/deferred shoes are no longer shown as graph explanation events or proposed purchases.
- Target rotation continuity is now explicit: when a non-race pair approaches its strategy-specific lifecycle limit and its retirement would leave fewer than two active training pairs, its successor is introduced during a short real-use handover before retirement.
- A replacement must be used immediately; it cannot be bought simply to sit idle.
- Retirement-driven slope changes are labelled Shoe lifecycle, not Training plan. Generic Training plan events are reserved for genuine sustained changes in the fixed running programme.
- Graph-event explanation cards now inherit the same physical-pair colour as their numbered graph markers.
- Balanced: target two active pairs; up to 100% model-specific planning life; short successor overlap; overall compromise of workout fit, protection, familiarity and purchase restraint.
- Performance: target two active training pairs plus dedicated Race Day specialist; up to 90% training-pair life; specialist quality-session role; race pair introduced only for real familiarisation.
- Protection / injury-conscious: target two active pairs; up to 80% life; strong cushioning/comfort/protection weighting; no automatic stability prescription; rehabilitation footwear uses active injury, comfort, familiarity and pronation context.
- Foot mechanics/pronation remains a contextual preference rather than a diagnosis or mandatory shoe category.
- Schema remains 10400.

AI Running Coach v13.8.2 — fixed three-mode shoe strategy engine — build 30802

- Shoes now exposes exactly three runner-facing strategies: Balanced, Performance, and Protection / injury-conscious.
- All modes target two active physical pairs without making two a hard limit.
- Balanced may use a suitable pair up to 100% of model-specific planning lifetime.
- Performance may use training pairs up to 90% of model-specific planning lifetime; Protection / injury-conscious uses up to 80%.
- New purchases are suppressed when the two available pairs can suitably cover the session; every purchase begins use immediately.
- Race Day mileage is capped at 250 km for every proposed Race Day pair.
- Legacy Minimum Pairs saved state migrates safely to Balanced.

AI Running Coach v13.8.2 — continuous active rotation — build 30802

- Fixes the loophole where the 25% weekly-share rule only applied to shoes already receiving mileage; a serviceable shoe could therefore disappear from the rotation at 0%.
- Every available, non-retired, non-Race-Day physical pair is now treated as active. In every week with running, each active non-race pair must receive at least 25% of that week's running volume.
- If a future shoe cannot sustain that meaningful weekly share while existing shoes remain serviceable, its purchase is deferred/removed rather than allowing it to sit unused.
- A future non-race purchase is blocked when it would create more active shoes than can mathematically satisfy the 25% rule.
- Existing serviceable pairs are actively rebalanced into weekly assignments rather than being abandoned early. This prevents a healthy pair such as the orange curve from going flat merely because another shoe scored better.
- Dedicated Race Day pairs remain exempt from the 25% weekly-share floor.
- Graph curves still end at final planned use/retirement and never extend beyond the retirement limit.
- Validation now catches a serviceable pair that stops being used while later running continues, including a true 0% weekly share.
- Existing training, prediction, recovery, injury progression, pathway, scoring and plan generation logic remains unchanged.
- Schema remains 10400.

AI Running Coach v13.8.2 — retirement-bounded graph + 25% active-rotation floor — build 30802

- A physical pair's graph curve now stops at its final planned use / projected retirement point. No horizontal extension is drawn after the pair leaves the rotation.
- No workout may be assigned after a pair reaches its model/condition/strategy-specific retirement mileage.
- Except for a dedicated Race Day pair, any physical pair used in a given week must carry at least 25% of that week's running volume.
- If a non-race pair would be below 25%, the engine first consolidates its work into another acceptable already-active pair.
- If consolidation is impossible, compatible work is redistributed toward that pair until the 25% weekly floor is reached, subject to workout fit and remaining service life.
- Future non-race pairs that become unused after this consolidation are removed from the purchase plan.
- Adds validation for post-retirement assignments and non-race weekly shares below 25%.
- Dedicated Race Day pairs are exempt from the 25% rule so familiarisation/race-specific usage is not artificially inflated.
- Existing training, prediction, recovery, injury progression, pathway, scoring and plan generation logic remains unchanged.
- Schema remains 10400.

AI Running Coach v13.8.2 — portfolio-first evidence-informed shoe engine — build 30802

- Minimum Pairs now treats purchase minimisation as a hard objective: acceptable remaining capacity in owned/earlier physical pairs is consumed before another training pair may be created.
- Separates ideal workout fit (ranking) from acceptable fit (purchase gating). A small score advantage can no longer trigger a purchase.
- Adds a portfolio compaction pass that removes a proposed future training purchase when its complete workload can be absorbed by physical pairs already in the rotation.
- Smart Rotation buys only for an uncovered role or genuine service-life capacity gap.
- Performance Focused may introduce a specialist only for a repeated meaningful quality block, not an isolated workout.
- Protection & Comfort retains conservative wear/cushioning priorities but still reuses suitable physical pairs before creating duplicates.
- Foot mechanics remains equipment context, not a diagnosis or automatic stability-shoe mandate.
- Race Day remains a separate hard constraint and must finish inside the selected pair's model-specific target mileage window.
- Existing training, prediction, recovery, injury progression, pathways, scoring and plan generation remain unchanged.
- Schema remains 10400.

AI Running Coach v13.8.2 — lifecycle consistency + Race Day hard target — build 30802

- Today rehab card now reflects today's saved check-in and changes to a completed/review state after the check-in is saved.
- Owned-shoe cards no longer show an empty "Buy next pair by" when no replacement is needed before Race Day.
- "Until handover" is incremental mileage still planned on that physical pair; projected handover total equals current mileage plus that incremental mileage.
- Future training purchases are explicitly linked to the physical pair they replace/complement.
- Replacement selection stays in the runner's current shoe family/current generation unless another current model is materially better for the actual programme role.
- Dedicated Race Day pairs are introduced only at the start of a real late-programme familiarisation block and are rejected if they would be bought and then barely used.
- The final Race Day proposal is a hard validity condition: the app displays a Race Day pair only when its final projected mileage is inside that pair's target window.
- Validation flags future pairs with insufficient meaningful use after purchase or a long idle gap.
- Existing training, prediction, recovery, injury progression, pathway, scoring and import logic remains unchanged.
- Schema remains 10400.

AI Running Coach v13.8.2 — shoe purchase/use + Race Day target integrity — build 30802

- Future physical pairs are now purchased on their first actual planned-use date; the graph can no longer show a new pair sitting unused for an extended period.
- Race Day candidates must be explicitly race-suitable profiles; ordinary recovery/daily trainers cannot silently become the Race Day pair merely because they score reasonably on comfort.
- Race Day planning first tunes an owned or already-required race-capable pair into its model/race-specific target mileage window by reallocating suitable existing workouts.
- A dedicated Race Day purchase is created only when no existing/planned race-capable pair can achieve the target window, and only when enough suitable existing workouts can provide the minimum familiarisation mileage.
- Race familiarisation uses workouts already in the fixed plan; it never creates extra training.
- Added validation for purchase-before-first-use and Race Day mileage outside the target window.
- Training, recovery, injury progression, pathway, scoring and import logic remain unchanged. Schema remains 10400.

AI Running Coach v13.8.2 — offline ASICS catalogue + rational physical-pair lifecycle planner — build 30802

- Adds a curated offline current-purchase ASICS catalogue snapshot covering neutral, guidance/support, maximum-cushion, daily, performance and race roles.
- Historical ASICS generations remain recognised for physical pairs already owned; future purchases are restricted to explicit current profiles.
- Current future-purchase profiles include NOVABLAST 6, SUPERBLAST 3, GEL-NIMBUS 28, GEL-CUMULUS 28, GEL-KAYANO 33, GT-2000 14, GT-1000 14, MEGABLAST, SONICBLAST, NOOSA TRI 16, METASPEED SKY TOKYO, METASPEED EDGE TOKYO and METASPEED RAY.
- Rebuilds the shoe planner around immutable fixed-programme demand: owned physical pairs → realistic remaining service capacity → workout allocation → only then necessary future purchases → backwards Race Day preparation.
- Purchase dates are calculated shortly before first meaningful use. Generated future pairs with no planned use are removed.
- Same-role purchases inside the selected strategy's minimum spacing window are consolidated when the earlier pair can rationally absorb the work.
- Foot mechanics / pronation is an equipment-selection preference, not a diagnosis or hard stability-shoe requirement.
- Service-life limits remain model/category ranges and are adjusted by shoe condition and selected approach rather than one universal retirement mileage.
- Race Day first considers owned and already-required future pairs; a dedicated race pair is created only when those cannot reach Race Day with suitable fit, familiarisation and wear.
- Graph mileage is rebuilt chronologically from physical-pair assignments only.
- Graph event markers are reduced to structural events and sustained material allocation shifts (about >=15 km/week and >=45% change across two-week averages); normal weekly variation is ignored.
- Future purchase cards explain the purchase reason, role, existing rotation relationship, first-use date and Race Day role.
- Rehab timeline remains 3 calendar days back + today + 4 days ahead; completed today stays anchored to today's saved prescription.
- Existing training-plan, prediction, recovery, injury progression, pathway, scoring and import logic is unchanged.
- Schema remains 10400.

AI Running Coach v13.8.2 — differentiated shoe strategies, current-model purchasing, slope explanations, 3-back/4-ahead rehab — build 30802

- Rehab timeline now shows three calendar days before today, today, and four days after today (8 cards total when the rehab plan spans the full window).
- Future shoe purchases no longer consider every historical ASICS generation. Owned historical models remain valid, but purchase proposals use the newest supported generation per family; a successfully loaded live ASICS portfolio overrides the offline snapshot.
- Smart rotation, Minimum-pairs rotation, Workout-specific performance and Protection & comfort first now materially change workout allocation, wear thresholds and future-pair creation.
- Minimum-pairs explicitly reuses one versatile future training pair where practical.
- Workout-specific performance explicitly separates quality/race-specific work from easy mileage and may introduce a dedicated quality pair.
- Protection & comfort first applies more conservative wear limits and stronger cushioning/protection preference.
- Significant week-to-week slope changes are numbered on the mileage graph and explained in foldouts as Training plan, Rehab plan or Shoe rotation changes.
- Existing training, prediction, recovery, injury progression, pathway and scoring logic remains untouched.
- Schema remains 10400.

AI Running Coach v13.8.2 — fresh physical-pair lifecycle planner + date-anchored rehab Today — build 30802

- Rehab Today is now anchored to the actual calendar date. After today's check-in is saved, the Today card keeps showing today's saved prescription and marks it checked-in/completed; adaptive changes apply only to future rehab dates.
- Replaces the prior shoe-purchase projection logic with a fresh physical-pair lifecycle planner. The running plan is immutable input.
- The new shoe planner works in this order: owned physical pairs and current mileage → service-life capacity → fixed future workouts → only then future purchase creation → Race Day reservation/familiarisation.
- Future physical pairs are created only when existing suitable capacity cannot cover the fixed programme; Smart rotation consolidates overlapping near-term purchases rather than independently spawning replacement pairs.
- Race Day is handled backwards from the target date. A planned pair can be reserved for Race Day; a separate race pair is created only when existing/planned pairs cannot reach the race in an appropriate mileage window.
- Graph curves are rebuilt chronologically from physical-pair assignments, preventing artificial mileage spikes or decreases.
- Purchase dates and first-use dates are generated from actual lifecycle need, and the graph displays them directly.
- User Foot mechanics / pronation is explicitly included in purchase-model ranking. Live ASICS models receive preference only after workout/race suitability and user context.
- No training-plan, injury progression, readiness, prediction, recovery, pathway or scoring calculations are changed.
- Schema remains 10400.

AI Running Coach v13.8.2 — graph-adjacent shoe strategy + verified live ASICS plan context — build 30802

- Moves Shoe Engine Approach inside the lifecycle chart, immediately above the plotted SVG graph.
- A successful live refresh now displays a green confirmation: live ASICS data confirmed, number of current models loaded, transport used, and the runner's saved pronation/foot-mechanics plus race distance.
- Live discovery now includes supportive ASICS families (GEL-KAYANO, GT-2000, GT-1000) as well as neutral/performance families; the runner's saved pronation is used as a modest equipment-fit preference, not as a diagnosis or hard biomechanical rule.
- If live access fails, the app clearly states that the local catalogue is active and does not claim live data was applied.
- Changing Shoe Engine Approach recalculates graph assignments and purchase planning immediately.
- No training, prediction, recovery, rehabilitation, pathway or scoring logic changed.
- Schema remains 10400.


- Corrects runtime and visible revision to v13.8.2 build 30802.
- Removes Shoes sections 03–06; Shoes now focuses on Active Rotation and the mileage/lifecycle graph.
- Keeps the runner-facing shoe approach selector immediately above the graph.
- Live refresh always exits its checking state; failed refreshes retain the local catalogue.
- Clears obsolete stored live-error text from earlier builds and limits automatic retries after failure.
- No training, prediction, recovery, rehabilitation, pathway, scoring or shoe-mileage calculations changed.
- Schema remains 10400.


- Fixes “No current ASICS running models could be validated” when official ASICS collection pages expose product names but omit product-detail link targets in browser/Reader output.
- Live discovery now validates current model names/generations directly from official ASICS family pages (NOVABLAST, SUPERBLAST, GEL-NIMBUS, GEL-CUMULUS, METASPEED) plus the fast-running collection.
- Direct product URLs are used for detailed manufacturer specifications when available; they are no longer required for a model to count as live-validated.
- Local structured profiles remain the recommendation-characteristic fallback for a live-validated model when the collection page does not expose all specifications.
- Manufacturer/live availability evidence and app-derived suitability remain separate.
- Cached/local catalogue remains available offline and when the live request is blocked.
- No training, prediction, recovery, rehabilitation, pathway, scoring or shoe-mileage calculations changed.
- Schema remains 10400.


- Fixes Settings treating Foot mechanics / pronation as if no setting changed. The field is now included in change detection.
- A shoe-context-only change no longer rebuilds the marathon plan. It saves the context, recalculates shoe recommendations/assignments, and explicitly leaves the training plan unchanged.
- The Settings preview distinguishes a training-plan rebuild from a Shoes-only context update.
- Catalogue parsing now supports Markdown links, relative ASICS links and raw HTML product links.
- The live button shows “Checking ASICS…” while working, caches successful data, preserves previous cached data on failure, and displays the failure reason rather than silently doing nothing.
- The live refresh now triggers shoe re-ranking after a successful catalogue update.
- No training, prediction, recovery, rehab, pathway, scoring or shoe-mileage source-of-truth calculations changed.
- Schema remains 10400.

AI Running Coach v13.8.2 — rehab check-in prescription consistency — build 30802

- Fixes rehabilitation check-ins showing an exercise set that can differ from the exercises scheduled on that day.
- Root cause: the check-in inferred “scheduled exercises” from the generic rehab day items, while the visible exercise cards are generated from guideExercises.
- The check-in now resolves the exact same rehab-calendar day object used by the seven-day schedule and uses that day’s guideExercises as the authoritative exercise list.
- Exercise name and dose are shown explicitly above the completion selector in the check-in.
- Historical completed check-ins retain their saved prescription snapshot for auditability; current/future days use the currently displayed rehab plan.
- The exact scheduled exercise list is stored with new check-ins as a snapshot.
- No rehabilitation progression, injury prediction, training, readiness, recovery, pathway, scoring or shoe-mileage calculations changed.
- Schema remains 10400.

AI Running Coach v13.8.2 — live ASICS catalogue + gait-aware shoe planning — build 30802

- Live ASICS NL catalogue refresh added to the shoe graph area. Direct ASICS fetch is attempted first; a Reader transport fallback is used where browser CORS blocks direct access. Results are cached; local profiles remain the offline fallback.
- Settings now includes Foot mechanics / pronation: Not sure, Neutral, Pronates / overpronates, Supinates / underpronates. This affects Shoes purchase/equipment ranking only.
- Future race-shoe purchase ranking now considers current live ASICS models, race distance, official support/goal information and the runner-entered foot-mechanics context.
- Removed the retirement paragraph requested by the user.
- Shoe-engine approaches now use materially different assignment priorities and participate in the assignment cache key, so the graph recalculates when the approach changes.
- Run-detail spacing rebuilt with unique V3 metric/evidence grids to eliminate legacy CSS collisions.
- No training, prediction, recovery, rehab, pathway, scoring or shoe-mileage source-of-truth calculations changed.
- Schema remains 10400.

AI Running Coach v13.8.2 — Shoes graph strategy control + Today visual lock — build 30802

- Moves the Shoe Engine Approach selector directly into the Mileage & Lifecycle graph card.
- Changing the approach recalculates future shoe assignments and redraws the shoe graph immediately, keeping the user at the graph section.
- Removes the Shoes-tab “What should you run in?” / Next Run tile. Today and Plan continue to show workout-specific shoe recommendations.
- Renumbers the remaining Shoes sections 01–06.
- Hard-locks Shoes Level-1 cards to the approved Today teal gradient/border system and nested cards to the consistent dark-teal Level-2 surface.
- No training, rehabilitation, prediction, recovery, pathway, scoring or shoe-mileage source-of-truth logic changed.
- Schema remains 10400.

AI Running Coach v13.8.2 — hard run-detail spacing + Today teal visual lock — build 30802

- Run-summary and pathway labels/values now contain explicit structural line breaks in the HTML, so narrow-screen text cannot collapse into strings such as DISTANCE11.00 km or RUN SIGNAL-0.04 even if CSS is temporarily stale.
- Logged-run detail Level-1 cards now use the exact approved Today teal gradient/border system.
- Nested metric/equipment/evidence/factor tiles use a consistent dark-teal Level-2 surface.
- Mobile pathway metrics stack cleanly when width is constrained.
- Service-worker same-origin assets are now network-first with offline cache fallback, preventing an older cached stylesheet/app script from being served ahead of a newly deployed build.
- No training, prediction, recovery, injury, pathway, scoring or shoe-mileage calculations changed.

AI Running Coach v13.8.2 — run duration input + log popup spacing — build 30802
...
AI Running Coach v13.8.2 — authoritative shoe reassignment for logged runs and rehab — build 30802
...
AI Running Coach v13.8.2 — permanent shoe deletion — build 30802
...
- Shoes with run, rehabilitation or future-plan dependencies can also be deleted after an explicit second confirmation.
- Permanent deletion removes shoe-usage and future-assignment links only; runs and rehabilitation check-ins remain intact and their shoe becomes unknown.
- Planned rehabilitation shoe references and future purchase references to the deleted pair are cleaned safely.
- Retirement remains the preferred non-destructive option when history should remain attached.
- No training, rehabilitation progression, prediction, readiness, recovery, pathway or scoring equations changed.
- Schema remains 10400.

AI Running Coach v13.8.2 — rehab shoe persistence + hard shoe lifecycle boundaries — build 30802
...
- Physical-pair replacement is now a hard lifecycle boundary. The planner no longer postpones a worn pair simply to consolidate purchases.
- Owned-shoe future graph curves are defensively truncated at the conservative replacement threshold; actual logged history is never capped or rewritten.
- Proposed replacement generations also respect realistic profile service-life limits.
- Workout rotation can still choose the best compatible shoe, but that optimisation may not extend a physical pair beyond its service-life planning boundary.
- No training, rehabilitation progression, prediction, readiness, recovery, pathway or scoring equations changed.
- Schema remains 10400.

AI Running Coach v13.8.2 — rehab shoe selection persistence fix — build 30802
...
AI Running Coach v13.8.2 — shoe lifecycle + rehab mileage integrity — build 30802
...
AI Running Coach v13.8.2 — physical-pair lifecycle rollover — build 30802
...
AI Running Coach v13.8.2 — cross-source rehab/run mileage deduplication — build 30802
...
AI Running Coach v13.8.2 — atomic rehab shoe usage — build 30802
...
AI Running Coach v13.8.2 — rehab shoe-mileage estimation fix — build 30802
...
AI Running Coach v13.8.2 — shoe planner render hotfix — build 30802

- Fixes the blank/unresponsive app introduced in v13.8.2.
...
- Portfolio-wide replacement optimisation from v13.8.2 remains enabled.
- No training, rehabilitation, prediction, readiness, recovery, pathway or scoring equations changed.
- Schema remains 10400.

AI Running Coach v13.8.2 — portfolio-optimised shoe purchasing — build 30802
...
AI Running Coach v13.8.2 — visible shoe-mileage confirmation — build 30802
...
AI Running Coach v13.8.2 — immediate shoe-mileage refresh after activity save — build 30802
...
AI Running Coach v13.8.2 — completed-day rehab freeze + future-only shoe projection — build 30802
...
AI Running Coach v13.8.2 — linked shoe-graph lifecycle events — build 30802
...
- VERSION 13.8.2 · BUILD 30802 · SCHEMA 10400.
- Repository remains flat.

AI Running Coach v13.8.2 — auditable shoe-curve drivers — build 30802
...
AI Running Coach v13.8.2 — startup render hotfix — build 30802

- Fixes the blank app shell introduced in v13.8.2.
...
- Changes the cache storage to startup-safe function-scope initialization while retaining the v13.8.2 performance optimizations.
- No training, prediction, pathway, recovery, injury, rehabilitation, scoring, import or plan-generation calculations changed.
- Schema remains 10400.
- Repository remains flat.

AI Running Coach v13.8.2 — Shoes performance regression fix — build 30802
...
AI Running Coach v13.8.2 — race-distance shoe choice + rehab shoe forecasting — build 30802
...
AI Running Coach v13.8.2 — proposed purchase catalogue + rehab-day shoe planning — build 30802
...
AI Running Coach v13.8.2 — shoe-plan synchronization and graph clarity — build 30802
...
AI Running Coach v13.8.2 — optimal race-day shoe lifecycle strategy — build 30802
...
AI Running Coach v13.8.2 — race-day shoe strategy lifecycle graph — build 30802
...
AI Running Coach v13.8.2 — shoe lifecycle chart race-date + target clarity — build 30802
...
AI Running Coach v13.8.2 — shoe plan visibility + race-day target + lifecycle chart clarity — build 30802
...
AI Running Coach v13.8.2 — Shoes programme mileage + lifecycle timeline — build 30802
...
AI Running Coach v13.8.2 — complete Shoes lifecycle and race-day planning module — build 30802
...
v13.8.2 build 30802 — shoe-plan clarity and evidence update
