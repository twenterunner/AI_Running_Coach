...
const BUILD=9910, SCHEMA=9910, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function diagnosisFamily(key=''){
 ...
 if(/strain|flexor|adductor|quad|sartorius|tfl|gastroc|soleus/.test(key))return'muscle';
 if(/tend|gtps|plantar/.test(key))return'tendon';
 if(/pfps|itbs|menisc|mcl|pes|patellar/.test(key))return'knee';
 if(/ankle_sprain|peroneal/.test(key))return'ankle';
 if(/bsi|sesamoid/.test(key))return'bone';
 if(/cecs|mortons/.test(key))return'neural';
 if(/foot|plantar|post_tib|navicular|met_|sesamoid|mortons/.test(key))return'foot';
 return'generic';
}
...
function explicitRedFlags(i,ranked=[]){const t=injuryText(i);let reasons=[];if(ranked.some(x=>x.urgent||diagnosisFamily(x.key)==='bone'))reasons.push('a higher-risk bone or tendon pattern is among the leading possibilities');if(/night pain|focal bone|point tender|unable to bear|cannot bear|marked weakness|deform|numb|breathless|calf swelling|fever/.test(t))reasons.push('a recorded symptom matches a clinical red flag');if(i.pop&&i.bruising&&nullableNumber(i.initialWalkPain)>=5)reasons.push('pop, bruising and major walking limitation were reported together');return[...new Set(reasons)]}
function workingDiagnosis(i){
 const clinician=String(i.clinicalDiagnosis||'').trim();
 if(clinician){const mapped=clinicianLibraryMatch(clinician),hasMap=!!mapped?.hits,days=nullableNumber(i.clinicianExpectedDays)??(hasMap?mapped.days:56);return{name:clinician,source:'Clinician-entered',strength:'Clinician-entered',evidence:['diagnosis entered from clinician information'],alternatives:['The app does not independently verify clinician information'],nominalDays:days,key:hasMap?mapped.key:'',family:hasMap?diagnosisFamily(mapped.key):'generic',ranked:[],missing:hasMap?[]:['expected recovery duration from the clinician'],urgent:!!mapped?.urgent,safetyReasons:explicitRedFlags(i,hasMap?[mapped]:[])};}
 const text=injuryText(i),initialPain=nullableNumber(i.initialPain),walk=nullableNumber(i.initialWalkPain);
 let ranked=RUNNER_INJURY_LIBRARY.map(d=>{const location=termHits(text,d.terms),mechanism=termHits(text,d.mechanisms),features=termHits(text,d.features);let score=location.length*5+mechanism.length*2.5+features.length*3;if(i.pop&&/strain|rupture|tear|sprain/.test(d.name.toLowerCase()))score+=3;if(i.bruising&&/strain|rupture|tear|sprain|stress injury/.test(d.name.toLowerCase()))score+=2;if(Number.isFinite(initialPain)&&initialPain>=7&&/strain|rupture|stress injury|sprain/.test(d.name.toLowerCase()))score+=1.5;if(Number.isFinite(walk)&&walk>=5&&/stress injury|rupture|strain|sprain/.test(d.name.toLowerCase()))score+=1;return{...d,score,location,mechanism,features};}).filter(x=>x.score>0).sort((a,b)=>b.score-a.score);
 if(!ranked.length){const region=(i.bodyRegion||i.location||'lower limb').trim();return{name:`Undifferentiated ${region.toLowerCase()} running-injury pattern`,source:'App pattern match',strength:'Limited',evidence:['insufficient differentiating symptom detail'],alternatives:['Muscle or tendon overload','Joint or bone stress injury','Referred or nerve-related pain'],nominalDays:56,key:'',family:'generic',ranked:[],missing:['exact pain location','gradual or sudden onset','activities that reproduce symptoms'],urgent:false,safetyReasons:explicitRedFlags(i,[])};}
...
function checkStatus(checks,test){const assessed=checks.filter(test.assessed),met=assessed.length?test.pass(assessed.at(-1)):null;return met===null?'unknown':met?'met':'notMet'}
function criterionState(i,p,stageIndex){const checks=sortedChecks(i),latest=checks.at(-1)||{},stable=checks.filter(c=>c.nextDayWorse===false&&c.newSwelling===false),runStable=stable.filter(c=>Number(c.runMinutes)>=10&&!c.alteredGait),tests={
 'Symptoms stable or improving':{assessed:c=>known(c.nextDayWorse),pass:()=>latest.nextDayWorse===false},
 'No new swelling or bruising':{assessed:c=>known(c.newSwelling),pass:()=>latest.newSwelling===false},
 'Walking pain ≤3/10':{assessed:c=>Number.isFinite(c.walkPain),pass:()=>p.walkPain<=3},
 'Walking pain ≤1/10':{assessed:c=>Number.isFinite(c.walkPain),pass:()=>p.walkPain<=1},
 'Current pain ≤3/10':{assessed:c=>Number.isFinite(c.pain),pass:()=>p.currentPain<=3},
 'Gentle double-leg loading tolerated':{assessed:c=>known(c.bridge),pass:()=>latest.bridge===true},
 'Current pain ≤2/10':{assessed:c=>Number.isFinite(c.pain),pass:()=>p.currentPain<=2},
 'Repeated bridge or hinge controlled':{assessed:c=>known(c.bridge),pass:()=>checks.filter(c=>c.bridge===true).length>=2},
 'No worse the next morning':{assessed:c=>known(c.nextDayWorse),pass:()=>stable.length>=2},
 'Walking pain 0/10':{assessed:c=>Number.isFinite(c.walkPain),pass:()=>p.walkPain===0},
 'Gentle hopping tolerated':{assessed:c=>known(c.hop),pass:()=>checks.filter(c=>c.hop===true).length>=2},
 'Pain remains ≤2/10':{assessed:c=>Number.isFinite(c.pain),pass:()=>checks.filter(c=>Number.isFinite(c.pain)&&c.pain<=2).slice(-2).length>=2},
 'No next-morning flare':{assessed:c=>known(c.nextDayWorse),pass:()=>stable.slice(-2).length>=2},
 'At least 10 minutes running tolerated':{assessed:c=>known(c.runMinutes),pass:()=>runStable.length>=2},
 'No altered gait':{assessed:c=>known(c.alteredGait),pass:()=>runStable.slice(-2).every(c=>c.alteredGait===false)&&runStable.length>=2},
 'At least 30 minutes easy running tolerated':{assessed:c=>known(c.runMinutes),pass:()=>stable.filter(c=>Number(c.runMinutes)>=30&&!c.alteredGait).length>=2},
 'Pain ≤1/10':{assessed:c=>Number.isFinite(c.pain),pass:()=>checks.filter(c=>Number.isFinite(c.pain)&&c.pain<=1).slice(-2).length>=2},
...
function injuryPrediction(i){
 ...
 const currentPain=nullableNumber(latest.pain)??nullableNumber(i.currentPain)??initialPain,walkPain=nullableNumber(latest.walkPain)??nullableNumber(i.currentWalkPain)??walkInitially,elapsed=Math.max(0,Math.floor((today()-dte(i.date))/DAY));
 let stage=0;for(let target=1;target<INJURY_STAGES.length;target++){const c=criterionState(i,{currentPain,walkPain},target);if(c.length&&c.every(x=>x.status==='met'))stage=target;else break;}if(diag.urgent)stage=Math.min(stage,1);const stageBase=[5,18,36,56,74,90][stage];let within=0;if(Number.isFinite(currentPain))within+=(3-currentPain)*2;if(Number.isFinite(walkPain))within+=(1-walkPain)*2;if(latest.bridge===true)within+=2;if(latest.hop===true)within+=2;if(Number(latest.runMinutes)>=10)within+=2;if(latest.nextDayWorse===true)within-=5;const completion=checks.length?Math.round(clamp(stageBase+Math.max(-6,Math.min(8,within)),0,98)):null,nominal=Math.round(clamp(elapsed/nominalTotal*100,0,100)),delta=completion===null?null:completion-nominal;let adjustment=0;if(checks.length>=3&&Number.isFinite(delta)){adjustment=Math.round(clamp(-delta*.18,-10,14));if(latest.nextDayWorse===true)adjustment+=4;}const total=Math.max(elapsed+7,nominalTotal+adjustment),remaining=Math.max(0,total-elapsed),central=new Date(today().getTime()+remaining*DAY),windowPad=checks.length>=7?7:checks.length>=3?12:18,windowStart=iso(new Date(central.getTime()-windowPad*DAY)),windowEnd=iso(new Date(central.getTime()+windowPad*DAY)),confidence=checks.length>=7?'Moderate':checks.length>=3?'Developing':'Low';return{nominalTotal,total,elapsed,remaining,stage,fullDate:iso(central),windowStart,windowEnd,confidence,currentPain,walkPain,completion,nominal,delta,latest,checks,diag,safetyHold:diag.urgent};
}
...
function comparisonFactors(i,p){let l=p.latest||{},f=[],add=(name,actual,nominal,status,reason)=>f.push({name,actual,nominal,status,reason}),unknown='unknown';add('Pain',valueText(p.currentPain),p.elapsed<14?'3–5/10':'0–3/10',!Number.isFinite(p.currentPain)?unknown:p.currentPain<=2?'ahead':p.currentPain>=5?'behind':'on',!Number.isFinite(p.currentPain)?'Pain has not been assessed.':p.currentPain<=2?'Pain is low enough for progressive loading.':p.currentPain>=5?'Pain remains higher than expected for this point.':'Pain is within the broad expected range.');add('Walking',Number.isFinite(p.walkPain)?`${p.walkPain}/10 pain`:'Not assessed',p.elapsed<10?'Some limitation':'Comfortable',!Number.isFinite(p.walkPain)?unknown:p.walkPain===0?'ahead':p.walkPain>=3?'behind':'on',!Number.isFinite(p.walkPain)?'Walking pain has not been assessed.':p.walkPain===0?'Normal walking has returned.':'Walking symptoms still limit progression.');add('Strength',l.bridge===true?'Controlled':l.bridge===false?'Not tolerated':'Not assessed',p.elapsed<21?'Beginning':'Controlled repeated loading',!known(l.bridge)?unknown:l.bridge?'ahead':'behind',!known(l.bridge)?'Strength tolerance has not been assessed.':l.bridge?'Controlled loading was reported.':'Controlled loading was not tolerated.');add('Impact',l.hop===true?'Tolerated':l.hop===false?'Not tolerated':'Not assessed',p.elapsed<28?'Usually not required':'Expected to be emerging',!known(l.hop)?unknown:l.hop?'ahead':p.elapsed>=28?'behind':'on',!known(l.hop)?'Impact tolerance has not been assessed.':l.hop?'Low-level impact was reported as tolerated.':'Impact was not tolerated.');add('Running',known(l.runMinutes)?`${Number(l.runMinutes)||0} min`:'Not assessed',p.elapsed<28?'Often 0 min':'Some easy exposure',!known(l.runMinutes)?unknown:Number(l.runMinutes)>=10?'ahead':p.elapsed>=35?'behind':'on',!known(l.runMinutes)?'Running tolerance has not been assessed.':Number(l.runMinutes)>=10?'Running exposure has started.':'Running tolerance remains a limiting factor.');add('Next morning',l.nextDayWorse===true?'Worse':l.nextDayWorse===false?'Stable':'Not assessed','Stable',!known(l.nextDayWorse)?unknown:l.nextDayWorse?'behind':'ahead',!known(l.nextDayWorse)?'Delayed response has not been assessed.':l.nextDayWorse?'The latest load caused a delayed flare.':'No delayed worsening was reported.');return f;}
...
function renderInjury(){
 const box=$('injuryList');if(!box)return;const injuries=state.injuries||[],add=$('addInjuryBtn');$('injuryIntro').innerHTML='';if(add)add.classList.toggle('hidden',injuries.length>0);if(!injuries.length){box.innerHTML='<article class="panel injuryEmpty"><h3>No active injury</h3><p class="muted">Record an injury to start criteria-based rehabilitation guidance.</p></article>';return;}
 box.innerHTML=injuries.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(i=>{let p=injuryPrediction(i),st=INJURY_STAGES[p.stage],diag=p.diag,next=Math.min(5,p.stage+1),criteria=criterionState(i,p,next),met=criteria.filter(x=>x.status==='met').length,unknown=criteria.filter(x=>x.status==='unknown').length,factors=comparisonFactors(i,p),exercises=exerciseList(i,p),milestones=milestoneStatus(p),blockers=criteria.filter(x=>x.status==='notMet'),needs=criteria.filter(x=>x.status==='unknown');const timelineText=p.safetyHold?'Progression paused pending assessment':`${fmtDate(p.windowStart)}–${fmtDate(p.windowEnd)}`;return`<article class="panel injuryCard"><div class="panelHead"><div><h3>${esc(i.location||'Injury')} · ${fmtDate(i.date)}</h3><p class="muted compact">${esc(i.mechanism||'Mechanism not entered')}</p></div><span class="status today">Stage ${p.stage+1}</span></div>
 <button data-injury-check="${i.id}" class="primary full injuryPrimaryAction">Complete today’s check-in</button>
 <div class="injuryKpiGrid"><div class="metric-card"><span>Current stage</span><strong class="viz-stat-value injuryStageValue">${esc(st.name)}</strong><small>${esc(st.goal)}</small></div><div class="metric-card"><span>Current pain</span><strong class="viz-stat-value">${valueText(p.currentPain)}</strong><small>Walking ${valueText(p.walkPain)}</small></div><div class="metric-card"><span>Estimated unrestricted-running window</span><strong class="viz-stat-value injuryDateValue">${timelineText}</strong><small>${p.safetyHold?'Do not use app progression':'Central estimate '+fmtDate(p.fullDate)+' · '+p.confidence.toLowerCase()+' confidence'}</small></div><div class="metric-card"><span>Evidence</span><strong class="viz-stat-value">${p.checks.length}</strong><small>daily check-ins</small></div></div>
 ${p.safetyHold?`<div class="note bad injurySafetyHold"><b>Rehabilitation progression paused</b><p>${diag.safetyReasons.map(esc).join(' · ')}. Seek appropriate clinical assessment before running, hopping or progressing loading.</p></div>`:''}
 <section class="injurySection injuryActionSection"><h4>Today’s focus</h4><div class="todayFocus"><strong>${p.safetyHold?'Protect the area and arrange assessment':p.stage<3?'Build the next functional criterion safely':p.stage<5?'Increase running tolerance without a flare':'Restore faster running and full training tolerance'}</strong><p>${p.safetyHold?'Do not use the exercise pathway to clear a suspected higher-risk pattern.':blockers.length?`Current blockers: ${blockers.map(x=>x.label.toLowerCase()).join(', ')}.`:needs.length?`Still to assess: ${needs.map(x=>x.label.toLowerCase()).join(', ')}.`:'All criteria are met; repeated stable evidence is already included before progression.'}</p></div>${exercises.map(x=>`<details class="exerciseDetail"><summary><span><b>${esc(x.name)}</b><small>${esc(x.dose)} · ${esc(x.purpose)}</small></span><em>Open guide</em></summary><div class="exerciseGuide"><div class="exerciseWhy"><b>Why today?</b><p>${esc(x.why)}</p></div><div><b>How to perform</b><ol>${x.steps.map(y=>`<li>${esc(y)}</li>`).join('')}</ol></div><div class="exerciseRules"><div><b>Pain rule</b><p>${p.safetyHold?'Do not test impact or progress loading until assessed.':'Keep pain at 0–2/10. Stop for sharp pain, altered movement, or symptoms that worsen later or next morning.'}</p></div><div><b>Progress when</b><p>${esc(x.progress)}</p></div></div></div></details>`).join('')}</section>
 <section class="injurySection"><h4>Recovery milestones</h4><div class="injuryMilestones">${milestones.map(m=>`<div class="milestone ${m.status}"><i>${m.status==='met'?'✓':m.status==='notMet'?'○':'?'}</i><span>${esc(m.name)}</span><b>${m.status==='met'?'Met':m.status==='notMet'?'Not met':'Not assessed'}</b></div>`).join('')}</div></section>
 <section class="injurySection"><div class="injurySectionHead"><div><h4>Next-stage requirements</h4><p class="muted compact">Stage ${next+1}: ${esc(INJURY_STAGES[next].name)} · repeated evidence is required</p></div><strong>${met}/${criteria.length} met${unknown?` · ${unknown} unknown`:''}</strong></div><div class="criteriaList">${criteria.map(c=>`<div class="criterion ${c.status}"><i>${c.status==='met'?'✓':c.status==='notMet'?'○':'?'}</i><span>${esc(c.label)}</span><b>${c.status==='met'?'Met':c.status==='notMet'?'Not met':'Not assessed'}</b></div>`).join('')}</div></section>
 <details class="injuryDisclosure"><summary>Working symptom pattern</summary><div class="diagnosisBox"><div><small>${esc(diag.source.toUpperCase())}</small><strong>${esc(diag.name)}</strong><span>${esc(diag.strength)} pattern match${diag.source==='Clinician-entered'?' · not independently verified':''}</span></div><p><b>Why:</b> ${diag.evidence.map(esc).join(' · ')||'limited symptom information'}</p>${diag.ranked?.length>1?`<div class="diagnosisRanking">${diag.ranked.map((d,n)=>`<div><i>${n+1}</i><span><b>${esc(d.name)}</b><small>${esc(d.region)}</small></span><strong>${esc(d.strength)}</strong></div>`).join('')}</div>`:''}${diag.missing?.length?`<div class="diagnosisMissing"><b>Information that would improve the match</b><p>${diag.missing.map(esc).join(' · ')}</p></div>`:''}<details><summary>Alternatives and diagnostic boundary</summary><p>${diag.alternatives.map(esc).join(' · ')}</p><p class="muted">This is a pattern-based working assumption, not a confirmed diagnosis. Examination or imaging may change it.</p></details></div></details>
 <details class="injuryDisclosure"><summary>Recovery rationale and comparison</summary><div class="timelineReason"><p>The ${esc(diag.name.toLowerCase())} pathway uses a nominal ${p.nominalTotal}-day central estimate.</p><p>You are ${p.elapsed} days after injury and currently in Stage ${p.stage+1}: ${esc(st.name)}.</p><p>Blank or unanswered fields are treated as not assessed and do not count toward progression.</p><p>The displayed date is a window because recovery cannot be predicted to one exact day.</p></div><div class="comparisonList">${factors.map(f=>`<div class="comparisonRow ${f.status}"><div><b>${esc(f.name)}</b><small>${esc(f.reason)}</small></div><span>${esc(f.actual)}</span><em>Nominal: ${esc(f.nominal)}</em></div>`).join('')}</div></details>
 <details class="injuryDisclosure"><summary>Detailed recovery progress</summary>${p.completion===null?'<p class="muted">Add daily check-ins to build an observed trajectory. No recovery score is inferred from calendar time alone.</p>':injuryTrajectorySvg(i,p)}</details>
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.9.1 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.9.1 stable build 9910');
})();