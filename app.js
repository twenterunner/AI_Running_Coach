...
function dailyCoachFocus(p){
 ...
 const paceState=pace.weekChange>.0005?'Improving':pace.weekChange<-.0005?'More conservative':'Stable';
 const loadState=load.weekChange>.0005?'Improving':load.weekChange<-.0005?'More conservative':'Stable';
 const readyState=ast.readiness==='Normal'?'Normal':ast.readiness==='Reduced'?'Reduced':'Restricted';
 cards.push(`<section class="todaySupportCard progressSupport"><div class="todaySupportHead"><span>${coachVisualIcon('trend')}</span><div><small>PROGRESS SNAPSHOT</small><h4>Where you stand</h4></div></div><div class="todayMiniMetrics qualitative"><div><small>Week distance</small><strong>${wd.actual.toFixed(1)}<em> / ${wd.planned.toFixed(1)} km</em></strong><span>${completion===null?'No volume target':`${completion}% complete`}</span></div><div><small>Readiness</small><strong>${esc(readyState)}</strong><span>Temporary recovery state</span></div><div><small>Fitness calibration</small><strong>${esc(paceState)}</strong><span>Pace & power trend</span></div><div><small>Load tolerance</small><strong>${esc(loadState)}</strong><span>Distance & load trend</span></div></div><button type="button" class="todayInlineLink" data-go="dashboard" data-anchor="progressAdaptationHome">View adaptation in Progress</button></section>`);
...
function weeklyReviewHtml(w=currentWeek()){
 ...
 <div class="weeklyReviewReasons"><b>Why</b>${r.reasons.length?r.reasons.map(x=>`<p>${esc(x)}</p>`).join(''):'<p>Evidence is still accumulating for this week.</p>'}</div>
