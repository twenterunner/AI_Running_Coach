...
function shoeGraphRationaleHtml(series,events,raceLifecycle){const rows=series.map(item=>{const current=shoeMileage(item.shoe),planned=item.planned.at(-1)?.km??current,added=Math.max(0,planned-current),event=events.find(e=>e.shoe.id===item.shoe.id),f=shoeForecast(item.shoe);return`<li><b>${esc(shoeDisplayName(item.shoe))}:</b> ${Math.round(current)} km accumulated today. The current rotation assigns ~${Math.round(added)} km to this exact pair before ${event?'its planned replacement':'race day'}.${event?` It is replaced on ${esc(shoeChartDate(event.date))} because the assigned programme first takes it into the expected ${Math.round(f.low)}–${Math.round(f.high)} km replacement range.`:' It does not reach its planned replacement threshold before race day.'}</li>`}).join('');const race=raceLifecycle?.preferred&&raceLifecycle.condition?`<li><b>Race-day shoe:</b> ${esc(shoeDisplayName(raceLifecycle.preferred))}. Desired race-day mileage is ${Math.round(raceLifecycle.condition.opt.minKm)}–${Math.round(raceLifecycle.condition.opt.maxKm)} km; the race planner projects ~${Math.round(raceLifecycle.condition.projected)} km on ${esc(shoeChartDate(state.setup?.raceDate))}.</li>`:raceLifecycle?.purchaseNeeded&&raceLifecycle.profile?`<li><b>Race-day shoe:</b> proposed ASICS ${esc(raceLifecycle.profile.family)} ${esc(raceLifecycle.profile.version)}. Desired race-day mileage is ${Math.round(raceLifecycle.targetWindow?.minKm||0)}–${Math.round(raceLifecycle.targetWindow?.maxKm||0)} km; the planned familiarisation currently gives ~${Math.round(raceLifecycle.projectedNew||0)} km on ${esc(shoeChartDate(state.setup?.raceDate))}.</li>`:'';return`<details class="shoeGraphRationale" open><summary>How the shoe plan gets you to Race Day</summary><div><p>This graph now uses <b>one replacement rule only</b>: the planned replacement date is the first future assigned workout that takes the current pair into its expected replacement-mileage range. The separate usage-rate estimate is deliberately not shown here because it is only a fallback forecast and made the lifecycle plan harder to interpret.</p><p>When a current pair is replaced, its line stops. The proposed replacement pair starts at 0 km and takes the later workouts that had been assigned to that role. The <b>race-day shoe strategy is always shown separately</b>, including a future race shoe that has not yet been bought.</p><ul>${rows}${race}</ul></div></details>`}
function shoeProgrammeMileageChartHtml(){
 ...
 const raceStrategyCard=raceStrategy?`<div class="shoeRaceStrategyCard"><small>RACE-DAY SHOE STRATEGY</small><b>${esc(raceStrategy.name)}</b><span>${raceStrategy.owned?'Owned now':'Proposed purchase'} · projected ~${Math.round(raceStrategy.finalKm)} km on ${esc(shoeChartDate(raceDate))}</span><strong>Desired race-day window ${Math.round(raceStrategy.window?.minKm||0)}–${Math.round(raceStrategy.window?.maxKm||0)} km</strong>${raceLifecycle.purchaseNeeded&&raceLifecycle.purchaseStart&&raceLifecycle.purchaseEnd?`<em>Buy ${esc(shoeChartDate(raceLifecycle.purchaseStart))} – ${esc(shoeChartDate(raceLifecycle.purchaseEnd))}, then use the planned familiarisation sessions shown by the race-shoe curve.</em>`:''}</div>`:'';
 return`<div class="shoeChartWrap shoeProgrammeChart"><div class="shoeChartRaceDate"><small>SHOE PLAN THROUGH RACE DAY</small><b>Race day · ${esc(shoeChartDate(raceDate))}</b></div>${raceStrategyCard}<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Current shoes, planned replacements and intended race-day shoe mileage through race day">${targetBand}<g class="shoeGrid">${grid}</g><line class="shoeTodayLine" x1="${todayX}" y1="${T}" x2="${todayX}" y2="${H-B}"/><line class="shoeRaceLine" x1="${raceX}" y1="${T}" x2="${raceX}" y2="${H-B}"/>${paths}${replacementProjection}${raceStrategyPath}${markers}${xlabels}<text class="yTitle" transform="translate(28 ${H/2}) rotate(-90)" text-anchor="middle">Accumulated km</text></svg><div class="shoeChartKey"><span><i class="actual"></i>Observed mileage</span><span><i class="planned"></i>Planned mileage on current pair</span><span><i class="replace"></i>Planned replacement</span><span><i class="newpair"></i>Proposed replacement pair</span><span><i class="raceshoe"></i>Intended race-day shoe</span><span><i class="raceband"></i>Desired race-day mileage window</span></div><div class="shoeChartLegend">${legend}</div>${shoeGraphRationaleHtml(series,rawEvents,raceLifecycle)}${eventHtml}</div>`}
