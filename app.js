...
const BUILD=10100, SCHEMA=10100, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function rehabExecutionMeta(check){
 ...
 // Backwards compatibility for check-ins saved before build 10100.
 if(!exercise){
...
function openInjuryCheck(i,existing=null){
 ...
 const syncRunningExecution=()=>{const minutes=nullableNumber(runMinutesField.value),status=runStatusField.value;if(Number.isFinite(minutes)&&minutes>0){if(['not_assessed','not_planned'].includes(status))runStatusField.value='completed';if(!locomotionField.value||['not_planned'].includes(locomotionField.value))locomotionField.value='completed';}else if(status==='completed'){if(!locomotionField.value||locomotionField.value==='not_planned')locomotionField.value='completed';}else if(status==='stopped'){if(!locomotionField.value||['not_planned','completed'].includes(locomotionField.value))locomotionField.value='stopped';}else if(status==='unable'){if(!locomotionField.value||['not_planned','completed'].includes(locomotionField.value))locomotionField.value='none';}};
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v10.0.10 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v10.0.10 stable build 10100');
})();