...
const BUILD=9710, SCHEMA=9710, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
const INJURY_STAGES=[
 {name:'Protect & settle',min:0,goal:'Calm symptoms and restore comfortable daily movement.',criteria:'Walking is comfortable; symptoms are stable or improving; no new swelling or bruising.'},
 {name:'Restore movement',min:1,goal:'Recover pain-limited range of motion and basic control.',criteria:'Normal walking, gentle mobility and easy double-leg loading with pain no higher than 2/10.'},
 {name:'Build capacity',min:2,goal:'Progress strength and single-leg control.',criteria:'Repeated bridge, hinge and calf-raise patterns are controlled, without worsening later that day or next morning.'},
 {name:'Return to run',min:3,goal:'Reintroduce impact with walk–run intervals.',criteria:'Brisk walk and low-level hopping/jogging in place are tolerated; pain stays at 0–2/10 and returns to baseline by the next morning.'},
 {name:'Rebuild running',min:4,goal:'Increase easy continuous running before speed or hills.',criteria:'Several easy runs completed without altered gait or a next-day symptom increase.'},
 {name:'Return to performance',min:5,goal:'Restore faster running, hills and sport-specific demands.',criteria:'Near-symmetrical strength and full-speed exposure are tolerated; clinician clearance when the injury was severe.'}
];
function injuryPrediction(i){
 ...
 let base=initialPain>=8||pop||walkInitially>=8?56:initialPain>=6||bruising||walkInitially>=5?35:initialPain>=3?21:10;
 ...
 let trend=0;if(checks.length>=2){const prev=checks.at(-2);trend=(Number(prev.pain)||0)-currentPain;}
 let modifier=currentPain>=6?14:currentPain>=4?7:currentPain<=1?-4:0;
 if(trend<0)modifier+=7;if(trend>=2)modifier-=3;
 const elapsed=Math.max(0,Math.floor((today()-dte(i.date))/DAY));
 const total=Math.max(elapsed+1,base+modifier), remaining=Math.max(0,total-elapsed);
 let stage=0;if(walkPain<=3&&currentPain<=5)stage=1;if(walkPain<=1&&currentPain<=3&&(latest.bridge??false))stage=2;if(currentPain<=2&&walkPain===0&&(latest.hop??false))stage=3;if((latest.runMinutes||0)>=10&&currentPain<=2&&!latest.nextDayWorse)stage=4;if((latest.runMinutes||0)>=30&&currentPain<=1&&!latest.nextDayWorse)stage=5;
 if(latest.nextDayWorse)stage=Math.max(0,stage-1);
 const startDate=iso(new Date(today().getTime()+remaining*DAY));
 const confidence=checks.length>=5?'Higher':checks.length>=2?'Moderate':'Low';
 return{base,total,elapsed,remaining,stage,startDate,confidence,currentPain,walkPain};
}
function renderInjury(){
 const box=$('injuryList');if(!box)return;const injuries=state.injuries||[];
 $('injuryIntro').innerHTML=`<div class="note"><b>Independent from your running plan.</b> Injury entries never alter workouts, race predictions or plan health. The timeline is a symptom-guided estimate, not a diagnosis or medical clearance.</div>`;
 box.innerHTML=injuries.length?injuries.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(i=>{let p=injuryPrediction(i),st=INJURY_STAGES[p.stage];return`<article class="panel injuryCard"><div class="panelHead"><div><h3>${esc(i.location||'Injury')} · ${fmtDate(i.date)}</h3><p class="muted compact">${esc(i.mechanism||'Mechanism not entered')}</p></div><span class="status ${p.stage>=3?'completed':p.currentPain>=5?'missed':'today'}">Stage ${p.stage+1}</span></div><div class="injuryForecast"><div><small>Estimated return-to-run window</small><strong>${p.stage>=3?'Running has started':fmtDate(p.startDate)}</strong><span>${p.remaining} estimated days remaining · ${p.confidence.toLowerCase()} confidence</span></div><div><small>Current stage</small><strong>${esc(st.name)}</strong><span>${esc(st.goal)}</span></div></div><div class="injuryTimeline">${INJURY_STAGES.map((x,n)=>`<div class="injuryStage ${n<p.stage?'done':n===p.stage?'active':''}"><i>${n+1}</i><div><b>${esc(x.name)}</b><small>${esc(x.criteria)}</small></div></div>`).join('')}</div><details><summary>Initial and current symptoms</summary><div class="guideGrid"><div><b>At injury</b><p>Pain ${Number(i.initialPain)||0}/10 · walking pain ${Number(i.initialWalkPain)||0}/10${i.pop?' · pop felt':''}${i.bruising?' · bruising':''}<br>${esc(i.initialSymptoms||'No symptom description')}</p></div><div><b>Latest</b><p>Pain ${p.currentPain}/10 · walking pain ${p.walkPain}/10<br>${esc((i.checkIns||[]).at(-1)?.symptoms||i.currentSymptoms||'No current symptom description')}</p></div></div></details><div class="buttonRow"><button data-injury-check="${i.id}" class="primary">Daily check-in</button><button data-injury-edit="${i.id}">Edit injury</button><button data-injury-delete="${i.id}" class="danger">Delete</button></div></article>`}).join(''):'<article class="panel"><p>No injury recorded. Add one to create a separate rehabilitation timeline.</p></article>';
}
function injuryForm(i={}){return`<h3>${i.id?'Edit':'Record'} injury</h3><div class="formGrid"><div class="field"><label>Injury date</label><input id="injDate" type="date" value="${i.date||iso(today())}"></div><div class="field"><label>Area / location</label><input id="injLocation" value="${esc(i.location||'')}" placeholder="e.g. left upper hamstring"></div><div class="field"><label>How did it happen?</label><input id="injMechanism" value="${esc(i.mechanism||'')}" placeholder="e.g. sprint, gradual onset"></div><div class="field"><label>Pain then 0–10</label><input id="injInitialPain" type="number" min="0" max="10" value="${i.initialPain??''}"></div><div class="field"><label>Walking pain then 0–10</label><input id="injInitialWalk" type="number" min="0" max="10" value="${i.initialWalkPain??''}"></div><div class="field"><label>Signs then</label><label><input id="injPop" type="checkbox" ${i.pop?'checked':''}> Pop / snap</label><label><input id="injBruise" type="checkbox" ${i.bruising?'checked':''}> Bruising or swelling</label></div><div class="field"><label>Symptoms then</label><textarea id="injInitialSymptoms">${esc(i.initialSymptoms||'')}</textarea></div><div class="field"><label>Symptoms now</label><textarea id="injCurrentSymptoms">${esc(i.currentSymptoms||'')}</textarea></div><div class="field"><label>Pain now 0–10</label><input id="injCurrentPain" type="number" min="0" max="10" value="${i.currentPain??''}"></div><div class="field"><label>Walking pain now 0–10</label><input id="injCurrentWalk" type="number" min="0" max="10" value="${i.currentWalkPain??''}"></div></div><div class="note bad"><b>Seek urgent assessment</b> for major trauma, inability to bear weight, marked weakness or deformity, rapidly increasing swelling, numbness, fever, severe night pain, or calf swelling/breathlessness.</div><button id="saveInjury" class="primary full">Save injury</button>`}
function openInjuryForm(i){$('modalContent').innerHTML=injuryForm(i);$('modal').className='modal';$('saveInjury').onclick=()=>{let obj={...(i||{}),id:i?.id||'inj-'+Date.now(),date:$('injDate').value,location:$('injLocation').value.trim(),mechanism:$('injMechanism').value.trim(),initialPain:Number($('injInitialPain').value),initialWalkPain:Number($('injInitialWalk').value),pop:$('injPop').checked,bruising:$('injBruise').checked,initialSymptoms:$('injInitialSymptoms').value.trim(),currentSymptoms:$('injCurrentSymptoms').value.trim(),currentPain:Number($('injCurrentPain').value),currentWalkPain:Number($('injCurrentWalk').value),checkIns:i?.checkIns||[]};if(!obj.date||!obj.location)return toast('Enter the injury date and location.',true);state.injuries=state.injuries||[];let n=state.injuries.findIndex(x=>x.id===obj.id);n>=0?state.injuries[n]=obj:state.injuries.push(obj);save();$('modal').className='modal hidden';renderInjury();toast('Injury timeline saved.')}}
function openInjuryCheck(i){$('modalContent').innerHTML=`<h3>Daily injury check-in</h3><div class="formGrid"><div class="field"><label>Date</label><input id="icDate" type="date" value="${iso(today())}"></div><div class="field"><label>Pain now 0–10</label><input id="icPain" type="number" min="0" max="10"></div><div class="field"><label>Walking pain 0–10</label><input id="icWalk" type="number" min="0" max="10"></div><div class="field"><label>Function today</label><label><input id="icBridge" type="checkbox"> Repeated bridge/hinge tolerated</label><label><input id="icHop" type="checkbox"> Gentle hop/jog-in-place tolerated</label></div><div class="field"><label>Running completed (minutes)</label><input id="icRun" type="number" min="0" value="0"></div><div class="field"><label>Worse the next morning?</label><select id="icWorse"><option value="false">No</option><option value="true">Yes</option></select></div><div class="field"><label>Symptoms / notes</label><textarea id="icSymptoms"></textarea></div></div><button id="saveCheck" class="primary full">Save and recalculate timeline</button>`;$('modal').className='modal';$('saveCheck').onclick=()=>{let c={date:$('icDate').value,pain:Number($('icPain').value),walkPain:Number($('icWalk').value),bridge:$('icBridge').checked,hop:$('icHop').checked,runMinutes:Number($('icRun').value)||0,nextDayWorse:$('icWorse').value==='true',symptoms:$('icSymptoms').value.trim()};if(!c.date||!Number.isFinite(c.pain)||!Number.isFinite(c.walkPain))return toast('Enter date, pain and walking pain.',true);i.checkIns=i.checkIns||[];let n=i.checkIns.findIndex(x=>x.date===c.date);n>=0?i.checkIns[n]=c:i.checkIns.push(c);save();$('modal').className='modal hidden';renderInjury();toast('Timeline recalculated from today’s feedback.')}}
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.7.1 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.7.1 stable build 9710');
})();