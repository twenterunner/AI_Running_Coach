...
function shoeProgrammeMileageChartHtml(){
 ...
 const replacementSeries=rawEvents.filter(e=>enabled.has(e.shoe.id)&&e.replacement).map(e=>{const purchaseDate=plannedTrainingPurchaseDate(e),pts=[{date:purchaseDate,km:0,purchase:true,driver:'rotation'}];let km=0;if(purchaseDate<e.date)pts.push({date:e.date,km:0,ready:true,driver:'rotation'});(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>e.date&&p.date<=raceDate).slice().sort((a,b)=>a.date.localeCompare(b.date)).forEach(p=>{if(plannedAssignment(p.id)?.shoeId!==e.shoe.id)return;const add=Number(p.distance)||0;km+=add;pts.push({date:p.date,km,deltaKm:add,type:p.type,driver:'training'})});futureRehabShoeUsage(e.shoe.id,null).filter(x=>x.date>e.date&&x.date<=raceDate).forEach(x=>{const add=Number(x.distanceKm)||0;km+=add;pts.push({date:x.date,km,deltaKm:add,type:'Rehab',driver:'rehab'})});pts.sort((a,b)=>a.date.localeCompare(b.date)||Number(a.km)-Number(b.km));if(pts.at(-1)?.date!==raceDate)pts.push({date:raceDate,km,driver:'end'});return{event:e,purchaseDate,points:pts,finalKm:km}});
