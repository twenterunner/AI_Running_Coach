...
function estimateRehabDistance(minutes,kind){const mins=Math.max(0,Number(minutes)||0),speed=kind==='walk'?4.8:kind==='run'?7.5:6.0;return{distanceKm:mins/60*speed,assumedSpeedKmh:speed,distanceMethod:'estimated'}}
...
function reconcileShoeUsage(){
 ...
 const planIds=new Set((state.plan||[]).filter(p=>p.type!=='Rest').map(p=>p.id));state.plannedShoeAssignments=(state.plannedShoeAssignments||[]).filter(a=>a&&planIds.has(a.planId)&&shoeIds.has(a.shoeId));
}
...
function plannedAssignment(planId){return (state.plannedShoeAssignments||[]).find(a=>a.planId===planId)||null}
function setPlannedShoe(planId,shoeId,source='user'){state.plannedShoeAssignments=(state.plannedShoeAssignments||[]).filter(a=>a.planId!==planId);if(shoeId)state.plannedShoeAssignments.push({id:`shoe-plan-${planId}`,planId,shoeId,source,updatedAt:new Date().toISOString()})}
...
function shoeRecommendation(plan,{excludeId=null}={}){
 ...
 const ranked=active.map(shoe=>{const p=shoeProfileForShoe(shoe),wear=shoeWearState(shoe),fam=shoeFamiliarity(shoe,plan.type);let weighted=0,total=0,reasons=[];
  ...
  const race=/Race$|Race Day|Race rehearsal/.test(plan.type),raceSuitable=(p.roles||[]).includes('race');let warning='';if(race&&!raceSuitable)score=Math.min(score,34);if(race&&raceSuitable&&fam.runs<2){score-=18;warning='Race shoe is theoretically suitable but has insufficient verified use.'}if(race&&raceSuitable&&wear.status==='replace'){score-=30;warning='This shoe is at or beyond its expected service-life range.'}
...
function shoeAutoAssignments(){if(!(state.shoes||[]).some(s=>s.status!=='retired'))return;const todayStr=iso(today()),future=(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>=todayStr).slice().sort((a,b)=>a.date.localeCompare(b.date));const user=new Map((state.plannedShoeAssignments||[]).filter(a=>a.source==='user').map(a=>[a.planId,a]));state.plannedShoeAssignments=(state.plannedShoeAssignments||[]).filter(a=>a.source==='user'||!(future.some(p=>p.id===a.planId)));future.forEach(p=>{if(user.has(p.id))return;const rec=shoeRecommendation(p);if(rec.best)setPlannedShoe(p.id,rec.best.shoe.id,'auto')})}
...
function racePlanHtml(){const plan=raceShoePlan();if(plan.status!=='ok')return'<div class="shoeEmpty">Race-day shoe planning needs a valid race date.</div>';if(!(state.shoes||[]).length&&!plan.purchaseNeeded)return'<div class="shoeEmpty">Add your current rotation to plan race-day shoe condition.</div>';const preferred=plan.preferred,cond=plan.condition;let main='';if(preferred){main=`<article class="raceShoeHero ${cond.code}"><div><small>RACE SHOE</small><h4>${esc(shoeDisplayName(preferred))}</h4><p>${Math.round(cond.current)} km now · ${Math.round(cond.projected)} km projected on race day</p></div><div><small>FAMILIARITY</small><b>${cond.fam.runs>=3?'Proven':cond.fam.runs>=1?'Building':'Unproven'}</b><span>${cond.fam.runs} verified runs · ${cond.fam.km.toFixed(1)} km</span></div><div><small>RACE-DAY OUTLOOK</small><b>${esc(cond.label)}</b><span>Target familiarisation window ~${Math.round(cond.opt.minKm)}–${Math.round(cond.opt.maxKm)} km</span></div></article>`}else main='<div class="shoeWarning">No owned active shoe currently provides a strong race-day match.</div>';
 const sessions=plan.famSessions.length?`<div class="raceFamiliarisation"><small>PLANNED FAMILIARISATION</small>${plan.famSessions.map(x=>`<div><span>${fmtDate(x.date)} · ${esc(x.type)}</span><b>~${Number(x.plannedShoeKm).toFixed(1)} km shoe exposure</b></div>`).join('')}</div>`:'<p class="muted">No suitable future familiarisation session is currently available in the existing plan.</p>';
 const purchase=plan.purchaseNeeded&&plan.profile?`<p class="shoePurchaseInline"><b>Future Race Day pair:</b> ASICS ${esc(plan.profile.family)} ${esc(plan.profile.version)} · suggested purchase ${plan.purchaseStart&&plan.purchaseEnd?`${fmtDate(plan.purchaseStart)}–${fmtDate(plan.purchaseEnd)}`:'timing building'} · projected ~${Math.round(plan.projectedNew)} km on Race Day.</p>`:`<p class="shoePurchaseInline"><b>No new race shoe currently required.</b> Preserve the selected pair for relevant sessions.</p>`;
 return`${main}${sessions}${purchase}${state.activeInjuryPlanId?'<p class="shoeWarning">Active rehabilitation reduces race-shoe planning confidence. The shoe plan adapts to available sessions and does not change rehabilitation progression.</p>':''}`}
...
function raceDayShoeTargetHtml(){const plan=raceShoePlan();if(plan.status==='no-race')return'';const shoe=plan.purchaseNeeded?null:plan.preferred,profile=plan.purchaseNeeded?plan.profile:(shoe?shoeProfileForShoe(shoe):null),window=plan.targetWindow||plan.condition?.opt;if(!profile||!window)return'';const name=shoe?shoeDisplayName(shoe):`ASICS ${profile.family} ${profile.version}`;const projected=shoe?Math.round(plan.condition?.projected||0):Math.round(plan.projectedNew||0);const familiarisation=plan.famSessions?.length||0;const stateText=projected>=window.minKm&&projected<=window.maxKm?'within the desired race-day mileage window':projected<window.minKm?'below the desired familiarity window unless planned familiarisation is completed':'above the desired race-day mileage window';return`<article class="shoeRaceTarget"><small>DESIRED RACE-DAY SITUATION</small><h4>${esc(name)}</h4><div class="shoeRaceTargetMetrics"><div><span>Target mileage window</span><b>${Math.round(window.minKm)}–${Math.round(window.maxKm)} km</b></div><div><span>Current plan projects</span><b>~${projected} km</b></div><div><span>Familiarisation</span><b>${familiarisation?`${familiarisation} suitable session${familiarisation===1?'':'s'}`:'Existing verified use'}</b></div></div><p>The shoe planner is working toward race day with this shoe sufficiently familiar, but not unnecessarily worn. The present projection is ${esc(stateText)}.</p></article>`}
...
function shoeProgrammeMileageChartHtml(){
 ...
 const minD=dte(dates[0]).getTime(),maxD=Math.max(minD+DAY,dte(raceDate).getTime()),maxY=Math.max(10,Math.ceil(Math.max(...values)*1.18/50)*50),W=920,H=500,L=112,R=34,T=112,B=88,x=d=>L+(dte(d).getTime()-minD)/(maxD-minD)*(W-L-R),y=v=>T+(maxY-v)/maxY*(H-T-B),palette=['#3dd6c6','#79d69b','#67a7ff','#f1c56a','#b9a3ff','#ff9f9f','#7ee7ff'];
 ...
 const paths=series.map((s,i)=>{const c=palette[i%palette.length],actualPts=s.actual.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' '),plannedPts=s.planned.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ');return`${actualPts?`<polyline class="shoeLine shoeActualLine" style="--shoe-color:${c}" points="${actualPts}"/>`:''}${plannedPts?`<polyline class="shoeLine shoePlannedLine" style="--shoe-color:${c}" points="${plannedPts}"/>`:''}`}).join('');
 const replacementProjection=replacementSeries.map(item=>{const e=item.event,c=palette[Math.max(0,shoes.findIndex(s=>s.id===e.shoe.id))%palette.length];return`<polyline class="shoeLine shoeNewPairLine" style="--shoe-color:${c}" points="${item.points.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ')}"/>`}).join('');
 const raceStrategyPath=raceStrategy?`<polyline class="shoeLine shoeRaceStrategyLine" points="${raceStrategy.points.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ')}"/><circle class="shoeRaceStrategyPoint" cx="${raceX}" cy="${y(raceStrategy.finalKm)}" r="8"/>`:'';
 ...
 const legend=series.map((s,i)=>`<span class="shoeLegendItem"><span class="shoeLegendSwatch" style="--shoe-color:${palette[i%palette.length]}"></span>${esc(shoeDisplayName(s.shoe))}</span>`).join('');
 const eventHtml=rawEvents.length?`<div class="shoeLifecycleEvents">${rawEvents.map(e=>`<article><small>PLANNED REPLACEMENT</small><b>${esc(shoeDisplayName(e.shoe))}</b><span>${esc(shoeChartDate(e.date))} · ~${Math.round(e.km)} km</span><strong>Proposed replacement: ASICS ${esc(e.replacement?.family||'running shoe')} ${esc(e.replacement?.version||'')}</strong></article>`).join('')}</div>`:'';
...
function futureRehabShoeUsage(shoeId,days=null){const injury=(state.injuries||[]).find(x=>x.id===state.activeInjuryPlanId);if(!injury)return[];const progress=injuryPrediction(injury),start=iso(today()),limit=days?new Date(today().getTime()+days*DAY):dte(state.setup?.raceDate||start),end=iso(limit);const out=[];for(let d=dte(start),n=0;d<=dte(end)&&n<120;d=new Date(d.getTime()+DAY),n++){const date=iso(d),day=rehabCalendarDay(injury,progress,date,rehabPlanDayIndex(injury,date));if(!rehabDayNeedsShoe(day))continue;const sid=plannedRehabShoeForDay(injury,day);if(sid!==shoeId)continue;const exp=rehabExpectedDistance(day);if(exp.totalKm>0)out.push({date,distanceKm:exp.totalKm,sourceType:'planned_rehab',distanceMethod:'estimated',walkMinutes:exp.walkMinutes,runMinutes:exp.runMinutes})}return out}
