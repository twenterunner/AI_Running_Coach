...
function shoeGraphRationaleHtml(series,events,raceLifecycle){const rows=series.map(item=>{const current=shoeMileage(item.shoe),planned=item.planned.at(-1)?.km??current,added=Math.max(0,planned-current),event=events.find(e=>e.shoe.id===item.shoe.id),f=shoeForecast(item.shoe);return`<li><b>${esc(shoeDisplayName(item.shoe))}:</b> starts at ${Math.round(current)} km today, receives ~${Math.round(added)} km from current planned assignments, and therefore reaches ~${Math.round(planned)} km by race day.${event?` Its planned replacement point is around ${esc(fmtDate(event.date))}.`:(f.lowDate?` Its nominal replacement window begins around ${esc(fmtDate(f.lowDate))}, ${f.lowDate>state.setup.raceDate?'after':'before'} race day.`:' There is not enough usage evidence for a nominal replacement date.')}</li>`}).join('');const race= raceLifecycle?.preferred&&raceLifecycle.condition?`<li><b>Race-day objective:</b> ${esc(shoeDisplayName(raceLifecycle.preferred))} is currently projected at ~${Math.round(raceLifecycle.condition.projected)} km on race day versus a desired ${Math.round(raceLifecycle.condition.opt.minKm)}–${Math.round(raceLifecycle.condition.opt.maxKm)} km familiarity/service window.</li>`:raceLifecycle?.purchaseNeeded&&raceLifecycle.profile?`<li><b>Race-day objective:</b> the planner proposes ASICS ${esc(raceLifecycle.profile.family)} ${esc(raceLifecycle.profile.version)} and targets ~${Math.round(raceLifecycle.projectedNew||0)} km by race day.</li>`:'';return`<details class="shoeGraphRationale"><summary>Why does this graph look like this?</summary><div><p>The solid section is what has actually accumulated. The dashed section is not a smooth forecast: it steps upward only when the current training plan assigns a workout to that shoe. Flat sections therefore mean the other shoe is being used, a rest day occurs, or that shoe is deliberately being preserved.</p><ul>${rows}${race}</ul><p>Amber markers show the first planned point where a shoe enters its expected replacement range. Blue nominal markers show the replacement window implied by the current usage-rate forecast; the two can differ because planned future assignments are more specific than a weekly-rate extrapolation.</p></div></details>`}
function shoeProgrammeMileageChartHtml(){const shoes=(state.shoes||[]).filter(s=>s.status!=='retired'||s.showInHistory!==false);if(!shoes.length)return'<div class="shoeEmpty">Add a shoe to start mileage planning.</div>';const enabled=new Set((state.shoeChartVisible||shoes.map(s=>s.id)).filter(id=>shoes.some(s=>s.id===id)));if(!enabled.size)shoes.forEach(s=>enabled.add(s.id));const raceDate=state.setup?.raceDate||iso(today()),todayStr=iso(today()),programStart=state.setup?.planStart||todayStr,rawEvents=plannedReplacementEvents(),eventByShoe=new Map(rawEvents.map(e=>[e.shoe.id,e])),series=shoes.filter(s=>enabled.has(s.id)).map(s=>{const actual=shoeActualProgrammeSeries(s,programStart,todayStr),rawPlanned=shoePlannedMileageSeries(s),event=eventByShoe.get(s.id),planned=event?rawPlanned.filter(p=>p.date<=event.date):rawPlanned;return{shoe:s,actual,planned}});const dates=[programStart,todayStr,raceDate,...series.flatMap(x=>x.actual.map(p=>p.date)),...series.flatMap(x=>x.planned.map(p=>p.date))].filter(Boolean).sort(),values=series.flatMap(x=>[...x.actual,...x.planned].map(p=>p.km)).filter(Number.isFinite);if(!values.length)return'<div class="shoeEmpty">No tracked or planned mileage yet.</div>';const minD=dte(dates[0]).getTime(),maxD=Math.max(minD+DAY,dte(raceDate).getTime()),maxY=Math.max(10,Math.ceil(Math.max(...values)*1.12/50)*50),W=760,H=390,L=66,R=22,T=82,B=66,x=d=>L+(dte(d).getTime()-minD)/(maxD-minD)*(W-L-R),y=v=>T+(maxY-v)/maxY*(H-T-B),palette=['#3dd6c6','#79d69b','#67a7ff','#f1c56a','#b9a3ff','#ff9f9f','#7ee7ff'];let grid='';for(let i=0;i<=4;i++){const v=maxY*i/4,yy=y(v);grid+=`<line x1="${L}" y1="${yy}" x2="${W-R}" y2="${yy}"/><text x="${L-10}" y="${yy+5}" text-anchor="end">${Math.round(v)}</text>`}const paths=series.map((s,i)=>{const c=palette[i%palette.length],actualPts=s.actual.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' '),plannedPts=s.planned.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ');return`${actualPts?`<polyline class="shoeLine shoeActualLine" style="--shoe-color:${c}" points="${actualPts}"/>`:''}${plannedPts?`<polyline class="shoeLine shoePlannedLine" style="--shoe-color:${c}" points="${plannedPts}"/>`:''}`}).join('');const events=rawEvents,replacementProjection=events.filter(e=>enabled.has(e.shoe.id)&&e.replacement&&e.date<=raceDate).map(e=>{let km=0;const pts=[{date:e.date,km:0}];(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>=e.date&&p.date<=raceDate).slice().sort((a,b)=>a.date.localeCompare(b.date)).forEach(p=>{if(plannedAssignment(p.id)?.shoeId!==e.shoe.id)return;km+=Number(p.distance)||0;pts.push({date:p.date,km})});if(pts.at(-1).date!==raceDate)pts.push({date:raceDate,km});return`<polyline class="shoeLine shoeNewPairLine" points="${pts.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ')}"/>`}).join('');const raceLifecycle=raceShoePlan(),racePurchaseSeries=raceLifecycle.purchaseNeeded&&raceLifecycle.profile&&raceLifecycle.famSessions?.length?(()=>{let km=0;const first=raceLifecycle.famSessions[0].date,pts=[{date:first,km:0}];raceLifecycle.famSessions.forEach(p=>{km+=Number(p.plannedShoeKm)||0;pts.push({date:p.date,km})});if(pts.at(-1).date!==raceDate)pts.push({date:raceDate,km});return`<polyline class="shoeLine shoeRacePurchaseLine" points="${pts.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ')}"/>`})():'';const todayX=x(todayStr),raceX=x(raceDate),markers=events.filter(e=>enabled.has(e.shoe.id)&&e.date>=dates[0]&&e.date<=raceDate).map((e,i)=>{const xx=x(e.date),yy=Math.max(T+12,y(Math.min(maxY,e.km))),labelY=22+(i%2)*18;return`<g class="shoeReplacementMarker"><line x1="${xx}" y1="${T}" x2="${xx}" y2="${H-B}"/><circle cx="${xx}" cy="${yy}" r="6"/><text x="${Math.min(W-R-4,Math.max(L+4,xx))}" y="${labelY}" text-anchor="${xx>W*.72?'end':xx<W*.28?'start':'middle'}">Planned replace ${esc(e.shoe.model)} · ${esc(fmtDate(e.date))}</text></g>`}).join('');const nominal=series.map((s,i)=>({shoe:s.shoe,forecast:shoeForecast(s.shoe),i})).filter(o=>o.forecast.lowDate).map((o,i)=>{const inRange=o.forecast.lowDate<=raceDate,xx=inRange?x(o.forecast.lowDate):raceX,labelY=58+(i%2)*16,anchor=inRange?(xx>W*.72?'end':xx<W*.28?'start':'middle'):'end',label=inRange?`Nominal ${o.shoe.model} · ${fmtDate(o.forecast.lowDate)}`:`${o.shoe.model} nominal after race · ${fmtDate(o.forecast.lowDate)}`;return`<g class="shoeNominalMarker ${inRange?'':'afterRace'}"><line x1="${xx}" y1="${T}" x2="${xx}" y2="${H-B}"/><path d="M ${xx-5} ${T+4} L ${xx+5} ${T+4} L ${xx} ${T+13} Z"/><text x="${Math.min(W-R-4,Math.max(L+4,xx))}" y="${labelY}" text-anchor="${anchor}">${esc(label)}</text></g>`}).join('');const xlabels=[programStart,todayStr,raceDate].filter((v,i,a)=>v&&a.indexOf(v)===i).map(d=>`<text class="xLab" x="${x(d)}" y="${H-20}" text-anchor="${d===programStart?'start':d===raceDate?'end':'middle'}">${d===todayStr?'Today':d===raceDate?'Race day':dte(d).toLocaleDateString(undefined,{month:'short',year:'2-digit'})}</text>`).join('');const legend=shoes.map((s,i)=>`<label class="shoeLegendToggle"><input type="checkbox" data-shoe-chart-toggle="${esc(s.id)}" ${enabled.has(s.id)?'checked':''}><i style="--shoe-color:${palette[i%palette.length]}"></i><span>${esc(shoeDisplayName(s))}</span></label>`).join('');const eventHtml=events.length?`<div class="shoeLifecycleEvents">${events.map(e=>`<article><small>PLANNED REPLACEMENT · ${esc(fmtDate(e.date).toUpperCase())}</small><b>Replace ${esc(shoeDisplayName(e.shoe))}</b><span>${esc(e.range)} expected window · ${esc(e.method)}</span>${e.replacement?`<strong>Proposed new pair: ASICS ${esc(e.replacement.family)} ${esc(e.replacement.version)}</strong>`:'<strong>Choose a suitable replacement for the same role.</strong>'}</article>`).join('')}</div>`:'<div class="shoeEmpty compact">No shoe is currently projected to enter its replacement range before race day.</div>';return`<div class="shoeChartWrap shoeProgrammeChart"><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Actual accumulated shoe mileage versus planned mileage through race day"><g class="shoeGrid">${grid}</g><line class="shoeTodayLine" x1="${todayX}" y1="${T}" x2="${todayX}" y2="${H-B}"/><line class="shoeRaceLine" x1="${raceX}" y1="${T}" x2="${raceX}" y2="${H-B}"/>${paths}${replacementProjection}${racePurchaseSeries}${nominal}${markers}${xlabels}<text class="yTitle" transform="translate(18 ${H/2}) rotate(-90)" text-anchor="middle">Accumulated km</text></svg><div class="shoeChartKey"><span><i class="actual"></i>Actual accumulated mileage</span><span><i class="planned"></i>Planned cumulative mileage</span><span><i class="replace"></i>Planned replacement point</span><span><i class="nominal"></i>Nominal replacement date</span><span><i class="newpair"></i>Proposed new-pair mileage</span></div><div class="shoeChartLegend">${legend}</div>${shoeGraphRationaleHtml(series,events,raceLifecycle)}${eventHtml}${raceLifecycle.purchaseNeeded&&raceLifecycle.profile?`<div class="shoeLifecycleEvents racePurchaseEvent"><article><small>${raceLifecycle.purchaseStart?`BUY AROUND ${esc(fmtDate(raceLifecycle.purchaseStart).toUpperCase())}`:`FUTURE PURCHASE`}</small><b>Proposed race shoe: ASICS ${esc(raceLifecycle.profile.family)} ${esc(raceLifecycle.profile.version)}</b><span>${raceLifecycle.purchaseStart&&raceLifecycle.purchaseEnd?`${esc(fmtDate(raceLifecycle.purchaseStart))} – ${esc(fmtDate(raceLifecycle.purchaseEnd))}`:`Purchase timing remains uncertain`}</span><strong>Projected race-day mileage ~${Math.round(raceLifecycle.projectedNew||0)} km</strong></article></div>`:``}</div>`}
