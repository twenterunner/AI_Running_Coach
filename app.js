...
function openRunDetails(runId){
 ...
 $('saveRunEdit').onclick=()=>{
   try{
    ...
    state.runs.splice(i,1);const before=postRunCoachSnapshot(updated.date);state.runs.splice(i,0,updated);assignRunShoe(updated,updated.shoeId||null,'run-edit');recordPredictionSnapshot(updated.date,'Run update',updated.id);const after=postRunCoachSnapshot(updated.date);updated.coachUpdate=postRunCoachUpdate(updated,before,after);save();refreshShoeActualsAfterActivity();renderAll();$('modalContent').innerHTML=runAnalysisStackHtml(updated,false)+`<button id="closeCoachUpdate" class="primary full" type="button">Done</button>`;$('modal').className='modal logDetailModal';$('closeCoachUpdate').onclick=closeDialog;toast('Run updated and coaching update recalculated.');
...
$('manualRunBtn').onclick=()=>{
 ...
 bindEditorPlanRefresh(r);
 $('saveRunEdit').onclick=()=>{
   try{let created=updatedRunFromForm(r),before=postRunCoachSnapshot(created.date);state.runs.push(created);assignRunShoe(created,created.shoeId||null,'manual-run');recordPredictionSnapshot(created.date,'Run saved',created.id);let after=postRunCoachSnapshot(created.date);created.coachUpdate=postRunCoachUpdate(created,before,after);save();refreshShoeActualsAfterActivity();renderAll();$('modalContent').innerHTML=runAnalysisStackHtml(created,false)+`<button id="closeCoachUpdate" class="primary full" type="button">Done</button>`;$('modal').className='modal logEditModal';$('closeCoachUpdate').onclick=closeDialog;toast('Run saved and coaching update calculated.')}catch(err){toast(err.message,true)}
 };
};
 ...
 try{
   ...
   $('saveImport').onclick=()=>{
    try{
      ...
      const savedRun={...preview};state.runs.push(savedRun);if($('iShoe'))assignRunShoe(savedRun,$('iShoe').value||null,'import');reconcileExactDateMatches();
      ...
      save();refreshShoeActualsAfterActivity();$('importPreview').className='hidden';preview=null;input.value='';renderAll();
