...
function workoutScoreDetails(run,plan=run?.planId?state.plan.find(p=>p.id===run.planId):null){
 ...
 const ia=run.intervalAnalysis?.structured&&run.intervalAnalysis?.work?.length?run.intervalAnalysis:null,ip=ia?ia.work.map(x=>x.paceScore).filter(Number.isFinite):[],iw=ia?ia.work.map(x=>x.powerScore).filter(Number.isFinite):[];
 if(plan?.zone?.pace>0){if(ip.length>=2)add('pace','Interval pace execution',avg(ip),profile.pace,`${ip.length} FIT work repetitions compared directly with ${pace(plan.zone.pace)} target.`,1);else{const ratio=plan.zone.pace/actualPace,score=clamp(100-Math.abs(1-ratio)*145,45,100);add('pace','Pace execution',score,profile.pace,`${pace(actualPace)} whole-run average versus ${pace(plan.zone.pace)} target for the prescribed target portion.${aggregateNote}`,reliability)}}
 if(Number(run.avgPower)>0&&plan?.zone?.power>0){if(iw.length>=2)add('power','Interval power execution',avg(iw),profile.power,`${iw.length} FIT work repetitions compared directly with ${Math.round(plan.zone.power)} W target.`,1);else{const ratio=Number(run.avgPower)/plan.zone.power,score=clamp(100-Math.abs(1-ratio)*140,45,100);add('power','Power execution',score,profile.power,`${Math.round(Number(run.avgPower))} W whole-run average versus ${Math.round(plan.zone.power)} W target.${aggregateNote}`,reliability)}}
...
function adaptiveFactorDetails(w){
 ...
 if(previousEnd>today()){const prior=w>1?adaptiveFactorDetails(w-1).cumulativeFactor:1;return{factor:prior,rawFactor:prior,baseFactor:prior,cumulativeFactor:prior,weeklyLearningDelta:0,temporaryAdjustment:0,items:[{name:'Pending evidence',adjustment:0,detail:`Week ${previousWeek} is not complete. The cumulative load-tolerance factor remains ${prior.toFixed(3)} until that week ends.`}],previousWeek,plannedKm:null,completedKm:null,status:'pending'};}
 ...
 let aerobic=state.runs.filter(r=>['Easy','Recovery','Long run'].includes(r.type)&&Number(r.durationSec)>=2700&&dte(r.date)>=new Date(weekStart(w).getTime()-35*DAY)&&dte(r.date)<weekStart(w)&&dte(r.date)<=today());
 ...
 const previousCumulative=w>1?adaptiveFactorDetails(w-1).cumulativeFactor:1;
 const weeklyLearningDelta=trainingResponse-1;
 const cumulativeFactor=clamp(previousCumulative*(1+weeklyLearningDelta),state.setup.minFactor,state.setup.maxFactor);
 const temporaryAdjustment=(hrv.factor-1)+painAdj.adjustment;
 const rawFactor=cumulativeFactor*(1+temporaryAdjustment),boundedFactor=clamp(rawFactor,state.setup.minFactor,state.setup.maxFactor);
 return{factor:boundedFactor,rawFactor,baseFactor:previousCumulative,cumulativeFactor,weeklyLearningDelta,temporaryAdjustment,trainingResponse,hrvFactor:hrv.factor,hrv,items,previousWeek,plannedKm:prev.planned,completedKm:prev.actual,status:'calculated'};
}
...
function provisionalWeeklyAdjustment(w=currentWeek()){
 ...
 add('Pain',painAdj.adjustment,painAdj.detail);
 const baseFactor=adaptiveFactorDetails(current).cumulativeFactor;
 const weeklyLearningDelta=trainingResponse-1;
 const cumulativeFactor=clamp(baseFactor*(1+weeklyLearningDelta),state.setup.minFactor,state.setup.maxFactor);
 const temporaryAdjustment=(hrv.ready?hrv.factor-1:0)+painAdj.adjustment;
 ...
 return{factor,rawFactor,baseFactor,cumulativeFactor,weeklyLearningDelta,temporaryAdjustment,items,status:'provisional',week:current};
}
...
function weeklyReviewData(w=currentWeek()){
 ...
 const appliedLoad=adaptiveFactorDetails(w),nextLoad=w<weeks()?adaptiveFactorDetails(w+1):appliedLoad;
 ...
 const nextWeek=Math.min(weeks(),w+1),nextPlan=state.plan.filter(p=>p.week===nextWeek&&!['Rest','Race Day'].includes(p.type)).slice(0,5);
 const paceRatio=appliedPace>0?provisionalPace/appliedPace:1;
 const changes=nextPlan.map(p=>{const z=zone(p.type,p.date);return{date:p.date,type:p.type,distance:p.distance,pace:z.pace,nextPace:z.pace/Math.max(.8,paceRatio),power:z.power,nextPower:z.power*paceRatio}});
 const reasons=[];
 ...
 const paceDelta=provisionalPace-appliedPace,loadDelta=Number(nextLoad.cumulativeFactor||1)-Number(appliedLoad.cumulativeFactor||1);
 return{week:w,closed,completion,avgScore,drift,maxPain,appliedPace,provisionalPace,paceDelta,appliedLoad:Number(appliedLoad.cumulativeFactor||1),nextLoad:Number(nextLoad.cumulativeFactor||1),loadDelta,readiness:1+(Number(nextLoad.temporaryAdjustment)||0),reasons,changes};
}
function weeklyReviewHtml(w=currentWeek()){
 const r=weeklyReviewData(w),status=r.closed?'WEEKLY REVIEW COMPLETE':'PROVISIONAL WEEKLY REVIEW',paceText=`${r.appliedPace.toFixed(3)} → ${r.provisionalPace.toFixed(3)}`,loadText=`${r.appliedLoad.toFixed(3)} → ${r.nextLoad.toFixed(3)}`;
 const rows=r.changes.map(x=>{const pc=Math.abs(x.nextPace-x.pace)>=.5,pw=Math.abs(x.nextPower-x.power)>=1;return`<div class="weeklyReviewChange"><div><b>${fmtDate(x.date)} · ${esc(x.type)}</b><small>${Number(x.distance).toFixed(1)} km</small></div><span>${pc?`${pace(x.pace)} → ${pace(x.nextPace)}`:`${pace(x.pace)} · held`}</span><span>${pw?`${Math.round(x.power)} → ${Math.round(x.nextPower)} W`:`${Math.round(x.power)} W · held`}</span></div>`}).join('');
 return`<section class="weeklyReviewCard ${r.closed?'complete':'provisional'}"><div class="weeklyReviewHead"><div><small>${status}</small><h3>Week ${r.week} adaptation review</h3></div><span>${r.closed?'Applied':'Building'}</span></div><div class="weeklyReviewFactors"><div><small>Pace & Power</small><strong>${paceText}</strong><em>${signedFactorDelta(r.paceDelta)}</em></div><div><small>Distance & Load</small><strong>${loadText}</strong><em>${signedFactorDelta(r.loadDelta)}</em></div><div><small>Readiness</small><strong>${r.readiness.toFixed(3)}</strong><em>temporary</em></div></div><div class="weeklyReviewReasons"><b>Why</b>${r.reasons.length?r.reasons.map(x=>`<p>${esc(x)}</p>`).join(''):'<p>Evidence is still accumulating for this week.</p>'}</div><details><summary>Prescription impact for Week ${Math.min(weeks(),r.week+1)}</summary><div class="weeklyReviewChanges">${rows||'<p class="muted">No upcoming running sessions to compare.</p>'}</div><p class="muted compact">${r.closed?'The weekly review is finalised from completed evidence.':'These are provisional pace/power effects. Learned pathway changes commit when the week closes; acute readiness may still reduce an upcoming session sooner.'}</p></details></section>`;
}
...
function renderPlan(){
 const ast=athleteState(state.weekView||currentWeek());if($('fitnessEvidence')){
  ...
  $('fitnessEvidence').innerHTML=`<div class="adaptationPathways"><section class="adaptationPath pacePath"><div class="pathHeader"><span>PACE & POWER PATHWAY</span><strong>${fitnessFactor.toFixed(3)}</strong></div><div class="pathDecision"><h3>${esc(paceDecision)}</h3><small>Fitness calibration</small></div><p>Completed-run execution, assessments and races determine whether future pace and power targets should change.</p><div class="pathFactorSummary"><div><small>Current factor</small><b>${fitnessFactor.toFixed(3)}</b></div><div><small>Change this week</small><b>${signedFactorDelta(ast.pathways.pace.weekChange)}</b></div><div><small>Since programme start</small><b>${signedFactorDelta(ast.pathways.pace.sinceStart)}</b></div></div><div class="pathStats"><span>Evidence confidence <b>${ast.evidenceConfidence}%</b></span><span>Status <b>${esc(ast.fitnessTrend)}</b></span></div><p class="pathAction">${esc(paceAction)}</p><details class="stateDetails"><summary>Show pace and power calculation</summary><p>${esc(confidenceMeaning)}</p><div class="confidenceBands"><span><b>0–19%</b> Very limited</span><span><b>20–49%</b> Early</span><span><b>50–79%</b> Moderate</span><span><b>80–100%</b> Strong</span></div><p class="muted compact">Confidence is the amount of weighted evidence accumulated since the ${esc(evidence.anchorType.toLowerCase())} on ${fmtDate(evidence.anchorDate)}. It is not a fitness score. Targets change only when the accumulated fitness signal produces at least a 0.5% calibration step.</p><div class="evidenceEventList">${evidenceRows.length?evidenceRows.map(e=>`<div><span>${fmtDate(e.date)} · ${esc(e.type)}</span><b>Execution ${Math.round(e.score)}/100</b><small>${(e.confidence/4*100).toFixed(1)} confidence points · fitness signal ${e.delta>=0?'+':''}${e.delta.toFixed(2)}</small></div>`).join(''):'<p class="muted">No qualifying completed-run evidence has accumulated since the current baseline.</p>'}</div><div class="adjustmentCalcTotal"><span>Applied pace & power factor</span><b>${fitnessFactor.toFixed(3)}</b><small>1.000 means no change. Applied to future pace and power targets; heart-rate targets remain stable.</small></div></details></section><section class="adaptationPath loadPath"><div class="pathHeader"><span>DISTANCE & LOAD PATHWAY</span><strong>${ast.pathways.load.current.toFixed(3)}</strong></div><div class="pathDecision"><h3>${ast.direction}${ast.pct?` ${ast.pct}%`:''} this week</h3><small>Load calibration</small></div><p>Recovery, pain, completed load, efficiency and cardiac drift determine future distance and session load.</p><div class="pathFactorSummary"><div><small>Current cumulative factor</small><b>${ast.pathways.load.current.toFixed(3)}</b></div><div><small>Change this week</small><b>${signedFactorDelta(ast.pathways.load.weekChange)}</b></div><div><small>Since programme start</small><b>${signedFactorDelta(ast.pathways.load.sinceStart)}</b></div></div><div class="pathStats"><span>Provisional cumulative factor <b>${Number(calc.cumulativeFactor||calc.factor).toFixed(3)}</b></span><span>Temporary recovery modifier <b>${signedFactorDelta(calc.temporaryAdjustment||0)}</b></span><span>HRV <b>${recoveryImpact?`${recoveryImpact}%`:'0%'}</b></span><span>Training tolerance <b>${esc(ast.tolerance)}</b></span><span>Pain <b>${ast.pain.count?`${ast.pain.max}/10`:'No data'}</b></span></div><p class="pathAction">${esc(painAction)} ${esc(completionText)}.</p><details class="stateDetails"><summary>Show distance and load calculation</summary><div class="adjustmentCalcHeader appliedCalc"><b>${appliedTitle}</b><span>${ast.applied.cumulativeFactor.toFixed(3)}</span></div><p class="muted compact">The cumulative factor carries forward between weeks. HRV and pain are applied separately as temporary modifiers and are not compounded into long-term load tolerance.</p><div class="adjustmentCalcHeader"><b>${calcTitle}</b><span>Current estimate ${calc.factor.toFixed(3)}</span></div><div class="adjustmentCalcTable">${calcRows.map((i,idx)=>`<div class="adjustmentCalcRow"><span>${esc(i.name)}</span><b>${idx===0?Number(calc.baseFactor||1).toFixed(3):`${i.adjustment>=0?'+':''}${(i.adjustment*100).toFixed(0)}%`}</b><small>${esc(i.detail)}</small></div>`).join('')}<div class="adjustmentCalcTotal"><span>${ast.weekComplete?'Cumulative factor after weekly learning':'Provisional cumulative factor'}</span><b>${Number(calc.cumulativeFactor||calc.factor).toFixed(3)}</b><small>${ast.weekComplete?'Carried forward into the next week; temporary recovery modifiers are applied separately.':'Visible during the week for transparency; completed-load scoring is added only when the week closes.'} Limited to ${Number(state.setup.minFactor).toFixed(3)}–${Number(state.setup.maxFactor).toFixed(3)}.</small></div></div></details></section></div><section class="pathwayHistoryPanel"><div class="panelHead"><div><h4>Pathway capability history</h4><p class="muted compact">The graph shows cumulative learned capability only: Pace & Power and Distance & Load, each relative to the 1.000 programme baseline. Temporary readiness is deliberately excluded because it can change from week to week and is not learned capability.</p></div></div>${pathwayHistorySvg(ast.pathways.history)}</section><section class="planFactorUsePanel"><h4>What the plan currently uses</h4><div class="factorUseGrid"><article><small>Learned pace capability</small><b>${ast.pathways.pace.current.toFixed(3)}</b><p>Applied to future pace and power targets. Heart-rate targets remain unchanged.</p></article><article><small>Learned load capability</small><b>${ast.applied.cumulativeFactor.toFixed(3)}</b><p>The cumulative amount of distance and session load the model has learned you can tolerate.</p></article><article><small>Current readiness modifier</small><b>${(1+(ast.applied.temporaryAdjustment||0)).toFixed(3)}</b><p>Temporary HRV and pain moderation for this week; it does not change either cumulative pathway line.</p></article><article class="effectiveUse"><small>Effective load factor this week</small><b>${ast.applied.factor.toFixed(3)}</b><p>${ast.applied.cumulativeFactor.toFixed(3)} learned load capability × ${(1+(ast.applied.temporaryAdjustment||0)).toFixed(3)} readiness, bounded by plan safeguards.</p></article></div></section><div class="adjustmentSchedule"><b>When changes apply</b><p>Pace and power targets use the current cumulative Pace & Power factor ${ast.pathways.pace.current.toFixed(3)} whenever sufficient evidence crosses the 0.5% calibration threshold. Week ${ast.currentWeek} distance and session load use cumulative Distance & Load factor ${ast.applied.cumulativeFactor.toFixed(3)} multiplied by the current readiness modifier ${(1+(ast.applied.temporaryAdjustment||0)).toFixed(3)}, producing effective load factor ${ast.applied.factor.toFixed(3)}. The next load review is finalised after ${fmtDate(ast.reviewDate)} and applies from ${fmtDate(ast.nextStart)}.</p></div>`;
 }
 if(!state.weekView)state.weekView=currentWeek();let arr=state.plan.filter(p=>p.week===state.weekView),wd=weekData(state.weekView),afd=adaptiveFactorDetails(state.weekView),factor=arr[0]?.factor||afd.factor||1;let factorText=afd.status==='pending'?`cumulative load factor pending · currently ${factor.toFixed(2)}`:afd.status==='calculated'?`cumulative load factor ${Number(afd.cumulativeFactor||factor).toFixed(2)} · based on Week ${afd.previousWeek}`:`cumulative load factor ${Number(afd.cumulativeFactor||factor).toFixed(2)}`;$('weekHeader').innerHTML=`<b>Week ${state.weekView} · ${detailedPhase(state.weekView)}</b><br><span class="muted">${fmtDate(iso(weekStart(state.weekView)))} · ${wd.planned.toFixed(1)} km planned · ${wd.actual.toFixed(1)} km completed · ${factorText}</span>`;
...
function intervalFitAnalysis(laps,plan){
 ...
 if(!structured)return{available:true,structured:false,laps,quality:'lap data available',reason:'FIT lap data are available, but the run is not matched to a structured workout.'};
 const expected=Math.max(0,Number(plan.repetitions)||0),candidates=laps.filter(l=>Number(l.durationSec)>20&&Number(l.distanceKm)>.05);
 if(!candidates.length)return null;
 const medPower=median(candidates.map(x=>x.avgPower).filter(Number.isFinite)),medPace=median(candidates.map(x=>x.paceSecPerKm).filter(Number.isFinite));
 let work=candidates.filter(l=>(Number.isFinite(l.avgPower)&&Number.isFinite(medPower)&&l.avgPower>=medPower*.98)||(Number.isFinite(l.paceSecPerKm)&&Number.isFinite(medPace)&&l.paceSecPerKm<=medPace*1.02));
 if(expected>=2&&work.length>expected)work=work.slice().sort((a,b)=>{
   const az=Number.isFinite(a.avgPower)&&plan.zone?.power?Math.abs(a.avgPower-plan.zone.power)/plan.zone.power:Number.isFinite(a.paceSecPerKm)&&plan.zone?.pace?Math.abs(a.paceSecPerKm-plan.zone.pace)/plan.zone.pace:0;
   const bz=Number.isFinite(b.avgPower)&&plan.zone?.power?Math.abs(b.avgPower-plan.zone.power)/plan.zone.power:Number.isFinite(b.paceSecPerKm)&&plan.zone?.pace?Math.abs(b.paceSecPerKm-plan.zone.pace)/plan.zone.pace:0;
   return az-bz}).slice(0,expected).sort((a,b)=>a.index-b.index);
 if(expected>=2&&work.length<Math.max(2,Math.floor(expected*.6)))return{available:true,structured:true,laps,work:[],expectedReps:expected,quality:'limited',reason:`Only ${work.length} work laps could be identified confidently versus ${expected} prescribed.`};
 const targetPace=Number(plan.zone?.pace),targetPower=Number(plan.zone?.power);
 const rows=work.map((l,i)=>({...l,rep:i+1,paceScore:Number.isFinite(l.paceSecPerKm)&&targetPace>0?clamp(100-Math.abs(1-targetPace/l.paceSecPerKm)*145,45,100):null,powerScore:Number.isFinite(l.avgPower)&&targetPower>0?clamp(100-Math.abs(1-l.avgPower/targetPower)*140,45,100):null}));
 const scores=[avg(rows.map(x=>x.paceScore).filter(Number.isFinite)),avg(rows.map(x=>x.powerScore).filter(Number.isFinite))].filter(Number.isFinite);
 const cut=Math.max(1,Math.floor(rows.length/2)),first=rows.slice(0,cut),last=rows.slice(cut);
 const mean=(a,k)=>avg(a.map(x=>Number(x[k])).filter(Number.isFinite)),p1=mean(first,'avgPower'),p2=mean(last,'avgPower'),t1=mean(first,'paceSecPerKm'),t2=mean(last,'paceSecPerKm');
 const ratios=rows.map(x=>Number.isFinite(x.avgPower)&&targetPower>0?x.avgPower/targetPower:Number.isFinite(x.paceSecPerKm)&&targetPace>0?targetPace/x.paceSecPerKm:null).filter(Number.isFinite),mr=avg(ratios);
 const variance=mr&&ratios.length>1?avg(ratios.map(v=>(v-mr)**2)):null;
 return{available:true,structured:true,laps,work:rows,expectedReps:expected,repScore:scores.length?Math.round(avg(scores)):null,fadePower:Number.isFinite(p1)&&p1>0&&Number.isFinite(p2)?(p2/p1-1)*100:null,fadePace:Number.isFinite(t1)&&t1>0&&Number.isFinite(t2)?(t2/t1-1)*100:null,consistencyCv:variance!=null?Math.sqrt(variance)/mr*100:null,quality:rows.length>=Math.max(2,expected||2)?'high':'moderate',reason:`${rows.length}${expected?` of ${expected}`:''} work repetitions identified from FIT laps.`};
}
function refreshIntervalAnalysis(run,plan){if(!run?.fitLaps?.length)return null;return run.intervalAnalysis=intervalFitAnalysis(run.fitLaps,plan)}
...
function postRunCoachUpdate(run,before,after){
 ...
 const appliedPaceFactor=after.paceFactor||1,provisionalPaceFactor=after.paceProvisional||appliedPaceFactor,paceRatio=provisionalPaceFactor/appliedPaceFactor;
 const prescriptionChanges=(after.upcoming||[]).map(a=>{const hypotheticalPace=Number(a.pace)/Math.max(.8,paceRatio),hypotheticalPower=Number(a.power)*paceRatio;return{id:a.id,date:a.date,type:a.type,beforeDistance:a.distance,afterDistance:a.distance,distanceDeltaKm:0,beforePace:a.pace,afterPace:hypotheticalPace,paceDeltaSec:hypotheticalPace-Number(a.pace),beforePower:a.power,afterPower:hypotheticalPower,powerDeltaW:hypotheticalPower-Number(a.power),week:a.week}}).filter(Boolean);
 const changedPrescriptions=prescriptionChanges.filter(x=>Math.abs(x.paceDeltaSec)>=.5||Math.abs(x.powerDeltaW)>=1);
...
function intervalAnalysisHtml(r){
 ...
 if(!a.work?.length)return`<section class="intervalAnalysisCard"><div class="intervalAnalysisHead"><div><span>INTERVAL-LEVEL FIT ANALYSIS</span><h3>Lap evidence limited</h3></div><b>${esc(a.quality||'Limited')}</b></div><p>${esc(a.reason||'Work repetitions could not be mapped confidently.')}</p></section>`;
 const rows=a.work.map(x=>`<div class="intervalRepRow"><strong>Rep ${x.rep}</strong><span>${Number.isFinite(x.distanceKm)?x.distanceKm.toFixed(2)+' km':'—'}</span><span>${Number.isFinite(x.paceSecPerKm)?pace(x.paceSecPerKm):'—'}</span><span>${Number.isFinite(x.avgPower)?Math.round(x.avgPower)+' W':'—'}</span><span>${Number.isFinite(x.avgHr)?Math.round(x.avgHr)+' bpm':'—'}</span></div>`).join('');
 const fade=Number.isFinite(a.fadePower)?`Power ${a.fadePower>=0?'+':''}${a.fadePower.toFixed(1)}%`:Number.isFinite(a.fadePace)?`Pace ${a.fadePace>=0?'+':''}${a.fadePace.toFixed(1)}%`:'—';
 return`<section class="intervalAnalysisCard"><div class="intervalAnalysisHead"><div><span>INTERVAL-LEVEL FIT ANALYSIS</span><h3>${a.work.length}${a.expectedReps?` / ${a.expectedReps}`:''} work reps identified</h3></div><b>${Number.isFinite(a.repScore)?a.repScore+'/100':'Analysed'}</b></div><div class="intervalSummary"><div><small>Rep execution</small><strong>${Number.isFinite(a.repScore)?a.repScore+'/100':'—'}</strong></div><div><small>Consistency</small><strong>${Number.isFinite(a.consistencyCv)?a.consistencyCv.toFixed(1)+'% CV':'—'}</strong></div><div><small>Late-rep change</small><strong>${fade}</strong></div></div><div class="intervalRepTable"><div class="intervalRepHeader"><b>Rep</b><b>Distance</b><b>Pace</b><b>Power</b><b>HR</b></div>${rows}</div><p class="muted compact">${esc(a.reason)} Sufficient lap evidence replaces whole-run pace/power averages in execution scoring.</p></section>`;
}
...
function runExecutionBreakdownHtml(r){
 ...
 const intervalHtml=intervalAnalysisHtml(r);
...
function rehabAdherenceSummary(i,days=7){
 const cutoff=new Date(today().getTime()-(days-1)*DAY),planStart=dte(rehabPlanStart(i)),checks=sortedChecks(i).filter(c=>dte(c.date)>=cutoff&&dte(c.date)>=planStart&&dte(c.date)<=today());
