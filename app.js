...
function raceShoePlan(){const active=(state.shoes||[]).filter(s=>s.status!=='retired'),raceDate=state.setup?.raceDate;if(!raceDate)return{status:'no-race',confidence:'Low'};const racePlan={id:'race-shoe-eval',type:'Race Day',date:raceDate,distance:Number(state.setup?.raceDistance)||0,surface:'road'};const ranked=shoeRecommendation(racePlan).ranked.filter(x=>(x.profile.roles||[]).includes('race'));let preferred=ranked[0]?.shoe||null,condition=preferred?raceShoeCondition(preferred):null;let purchaseNeeded=!preferred||['replace','high-wear','unsuitable'].includes(condition?.code);let reason='';if(!preferred)reason='No owned active shoe is sufficiently race-suitable.';else if(condition.code==='replace'||condition.code==='high-wear')reason='The best owned race shoe is projected to be too worn or close to its expected service range by race day.';else if(condition.code==='unsuitable')reason='The current rotation does not contain an appropriate race-day shoe.';const profile=purchaseNeeded?recommendedPurchaseProfile():null;const targetWindow=profile?raceOptimalWindow(profile,state.setup.raceDistance):condition?.opt;const ownedTarget=condition&&['too-new','familiarise'].includes(condition.code)?Math.max(0,condition.opt.minKm-condition.fam.km):0;const targetKm=purchaseNeeded?Math.max(20,((Number(targetWindow?.minKm)||25)+(Number(targetWindow?.maxKm)||75))/2):ownedTarget;const famSessions=(purchaseNeeded||preferred)?raceFamiliarisationSessions(preferred||{id:'planned',brand:'ASICS',model:profile?.family,version:profile?.version,characteristics:{roles:profile?.roles}},targetKm):[];const first=famSessions[0]?.date||null;let purchaseStart=null,purchaseEnd=null;if(purchaseNeeded&&first){purchaseEnd=iso(new Date(dte(first).getTime()-3*DAY));purchaseStart=iso(new Date(dte(first).getTime()-21*DAY));if(purchaseStart<iso(today()))purchaseStart=iso(today())}else if(purchaseNeeded){const rd=dte(raceDate);purchaseEnd=iso(new Date(rd.getTime()-28*DAY));purchaseStart=iso(new Date(rd.getTime()-49*DAY));if(purchaseStart<iso(today()))purchaseStart=iso(today())}const plannedKm=famSessions.reduce((a,x)=>a+Number(x.plannedShoeKm||0),0),projectedNew=plannedKm;let confidence=preferred&&condition?.fam.runs>=3?'High':'Medium';if(state.activeInjuryPlanId)confidence=confidence==='High'?'Medium':'Low';if(purchaseNeeded&&!famSessions.length)confidence='Low';const existingPlanned=(state.plannedShoePurchases||[]).find(x=>x.status==='planned'&&x.intendedRole==='race')||null;return{status:'ok',preferred,condition,ranked,purchaseNeeded,reason,profile,targetWindow,targetKm,famSessions,plannedKm,projectedNew,purchaseStart,purchaseEnd,confidence,existingPlanned}}
...
function todayShoeRowHtml(plan){if(!plan||(state.shoes||[]).filter(s=>s.status!=='retired').length===0)return'';const rec=shoeRecommendationForPlan(plan);if(!rec.best)return'';return`<div class="todayShoeRow"><span>${todayPictogram('shoe')}</span><div><small>SHOE</small><b>${esc(shoeDisplayName(rec.best.shoe))}</b><p>Recommended for today’s ${esc(plan.type)}</p></div><details><summary>Why this shoe?</summary><ul>${rec.best.reasons.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>${rec.best.warning?`<p class="shoeWarning">${esc(rec.best.warning)}</p>`:''}${rec.alternative?`<p><b>Alternative:</b> ${esc(shoeDisplayName(rec.alternative.shoe))}</p>`:''}</details></div>`}
function planShoeRowHtml(plan){if(!plan||plan.type==='Rest'||(state.shoes||[]).filter(s=>s.status!=='retired').length===0)return'';const rec=shoeRecommendationForPlan(plan);if(!rec.best)return'';return`<div class="planShoeRow"><span>${todayPictogram('shoe')}</span><div><small>SHOE</small><b>Recommended: ${esc(shoeDisplayName(rec.best.shoe))}</b>${rec.alternative?`<p>Alternative: ${esc(shoeDisplayName(rec.alternative.shoe))}</p>`:''}<small>${rec.best.label}${rec.manual?' · manually selected':''}</small></div><button type="button" class="secondary small" data-choose-plan-shoe="${esc(plan.id)}">Choose another shoe</button></div>`}
...
function shoeGraphRationaleHtml(series,events,raceLifecycle){const rows=series.map(item=>{const current=shoeMileage(item.shoe),planned=item.planned.at(-1)?.km??current,added=Math.max(0,planned-current),event=events.find(e=>e.shoe.id===item.shoe.id),f=shoeForecast(item.shoe);return`<li><b>${esc(shoeDisplayName(item.shoe))}:</b> ${Math.round(current)} km accumulated today. The current rotation assigns ~${Math.round(added)} km to this exact pair before ${event?'its planned replacement':'race day'}.${event?` It is replaced on ${esc(shoeChartDate(event.date))} because the assigned programme first takes it into the expected ${Math.round(f.low)}–${Math.round(f.high)} km replacement range.`:' It does not reach its planned replacement threshold before race day.'}</li>`}).join('');const race=raceLifecycle?.purchaseNeeded&&raceLifecycle.profile?`<li><b>Race-day pair:</b> proposed ASICS ${esc(raceLifecycle.profile.family)} ${esc(raceLifecycle.profile.version)}. The current best race-capable pair is not carried through to Race Day when it is projected to be too worn. The new race-day pair is targeted at ${Math.round(raceLifecycle.targetWindow?.minKm||0)}–${Math.round(raceLifecycle.targetWindow?.maxKm||0)} km, with ~${Math.round(raceLifecycle.projectedNew||0)} km of planned familiarisation by ${esc(shoeChartDate(state.setup?.raceDate))}.</li>`:raceLifecycle?.preferred&&raceLifecycle.condition?`<li><b>Race-day pair:</b> ${esc(shoeDisplayName(raceLifecycle.preferred))}. It remains the intended Race Day pair only because its planned familiarisation keeps it inside the desired ${Math.round(raceLifecycle.condition.opt.minKm)}–${Math.round(raceLifecycle.condition.opt.maxKm)} km window.</li>`:'';return`<details class="shoeGraphRationale" open><summary>How the shoe plan gets you to Race Day</summary><div><p>This graph now uses <b>one replacement rule only</b>: the planned replacement date is the first future assigned workout that takes the current pair into its expected replacement-mileage range. The separate usage-rate estimate is deliberately not shown here because it is only a fallback forecast and made the lifecycle plan harder to interpret.</p><p>When a current pair is replaced, its line stops. The proposed replacement pair starts at 0 km and takes the later workouts that had been assigned to that role. The <b>race-day pair is planned separately from training replacements</b>. If an owned race-capable pair would be too worn, it is not projected forward as the Race Day pair; a new pair is introduced later and receives only suitable familiarisation mileage.</p><ul>${rows}${race}</ul></div></details>`}
function shoeProgrammeMileageChartHtml(){
 ...
 const enabled=new Set((state.shoeChartVisible||shoes.map(s=>s.id)).filter(id=>shoes.some(s=>s.id===id)));if(!enabled.size)shoes.forEach(s=>enabled.add(s.id));
 ...
 const series=shoes.filter(s=>enabled.has(s.id)).map(s=>{const actual=shoeActualProgrammeSeries(s,programStart,todayStr),rawPlanned=shoePlannedMileageSeries(s),event=eventByShoe.get(s.id),planned=event?rawPlanned.filter(p=>p.date<=event.date):rawPlanned;return{shoe:s,actual,planned}});
 const raceLifecycle=raceShoePlan(),targetWindow=raceLifecycle?.targetWindow||raceLifecycle?.condition?.opt||null;
 const replacementSeries=rawEvents.filter(e=>enabled.has(e.shoe.id)&&e.replacement).map(e=>{let km=0;const pts=[{date:e.date,km:0}];(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>e.date&&p.date<=raceDate).slice().sort((a,b)=>a.date.localeCompare(b.date)).forEach(p=>{if(plannedAssignment(p.id)?.shoeId!==e.shoe.id)return;km+=Number(p.distance)||0;pts.push({date:p.date,km})});if(pts.at(-1)?.date!==raceDate)pts.push({date:raceDate,km});return{event:e,points:pts,finalKm:km}});
 let raceStrategy=null;if(raceLifecycle?.purchaseNeeded&&raceLifecycle.profile){const first=raceLifecycle.famSessions?.[0]?.date||raceLifecycle.purchaseEnd||raceLifecycle.purchaseStart||todayStr,pts=[{date:first,km:0}];let km=0;(raceLifecycle.famSessions||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)).forEach(p=>{if(p.date<first||p.date>raceDate)return;km+=Number(p.plannedShoeKm)||0;pts.push({date:p.date,km})});if(pts.at(-1)?.date!==raceDate)pts.push({date:raceDate,km});raceStrategy={name:`ASICS ${raceLifecycle.profile.family} ${raceLifecycle.profile.version}`,owned:false,points:pts,finalKm:km,window:targetWindow}}else if(raceLifecycle?.preferred&&raceLifecycle.condition){const shoe=raceLifecycle.preferred,base=shoeMileage(shoe),pts=[{date:todayStr,km:base}];let km=base;(raceLifecycle.famSessions||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)).forEach(p=>{if(p.date<todayStr||p.date>raceDate)return;km+=Number(p.plannedShoeKm)||0;pts.push({date:p.date,km})});const projected=km;if(pts.at(-1)?.date!==raceDate)pts.push({date:raceDate,km:projected});raceStrategy={name:shoeDisplayName(shoe),owned:true,points:pts,finalKm:projected,window:raceLifecycle.condition.opt}};
 const dates=[programStart,todayStr,raceDate,...series.flatMap(x=>x.actual.map(p=>p.date)),...series.flatMap(x=>x.planned.map(p=>p.date)),...replacementSeries.flatMap(x=>x.points.map(p=>p.date)),...(raceStrategy?.points||[]).map(p=>p.date)].filter(Boolean).sort(),values=[...series.flatMap(x=>[...x.actual,...x.planned].map(p=>p.km)),...replacementSeries.flatMap(x=>x.points.map(p=>p.km)),...(raceStrategy?.points||[]).map(p=>p.km)].filter(Number.isFinite);if(targetWindow)values.push(Number(targetWindow.maxKm)||0);if(!values.length)return'<div class="shoeEmpty">No tracked or planned mileage yet.</div>';
 const minD=dte(dates[0]).getTime(),maxD=Math.max(minD+DAY,dte(raceDate).getTime()),maxY=Math.max(10,Math.ceil(Math.max(...values)*1.16/50)*50),W=920,H=560,L=98,R=34,T=112,B=94,x=d=>L+(dte(d).getTime()-minD)/(maxD-minD)*(W-L-R),y=v=>T+(maxY-v)/maxY*(H-T-B),palette=['#3dd6c6','#79d69b','#67a7ff','#f1c56a','#b9a3ff','#ff9f9f','#7ee7ff'];
 let grid='';for(let i=0;i<=4;i++){const v=maxY*i/4,yy=y(v);grid+=`<line x1="${L}" y1="${yy}" x2="${W-R}" y2="${yy}"/><text x="${L-18}" y="${yy+7}" text-anchor="end">${Math.round(v)}</text>`}
 const raceX=x(raceDate),todayX=x(todayStr),targetBand=targetWindow?(()=>{const low=Math.max(0,Number(targetWindow.minKm)||0),high=Math.min(maxY,Number(targetWindow.maxKm)||0),yy1=y(high),yy2=y(low),bandX=Math.max(L,raceX-92),name=raceStrategy?.name||'Race shoe';return`<g class="shoeRaceTargetBand"><rect x="${bandX}" y="${yy1}" width="${Math.max(18,raceX-bandX)}" height="${Math.max(5,yy2-yy1)}"/><text x="${raceX-8}" y="${Math.max(T+22,yy1-10)}" text-anchor="end">Race target · ${esc(name)} · ${Math.round(low)}–${Math.round(high)} km</text></g>`})():'';
 ...
 const replacementProjection=replacementSeries.map(item=>{const e=item.event,c=palette[Math.max(0,shoes.findIndex(s=>s.id===e.shoe.id))%palette.length],label=`Training replacement · ${e.replacement.family} ${e.replacement.version}`;return`<polyline class="shoeLine shoeNewPairLine" style="--shoe-color:${c}" points="${item.points.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ')}"/><text class="shoeNewPairLabel" x="${raceX-8}" y="${Math.max(T+30,y(item.finalKm)-10)}" text-anchor="end">${esc(label)} · ~${Math.round(item.finalKm)} km at race day</text>`}).join('');
 const raceStrategyPath=raceStrategy?`<polyline class="shoeLine shoeRaceStrategyLine" points="${raceStrategy.points.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ')}"/><circle class="shoeRaceStrategyPoint" cx="${raceX}" cy="${y(raceStrategy.finalKm)}" r="8"/><text class="shoeRaceStrategyLabel" x="${raceX-10}" y="${Math.max(T+34,y(raceStrategy.finalKm)-14)}" text-anchor="end">RACE-DAY PAIR · ${esc(raceStrategy.name)} · ~${Math.round(raceStrategy.finalKm)} km</text>`:'';
 const markers=rawEvents.filter(e=>enabled.has(e.shoe.id)&&e.date>=dates[0]).map((e,i)=>{const xx=x(e.date),yy=Math.max(T+12,y(Math.min(maxY,e.km))),labelY=34+(i%2)*26;return`<g class="shoeReplacementMarker"><line x1="${xx}" y1="${T}" x2="${xx}" y2="${H-B}"/><circle cx="${xx}" cy="${yy}" r="8"/><text x="${Math.min(W-R-4,Math.max(L+4,xx))}" y="${labelY}" text-anchor="${xx>W*.72?'end':xx<W*.28?'start':'middle'}">Replace ${esc(e.shoe.model)} · ${esc(shoeChartDate(e.date))}</text></g>`}).join('');
 const xlabels=[programStart,todayStr,raceDate].filter((v,i,a)=>v&&a.indexOf(v)===i).map(d=>`<text class="xLab" x="${x(d)}" y="${H-28}" text-anchor="${d===programStart?'start':d===raceDate?'end':'middle'}">${d===todayStr?'Today · '+shoeChartDate(d):d===raceDate?'Race day · '+shoeChartDate(d):shoeChartDate(d)}</text>`).join('');
 const legend=shoes.map((s,i)=>`<label class="shoeLegendToggle"><input type="checkbox" data-shoe-chart-toggle="${esc(s.id)}" ${enabled.has(s.id)?'checked':''}><i style="--shoe-color:${palette[i%palette.length]}"></i><span>${esc(shoeDisplayName(s))}</span></label>`).join('');
 const eventHtml=rawEvents.length?`<div class="shoeLifecycleEvents">${rawEvents.map(e=>{const f=shoeForecast(e.shoe);return`<article><small>PLANNED REPLACEMENT · ${esc(shoeChartDate(e.date).toUpperCase())}</small><b>Replace ${esc(shoeDisplayName(e.shoe))}</b><span>The assigned programme first takes this pair into its expected ${Math.round(f.low)}–${Math.round(f.high)} km replacement range here.</span>${e.replacement?`<strong>Proposed new pair: ASICS ${esc(e.replacement.family)} ${esc(e.replacement.version)}</strong>`:'<strong>Choose a suitable replacement for the same role.</strong>'}</article>`}).join('')}</div>`:'<div class="shoeEmpty compact">No current pair is planned to reach its replacement range before Race Day.</div>';
 const raceStrategyCard=raceStrategy?`<div class="shoeRaceStrategyCard"><small>RACE-DAY PAIR STRATEGY</small><b>${esc(raceStrategy.name)}</b><span>${raceStrategy.owned?'Current pair retained for Race Day':'New pair proposed for Race Day'} · projected ~${Math.round(raceStrategy.finalKm)} km on ${esc(shoeChartDate(raceDate))}</span><strong>Desired race-day window ${Math.round(raceStrategy.window?.minKm||0)}–${Math.round(raceStrategy.window?.maxKm||0)} km</strong>${raceLifecycle.purchaseNeeded&&raceLifecycle.purchaseStart&&raceLifecycle.purchaseEnd?`<em>Buy ${esc(shoeChartDate(raceLifecycle.purchaseStart))} – ${esc(shoeChartDate(raceLifecycle.purchaseEnd))}, then use the planned familiarisation sessions shown by the race-shoe curve.</em>`:''}</div>`:'';
 return`<div class="shoeChartWrap shoeProgrammeChart"><div class="shoeChartRaceDate"><small>SHOE PLAN THROUGH RACE DAY</small><b>Race day · ${esc(shoeChartDate(raceDate))}</b></div>${raceStrategyCard}<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Current shoes, planned replacements and intended race-day shoe mileage through race day">${targetBand}<g class="shoeGrid">${grid}</g><line class="shoeTodayLine" x1="${todayX}" y1="${T}" x2="${todayX}" y2="${H-B}"/><line class="shoeRaceLine" x1="${raceX}" y1="${T}" x2="${raceX}" y2="${H-B}"/>${paths}${replacementProjection}${raceStrategyPath}${markers}${xlabels}<text class="yTitle" transform="translate(28 ${H/2}) rotate(-90)" text-anchor="middle">Accumulated km</text></svg><div class="shoeChartKey"><span><i class="actual"></i>Observed mileage</span><span><i class="planned"></i>Planned mileage on current pair</span><span><i class="replace"></i>Planned replacement</span><span><i class="newpair"></i>Proposed training replacement</span><span><i class="raceshoe"></i>Planned race-day pair</span><span><i class="raceband"></i>Desired race-day mileage window</span></div><div class="shoeChartLegend">${legend}</div>${shoeGraphRationaleHtml(series,rawEvents,raceLifecycle)}${eventHtml}</div>`}
function shoeMileageChartHtml(){return shoeProgrammeMileageChartHtml()}
function shoeRotationCard(shoe){const f=shoeForecast(shoe),p=shoeProfileForShoe(shoe),primary=(p.roles||[])[0]||'mixed';return`<article class="shoeRotationCard ${shoeStatusClass(f.status)}" data-shoe-detail="${esc(shoe.id)}" tabindex="0"><div class="shoeCardHead"><span>${todayPictogram('shoe')}</span><div><small>${esc(shoe.brand||'')}</small><h4>${esc([shoe.model,shoe.version].filter(Boolean).join(' '))}</h4>${shoe.nickname?`<p>${esc(shoe.nickname)}</p>`:''}</div><span class="shoeStatus ${shoeStatusClass(f.status)}">${esc(f.label)}</span></div><div class="shoeCardMetrics"><div><small>CURRENT MILEAGE</small><b>${Math.round(f.km)} km</b></div><div><small>PRIMARY USE</small><b>${esc(SHOE_ROLE_LABELS[primary]||primary)}</b></div><div><small>EXPECTED WINDOW</small><b>${Math.round(f.low)}–${Math.round(f.high)} km</b></div><div><small>EST. REMAINING</small><b>~${Math.round((f.remainingLow+f.remainingHigh)/2)} km</b></div></div><div class="shoeCardFooter"><span>Projected replacement</span><b>${esc(shoeForecastDateText(f))}</b></div></article>`}
