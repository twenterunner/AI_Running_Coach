...
const BUILD=10300, SCHEMA=10300, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function pathwayFactorHistory(asOfWeek=currentWeek()){
 ...
 for(let w=1;w<=endWeek;w++){
  ...
  const load=adaptiveFactorDetails(w).factor;
...
function renderPlan(){
 const ast=athleteState(state.weekView||currentWeek());if($('fitnessEvidence')){
  ...
  const calcRows=[{name:'Starting factor',adjustment:0,detail:'Each weekly load calculation starts at 1.000; its history shows the weekly modifier rather than a cumulative multiplier.'},...(calc.items||[])];
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v10.0.30 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v10.0.30 stable build 10300');
})();