...
const BUILD=9340, SCHEMA=9340, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function workoutScore(run,plan=run?.planId?state.plan.find(p=>p.id===run.planId):null){if(!run)return null;const actualKm=Number(run.distanceKm)||0,dur=Number(run.durationSec)||0;if(!(actualKm>0&&dur>0))return null;const plannedKm=Math.max(.1,Number(plan?.distance)||actualKm),completion=clamp(actualKm/plannedKm*100,0,110);let components=[{v:completion,w:.38}],actualPace=dur/actualKm;if(plan?.zone?.pace>0){let ratio=plan.zone.pace/actualPace;components.push({v:clamp(100-Math.abs(1-ratio)*160,45,105),w:.20})}if(Number(run.avgPower)>0&&plan?.zone?.power>0){let ratio=Number(run.avgPower)/plan.zone.power;components.push({v:clamp(100-Math.abs(1-ratio)*150,45,105),w:.22})}if(Number(run.avgHr)>0&&plan?.zone?.hr>0){let excess=(Number(run.avgHr)-plan.zone.hr)/Math.max(1,plan.zone.hr);components.push({v:clamp(100-excess*180,55,105),w:.10})}if(Number.isFinite(Number(run.powerDrift)))components.push({v:clamp(100-Math.max(0,Number(run.powerDrift)-2)*6,55,102),w:.10});let ws=sum(components.map(c=>c.v*c.w))/sum(components.map(c=>c.w));if(Number(run.pain)>=5)ws=Math.min(ws,70);else if(Number(run.pain)>=3)ws=Math.min(ws,82);return Math.round(clamp(ws,0,100))}
...
function athleteState(w=currentWeek()){
 const current=Math.max(1,Math.min(weeks(),w)),next=Math.min(weeks(),current+1),ev=trainingEvidence(),applied=adaptiveFactorDetails(current),nextAfd=adaptiveFactorDetails(next),pain=recoveryPainState(),hrv=hrvModel();
 ...
 const adjustment=nextAfd.factor,isPending=nextAfd.status==='pending',direction=isPending?'Pending review':adjustment<.995?'Reduce':adjustment>1.005?'Increase':'Maintain',pct=isPending?0:Math.round(Math.abs(adjustment-1)*100),reasons=[];
 if(isPending)reasons.push(`Week ${current} is still in progress; no completion penalty is applied before the review`);
 else nextAfd.items.forEach(i=>{if(Math.abs(Number(i.adjustment)||0)>.0001)reasons.push(`${i.name.toLowerCase()} ${i.adjustment>0?'+':''}${Math.round(i.adjustment*100)}%`)});
 if(!reasons.length)reasons.push('all calculated inputs are neutral');
 const nextStart=weekStart(next),reviewDate=iso(new Date(nextStart.getTime()-DAY));
 return{currentWeek:current,nextWeek:next,fitnessTrend,fitnessIndex:ev.index,fitnessDelta,evidenceConfidence:ev.confidence,fitnessEvidence:ev,readiness,tolerance,completion,weekComplete,adjustment,direction,pct,reasons,applied,nextAfd,isPending,pain,hrv,reviewDate,nextStart:iso(nextStart)};
}
...
function evidenceBasedCoach(engine){
 const c=engine.c,ast=athleteState(engine.cw),components=uniqueComponents(c.components.filter(i=>i.hasEvidence&&Number.isFinite(i.displayScore)));
 ...
 let conclusion;
 const planDecision=ast.isPending?`The next weekly plan adjustment is pending the end-of-week review; the current week remains at factor ${ast.applied.factor.toFixed(2)}.`:`The next weekly plan adjustment is ${ast.adjustment.toFixed(2)} (${ast.direction}${ast.pct?` ${ast.pct}%`:''}).`;
 if(recoveryConstraint)conclusion=`Recovery or pain evidence currently limits safe progression. ${planDecision} ${current}`;
 else if(opportunities[0])conclusion=`${planDecision} ${current} The highest-priority verified opportunity is ${opportunities[0].name.toLowerCase()}.`;
 else conclusion=`${planDecision} ${current} No major evidence-backed limiter is currently identified; preserve consistency and recovery.`;
 let actionsList=[];
 ...
 return{conclusion,current,projected,strengths,opportunities,actions:actionsList,evidenceCoverage:Math.round(c.evidenceCoverage*100),recoveryConstraint,athleteState:ast};
}
...
function coachReportHtml(report,compact=false){
 ...
 const ast=report.athleteState,decision=ast.isPending?'Pending review':`${ast.adjustment.toFixed(2)} · ${ast.direction}${ast.pct?` ${ast.pct}%`:''}`;
 return `<div class="coachReport ${compact?'compactReport':''}"><div class="coachVerdict"><span>Evidence-based assessment</span><p class="coachConclusion">${esc(report.conclusion)}</p><p class="coachProjection">${esc(report.projected)}</p><small>Evidence coverage ${report.evidenceCoverage}% · Statements below use only logged, configured or plan-linked data.</small></div><div class="coachStateStrip"><div><span>Fitness targets</span><b>${ast.fitnessDelta>=0?'+':''}${ast.fitnessDelta.toFixed(1)}%</b></div><div><span>Recovery</span><b>${esc(ast.readiness)}</b></div><div><span>Pain</span><b>${ast.pain.count?`${ast.pain.max}/10`:'No data'}</b></div><div><span>Weekly plan</span><b>${esc(decision)}</b></div></div><div class="coachEvidenceGrid"><section><h4>Verified strengths</h4>${strengths}</section><section><h4>Priority opportunities</h4>${opportunities}</section></div><section class="coachActions"><h4>Next actions</h4>${report.actions.map((a,i)=>`<div class="coachAction"><strong>${i+1}</strong><div><b>${esc(a.title)}</b><p>${esc(a.text)}</p><small>Based on: ${esc(a.source)}</small></div></div>`).join('')}</section></div>`;
}
...
function renderPlan(){
 ...
 const calc=ast.nextAfd.status==='calculated'?ast.nextAfd:ast.applied;
 const calcTitle=ast.nextAfd.status==='calculated'?`Week ${ast.nextWeek} calculation`:`Current Week ${ast.currentWeek} applied calculation`;
 const calcRows=[{name:'Starting factor',adjustment:0,detail:'Every weekly calculation starts at 1.00.'},...(calc.items||[])];
 const evidenceRows=(evidence.events||[]).slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,8);
 const confidenceMeaning=evidence.confidence<20?'Very limited evidence; targets are held stable.':evidence.confidence<50?'Early evidence; only small, accumulated changes can affect targets.':evidence.confidence<80?'Moderate evidence; recent performance can meaningfully calibrate targets.':'Strong evidence; the target calibration is well supported by recent training and tests.';
 $('fitnessEvidence').innerHTML=`<div class="athleteStateGrid"><div><span>Fitness calibration</span><strong>${fitnessTarget>=0?'+':''}${fitnessTarget.toFixed(1)}%</strong><small>${ast.fitnessTrend} · applied to future pace and power targets</small></div><div><span>Fitness evidence confidence</span><strong>${ast.evidenceConfidence}%</strong><small>${esc(confidenceMeaning)}</small></div><div><span>Recovery effect</span><strong>${recoveryImpact?`${recoveryImpact}% load`:'0% load change'}</strong><small>${ast.hrv.ready?`${Math.round(ast.hrv.rolling)} ms recent vs ${Math.round(ast.hrv.baseline)} ms baseline`:'No HRV baseline yet; no HRV penalty applied'}</small></div><div><span>Injury action</span><strong>${ast.pain.count?`Pain max ${ast.pain.max}/10`:'No pain data'}</strong><small>${painAction}</small></div><div><span>Training tolerance</span><strong>${ast.tolerance}</strong><small>${completionText}</small></div><div class="stateDecision"><span>Next-week adjustment</span><strong>${ast.isPending?'Pending review':`${ast.adjustment.toFixed(2)} · ${ast.direction}${ast.pct?` ${ast.pct}%`:''}`}</strong><small>${esc(ast.reasons.join(' · '))}</small></div></div><div class="adjustmentSchedule"><b>When it updates</b><p>Current Week ${ast.currentWeek} uses factor ${ast.applied.factor.toFixed(2)}. The next calculation is finalised after ${fmtDate(ast.reviewDate)} and applies from ${fmtDate(ast.nextStart)} to Week ${ast.nextWeek}. Runs, pain and HRV logged before then feed that review. Pace and power targets update separately when accumulated fitness evidence produces at least a 0.5% calibrated change.</p></div><details class="stateDetails"><summary>How the weekly adjustment is calculated</summary><div class="adjustmentCalcHeader"><b>${calcTitle}</b><span>Result ${calc.factor.toFixed(2)}</span></div><div class="adjustmentCalcTable">${calcRows.map((i,idx)=>`<div class="adjustmentCalcRow"><span>${esc(i.name)}</span><b>${idx===0?'1.00':`${i.adjustment>=0?'+':''}${(i.adjustment*100).toFixed(0)}%`}</b><small>${esc(i.detail)}</small></div>`).join('')}<div class="adjustmentCalcTotal"><span>Bounded final factor</span><b>${calc.factor.toFixed(2)}</b><small>Limited to the configured range ${Number(state.setup.minFactor).toFixed(2)}–${Number(state.setup.maxFactor).toFixed(2)}.</small></div></div>${ast.isPending?`<p class="muted compact">The next-week result is not final yet. Current-week completion is shown for awareness but is not penalised before the week closes.</p>`:''}</details><details class="stateDetails"><summary>How to interpret ${ast.evidenceConfidence}% fitness evidence confidence</summary><p>${esc(confidenceMeaning)}</p><div class="confidenceBands"><span><b>0–19%</b> Very limited</span><span><b>20–49%</b> Early</span><span><b>50–79%</b> Moderate</span><span><b>80–100%</b> Strong</span></div><p class="muted compact">Confidence measures how much weighted evidence has accumulated since the ${esc(evidence.anchorType.toLowerCase())} on ${fmtDate(evidence.anchorDate)}. It is not a fitness score and does not mean the app is “4% sure” that you are fit.</p><div class="evidenceEventList">${evidenceRows.length?evidenceRows.map(e=>`<div><span>${fmtDate(e.date)} · ${esc(e.type)}</span><b>Workout ${Math.round(e.score)}/100</b><small>${(e.confidence/4*100).toFixed(1)} percentage points of confidence · fitness signal ${e.delta>=0?'+':''}${e.delta.toFixed(2)}</small></div>`).join(''):'<p class="muted">No qualifying completed-run evidence has accumulated since the current baseline.</p>'}</div></details>`;
 }
...
function renderRecovery(){
 ...
 const recoveryChange=Math.round((hrv.factor-1)*100);
 const painNote=(pain.max??0)>=5?'Pain is high enough to restrict demanding training.':(pain.max??0)>=3?'Pain should be monitored and may limit progression.':'Pain currently adds no reduction.';
 $('recoveryAdaptive').innerHTML=`<div class="recoveryContribution"><span>Contribution to next weekly review</span><strong class="${recoveryChange<0?'negative':''}">${recoveryChange?`${recoveryChange}%`:'0%'}</strong><p>${esc(hrv.detail)} ${esc(painNote)}</p><small>The complete Weekly Plan Adjustment, including completed load, performance and calculation timing, is shown once on the Plan tab.</small></div>`;
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.3.4 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.3.4 stable build 9340');
})();