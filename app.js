...
function buildPlan(){
 ...
 for(let w=1;w<=total;w++){
   ...
   base*=factor;
   let prog=clamp((w-1)/Math.max(1,total-taper-2),0,1),long=state.setup.currentLongest+(state.setup.peakLong-state.setup.currentLongest)*prog;
   if(w%4===0&&ph==='Build')long*=.85;
   if(ph==='Taper'){let left=total-w;long=Math.min(long,left===0?12:left===1?18:24)}
   long=Math.min(long,base*.62);

   // Resolve each calendar date to its actual weekday configuration.
   let weekDates=[];
   ...
   let qualityCount=training.filter(x=>['Intervals','Tempo'].includes(x.cfg[2])).length;
   let easyCount=training.filter(x=>x.cfg[2]==='Easy').length;
   let remaining=isRaceWeek?0:Math.max(0,base-long);
   let qualityShare=qualityCount?remaining*.48/qualityCount:0,easyShare=easyCount?remaining*.52/easyCount:0;
...
function confidence(){
 ...
 let latest=state.assessments.filter(a=>a.valid).sort((a,b)=>b.date.localeCompare(a.date))[0],testTime=latest?latest.time:state.setup.testTime,testDist=latest?latest.distance:state.setup.testDistance,riegel=testTime*Math.pow(state.setup.raceDistance/testDist,1.06);
 let fitness=clamp(100-(riegel/state.setup.targetTime-1)*300,0,100),longest=Math.max(state.setup.currentLongest,...state.runs.map(r=>r.distanceKm),0),endurance=clamp(longest/state.setup.peakLong*100,0,100);
 let opportunities=due.length,consistency=opportunities?clamp(recent.length/opportunities*100,0,100):0,adherence=plannedKm?clamp(actual/plannedKm*100,0,100):0,rec=avg(recent.map(r=>r.recovery)),pain=avg(recent.map(r=>r.pain)),recovery=Number.isFinite(rec)?clamp(rec/5*75+(10-(pain||0))/10*25,0,100):50;
 let eff=recent.filter(r=>r.avgHr&&r.avgPower).map(r=>metrics(r).effect),economy=eff.length>=2?clamp(70+(eff.at(-1)/eff[0]-1)*500,0,100):50;
 let c=[['Fitness',fitness,.25],['Endurance',endurance,.20],['Consistency',consistency,.15],['Adherence',adherence,.15],['Recovery',recovery,.10],['Economy',economy,.15]].map(x=>({name:x[0],score:x[1],weight:x[2],impact:x[1]*x[2]}));
 return{components:c,overall:sum(c.map(x=>x.impact)),riegel,plannedKm,actual,longest}
}
...
function summariseCSV(rows){let h=rows[0].map(x=>x.trim()),idx=n=>h.indexOf(n),data=rows.slice(1).filter(r=>r.length>1),col=n=>data.map(r=>Number(r[idx(n)])).filter(Number.isFinite),pos=n=>avg(col(n).filter(v=>v>0));let ts=col('Timestamp'),distVals=col('Watch Distance (meters)'),dist=distVals.length?Math.max(...distVals)/1000:null,wkg=pos('Power (w/kg)'),powerW=pos('Power (W)')||pos('Power (Watts)')||(wkg? wkg*state.setup.bodyWeight:null),duration=ts.length?Math.max(...ts)-Math.min(...ts):null;if(!ts.length||!Number.isFinite(dist)||dist<=0||!Number.isFinite(duration)||duration<=0)throw Error('The CSV does not contain valid Stryd timestamp and distance data.');return{id:'stryd-'+Math.min(...ts)+'-'+Math.round(dist*1000),date:iso(new Date(Math.min(...ts)*1000)),type:'Easy',distanceKm:dist,durationSec:duration,avgHr:pos('Heart Rate (bpm)'),avgPower:powerW,cadence:pos('Cadence (spm)'),gct:idx('Ground Time (ms)')>=0?pos('Ground Time (ms)'):null,vo:idx('Vertical Oscillation (cm)')>=0?pos('Vertical Oscillation (cm)'):null,rpe:null,pain:null,recovery:null,temperature:null,notes:''}}
let preview=null;
...
function renderMetrics(){let rs=state.runs.slice().sort((a,b)=>a.date.localeCompare(b.date)),last=rs.at(-1),m=last?metrics(last):null;$('metricKpis').innerHTML=kpi('Latest pace',m?pace(m.pace):'—')+kpi('Distance / heartbeat',m?dec(m.dph,3):'—')+kpi('W / bpm',m?dec(m.wpb,3):'—')+kpi('Running effectiveness',m?dec(m.effect,3):'—')+kpi('W / kg',m?dec(m.wkg,2):'—')+kpi('Runs logged',rs.length);drawLine($('metricChart'),[{data:rs.map(r=>metrics(r).effect),color:'#2679bc'},{data:rs.map(r=>metrics(r).dph),color:'#12827c'}])}
