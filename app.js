...
const BUILD=10000, SCHEMA=10000, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function diagnosisFamily(key=''){
 ...
 if(/post_tib|navicular|met_|mortons|fhl|mtp|extensor|plantar|lateral_plantar/.test(key))return'foot';
...
const INJURY_REGION_KEYS={
 ...
 'Heel / arch':/lateral_plantar|plantar|post_tib|calcaneal_bsi|achilles_ins|fhl_tend/,
 'Forefoot / toes':/lateral_plantar|met_bsi|navicular_bsi|sesamoid|mortons|extensor_tend|fhl_tend|mtp_synovitis|fifth_met_bsi/
};
...
function clinicianVerification(i,app){
 const entered=String(i.clinicalDiagnosis||'').trim();if(!entered)return null;const mapped=clinicianLibraryMatch(entered),hasMap=!!mapped?.hits,appKey=app?.key||'',agrees=hasMap&&mapped.key===appKey;const inDifferential=hasMap&&app?.ranked?.some(x=>x.key===mapped.key);let status=agrees?'Supported by app pattern':inDifferential?'Plausible but not the leading app pattern':'Not supported by the current app pattern';let reasons=[];if(agrees)reasons.push('the independently ranked symptom pattern matches the clinician-entered diagnosis');else if(inDifferential)reasons.push('the clinician diagnosis remains in the app differential but another pattern currently scores higher');else reasons.push('the recorded symptom pattern does not currently reproduce the clinician-entered diagnosis');if(!hasMap)reasons.push('the clinician wording could not be mapped confidently to the app library');return{entered,mapped:hasMap?mapped:null,status,agrees,inDifferential,reasons};
}
...
function injuryCausePrevention(i,diag){
 ...
 if(!causes.length)causes.push('No single cause can be established from the current information.');return{causes:[...new Set(causes)],prevention:[...new Set(prevention)],needsRehab:!(diag.key==='lateral_plantar_overload'&&e.resolves48h&&!e.focalTenderness&&!e.hopPain&&!e.nightPain),selfManagement:diag.key==='lateral_plantar_overload'&&e.resolves48h&&!e.focalTenderness&&!e.hopPain&&!e.nightPain};
}
...
function runEvidence(checks){
 const completed=checks.filter(c=>c.runStatus==='completed'||Number.isFinite(c.runMinutes));
 ...
 const lastAttempt=checks.slice().reverse().find(c=>['completed','stopped','unable'].includes(c.runStatus)||Number.isFinite(c.runMinutes))||null;
...
function rehabExecutionMeta(check){
 const status=check?.rehabStatus||null;
 const map={
  stretch:{label:'Stretch goal achieved',short:'Stretch goal',className:'exceeded',score:100,assessed:true},
  completed:{label:'Completed as prescribed',short:'Completed',className:'completed',score:100,assessed:true},
  reduced:{label:'Completed with reduced volume',short:'Reduced dose',className:'partial',score:65,assessed:true},
  walking_only:{label:'Only walking completed',short:'Walking only',className:'partial',score:40,assessed:true},
  stopped:{label:'Started but stopped because of symptoms',short:'Stopped',className:'stopped',score:20,assessed:true},
  not_completed:{label:'Rehabilitation not performed',short:'Not completed',className:'missed',score:0,assessed:true},
  not_planned:{label:'Planned recovery / no rehabilitation',short:'Recovery day',className:'rest',score:null,assessed:false}
 };
 return map[status]||{label:'Not reported',short:'Not reported',className:'unknown',score:null,assessed:false};
}
...
function buildRehabCalendar(i,p){
 ...
 for(let offset=0;offset<7;offset++){
  const date=iso(new Date(today().getTime()+offset*DAY)),fresh=rehabCalendarDay(i,p,date,offset),previous=oldMap.get(date),check=checks.get(date),checkInCompleted=!!check,execution=rehabExecutionMeta(check);
...
function renderInjury(){
 ...
 <section class="injurySection clinicalReasoningSummary"><div class="injurySectionHead"><div><h4>Clinical reasoning summary</h4><p class="muted compact">Diagnosis, likely contributors, prevention and whether a formal rehabilitation pathway is currently justified.</p></div><strong>${clinicalPlan.selfManagement?'Self-management':'Rehabilitation'}</strong></div><div class="clinicalReasoningGrid"><div><b>Why this pattern?</b><p>${diag.evidence.slice(0,5).map(esc).join(' · ')||'More detail is needed.'}</p></div><div><b>Likely contributors</b><ul>${clinicalPlan.causes.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div><b>Prevention</b><ul>${clinicalPlan.prevention.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div><b>Plan decision</b><p>${clinicalPlan.needsRehab?'A staged rehabilitation plan is appropriate because the current pattern or functional status requires monitored progression.':'A formal rehabilitation pathway may not be necessary if symptoms settle fully within 24–48 hours and no focal tenderness, hopping pain, night pain or neurological symptoms are present. Prevention and load modification remain important.'}</p></div></div></section>
...
function openInjuryCheck(i,existing=null){
 ...
 <section class="injuryCheckSection"><h4>3. Rehabilitation load</h4><div class="formGrid">${selectField('icRehab','How much of today’s rehabilitation plan did you complete?',prev.rehabStatus,[['','Not assessed'],['not_planned','Planned recovery day / no rehabilitation'],['completed','Completed as prescribed'],['stretch','Completed plan and optional stretch goal'],['reduced','Completed with reduced volume'],['walking_only','Only walking completed'],['stopped','Started but stopped because of symptoms'],['not_completed','Did not perform rehabilitation']])}${triSelect('icBridge','Strength exercise tolerated with control?',prev.bridge,'Tolerated','Not tolerated')}${triSelect('icHop','Impact test or jog-in-place tolerated?',prev.hop,'Tolerated','Not tolerated')}</div></section>
 ...
 $('modal').className='modal';$('saveCheck').onclick=()=>{let runStatus=$('icRunStatus').value,runMinutes=nullableNumber($('icRun').value);if(['not_planned','not_assessed'].includes(runStatus))runMinutes=null;let c={date:$('icDate').value,pain:nullableNumber($('icPain').value),walkPain:nullableNumber($('icWalk').value),morningStiffness:nullableNumber($('icStiff').value),symptomTrend:$('icTrend').value||null,newSwelling:readTri('icSwelling'),walkMinutes:nullableNumber($('icWalkMinutes').value),stairs:readTri('icStairs'),confidence:nullableNumber($('icConfidence').value),rehabStatus:$('icRehab').value||null,bridge:readTri('icBridge'),hop:readTri('icHop'),runStatus,runMinutes,runPain:nullableNumber($('icRunPain').value),alteredGait:readTri('icGait'),nextDayWorse:readTri('icWorse'),loadResponse:$('icResponse').value||null,symptoms:$('icSymptoms').value.trim()};if(!c.date)return toast('Enter the check-in date.',true);const hasObs=Object.entries(c).some(([k,v])=>k!=='date'&&k!=='symptoms'&&v!==null&&v!==''&&v!==undefined);if(!hasObs&&!c.symptoms)return toast('Record at least one observation.',true);if(runStatus==='completed'&&!Number.isFinite(runMinutes))return toast('Enter the completed running minutes.',true);i.checkIns=i.checkIns||[];if(editing&&originalDate!==c.date&&i.checkIns.some(x=>x.date===c.date))return toast('A check-in already exists for that date.',true);let n=i.checkIns.findIndex(x=>x.date===(editing?originalDate:c.date));n>=0?i.checkIns[n]=c:i.checkIns.push(c);save();$('modal').className='modal hidden';renderInjury();toast(editing?'Check-in updated and full trajectory recalculated.':'Check-in saved. The full history—not this day alone—was used.');};if(editing)$('deleteCheck').onclick=()=>{if(confirm('Delete this check-in?')){i.checkIns=i.checkIns.filter(x=>x.date!==originalDate);save();$('modal').className='modal hidden';renderInjury();toast('Check-in deleted and trajectory recalculated.')}};
}
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v10.0.0 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v10.0.0 stable build 10000');
})();