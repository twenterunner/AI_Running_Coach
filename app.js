...
function shoeEngineStrategyHtml(){const priority=shoeRotationPriority(),profile=shoeRotationPriorityProfile(),weeks=shoePurchaseIntervalWeeks();return`<div class="shoeApproachControl shoeApproachFixed"><label>Shoe strategy</label><strong>SESSION SUITABILITY</strong><small>${esc(SHOE_ENGINE_STRATEGY.desc)}</small><div class="shoeStrategyControls"><div class="shoePriorityControl"><label for="shoeRotationPriority">Rotation preference</label><select id="shoeRotationPriority"><option value="purchase-spacing" ${priority==='purchase-spacing'?'selected':''}>Purchase spacing priority</option><option value="quality-session" ${priority==='quality-session'?'selected':''}>Quality-session priority</option></select><div class="shoePriorityActive">ACTIVE: ${esc(profile.short.toUpperCase())}</div><small>${esc(profile.desc)}</small></div><div class="shoeIntervalControl"><label for="shoePurchaseIntervalWeeks">Target purchase interval</label><div class="shoeIntervalInput"><input id="shoePurchaseIntervalWeeks" type="number" min="4" max="24" step="1" inputmode="numeric" value="${weeks}"><span>weeks</span></div><small>Soft target between successive planned shoe purchases. The engine may deviate when lifecycle, safe rotation, Race Day readiness or the selected session-priority policy requires it.</small></div></div></div>`}
