...
const BUILD=9860, SCHEMA=9860, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function reconcilePredictionHistory(){
 ...
 const validIds=new Set(events.map(e=>e.entityId));
 const invalid=history.some(x=>Number(x.predictionModelVersion)!==2||!Number.isFinite(Number(x.updateDelta)));
 const missing=events.some(e=>!history.some(x=>x.entityId===e.entityId));
 const orphaned=history.some(x=>x.entityId&&!validIds.has(x.entityId));
 if(!invalid&&!missing&&!orphaned)return false;
...
function renderDashboard(){
 ...
 const contributionRows=fitnessContributions.length?fitnessContributions.map(x=>{const potential=Number(x?.potential),realised=Number(x?.realised);const meta=[Number.isFinite(potential)?`Potential ${potential.toFixed(2)}%`:null,Number.isFinite(realised)?`Realised ${realised.toFixed(2)}%`:null].filter(Boolean).join(' · ');return `<div class="fitnessCalcRow"><span>${esc(x?.name||'Plan stimulus')}</span><b>${esc(meta||'—')}</b></div>`}).join(''):`<div class="fitnessCalcRow"><span>Plan-derived projection</span><b>+${(fitnessGainSafe*100).toFixed(2)}%</b></div>`;
 $('outlookGain').innerHTML=`<div><span>Expected improvement</span><b>${fmtTime(engine.projection.improvementSec)}</b></div><div><span>Target margin</span><b>${targetMargin>=0?fmtTime(targetMargin)+' faster':fmtTime(-targetMargin)+' slower'}</b></div><details class="outlookMetricDetail"><summary><span>Projected fitness</span><b>${projectedFitnessSafe.toFixed(1)}</b><small>Tap to show calculation</small></summary><div class="outlookMetricCalc"><p>Your predicted general race capability is <b>${Math.abs(projectedFitnessSafe-100).toFixed(1)}% ${projectedFitnessSafe>=100?'above':'below'}</b> the latest fitness-assessment baseline, before durability and taper adjustments.</p><div class="fitnessCalcTable">${contributionRows}<div class="fitnessCalcRow total"><span>Projected fitness gain</span><b>+${(fitnessGainSafe*100).toFixed(2)}%</b></div></div><p class="muted compact">The realised gain accounts for plan health, expected completion, recovery, training opportunity and diminishing returns.</p></div></details><div><span>Plan health</span><b>${Math.round(Number(health.score)||0)}/100</b></div>`;
...
const INJURY_STAGES=[
 {name:'Protect & settle',goal:'Calm symptoms and restore comfortable daily movement.',criteria:'Symptoms are stable or improving and normal daily movement is becoming easier.'},
 {name:'Restore movement',goal:'Recover comfortable range of motion and basic muscle control.',criteria:'Normal walking, stairs and easy double-leg loading are tolerated with pain no higher than 2/10.'},
 {name:'Build capacity',goal:'Progress strength, control and tolerance to repeated loading.',criteria:'Repeated strength work is controlled and symptoms return to baseline by the next morning.'},
 {name:'Return to run',goal:'Reintroduce impact with a structured walk–run progression.',criteria:'Brisk walking and low-level hopping are tolerated; pain remains 0–2/10 and settles by the next morning.'},
 {name:'Rebuild running',goal:'Increase easy continuous running before speed, hills or hard sessions.',criteria:'Several easy runs are completed without altered gait or a next-day symptom increase.'},
 {name:'Return to performance',goal:'Restore faster running, hills and sport-specific demands.',criteria:'Near-normal strength, confidence and full-speed exposure are tolerated; clinician clearance when the injury was severe.'}
];
const REHAB_EXERCISES={
 hamstring:[
  [
   {name:'Heel-dig isometric',dose:'5 × 20–30 sec',cue:'Heel close to the body. Press down without lifting the hips; keep pain at 0–2/10.'},
   {name:'Supported bridge hold',dose:'4 × 20 sec',cue:'Use both legs and a short lever. Stop if pain sharpens or increases after the set.'},
   {name:'Comfortable walking',dose:'2–4 × 5–10 min',cue:'Walk without limping. Shorten the bout before symptoms rise.'}
  ],
  [
   {name:'Double-leg bridge',dose:'3 × 8–12',cue:'Slow lift and lower. Keep the pelvis level and use a comfortable range.'},
   {name:'Standing hamstring curl',dose:'3 × 10',cue:'Use body weight only. Move slowly and avoid forcing end range.'},
   {name:'Hip-hinge drill',dose:'3 × 8',cue:'Push hips back with a neutral spine. Keep the range symptom-limited.'}
  ],
  [
   {name:'Long-lever bridge',dose:'3 × 8–10',cue:'Move the heels farther away only when the short-lever version is easy.'},
   {name:'Romanian deadlift',dose:'3 × 8',cue:'Start light. Use a controlled 3-second lowering phase.'},
   {name:'Slider curl regression',dose:'3 × 5–8',cue:'Slide out slowly with both legs and reduce range if symptoms increase.'}
  ],
  [
   {name:'Single-leg bridge',dose:'3 × 6–10 each',cue:'Keep the pelvis level and compare control between sides.'},
   {name:'Walk–run intervals',dose:'6 × 1 min run / 2 min walk',cue:'Easy pace, flat surface and normal gait. Stop for pain above 2/10.'},
   {name:'Low pogo hops',dose:'3 × 20 sec',cue:'Small, quiet contacts. Skip if hopping is painful or apprehensive.'}
  ],
  [
   {name:'Eccentric slider curl',dose:'3 × 6–8',cue:'Extend slowly and use both legs to return if needed.'},
   {name:'Easy continuous run',dose:'20–40 min',cue:'Increase duration before pace. Leave at least 24 hours between progression runs.'},
   {name:'Strides introduction',dose:'4–6 × 10–15 sec',cue:'Only after two symptom-stable easy runs; build from 60% to 75% speed.'}
  ],
  [
   {name:'Nordic regression',dose:'3 × 4–6',cue:'Use assistance and control the lowering phase; quality over depth.'},
   {name:'Progressive accelerations',dose:'6 × 40–60 m',cue:'Progress from 70% toward 90% over several symptom-stable sessions.'},
   {name:'Maintenance strength',dose:'2 sessions/week',cue:'Retain hip-dominant and knee-flexor strength after normal running resumes.'}
  ]
 ],
 calf:[
  [{name:'Calf isometric hold',dose:'5 × 30 sec',cue:'Use both legs if needed; keep symptoms tolerable.'},{name:'Gentle ankle movement',dose:'2 × 15',cue:'Move through a comfortable range without forcing stretch.'},{name:'Comfortable walking',dose:'Short frequent bouts',cue:'Avoid limping and reduce hills initially.'}],
  [{name:'Double-leg calf raise',dose:'3 × 10–15',cue:'Slow, controlled repetitions through comfortable range.'},{name:'Bent-knee calf raise',dose:'3 × 10',cue:'Keep the heel aligned and load gradually.'},{name:'Single-leg balance',dose:'3 × 30 sec',cue:'Use light support if required.'}],
  [{name:'Single-leg calf raise',dose:'3 × 8–12',cue:'Match height and control before adding load.'},{name:'Loaded calf raise',dose:'3 × 8',cue:'Add weight gradually; avoid large next-morning stiffness increase.'},{name:'Step-down',dose:'3 × 8 each',cue:'Control the knee and ankle over the foot.'}],
  [{name:'Pogo progression',dose:'3 × 20 sec',cue:'Start double-leg with quiet contacts.'},{name:'Walk–run intervals',dose:'6 × 1 min run / 2 min walk',cue:'Flat surface, easy pace and symptom response checked next morning.'},{name:'Calf raise maintenance',dose:'3 × 8–12',cue:'Continue loading on non-run days.'}],
  [{name:'Easy continuous run',dose:'20–40 min',cue:'Build duration before hills or speed.'},{name:'Single-leg hops',dose:'3 × 8',cue:'Compare height, control and confidence.'},{name:'Heavy calf raise',dose:'3 × 6–8',cue:'Progress load rather than adding excessive repetitions.'}],
  [{name:'Fast calf contacts',dose:'3 × 20 sec',cue:'Progress cadence and stiffness gradually.'},{name:'Hill or speed exposure',dose:'4–6 short reps',cue:'Add one demand at a time.'},{name:'Maintenance loading',dose:'2 sessions/week',cue:'Continue after full running return.'}]
 ],
 knee:[
  [{name:'Quadriceps set',dose:'5 × 20 sec',cue:'Tighten the thigh with the knee supported and comfortable.'},{name:'Sit-to-stand',dose:'3 × 6–10',cue:'Use a higher seat and controlled range.'},{name:'Comfortable walking',dose:'Short frequent bouts',cue:'Avoid swelling or limping.'}],
  [{name:'Wall sit',dose:'4 × 20–30 sec',cue:'Choose a shallow angle that stays tolerable.'},{name:'Low step-up',dose:'3 × 8 each',cue:'Control the knee over the middle toes.'},{name:'Bridge',dose:'3 × 10',cue:'Keep the pelvis level.'}],
  [{name:'Split squat',dose:'3 × 8 each',cue:'Use support and a comfortable depth.'},{name:'Step-down',dose:'3 × 8',cue:'Slow lowering and stable alignment.'},{name:'Single-leg balance',dose:'3 × 30 sec',cue:'Progress by reducing hand support.'}],
  [{name:'Low pogo hops',dose:'3 × 20 sec',cue:'Quiet, symmetrical contacts.'},{name:'Walk–run intervals',dose:'6 × 1 min run / 2 min walk',cue:'Flat surface and easy effort.'},{name:'Split squat',dose:'3 × 8',cue:'Maintain strength while impact returns.'}],
  [{name:'Easy continuous run',dose:'20–40 min',cue:'Increase time before intensity.'},{name:'Single-leg squat to box',dose:'3 × 6–8',cue:'Use a height that preserves control.'},{name:'Forward hops',dose:'3 × 6',cue:'Land quietly and confidently.'}],
  [{name:'Faster running exposure',dose:'4–6 short reps',cue:'Add speed gradually after stable easy running.'},{name:'Lateral hop progression',dose:'3 × 6 each',cue:'Control landing and direction change.'},{name:'Maintenance strength',dose:'2 sessions/week',cue:'Retain knee and hip capacity.'}]
 ],
 generic:[
  [{name:'Pain-limited isometric',dose:'5 × 20–30 sec',cue:'Contract the affected area gently without provoking sharp pain.'},{name:'Comfortable mobility',dose:'2 × 10',cue:'Move only through a comfortable range.'},{name:'Comfortable walking',dose:'Short frequent bouts',cue:'Avoid limping and symptom escalation.'}],
  [{name:'Basic double-leg strength',dose:'3 × 8–12',cue:'Use controlled tempo and comfortable range.'},{name:'Balance practice',dose:'3 × 30 sec',cue:'Use support as needed.'},{name:'Mobility',dose:'2 × 10',cue:'Do not force end range.'}],
  [{name:'Progressive resistance',dose:'3 × 8',cue:'Increase load only after a stable next-morning response.'},{name:'Single-leg control',dose:'3 × 6–10',cue:'Compare control and confidence.'},{name:'Functional movement',dose:'3 × 8',cue:'Choose a movement relevant to walking and running.'}],
  [{name:'Low-level hopping',dose:'3 × 20 sec',cue:'Small, quiet contacts and normal confidence.'},{name:'Walk–run intervals',dose:'6 × 1 min run / 2 min walk',cue:'Flat surface, easy effort and pain no higher than 2/10.'},{name:'Strength maintenance',dose:'2–3 exercises',cue:'Continue on non-run days.'}],
  [{name:'Easy continuous run',dose:'20–40 min',cue:'Increase duration before pace or hills.'},{name:'Single-leg strength',dose:'3 × 8',cue:'Aim for controlled, near-symmetrical movement.'},{name:'Running drills',dose:'4–6 short reps',cue:'Use relaxed technique and low intensity.'}],
  [{name:'Performance exposure',dose:'4–8 short reps',cue:'Restore speed, hills and direction change one variable at a time.'},{name:'Heavy strength',dose:'3 × 5–8',cue:'Maintain high-quality progressive loading.'},{name:'Maintenance programme',dose:'2 sessions/week',cue:'Continue after return to full running.'}]
 ]
};
function injuryProfile(i){const s=((i.location||'')+' '+(i.initialSymptoms||'')+' '+(i.mechanism||'')).toLowerCase();if(/hamstring|posterior thigh|back of thigh|glute|sit bone|ischial/.test(s))return'hamstring';if(/calf|achilles|heel|soleus/.test(s))return'calf';if(/knee|patella|quad|it band|iliotibial/.test(s))return'knee';return'generic'}
function initialInjuryDays(i){const pain=Number(i.initialPain)||0,walk=Number(i.initialWalkPain)||0;let days=pain>=8||i.pop||walk>=8?56:pain>=6||i.bruising||walk>=5?35:pain>=3?21:10;if(i.medicalDiagnosis)days=Math.max(days,21);return days}
function injuryStageFromLatest(i,latest){
 const currentPain=Number(latest?.pain??i.currentPain??i.initialPain??0),walkPain=Number(latest?.walkPain??i.currentWalkPain??i.initialWalkPain??0);
 let stage=0;
 if(walkPain<=3&&currentPain<=5)stage=1;
 if(walkPain<=1&&currentPain<=3&&latest&&(latest.stairs||latest.strength))stage=2;
 if(currentPain<=2&&walkPain===0&&latest?.hop)stage=3;
 if((Number(latest?.runMinutes)||0)>=10&&currentPain<=2&&!latest?.nextDayWorse)stage=4;
 if((Number(latest?.runMinutes)||0)>=30&&currentPain<=1&&!latest?.nextDayWorse&&(Number(latest?.confidence)||0)>=7)stage=5;
 if(latest&&(latest.nextDayWorse||latest.swelling))stage=Math.max(0,stage-1);
 return{stage,currentPain,walkPain};
}
function stageCriteria(p){
 const l=p.checks.at(-1)||{};
 const stable=!!p.hasCheckIns&&!l.nextDayWorse&&!l.swelling;
 return[
  [{label:'Symptoms stable or improving',met:stable},{label:'Daily movement becoming easier',met:p.walkPain<=3}],
  [{label:'Walking pain ≤2/10',met:p.walkPain<=2},{label:'Current pain ≤3/10',met:p.currentPain<=3},{label:'Stairs or basic strength tolerated',met:!!(l.stairs||l.strength)}],
  [{label:'Strength work controlled',met:!!l.strength},{label:'Pain ≤2/10',met:p.currentPain<=2},{label:'No worse next morning',met:p.hasCheckIns&&!l.nextDayWorse}],
  [{label:'Walking pain 0/10',met:p.walkPain===0},{label:'Gentle hopping tolerated',met:!!l.hop},{label:'Pain ≤2/10',met:p.currentPain<=2},{label:'No next-day flare',met:p.hasCheckIns&&!l.nextDayWorse}],
  [{label:'At least 10 min running',met:(Number(l.runMinutes)||0)>=10},{label:'Normal symptom response next day',met:p.hasCheckIns&&!l.nextDayWorse},{label:'Pain ≤2/10',met:p.currentPain<=2}],
  [{label:'At least 30 min easy running',met:(Number(l.runMinutes)||0)>=30},{label:'Pain ≤1/10',met:p.currentPain<=1},{label:'No next-day flare',met:p.hasCheckIns&&!l.nextDayWorse},{label:'Confidence ≥7/10',met:(Number(l.confidence)||0)>=7}]
 ];
}
function rehabCompletionScore(p){
 if(!p.hasCheckIns)return null;
 const currentCriteria=stageCriteria(p)[p.stage]||[],met=currentCriteria.filter(x=>x.met).length,fraction=currentCriteria.length?met/currentCriteria.length:0;
 // Six stages each represent one sixth of the route to unrestricted training.
 // A runner cannot score near 100 merely because pain is low while impact/running criteria remain untested.
 let score=((p.stage+fraction)/INJURY_STAGES.length)*100;
 if(p.stage<3)score=Math.min(score,49);       // running has not started
 else if(p.stage<4)score=Math.min(score,66);  // only walk-run exposure
 else if(p.stage<5)score=Math.min(score,84);  // easy running, not unrestricted
 if(p.checks.at(-1)?.nextDayWorse||p.checks.at(-1)?.swelling)score-=8;
 return clamp(Math.round(score),0,100);
}
function checkInScore(c,i={}){
 if(!c)return null;
 const temp={checks:[c],hasCheckIns:true,...injuryStageFromLatest(i,c)};
 return rehabCompletionScore(temp);
}
function injuryPrediction(i){
 const checks=(i.checkIns||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)),latest=checks.at(-1)||null;
 const elapsed=Math.max(0,Math.floor((today()-dte(i.date))/DAY)),base=initialInjuryDays(i);
 const stateNow=injuryStageFromLatest(i,latest),currentPain=stateNow.currentPain,walkPain=stateNow.walkPain,stage=stateNow.stage;
 const hasCheckIns=checks.length>0;
 const shell={checks,hasCheckIns,stage,currentPain,walkPain};
 const score=rehabCompletionScore(shell);
 const expectedScore=clamp(Math.round(elapsed/base*100),0,100);
 const delta=hasCheckIns?Math.round(score-expectedScore):null;
 const span=checks.length>=2?Math.max(0,Math.round((dte(checks.at(-1).date)-dte(checks[0].date))/DAY)):0;
 const comparisonReady=checks.length>=2&&span>=2;
 let pace=!hasCheckIns?'awaiting':!comparisonReady?'building':delta>=10?'faster':delta<=-10?'slower':'on-track';
 const scores=checks.map(c=>checkInScore(c,i)).filter(Number.isFinite),recent=scores.slice(-3),trend=recent.length>=2?recent.at(-1)-recent[0]:0;
 let adjustment=0;
 if(comparisonReady){adjustment=Math.round(-delta*.12);if(latest.nextDayWorse)adjustment+=4;if(trend>=10)adjustment-=2;if(trend<=-8)adjustment+=3;adjustment=clamp(adjustment,-10,21)}
 const total=Math.max(elapsed+1,base+adjustment),remaining=Math.max(0,total-elapsed),fullReturnDate=iso(new Date(today().getTime()+remaining*DAY));
 const confidence=checks.length>=7?'Higher':checks.length>=3?'Moderate':'Low',originalDate=iso(new Date(dte(i.date).getTime()+base*DAY));
 const dateShift=comparisonReady?Math.round((dte(fullReturnDate)-dte(originalDate))/DAY):null;
 const nominalStage=Math.min(5,Math.floor(expectedScore/(100/6)));
 return{base,total,elapsed,remaining,stage,startDate:fullReturnDate,fullReturnDate,originalDate,dateShift,confidence,currentPain,walkPain,score,expectedScore,delta,pace,trend,checks,hasCheckIns,comparisonReady,nominalStage,comparisonSpan:span};
}
function trajectorySvg(i,p){const checks=p.checks,W=700,H=230,left=42,right=18,top=22,bottom=38,cw=W-left-right,ch=H-top-bottom;const days=Math.max(7,p.elapsed+Math.max(1,p.remaining),p.base),x=d=>left+clamp(d/days,0,1)*cw,y=v=>top+(100-clamp(v,0,100))/100*ch;let expected=`M ${x(0)} ${y(0)} L ${x(p.base)} ${y(100)}`;let observed='';const points=checks.map(c=>({day:Math.max(0,Math.round((dte(c.date)-dte(i.date))/DAY)),score:checkInScore(c,i)})).filter(q=>Number.isFinite(q.score));points.forEach((q,n)=>observed+=(n?' L ':'M ')+x(q.day)+' '+y(q.score));return`<svg class="rehabTrajectory" viewBox="0 0 ${W} ${H}" role="img" aria-label="Observed and nominal rehabilitation trajectory"><line x1="${left}" y1="${y(25)}" x2="${W-right}" y2="${y(25)}"/><line x1="${left}" y1="${y(50)}" x2="${W-right}" y2="${y(50)}"/><line x1="${left}" y1="${y(75)}" x2="${W-right}" y2="${y(75)}"/><path class="expectedLine" d="${expected}"/><path class="observedLine" d="${observed}"/>${points.map(q=>`<circle cx="${x(q.day)}" cy="${y(q.score)}" r="5"/>`).join('')}<text x="${left}" y="${H-10}">Injury</text><text x="${W-right}" y="${H-10}" text-anchor="end">Full training</text><text x="${left+4}" y="${top+14}">Rehab completion</text></svg><div class="trajectoryLegend"><span class="observedKey">Criteria completed</span><span class="expectedKey">Nominal progression</span></div>`}
function progressionCriteriaHtml(p){
 const next=Math.min(INJURY_STAGES.length-1,p.stage+1),items=stageCriteria(p)[next]||[],met=items.filter(x=>x.met).length;
 return `<div class="progressCriteria"><div class="criteriaHead"><div><small>Requirements for stage ${next+1}</small><h3>${esc(INJURY_STAGES[next].name)}</h3></div><strong>${met}/${items.length} met</strong></div>${!p.hasCheckIns?'<div class="criteriaNotice">Complete a daily check-in to verify functional criteria. Current symptoms alone cannot prove progression.</div>':''}<div class="criteriaList">${items.map(x=>`<div class="criterion ${x.met?'met':'notMet'}"><i>${x.met?'✓':'○'}</i><span>${esc(x.label)}</span><b>${x.met?'Met':'Not met'}</b></div>`).join('')}</div></div>`;
}
function exerciseKind(e){const n=e.name.toLowerCase();return /run|walk|stride|acceleration|hill/.test(n)?'run':/hop|pogo|skip|bound/.test(n)?'hop':/bridge|heel-dig|curl/.test(n)?'bridge':/raise|squat|deadlift|hinge|strength|step/.test(n)?'strength':'mobility'}
function exerciseSheetData(e){
 const n=e.name.toLowerCase(),kind=exerciseKind(e);
 const specific={
  'heel-dig isometric':{purpose:'Introduce pain-limited hamstring loading without joint movement.',steps:[['Set up','Lie on your back with the affected knee bent about 30–60°. Place the heel firmly on the floor and relax the shoulders.'],['Contract','Press the heel down and gently pull it toward you without allowing it to slide. Keep the pelvis on the floor.'],['Hold and release','Build tension gradually, hold for the prescribed time, then relax fully for 20–30 seconds before repeating.']]},
  'supported bridge hold':{purpose:'Load the hamstrings and glutes with a stable two-leg position.',steps:[['Set up','Lie on your back with both knees bent and feet hip-width apart. Bring the heels close enough that the movement is comfortable.'],['Lift','Brace gently, press through both heels and lift the pelvis until the trunk and thighs form a comfortable line.'],['Hold and lower','Keep the pelvis level during the hold. Lower slowly over about three seconds and allow symptoms to settle before the next repetition.']]},
  'double-leg bridge':{purpose:'Restore controlled posterior-chain strength through a comfortable range.',steps:[['Set up','Lie on your back with knees bent, feet parallel and arms relaxed by your sides.'],['Lift','Press evenly through both heels and raise the pelvis without flaring the ribs or arching the lower back.'],['Return','Pause briefly at the top, then lower over three seconds while keeping the knees and pelvis aligned.']]},
  'standing hamstring curl':{purpose:'Restore knee-flexion strength with minimal external load.',steps:[['Set up','Stand tall beside a wall or chair for balance. Keep the thighs side by side.'],['Curl','Bend the affected knee and bring the heel toward the buttock only as far as comfortable. Do not move the hip forward.'],['Return','Lower the foot slowly until the knee is straight. Keep the movement smooth and avoid swinging.']]},
  'hip-hinge drill':{purpose:'Retrain hip-dominant movement while controlling hamstring lengthening.',steps:[['Set up','Stand with feet hip-width apart and knees softly bent. Place the hands on the hips or hold a light support.'],['Hinge','Push the hips backward while keeping the spine neutral and weight through the mid-foot and heel.'],['Return','Stop before pain or loss of control, squeeze the glutes and stand tall without leaning backward.']]},
  'long-lever bridge':{purpose:'Increase hamstring demand by moving the heels farther from the hips.',steps:[['Set up','Lie on your back with both heels farther away than in a normal bridge and toes relaxed upward.'],['Lift','Drive the heels down, brace the trunk and lift the pelvis only as high as you can keep it level.'],['Return','Hold briefly, then lower slowly. Reduce the lever length if cramping or pain rises.']]},
  'romanian deadlift':{purpose:'Build hamstring and hip strength through controlled lengthening.',steps:[['Set up','Stand tall holding a light weight close to the thighs. Keep the knees softly bent and shoulders relaxed.'],['Lower','Push the hips backward and lower the weight close to the legs for about three seconds while maintaining a neutral spine.'],['Stand','Stop at the first point of loss of control or symptom increase, then drive the hips forward to stand tall.']]},
  'slider curl regression':{purpose:'Introduce controlled hamstring lengthening with adjustable range.',steps:[['Set up','Lie on your back with both heels on sliders or towels, knees bent and hips raised slightly.'],['Slide out','Slowly slide both heels away while keeping the pelvis as level as possible. Use only a range you can control.'],['Return','Lower the pelvis if needed, bend both knees to return, then reset before the next repetition.']]},
  'single-leg bridge':{purpose:'Build unilateral hamstring and glute control before running impact.',steps:[['Set up','Lie on your back with one foot flat and the other leg raised so the thighs remain level. Brace the trunk gently.'],['Lift','Press through the heel of the working leg and raise the pelvis without rotating or dropping one side.'],['Hold and lower','Hold for two to three seconds, then lower slowly. Stop when the pelvis can no longer remain level.']]},
  'walk–run intervals':{purpose:'Reintroduce running impact in small, measurable doses.',steps:[['Prepare','Choose a flat predictable route. Walk briskly for five to ten minutes and confirm that gait is normal.'],['Alternate','Run at an easy conversational effort for the prescribed interval, then walk for the recovery interval. Keep strides short and relaxed.'],['Review','Stop if pain exceeds 2/10 or gait changes. Record symptoms immediately, later that day and the following morning.']]},
  'low pogo hops':{purpose:'Restore low-level elastic impact and confidence before continuous running.',steps:[['Set up','Stand tall with feet hip-width apart, knees softly bent and weight evenly distributed.'],['Hop','Use small two-leg hops from the ankles. Keep contacts quiet and land beneath the hips.'],['Reset','Maintain rhythm only while contacts stay symmetrical and comfortable. Stop for pain, guarding or increasing stiffness.']]},
  'eccentric slider curl':{purpose:'Develop higher hamstring capacity during controlled lengthening.',steps:[['Set up','Lie on your back with both heels on sliders and lift the hips into a stable bridge.'],['Lengthen','Slowly slide the heels away over three to five seconds while keeping the pelvis elevated and level.'],['Return','Lower the hips and use both legs to bring the heels back. Reset fully before the next repetition.']]},
  'easy continuous run':{purpose:'Build uninterrupted easy-running tolerance before speed or hills.',steps:[['Prepare','Warm up with brisk walking and confirm pain is no higher than 2/10 with normal gait.'],['Run','Run continuously at an easy conversational effort on a flat surface. Keep cadence and stride natural.'],['Review','Finish before fatigue changes technique. Do not increase duration unless symptoms are stable later and the next morning.']]},
  'strides introduction':{purpose:'Reintroduce controlled faster running after stable easy runs.',steps:[['Prepare','Complete an easy run or thorough warm-up first. Use a flat straight surface.'],['Accelerate','Build smoothly from easy running to the prescribed moderate speed over several seconds; do not sprint.'],['Decelerate','Ease down gradually and walk until fully recovered. Stop if mechanics become protective or symptoms rise.']]},
  'nordic regression':{purpose:'Build high-load eccentric hamstring strength with assistance.',steps:[['Set up','Kneel on padding with ankles securely held. Keep hips extended and trunk in a straight line from knees to shoulders.'],['Lower','Lean forward from the knees, resisting the fall with the hamstrings. Use hands, a band or limited depth for assistance.'],['Return','Catch with the hands before control is lost, push lightly back to the start and reset posture.']]},
  'progressive accelerations':{purpose:'Restore high-speed running exposure in controlled increments.',steps:[['Prepare','Warm up fully with easy running, drills and symptom-free strides. Use a flat straight surface.'],['Accelerate','Increase speed smoothly over 40–60 m to the planned percentage; avoid an explosive first step.'],['Recover','Decelerate gradually and take a full walking recovery. Progress speed only after stable later and next-morning symptoms.']]}
 };
 const chosen=specific[n]||{};
 let purpose=chosen.purpose||(kind==='run'?'Restore normal running tolerance without provoking a delayed flare.':kind==='hop'?'Reintroduce elastic impact and confidence with controlled contacts.':kind==='bridge'?'Load the posterior chain while keeping the pelvis controlled.':kind==='strength'?'Build tissue capacity and movement control through progressive resistance.':'Restore comfortable movement and muscle control.');
 let steps=chosen.steps||(kind==='run'?[['Prepare','Choose a flat, predictable surface. Start with relaxed walking and normal posture.'],['Perform','Use an easy conversational effort. Keep stride short, quiet and symmetrical.'],['Finish','Stop before form changes. Record pain during, later that day and next morning.']]:kind==='hop'?[['Set','Stand tall with feet under the hips and knees softly bent.'],['Contact','Use small, quiet contacts. Keep the knee and foot aligned.'],['Reset','Land under control, pause if needed and stop if pain or apprehension rises.']]:kind==='bridge'?[['Start','Lie on your back with knees bent and heels planted. Brace gently.'],['Lift','Drive through the heel and lift the hips without twisting or arching.'],['Control','Hold briefly, lower slowly and keep both sides of the pelvis level.']]:kind==='strength'?[['Set position','Use a stable stance and a comfortable symptom-limited range.'],['Load','Move slowly through the working phase with good alignment and control.'],['Return','Use a controlled lowering phase. Stop before technique deteriorates.']]:[['Start','Set up in a comfortable, supported position.'],['Move','Move only through a smooth, symptom-tolerable range.'],['Reassess','Return slowly and check that symptoms settle immediately after the set.']]);
 let target=kind==='bridge'?'Primary: hamstrings and gluteus maximus. Secondary: trunk and pelvic stabilisers.':kind==='strength'?'Primary: the muscle group being loaded. Secondary: hip, knee, ankle and trunk stabilisers needed to maintain alignment.':kind==='hop'?'Primary: calf, hamstring, quadriceps and gluteal muscles working elastically.':'Primary: the injured region and the muscles controlling the relevant movement. Secondary: surrounding stabilisers.';
 let normal=kind==='run'?'Relaxed, symmetrical running effort with no protective shortening of the stride.':kind==='hop'?'Light, springy muscular effort and quiet symmetrical contacts.':'Local muscular effort, mild fatigue or a gentle stretch that settles quickly.';
 let avoid='Sharp pain, sudden pulling, altered gait, increasing swelling, loss of control, or symptoms that are worse later or the next morning.';
 let mistakes=kind==='bridge'?['Pelvis dropping or rotating','Pushing mainly through the toes','Over-arching the lower back','Rushing the lowering phase']:kind==='strength'?['Using more range than can be controlled','Knee or foot collapsing inward','Rushing the lowering phase','Adding load despite next-day worsening']:kind==='hop'?['Loud or stiff landings','Unequal loading','Increasing height too early','Continuing with apprehension or pain']:kind==='run'?['Increasing pace and duration together','Running with altered gait','Ignoring later or next-morning worsening','Progressing to hills too early']:['Forcing end range','Holding the breath','Moving through sharp pain','Using momentum instead of control'];
 let regression=kind==='run'?'Shorten each run interval or return to brisk walking.':kind==='hop'?'Use two legs, smaller contacts or supported heel raises.':kind==='bridge'?'Use both legs, shorten the lever or reduce lift height.':kind==='strength'?'Reduce range, resistance or use hand support.':'Reduce range and use more external support.';
 ...
 return{kind,purpose,steps,target,normal,avoid,mistakes,regression,progression};
}
function exerciseSheet(e,n){const d=exerciseSheetData(e);return `<details class="exerciseSheet"><summary><span class="exerciseIndex">${n+1}</span><span class="exerciseSummary"><b>${esc(e.name)}</b><small>${esc(d.purpose)}</small></span><span class="exerciseDose">${esc(e.dose)}</span><span class="sheetChevron">⌄</span></summary><div class="exerciseSheetBody"><div class="exerciseMeta"><span><b>${esc(e.dose)}</b><small>Prescription</small></span><span><b>0–2/10</b><small>Pain guideline</small></span><span><b>No flare</b><small>Next morning</small></span></div><div class="writtenExerciseSteps">${d.steps.map((x,k)=>`<div class="writtenStep"><i>${k+1}</i><div><b>${esc(x[0])}</b><p>${esc(x[1])}</p></div></div>`).join('')}</div><div class="exerciseSheetGrid textOnly"><div class="sheetPanel target"><h4>Target and expected sensation</h4><p><b>Muscles:</b> ${esc(d.target)}</p><p><b>Normal:</b> ${esc(d.normal)}</p><p><b>Not acceptable:</b> ${esc(d.avoid)}</p></div><div class="sheetPanel cues"><h4>Coaching cues</h4><ul><li>${esc(e.cue)}</li><li>Use slow, controlled repetitions and normal breathing.</li><li>Stop before technique or symmetry deteriorates.</li></ul></div><div class="sheetPanel mistakes"><h4>Common mistakes</h4><ul>${d.mistakes.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div></div><div class="exerciseDecision"><div><small>Pain guideline</small><b>Keep pain at 0–2/10</b><p>Stop for sharp pain, altered gait or symptoms that worsen later or next morning.</p></div><div><small>Regress</small><b>Make it easier</b><p>${esc(d.regression)}</p></div><div><small>Progress</small><b>When stable</b><p>${esc(d.progression)}</p></div></div></div></details>`}
...
function nominalEvidence(p){
 ...
 const add=(label,actual,nominal,status,why)=>signals.push({label,actual,nominal,status,why});
 const nominalPain=Math.max(0,Math.round(6-(p.expectedScore/100)*6));
 add('Pain',`${p.currentPain}/10`,`about ${nominalPain}/10`,p.currentPain<=nominalPain?'ahead':'behind',p.currentPain<=nominalPain?'Pain is lower than the nominal symptom level for this point.':'Pain remains higher than the nominal symptom level for this point.');
 const nominalWalk=expectedStage>=2?'0–1/10':expectedStage>=1?'≤2/10':'may still be limited';
 const walkAhead=expectedStage<1?p.walkPain<=3:expectedStage<2?p.walkPain<=2:p.walkPain<=1;
 add('Walking',`${p.walkPain}/10 pain`,nominalWalk,walkAhead?'ahead':'behind',walkAhead?'Walking tolerance meets or exceeds the nominal stage expectation.':'Walking pain is still above the nominal stage expectation.');
 if(p.hasCheckIns){
  const expectedStrength=expectedStage>=2,actualStrength=!!l.strength;
  add('Strength control',actualStrength?'Tolerated':'Not yet demonstrated',expectedStrength?'Expected':'Not yet required',actualStrength===expectedStrength||actualStrength?'ahead':'behind',actualStrength?'Controlled strength is supporting progression.':'Strength control is the clearest missing functional criterion.');
  const expectedHop=expectedStage>=3,actualHop=!!l.hop;
  add('Impact tolerance',actualHop?'Hop tolerated':'Not yet demonstrated',expectedHop?'Expected':'Not yet required',actualHop===expectedHop||actualHop?'ahead':'behind',actualHop?'Impact tolerance is ahead of or consistent with nominal.':expectedHop?'Hopping is behind the nominal functional stage.':'Hopping is not yet expected at this stage.');
  const expectedRun=expectedStage>=4,actualRun=(Number(l.runMinutes)||0)>=10;
  add('Running exposure',`${Number(l.runMinutes)||0} min`,expectedRun?'≥10 min expected':'Not yet expected',actualRun===expectedRun||actualRun?'ahead':'behind',actualRun?'Running tolerance is supporting a later-stage score.':expectedRun?'Running exposure is behind nominal.':'Running is correctly not yet required.');
  add('Next-morning response',l.nextDayWorse?'Worse':'Stable','Stable',l.nextDayWorse?'behind':'ahead',l.nextDayWorse?'Delayed worsening indicates the latest load exceeded current tolerance.':'No delayed flare supports progression.');
 }
 return signals;
}
function trajectoryComparisonHtml(p){
 if(!p.hasCheckIns)return`<div class="trajectoryComparison awaiting"><b>No comparison yet</b><p>Your first check-in establishes the observed baseline. The nominal line is based only on the initial estimated rehabilitation duration.</p></div>`;
 const signals=nominalEvidence(p),ahead=signals.filter(x=>x.status==='ahead'),behind=signals.filter(x=>x.status==='behind');
 if(!p.comparisonReady)return`<div class="trajectoryComparison awaiting"><b>Building a reliable trajectory</b><p>Current criteria completion is <strong>${p.score}%</strong>; nominal progress at injury day ${p.elapsed} is <strong>${p.expectedScore}%</strong>. Two check-ins spanning at least two days are required before classifying the overall trajectory.</p><div class="evidenceRows">${signals.map(x=>`<div class="evidenceRow ${x.status}"><span>${esc(x.label)}</span><b>${esc(x.actual)}</b><small>${esc(x.why)}</small></div>`).join('')}</div></div>`;
 const word=p.delta>0?'ahead of':p.delta<0?'behind':'aligned with';
 const summary=p.pace==='faster'?`${ahead.length} observed signals are ahead of nominal and ${behind.length} are behind.`:p.pace==='slower'?`${behind.length} observed signals are behind nominal and ${ahead.length} are ahead.`:`The observed signals are mixed and broadly close to nominal.`;
 return`<div class="trajectoryComparison ${p.pace==='faster'?'good':p.pace==='slower'?'bad':'warn'}"><b>Why recovery is ${p.pace==='faster'?'faster':p.pace==='slower'?'slower':'close to'} nominal</b><div class="compareGrid"><span>Your criteria completion<strong>${p.score}%</strong></span><span>Nominal at day ${p.elapsed}<strong>${p.expectedScore}%</strong></span><span>Difference<strong>${Math.abs(p.delta)} points ${word} nominal</strong></span><span>Stage comparison<strong>${esc(INJURY_STAGES[p.stage].name)} vs nominal ${esc(INJURY_STAGES[p.nominalStage].name)}</strong></span></div><p>${esc(summary)} The classification uses pain together with walking, strength, impact, running and next-morning response.</p><div class="evidenceRows">${signals.map(x=>`<div class="evidenceRow ${x.status}"><span>${esc(x.label)}</span><b>${esc(x.actual)} <em>vs ${esc(x.nominal)}</em></b><small>${esc(x.why)}</small></div>`).join('')}</div></div>`;
}
function injuryGuidance(i,p){const latest=p.checks.at(-1);let headline,body,action;if(!latest){headline='Complete your first daily check-in';body='No recovery trend is calculated yet. The injury date and current symptoms provide only an initial full-training estimate.';action='Record today’s baseline before progressing exercises.'}else if(latest.nextDayWorse||latest.swelling||p.trend<=-8){headline='Hold or step back today';body='Your latest response suggests the previous load exceeded current tolerance.';action='Repeat the last well-tolerated level, reduce volume by about 25–40%, and reassess tomorrow.'}else if(!p.comparisonReady){headline='Building your recovery trajectory';body='One observation cannot establish whether recovery is faster or slower than nominal.';action='Complete the prescribed work and add another check-in after the next-morning response.'}else if(p.pace==='faster'){headline='Progressing faster than nominal';body=`Functional criteria completion is ${p.score}% versus ${p.expectedScore}% nominal at this point.`;action='Progress only one variable today: repetitions, resistance, impact or running duration—not several together.'}else if(p.pace==='slower'){headline='Progressing slower than nominal';body=`Functional criteria completion is ${p.score}% versus ${p.expectedScore}% nominal at this point.`;action='Keep the current stage, prioritise consistent low-irritability loading, and seek clinical review if the plateau persists or red flags appear.'}else{headline='Recovery is tracking close to nominal';body=`Functional criteria completion is ${p.score}% versus ${p.expectedScore}% nominal.`;action='Complete today’s exercises and progress only when the next-morning response remains stable.'}return{headline,body,action}}
function weeklyRehabSummary(p){const recent=p.checks.filter(c=>dte(c.date)>=new Date(today().getTime()-6*DAY));if(!recent.length)return'No check-ins in the last 7 days.';const first=recent[0],last=recent.at(-1),painChange=(Number(last.pain)||0)-(Number(first.pain)||0),run=Math.max(...recent.map(c=>Number(c.runMinutes)||0));return`${recent.length} check-in${recent.length===1?'':'s'} · pain ${painChange<0?Math.abs(painChange)+' lower':painChange>0?painChange+' higher':'unchanged'} · longest run ${run} min`}
function injuryClinicalAssessment(i){
 if(i.medicalDiagnosis)return{label:'Recorded clinical diagnosis',diagnosis:i.medicalDiagnosis,confidence:'User/clinician supplied',basis:['Entered in the injury record'],alternatives:[]};
 const profile=injuryProfile(i),text=((i.location||'')+' '+(i.mechanism||'')+' '+(i.initialSymptoms||'')).toLowerCase();
 if(profile==='hamstring'){
  const acute=/sprint|accelerat|sudden|sharp|pull/.test(text),proximal=/upper|proximal|glute|sit bone|ischial/.test(text),severity=(i.bruising||i.pop||Number(i.initialWalkPain)>=5||Number(i.initialPain)>=6);
  return{label:'Working clinical assumption — not a diagnosis',diagnosis:`${severity?'Moderate':'Mild-to-moderate'} ${proximal?'proximal ':''}hamstring muscle–tendon strain${severity?' (often described clinically as grade 2)':''}`,confidence:acute?'Moderate':'Low to moderate',basis:[acute?'Acute sprinting or rapid-loading mechanism':'Posterior-thigh symptom pattern',i.bruising?'Bruising/swelling reported':'No bruising entered',i.pop?'Pop or snap reported':'No pop entered',`Initial pain ${Number(i.initialPain)||0}/10 and walking pain ${Number(i.initialWalkPain)||0}/10`],alternatives:['Lower-grade hamstring strain','Proximal hamstring tendon injury or tendinopathy','Referred pain from the hip or lower back']};
 }
 const names={calf:'Calf muscle–tendon strain or Achilles-related overload',knee:'Running-related knee soft-tissue overload',generic:'Lower-limb soft-tissue injury'};
 return{label:'Working clinical assumption — not a diagnosis',diagnosis:names[profile],confidence:'Low to moderate',basis:[`Location pattern: ${i.location||'not specified'}`,`Mechanism: ${i.mechanism||'not specified'}`,`Initial pain ${Number(i.initialPain)||0}/10 and walking pain ${Number(i.initialWalkPain)||0}/10`],alternatives:['A different muscle or tendon injury','Joint or bone-related pain','Referred symptoms from another region']};
}
function timelineRationaleHtml(i,p){
 const severity=Number(i.initialPain)>=8||i.pop||Number(i.initialWalkPain)>=8?'higher-severity markers':Number(i.initialPain)>=6||i.bruising||Number(i.initialWalkPain)>=5?'moderate-severity markers':Number(i.initialPain)>=3?'mild-to-moderate markers':'mild markers';
 const adjustment=p.dateShift==null?'No trajectory adjustment is applied until at least two check-ins span two days.':p.dateShift===0?'Observed progress is close enough to nominal that the original estimate remains unchanged.':`Observed progress shifts the forecast ${Math.abs(p.dateShift)} day${Math.abs(p.dateShift)===1?'':'s'} ${p.dateShift<0?'earlier':'later'}.`;
 const latest=p.checks.at(-1)||{};
 const factors=[
  [`Starting range`,`${p.base} days`,`Derived from ${severity}: initial pain ${Number(i.initialPain)||0}/10, walking pain ${Number(i.initialWalkPain)||0}/10${i.bruising?', bruising/swelling':''}${i.pop?', and a reported pop/snap':''}.`],
  ['Current stage',`Stage ${p.stage+1} of 6`,`${INJURY_STAGES[p.stage].name}: ${INJURY_STAGES[p.stage].goal}`],
  ['Pain and walking',`${p.currentPain}/10 · ${p.walkPain}/10`,p.currentPain<=2&&p.walkPain<=1?'These support progression, but do not prove impact or running readiness.':'These remain a limiting factor in the estimate.'],
  ['Functional evidence',`${latest.strength?'strength ✓':'strength not shown'} · ${latest.hop?'hop ✓':'hop not shown'} · ${Number(latest.runMinutes)||0} min run`,'Unrestricted training requires strength, impact and sustained easy-running evidence—not symptom reduction alone.'],
  ['Delayed response',p.hasCheckIns?(latest.nextDayWorse?'Worse next morning':'No flare recorded'):'Not yet observed',latest.nextDayWorse?'A next-day flare delays progression because the previous load exceeded current tolerance.':'A stable next morning supports, but does not by itself guarantee, progression.']
 ];
 return `<details class="timelineRationale"><summary>Why this full-training date?</summary><div class="timelineRationaleIntro"><b>${fmtDate(p.fullReturnDate)} is an estimate for unrestricted running training</b><p>The model starts with a symptom-severity range, then adjusts only after repeated observations. It is not the date of the first walk–run session and it is not medical clearance.</p></div><div class="rationaleRows">${factors.map(x=>`<div><span>${esc(x[0])}</span><b>${esc(x[1])}</b><p>${esc(x[2])}</p></div>`).join('')}</div><div class="note"><b>Forecast adjustment</b><p>${esc(adjustment)}</p></div></details>`;
}
function diagnosisHtml(i){const d=injuryClinicalAssessment(i);return `<div class="clinicalAssessment"><div class="clinicalHead"><div><small>${esc(d.label)}</small><h3>${esc(d.diagnosis)}</h3></div><span>${esc(d.confidence)} confidence</span></div><div class="clinicalColumns"><div><b>Why this is assumed</b><ul>${d.basis.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div><b>Important alternatives</b><ul>${(d.alternatives.length?d.alternatives:['No alternatives listed for a recorded diagnosis']).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div></div><p class="clinicalBoundary">This is a rehabilitation working assumption, not a confirmed diagnosis. Persistent weakness, marked bruising, pain at the sitting bone, recurrent symptoms, or failure to progress warrants clinical assessment and may require imaging.</p></div>`}
function renderInjury(){
 const box=$('injuryList');if(!box)return;const injuries=state.injuries||[];
 $('injuryIntro').innerHTML=`<div class="rehabIntro"><div><b>Adaptive rehabilitation coach</b><p>Exercises, stage criteria and the estimated date for full unrestricted running training update from your daily response. Injury data remains independent from plan generation and race prediction.</p></div><div class="rehabLoop"><span>Check in</span><i>→</i><span>Exercise</span><i>→</i><span>Reassess</span><i>→</i><span>Progress</span></div></div>`;
 box.innerHTML=injuries.length?injuries.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(i=>{
  const p=injuryPrediction(i),st=INJURY_STAGES[p.stage],g=injuryGuidance(i,p),shift=p.dateShift==null?'Not adjusted yet':p.dateShift===0?'Unchanged':`${Math.abs(p.dateShift)} day${Math.abs(p.dateShift)===1?'':'s'} ${p.dateShift<0?'earlier':'later'}`;
  const paceClass=p.pace==='faster'?'good':p.pace==='slower'?'bad':'warn';
  const trajectoryText=p.pace==='awaiting'?'Awaiting check-in':p.pace==='building'?'Building trajectory':p.pace==='faster'?'Faster than nominal':p.pace==='slower'?'Slower than nominal':'Close to nominal';
  const deltaText=!p.comparisonReady?'Comparison pending':`${p.delta>0?'+':''}${p.delta} points`;
  return`<article class="panel injuryCard"><div class="panelHead"><div><h3>${esc(i.location||'Injury')} · ${fmtDate(i.date)}</h3><p class="muted compact">${esc(i.mechanism||'Mechanism not entered')}</p></div><span class="status ${p.stage>=3?'completed':p.currentPain>=5?'missed':'today'}">Stage ${p.stage+1}</span></div>
  <div class="rehabHero"><div class="recoveryGauge ${p.hasCheckIns?'':'unscored'}" style="--recovery:${p.score??0}%"><div><strong>${p.score??'—'}</strong><span>${p.hasCheckIns?'rehab complete':'awaiting baseline'}</span></div></div><div><small>Current rehabilitation stage</small><h3>${esc(st.name)}</h3><p>${esc(st.goal)}</p><div class="trajectoryBadge ${paceClass}">${trajectoryText} · ${deltaText}</div></div></div>
  ${diagnosisHtml(i)}
  <p class="scoreMeaning"><b>What the score means:</b> percentage of the criteria-based route to <strong>full unrestricted running training</strong>. Low pain alone cannot produce a high score; impact, running duration, next-day response and confidence must also be demonstrated.</p>
  ${progressionCriteriaHtml(p)}
  <div class="injuryForecast three"><div><small>Estimated full unrestricted training</small><strong>${p.stage>=5&&p.score>=95?'Criteria nearly complete':fmtDate(p.fullReturnDate)}</strong><span>${p.remaining} estimated days remaining · ${p.confidence.toLowerCase()} confidence</span></div><div><small>Original full-training estimate</small><strong>${fmtDate(p.originalDate)}</strong><span>${shift}</span></div><div><small>Last 7 days</small><strong>${p.checks.length?p.trend>0?'Improving':p.trend<0?'Declining':'Stable':'Awaiting data'}</strong><span>${weeklyRehabSummary(p)}</span></div></div>
  ${timelineRationaleHtml(i,p)}
  ${trajectoryComparisonHtml(p)}
  <div class="coachDirective ${paceClass}"><b>${esc(g.headline)}</b><p>${esc(g.body)}</p><strong>Today: ${esc(g.action)}</strong></div>
  <article class="rehabVisual"><div class="panelHead"><div><h3>Recovery trajectory</h3><p class="muted compact">Observed criteria completion compared with nominal progression toward unrestricted training.</p></div></div>${trajectorySvg(i,p)}</article>
  <div class="rehabSectionHead"><div><span>Today</span><h3>Exercise prescription</h3></div><small>Tap any exercise to open detailed written setup, movement, technique, pain-limit, regression and progression guidance. No image or video is used.</small></div><div class="exerciseGrid">${rehabExercises(i,p)}</div>
  <div class="rehabSectionHead"><div><span>Progression</span><h3>All rehabilitation stages</h3></div><small>Stage 4 starts walk–run exposure. Stage 6 completes the route to unrestricted training.</small></div><div class="injuryTimeline visualTimeline">${INJURY_STAGES.map((x,n)=>`<div class="injuryStage ${n<p.stage?'done':n===p.stage?'active':''}"><i>${n<p.stage?'✓':n+1}</i><div><b>${esc(x.name)}</b><small>${esc(x.criteria)}</small></div></div>`).join('')}</div>
  <details><summary>Initial symptoms, latest response and model inputs</summary><div class="guideGrid"><div><b>At injury</b><p>Pain ${Number(i.initialPain)||0}/10 · walking pain ${Number(i.initialWalkPain)||0}/10${i.pop?' · pop felt':''}${i.bruising?' · bruising/swelling':''}<br>${esc(i.initialSymptoms||'No symptom description')}</p></div><div><b>Latest</b><p>Pain ${p.currentPain}/10 · walking pain ${p.walkPain}/10<br>${esc(p.checks.at(-1)?.symptoms||i.currentSymptoms||'No current symptom description')}</p></div></div></details>
  <div class="buttonRow"><button data-injury-check="${i.id}" class="primary">Daily check-in</button><button data-injury-edit="${i.id}">Edit injury</button><button data-injury-delete="${i.id}" class="danger">Delete</button></div></article>`
 }).join(''):'<article class="panel injuryEmpty"><div class="injuryEmptyIcon">✚</div><h3>Start an adaptive rehabilitation journey</h3><p>Record an injury to receive a staged exercise prescription, daily guidance, progress tracking and an adjustable estimate for full unrestricted running training.</p><button id="emptyAddInjury" class="primary">Record an injury</button></article>';
}
function injuryForm(i={}){return`<h3>${i.id?'Edit':'Record'} injury</h3><p class="muted">This establishes the starting point. The app will not diagnose an injury; a clinical diagnosis can be recorded when available.</p><div class="formGrid"><div class="field"><label>Injury date</label><input id="injDate" type="date" value="${i.date||iso(today())}"></div><div class="field"><label>Area / location</label><input id="injLocation" value="${esc(i.location||'')}" placeholder="e.g. left upper hamstring"></div><div class="field"><label>How did it happen?</label><input id="injMechanism" value="${esc(i.mechanism||'')}" placeholder="e.g. sprint, gradual onset"></div><div class="field"><label>Clinical diagnosis (optional)</label><input id="injDiagnosis" value="${esc(i.medicalDiagnosis||'')}" placeholder="e.g. grade 2 hamstring strain"></div><div class="field"><label>Pain then 0–10</label><input id="injInitialPain" type="number" min="0" max="10" value="${i.initialPain??''}"></div><div class="field"><label>Walking pain then 0–10</label><input id="injInitialWalk" type="number" min="0" max="10" value="${i.initialWalkPain??''}"></div><div class="field"><label>Signs then</label><label><input id="injPop" type="checkbox" ${i.pop?'checked':''}> Pop / snap</label><label><input id="injBruise" type="checkbox" ${i.bruising?'checked':''}> Bruising or swelling</label></div><div class="field"><label>Symptoms then</label><textarea id="injInitialSymptoms">${esc(i.initialSymptoms||'')}</textarea></div><div class="field"><label>Symptoms now</label><textarea id="injCurrentSymptoms">${esc(i.currentSymptoms||'')}</textarea></div><div class="field"><label>Pain now 0–10</label><input id="injCurrentPain" type="number" min="0" max="10" value="${i.currentPain??''}"></div><div class="field"><label>Walking pain now 0–10</label><input id="injCurrentWalk" type="number" min="0" max="10" value="${i.currentWalkPain??''}"></div></div><div class="note bad"><b>Seek urgent assessment</b> for major trauma, inability to bear weight, marked weakness or deformity, rapidly increasing swelling, numbness, fever, severe night pain, or calf swelling/breathlessness.</div><button id="saveInjury" class="primary full">Save injury and create rehabilitation plan</button>`}
function openInjuryForm(i){$('modalContent').innerHTML=injuryForm(i);$('modal').className='modal';$('saveInjury').onclick=()=>{let obj={...(i||{}),id:i?.id||'inj-'+Date.now(),date:$('injDate').value,location:$('injLocation').value.trim(),mechanism:$('injMechanism').value.trim(),medicalDiagnosis:$('injDiagnosis').value.trim(),initialPain:Number($('injInitialPain').value),initialWalkPain:Number($('injInitialWalk').value),pop:$('injPop').checked,bruising:$('injBruise').checked,initialSymptoms:$('injInitialSymptoms').value.trim(),currentSymptoms:$('injCurrentSymptoms').value.trim(),currentPain:Number($('injCurrentPain').value),currentWalkPain:Number($('injCurrentWalk').value),checkIns:i?.checkIns||[]};if(!obj.date||!obj.location)return toast('Enter the injury date and location.',true);state.injuries=state.injuries||[];let n=state.injuries.findIndex(x=>x.id===obj.id);n>=0?state.injuries[n]=obj:state.injuries.push(obj);save();$('modal').className='modal hidden';renderInjury();toast('Adaptive rehabilitation plan created.')}}
function openInjuryCheck(i){const last=(i.checkIns||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)).at(-1)||{};$('modalContent').innerHTML=`<h3>Daily rehabilitation check-in</h3><p class="muted">Record the response you have now. Next-morning response is especially important after running or a harder exercise session.</p><div class="formGrid"><div class="field"><label>Date</label><input id="icDate" type="date" value="${iso(today())}"></div><div class="field"><label>Pain now 0–10</label><input id="icPain" type="number" min="0" max="10" value="${last.pain??''}"></div><div class="field"><label>Morning stiffness 0–10</label><input id="icStiffness" type="number" min="0" max="10" value="${last.stiffness??''}"></div><div class="field"><label>Walking pain 0–10</label><input id="icWalk" type="number" min="0" max="10" value="${last.walkPain??''}"></div><div class="field"><label>Walking completed (minutes)</label><input id="icWalkMin" type="number" min="0" value="${last.walkMinutes??0}"></div><div class="field"><label>Exercises completed</label><select id="icExercises"><option value="all">All prescribed</option><option value="some">Some</option><option value="none">None</option></select></div><div class="field"><label>Function today</label><label><input id="icStairs" type="checkbox" ${last.stairs?'checked':''}> Stairs comfortable</label><label><input id="icStrength" type="checkbox" ${last.strength?'checked':''}> Prescribed strength controlled</label><label><input id="icHop" type="checkbox" ${last.hop?'checked':''}> Gentle hop/jog-in-place tolerated</label></div><div class="field"><label>Running completed (minutes)</label><input id="icRun" type="number" min="0" value="${last.runMinutes??0}"></div><div class="field"><label>Confidence in the injured area 0–10</label><input id="icConfidence" type="number" min="0" max="10" value="${last.confidence??5}"></div><div class="field"><label>Swelling or new bruising?</label><select id="icSwelling"><option value="false">No</option><option value="true">Yes</option></select></div><div class="field"><label>Worse later or next morning?</label><select id="icWorse"><option value="false">No</option><option value="true">Yes</option></select></div><div class="field"><label>Symptoms / notes</label><textarea id="icSymptoms">${esc(last.symptoms||'')}</textarea></div></div><button id="saveCheck" class="primary full">Save and update guidance</button>`;$('modal').className='modal';$('saveCheck').onclick=()=>{let c={date:$('icDate').value,pain:Number($('icPain').value),stiffness:Number($('icStiffness').value),walkPain:Number($('icWalk').value),walkMinutes:Number($('icWalkMin').value)||0,exercises:$('icExercises').value,stairs:$('icStairs').checked,strength:$('icStrength').checked,bridge:$('icStrength').checked,hop:$('icHop').checked,runMinutes:Number($('icRun').value)||0,confidence:Number($('icConfidence').value),swelling:$('icSwelling').value==='true',nextDayWorse:$('icWorse').value==='true',symptoms:$('icSymptoms').value.trim()};if(!c.date||![c.pain,c.stiffness,c.walkPain,c.confidence].every(Number.isFinite))return toast('Enter date, pain, stiffness, walking pain and confidence.',true);i.checkIns=i.checkIns||[];let n=i.checkIns.findIndex(x=>x.date===c.date);n>=0?i.checkIns[n]=c:i.checkIns.push(c);save();$('modal').className='modal hidden';renderInjury();toast('Exercises and recovery trajectory updated.')}}
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.8.6 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.8.6 stable build 9860');
})();