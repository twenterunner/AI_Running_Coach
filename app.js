...
function shoeProgrammeMileageChartHtml(){
 ...
 const markers=rawEvents.filter(e=>enabled.has(e.shoe.id)).map(e=>{const xx=x(e.date),yy=y(Math.min(maxY,e.km));return`<g class="shoeReplacementMarker"><line x1="${xx}" y1="${T}" x2="${xx}" y2="${H-B}"/><circle cx="${xx}" cy="${yy}" r="7"/></g>`}).join('');
 ...
 const allocationHtml=uniqueEvents.length?`<div class="shoeAllocationRationale"><div class="shoeAllocationRationaleHead"><small>WHY THE CURVES CHANGE</small><b>What drives the slope changes?</b><p>Only material changes are listed. Each is classified by its actual source.</p></div>${uniqueEvents.map(e=>`<article class="shoeAllocationEvent driver-${e.driver.toLowerCase().replace(/[^a-z]+/g,'-')}"><div><span class="shoeDriverTag">${esc(e.driver)}</span><time>${esc(shoeChartDate(e.date))}</time></div><b>${esc(e.title)}</b><p>${esc(e.text)}</p></article>`).join('')}</div>`:'';
...
function openRunDetails(runId){
 ...
 $('deleteEditedRun').onclick=()=>{
   ...
   state.runs=state.runs.filter(x=>x.id!==r.id);state.shoeUsage=(state.shoeUsage||[]).filter(u=>u.runId!==r.id);state.predictionHistory=(state.predictionHistory||[]).filter(x=>x.entityId!==r.id);save();$('modal').className='modal hidden';renderAll();toast('Run deleted.');
 };
}
