...
function dailyCoachFocus(p){
 ...
 const recovery=ast.readiness==='Normal'?'Recovery signals currently support normal progression.':ast.readiness==='Reduced'?'Recovery signals suggest a cautious day; reduce optional effort.':'Recovery signals currently restrict progression; prioritise recovery over added training.';
 const pain=!ast.pain.count?'No recent pain data are available. Log pain after any activity.':ast.pain.max>=5?'Recent pain evidence says to stop demanding running and reassess before the next session.':ast.pain.max>=3?'Recent pain evidence says to keep activity easy and avoid adding load or intensity.':'Recent pain evidence does not currently restrict the planned session.';
 const injury=state.activeInjuryPlanId?'<p><b>Active injury plan:</b> Follow today’s rehabilitation prescription and complete the injury check-in; it takes priority over optional activity.</p>':'';
 if(p){
  return `<div class="todayCoachGrid"><section class="todayFocusPrimary"><span>Today’s priority</span><h4>${esc(p.type)}</h4><p>${esc(p.purpose)}</p></section><section><span>Execution cue</span><p>${esc(p.coach)}</p></section><section><span>Readiness check</span><p>${esc(recovery)} ${esc(pain)}</p>${injury}</section></div>`;
 }
 const next=state.plan.filter(item=>item.date>iso(today())&&!['Rest'].includes(item.type)).sort((a,b)=>a.date.localeCompare(b.date))[0];
 const nextText=next?`Next planned session: ${next.type} on ${fmtDate(next.date)}${Number(next.distance)>0?` · ${next.distance.toFixed(1)} km`:''}.`:'No future training session is currently scheduled.';
 return `<div class="todayCoachGrid"><section class="todayFocusPrimary"><span>Today’s priority</span><h4>Recovery and preparation</h4><p>No workout is scheduled. Do not add a catch-up session; use rest or comfortable everyday movement.</p></section><section><span>What comes next</span><p>${esc(nextText)}</p></section><section><span>Readiness check</span><p>${esc(recovery)} ${esc(pain)}</p>${injury}</section></div>`;
}
...
function stageSpecificStretchGoal(i,p,type,walk,items){
 ...
 if(p.safetyHold||family==='bone')return{offered:false,title:'No stretch goal today',text:'Do not increase loading. Follow the protective plan and obtain the recommended clinical assessment.',why:'Higher-risk patterns should not use optional progression before clearance.'};
 if(unavailable)return{offered:false,title:'Stretch goal unavailable today',text:'Complete only today’s prescribed rehabilitation. Do not add volume while pain, gait, swelling or delayed response is unfavourable.',why:'A stretch goal is withheld because current evidence does not support extra loading.'};
 if(stage===0)return{offered:false,title:'No stretch goal today',text:'Recovery and comfortable movement are the goal. Do not increase loading today.',why:'The first phase prioritises settling symptoms rather than exceeding the prescription.'};
 if(type==='impact')return{offered:true,title:'Optional stretch goal',text:`Complete the impact assessment exactly as prescribed. If every repetition is controlled and pain remains ≤2/10, add 5 minutes of walking (${walk.target+5} minutes total). Do not add hops or jogging.`,why:'Today is an assessment day. Extra walking is lower risk, while extra impact would make the response harder to interpret.'};
 if(stage===1)return{offered:true,title:'Optional stretch goal',text:`If the prescribed work feels easier than expected and pain remains ≤2/10 with normal gait, add 5 minutes of walking (${walk.target+5} minutes total).`,why:'A small walking increase tests daily-function capacity without changing exercise load.'};
 if(stage===2){
  let text='If all prescribed repetitions are controlled and pain remains ≤2/10, add one extra set or 2–3 repetitions to the final set—choose one, not both.';
  if(family==='tendon')text='If all prescribed repetitions are controlled and symptoms remain ≤2/10, add one extra controlled set to the primary loading exercise; do not add speed or impact.';
  if(['foot','ankle','knee','hip'].includes(family))text='If all prescribed exercises are controlled and symptoms remain ≤2/10, add one extra set to the primary control exercise; do not add impact.';
  return{offered:true,title:'Optional stretch goal',text,why:'This adds a small strength dose while keeping the day’s main loading variable controlled.'};
 }
 if(stage===3)return{offered:true,title:'Optional stretch goal',text:'If every planned interval is completed with pain ≤2/10 and normal gait, add one additional walk–run interval only. Do not lengthen every running interval.',why:'One extra interval increases exposure gradually without changing several variables at once.'};
 return{offered:true,title:'Optional stretch goal',text:'If the full prescribed run is completed with pain ≤2/10 and normal gait, increase total running time by no more than 5–10%. Confirm that symptoms are unchanged later and the next morning before retaining the increase.',why:'A small volume increase is appropriate only after stable easy-running tolerance.'};
}
...
function openInjuryCheck(i,existing=null){
 ...
 const setConsistencyMessage=()=>{const rs=runStatusField.value,mins=nullableNumber(runMinutesField.value),walkMins=nullableNumber(walkMinutesField.value),target=activeScheduled.walkingTargetMinutes,lo=locomotionField.value,ex=exerciseField.value,sg=stretchField.value;let parts=[];
  ...
  if(sg==='achieved')parts.push('Optional stretch goal achieved.');
...
if('serviceWorker'in navigator&&(location.protocol==='https:'||['localhost','127.0.0.1'].includes(location.hostname)))navigator.serviceWorker.register(`service-worker.js?v=${BUILD}-ui1`,{updateViaCache:'none'}).then(reg=>reg.update()).catch(()=>{});
