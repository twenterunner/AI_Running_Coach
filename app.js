...
const SHOE_ENGINE_STRATEGY={key:'session-suitability',label:'Session suitability',desc:'Every session is ranked by suitability. The best-suited shoes are protected for the sessions that add most race value; lower-priority sessions may use the next-best pair so each principal available pair reaches at least 25% of weekly volume. Purchases are made only when the useful 2–3 pair rotation or lifecycle genuinely requires one. New purchases are preferably separated by about 12 weeks; the engine may delay a later purchase or bring an earlier one forward by reallocating suitable mileage.'};
...
function shoeEngineRepairPurchaseSpacing(result,allSessions,manual){
 ...
 const moveCost=(fromPair,toPair,plan,row)=>{
  ...
  // Race-specific quality is deliberately expensive, but not forbidden.
  const quality=/Race-pace|Marathon-specific|Half-marathon-specific|Specific|rehearsal|Threshold|interval|Long|Progression|tempo/i.test(String(plan.type||''))?2.2:1;
  return imp*Math.max(1,loss)*quality;
 };
 ...
 const tryAdvanceEarlier=(purchases,index,buyDate)=>{
  ...
  prev.spacingAdjusted=true;prev.spacingReason=`Purchase brought forward to ${newFirst.date} so the next planned purchase is better separated; lower-value sessions are preferred for the additional early mileage, with quality-session compromise allowed only when needed.`;
  return true
 };
...
function freshShoeLifecyclePlan(){
 const now=iso(today()),raceDate=state.setup?.raceDate||now,manual=new Map((state.plannedShoeAssignments||[]).filter(a=>a.source==='user').map(a=>[a.planId,a.shoeId])),fixed=(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>=now&&p.date<=raceDate).map(p=>({...p,rehab:false,importance:shoeEngineSessionImportance(p)})).sort((a,b)=>a.date.localeCompare(b.date)),rehab=shoeEngineBuildRehabSessions(now,raceDate),allSessions=[...fixed,...rehab].sort((a,b)=>a.date.localeCompare(b.date)||Number(b.rehab)-Number(a.rehab)),injury=(state.injuries||[]).find(x=>x.id===state.activeInjuryPlanId),rehabStamp=injury?JSON.stringify({id:injury.id,plannedRehabShoes:injury.plannedRehabShoes||{},checks:(injury.checkIns||[]).map(c=>[c.date,c.walkMinutes,c.runMinutes,c.rehabShoeId,c.pain,c.hop,c.bridge]),updatedAt:injury.updatedAt||injury.lastUpdated||''}):'',stamp=['race-first-spacing-v4-final-ledger-sync',state.storageRevision||0,runnerFootMechanics(),raceDate,OFFLINE_ASICS_CATALOGUE_VERSION,fixed.map(p=>`${p.id}:${p.date}:${p.type}:${p.distance}:${p.surface||''}`).join(';'),(state.shoes||[]).map(sh=>`${sh.id}:${sh.status}:${shoeMileage(sh).toFixed(2)}:${sh.condition||sh.conditionFeedback||''}`).join(';'),[...manual].join(';'),rehabStamp].join('|');
 ...
 const preRaceResult={pairs,assignments,purchases,events};shoeEngineRepairActiveRotation(preRaceResult,allSessions,manual,purchases,events,now);shoeEngineRepairPurchaseSpacing(preRaceResult,allSessions,manual);
 ...
 const result={strategy:'session-suitability',now,raceDate,fixed,allSessions,pairs,assignments,purchases,events,racePair:raceChoice?.pair||null,raceWindow:raceChoice?.window||null,catalogueSource:'offline',catalogueVersion:OFFLINE_ASICS_CATALOGUE_VERSION,footMechanics:runnerFootMechanics(),engine:'race-first-spacing-v4-final-ledger-sync'};
 result.pairs.forEach(pair=>lifecycleRebuildPoints(pair,now,raceDate,fixed));
 if(result.racePair&&!shoeEngineFinalizeRaceDay(result))result.racePair=null;result.purchases=result.purchases.filter(b=>{const p=result.pairs.find(x=>x.id===b.pairId);return p&&(p.assignments.length||p===result.racePair)}).map(b=>{const p=result.pairs.find(x=>x.id===b.pairId),first=p.assignments.slice().sort((a,b)=>a.date.localeCompare(b.date))[0];if(first){b.purchaseDate=first.date;b.firstUseDate=first.date;p.plannedEntryDate=first.date;p.availableDate=first.date;p.purchaseDate=first.date}b.reason=String(b.reason||'').replace(/\s*Purchase spacing target:\s*\d+ days[\s\S]*?Race Day readiness\./g,'').replace(/\s*Purchase-spacing optimisation[^.]*\./g,'').trim();if(b.spacingReason)b.reason=(b.reason?b.reason+' ':'')+b.spacingReason;return b}).sort((a,b)=>a.purchaseDate.localeCompare(b.purchaseDate));result.pairs=result.pairs.filter(p=>p.owned||p.assignments.length||p===result.racePair);result.validationIssues=shoeEngineValidation(result);result.softTargets=shoeEngineSoftTargets(result);result.valid=result.validationIssues.length===0;result.hardInvariantFailures=result.validationIssues.length;freshShoePlanCache={stamp,value:result};return result
}
