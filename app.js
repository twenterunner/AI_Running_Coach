...
function shoePlannedUsage(shoeId,days=null){const now=iso(today()),end=days?iso(new Date(today().getTime()+days*DAY)):state.setup.raceDate;let km=0;(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>=now&&p.date<=end).forEach(p=>{const a=plannedAssignment(p.id);if(a?.shoeId===shoeId)km+=Number(p.distance)||0});return km}
...
function shoePlannedUsageThrough(shoeId,date){const now=iso(today()),end=date||state.setup.raceDate;let km=0;(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>=now&&p.date<=end).forEach(p=>{const a=plannedAssignment(p.id);if(a?.shoeId===shoeId)km+=Number(p.distance)||0});return km}
...
function racePlanHtml(){const plan=raceShoePlan();if(plan.status!=='ok')return'<div class="shoeEmpty">Race-day shoe planning needs a valid race date.</div>';if(!(state.shoes||[]).length&&!plan.purchaseNeeded)return'<div class="shoeEmpty">Add your current rotation to plan race-day shoe condition.</div>';const preferred=plan.preferred,cond=plan.condition;let main='';if(preferred){main=`<article class="raceShoeHero ${cond.code}"><div><small>RACE SHOE</small><h4>${esc(shoeDisplayName(preferred))}</h4><p>${Math.round(cond.current)} km now · ${Math.round(cond.projected)} km projected on race day</p></div><div><small>FAMILIARITY</small><b>${cond.fam.runs>=3?'Proven':cond.fam.runs>=1?'Building':'Unproven'}</b><span>${cond.fam.runs} verified runs · ${cond.fam.km.toFixed(1)} km</span></div><div><small>RACE-DAY OUTLOOK</small><b>${esc(cond.label)}</b><span>Target familiarisation window ~${Math.round(cond.opt.minKm)}–${Math.round(cond.opt.maxKm)} km</span></div></article>`}else main='<div class="shoeWarning">No owned active shoe currently provides a strong race-day match.</div>';
 ...
 const purchase=plan.purchaseNeeded&&plan.profile?`<article class="shoePurchasePlan"><div><small>MODEL RECOMMENDATION</small><h4>ASICS ${esc(plan.profile.family)} ${esc(plan.profile.version)}</h4><p>${esc(plan.reason)}</p></div><div class="shoePurchaseMetrics"><div><small>PURCHASE WINDOW</small><b>${plan.purchaseStart&&plan.purchaseEnd?`${fmtDate(plan.purchaseStart)} – ${fmtDate(plan.purchaseEnd)}`:'Insufficient evidence'}</b></div><div><small>PROJECTED RACE-DAY MILEAGE</small><b>~${Math.round(plan.projectedNew)} km</b></div><div><small>PLANNING CONFIDENCE</small><b>${esc(plan.confidence)}</b></div></div>${plan.existingPlanned?`<span class="shoeMatchBadge">Planned purchase saved</span>`:`<button id="saveRacePurchasePlan" class="secondary small" type="button">Save this purchase plan</button>`}</article>`:`<article class="shoeNoPurchase"><b>No new race shoe currently required</b><p>The best owned race shoe is projected to remain usable; preserve it for relevant sessions rather than adding unnecessary mileage.</p></article>`;
...
function shoePlannedMileageSeries(shoe){const now=iso(today()),raceDate=state.setup?.raceDate||now,current=shoeMileage(shoe),plans=(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>=now&&p.date<=raceDate).slice().sort((a,b)=>a.date.localeCompare(b.date)),points=[{date:now,km:current,projected:true}];let km=current;plans.forEach(p=>{const a=plannedAssignment(p.id);if(a?.shoeId!==shoe.id)return;km+=Number(p.distance)||0;points.push({date:p.date,km,projected:true,planId:p.id,type:p.type})});if(points.at(-1)?.date!==raceDate)points.push({date:raceDate,km,projected:true,raceDay:true});return points}
...
function rehabShoePlanHtml(injury,day){if(!rehabDayNeedsShoe(day)||!(state.shoes||[]).some(s=>s.status!=='retired'))return'';const selected=plannedRehabShoeId(injury,day.date);return`<div class="rehabShoePlan"><span>${todayPictogram('shoe')}</span><div><small>SHOE</small><select data-rehab-shoe-plan="${esc(injury.id)}" data-rehab-shoe-date="${esc(day.date)}">${shoeSelectOptions(selected,true)}</select><p>Used for rehabilitation shoe planning and copied into the daily check-in unless changed.</p></div></div>`}
