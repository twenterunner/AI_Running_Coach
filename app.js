...
const BUILD=9830, SCHEMA=9830, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function exerciseVisual(e){
 const n=e.name.toLowerCase(),kind=/run|walk|stride|acceleration|hill/.test(n)?'run':/hop|pogo|skip|bound/.test(n)?'hop':/bridge|heel-dig|curl/.test(n)?'bridge':/raise|squat|deadlift|hinge|strength|step/.test(n)?'strength':'mobility';
 const art={
  run:`<svg viewBox="0 0 120 78" aria-hidden="true"><circle cx="61" cy="15" r="7"/><path d="M59 24l-9 18 16 9 10-17M50 42L34 62M66 51l18 14M54 29L39 36M66 30l13 8"/></svg>`,
  hop:`<svg viewBox="0 0 120 78" aria-hidden="true"><circle cx="60" cy="14" r="7"/><path d="M60 23v22M60 30L45 40M60 30l15 10M60 45L47 62M60 45l14 17"/><path class="ground" d="M34 68h52"/></svg>`,
  bridge:`<svg viewBox="0 0 120 78" aria-hidden="true"><circle cx="87" cy="43" r="7"/><path d="M80 47L60 35 38 53M60 35L77 57M38 53H22M77 57h21"/><path class="ground" d="M18 65h84"/></svg>`,
  strength:`<svg viewBox="0 0 120 78" aria-hidden="true"><circle cx="60" cy="14" r="7"/><path d="M60 23v24M60 30L43 39M60 30l17 9M60 47L48 65M60 47l13 18"/><path d="M37 35v9M31 38h12M83 35v9M77 38h12"/></svg>`,
  mobility:`<svg viewBox="0 0 120 78" aria-hidden="true"><circle cx="60" cy="15" r="7"/><path d="M60 23v25M60 30L40 26M60 30l20-4M60 48L47 65M60 48l13 17"/><path class="arc" d="M34 22c8-12 44-12 52 0"/></svg>`
 }[kind];
 return `<div class="exerciseVisual ${kind}">${art}</div>`;
}
function rehabExercises(i,p){const list=(REHAB_EXERCISES[injuryProfile(i)]||REHAB_EXERCISES.generic)[p.stage]||[];return list.map((e,n)=>`<div class="exerciseCard">${exerciseVisual(e)}<div class="exerciseCopy"><div class="exerciseNumber">${n+1}</div><b>${esc(e.name)}</b><strong>${esc(e.dose)}</strong><p>${esc(e.cue)}</p></div></div>`).join('')}
function trajectoryComparisonHtml(p){
 ...
 if(!p.comparisonReady)return`<div class="trajectoryComparison awaiting"><b>Building a reliable trajectory</b><p>Current criteria completion is <strong>${p.score}%</strong>; nominal progress at injury day ${p.elapsed} is <strong>${p.expectedScore}%</strong>. At least two check-ins across two or more days are required before calling progress faster or slower.</p></div>`;
 const word=p.delta>0?'ahead of':p.delta<0?'behind':'aligned with';
 return`<div class="trajectoryComparison ${p.pace==='faster'?'good':p.pace==='slower'?'bad':'warn'}"><b>Why this is ${p.pace==='faster'?'faster':p.pace==='slower'?'slower':'on track'} than nominal</b><div class="compareGrid"><span>Your criteria completion<strong>${p.score}%</strong></span><span>Nominal at day ${p.elapsed}<strong>${p.expectedScore}%</strong></span><span>Difference<strong>${Math.abs(p.delta)} points ${word} nominal</strong></span><span>Stage comparison<strong>${esc(INJURY_STAGES[p.stage].name)} vs nominal ${esc(INJURY_STAGES[p.nominalStage].name)}</strong></span></div><p>The comparison uses functional stage criteria—not pain alone—and the full-training date moves only after repeated check-ins establish a trend.</p></div>`;
}
...
function renderInjury(){
 ...
 box.innerHTML=injuries.length?injuries.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(i=>{
  ...
  <div class="rehabSectionHead"><div><span>Today</span><h3>Exercise prescription</h3></div><small>Animated movement guides are illustrative. Follow the written cues and stop if pain becomes sharp, gait changes or symptoms are worse later or next morning.</small></div><div class="exerciseGrid">${rehabExercises(i,p)}</div>
