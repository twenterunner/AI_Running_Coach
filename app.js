...
function renderInjury(){
 ...
 box.innerHTML=injuries.length?injuries.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(i=>{
  ...
  <div class="rehabSectionHead"><div><span>Today</span><h3>Exercise prescription</h3></div><small>Tap any exercise to open its complete step-by-step sheet. The illustrations, written cues, pain limit, regressions and progression rules are available offline; no video is used.</small></div><div class="exerciseGrid">${rehabExercises(i,p)}</div>
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.8.3 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.8.3 stable build 9830');
})();