...
function normaliseState(input){
 ...
 return{...base,...src,schemaVersion:8400,setup,days:Array.isArray(src.days)?src.days:base.days,runs,assessments,plan,weekView:Number(src.weekView)||null};
}
...
function buildPlan(){
 ...
 state.plan=out;state.schemaVersion=8400;state.lastPlanHealth=validatePlan(out);save()
}
...
if(state.schemaVersion!==8400){state.plan=[];buildPlan()}else if(!state.plan?.length)buildPlan();
...
function renderAll(){[renderDashboard,renderToday,renderPlan,renderRuns,renderMetrics,renderAssessments,renderCoach,renderRace,renderSettings,renderPlanHealth].forEach(fn=>{try{fn()}catch(err){recordDiagnostic('Render failure in '+fn.name,err)}});renderDiagnostics()}
...
renderAll();
console.info('AI Running Coach v8.4.0 stable build 8400');
})();