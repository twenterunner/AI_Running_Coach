...
function shoeMileageChartHtml(){const shoes=(state.shoes||[]).filter(s=>s.status!=='retired'||s.showInHistory!==false);if(!shoes.length)return'<div class="shoeEmpty">Add a shoe to start mileage history.</div>';const enabled=new Set((state.shoeChartVisible||shoes.map(s=>s.id)).filter(id=>shoes.some(s=>s.id===id)));if(!enabled.size)shoes.forEach(s=>enabled.add(s.id));const series=shoes.filter(s=>enabled.has(s.id)).map(s=>({shoe:s,points:shoeMileageSeries(s)}));const dates=series.flatMap(x=>x.points.map(p=>p.date)).sort(),values=series.flatMap(x=>x.points.map(p=>p.km)).filter(Number.isFinite);if(!dates.length||!values.length)return'<div class="shoeEmpty">No tracked mileage yet.</div>';const minD=dte(dates[0]).getTime(),maxD=Math.max(minD+DAY,dte(dates.at(-1)).getTime()),maxY=Math.max(10,Math.ceil(Math.max(...values)*1.12/50)*50),W=720,H=300,L=52,R=16,T=18,B=46,x=d=>L+(dte(d).getTime()-minD)/(maxD-minD)*(W-L-R),y=v=>T+(maxY-v)/maxY*(H-T-B);const palette=['#3dd6c6','#79d69b','#67a7ff','#f1c56a','#b9a3ff','#ff9f9f','#7ee7ff'];let grid='';for(let i=0;i<=4;i++){const v=maxY*i/4,yy=y(v);grid+=`<line x1="${L}" y1="${yy}" x2="${W-R}" y2="${yy}"/><text x="${L-8}" y="${yy+4}" text-anchor="end">${Math.round(v)}</text>`}const paths=series.map((s,i)=>{const pts=s.points.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ');return`<polyline class="shoeLine shoeLine${i}" style="--shoe-color:${palette[i%palette.length]}" points="${pts}"/>`}).join('');const xlabels=[dates[0],dates[Math.floor((dates.length-1)/2)],dates.at(-1)].filter((v,i,a)=>a.indexOf(v)===i).map(d=>`<text class="xLab" x="${x(d)}" y="${H-16}" text-anchor="middle">${dte(d).toLocaleDateString(undefined,{month:'short',year:'2-digit'})}</text>`).join('');const legend=shoes.map((s,i)=>`<label class="shoeLegendToggle"><input type="checkbox" data-shoe-chart-toggle="${esc(s.id)}" ${enabled.has(s.id)?'checked':''}><i style="--shoe-color:${palette[i%palette.length]}"></i><span>${esc(shoeDisplayName(s))}</span></label>`).join('');return`<div class="shoeChartWrap"><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Accumulated shoe mileage over calendar time"><g class="shoeGrid">${grid}</g>${paths}${xlabels}<text class="yTitle" transform="translate(14 ${H/2}) rotate(-90)" text-anchor="middle">Accumulated km</text></svg><div class="shoeChartLegend">${legend}</div><p class="muted compact">Entered starting mileage is anchored only to the shoe’s tracking start date; earlier accumulation is not fabricated.</p></div>`}
