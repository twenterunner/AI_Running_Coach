...
function shoeProgrammeMileageChartHtml(){
 ...
 const ownedLegend=series.map((s,i)=>`<span class="shoeLegendItem"><span class="shoeLegendSwatch" style="--shoe-color:${palette[i%palette.length]}"></span><b>OWNED</b> ${esc(shoeDisplayName(s.shoe))}</span>`).join(''),futureLegend=replacementSeries.map(item=>`<span class="shoeLegendItem future"><span class="shoeLegendSwatch dotted"></span><b>FUTURE</b> ${esc(`ASICS ${item.event.replacement?.family||'running shoe'} ${item.event.replacement?.version||''}`)}</span>`).join(''),raceLegend=raceStrategy&&!raceStrategy.owned?`<span class="shoeLegendItem race"><span class="shoeLegendSwatch race"></span><b>FUTURE RACE</b> ${esc(raceStrategy.name)}</span>`:'';const legend=ownedLegend+futureLegend+raceLegend;
