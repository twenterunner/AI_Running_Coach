...
const BUILD=9900, SCHEMA=9900, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
const INJURY_EXERCISES={
 ...
 generic:[
  {stage:0,name:'Comfortable mobility and walking',dose:'5–10 minutes, several times daily',purpose:'Maintain movement without provoking symptoms.',steps:['Use a comfortable range only.','Keep movement slow and controlled.','Stop for sharp pain or worsening symptoms.'],why:'Early rehabilitation prioritises symptom stability and normal daily function.',progress:'Increase duration before intensity.'},
  {stage:2,name:'Controlled strength pattern',dose:'3 sets of 8–12 repetitions',purpose:'Restore strength and control in the affected movement pattern.',steps:['Choose a stable supported position.','Move slowly through a pain-limited range.','Use a 3-second lowering phase.','Stop before technique deteriorates.'],why:'Strength and control must recover before impact or running exposure.',progress:'Progress when pain stays ≤2/10 and symptoms are no worse the next morning.'},
  {stage:3,name:'Low-level impact preparation',dose:'3 sets of 20 seconds',purpose:'Test impact tolerance before walk–run progression.',steps:['Begin with gentle marching or jogging in place.','Keep contacts quiet and symmetrical.','Stop for pain, guarding or loss of rhythm.'],why:'Impact tolerance is the key gateway to running.',progress:'Progress only when the next-morning response remains stable.'}
 ]
};
...
function injuryText(i){return [i.bodyRegion,i.location,i.mechanism,i.onset,i.initialSymptoms,i.currentSymptoms,i.painTriggers].filter(Boolean).join(' ').toLowerCase()}
function injuryLocationType(i){let t=injuryText(i);return /hamstring|posterior thigh|back of thigh|ischial|sit bone|buttock/.test(t)?'hamstring':'generic'}
function termHits(text,terms=[]){return terms.filter(x=>text.includes(x))}
function workingDiagnosis(i){
 const clinician=String(i.clinicalDiagnosis||'').trim();
 if(clinician)return{name:clinician,confidence:100,evidence:['clinician diagnosis entered by the user'],alternatives:['The app does not independently verify a clinician diagnosis'],nominalDays:56,ranked:[{name:clinician,confidence:100,region:'Clinician supplied'}],missing:[]};
 const text=injuryText(i),initialPain=Number(i.initialPain)||0,walk=Number(i.initialWalkPain)||0;
 let ranked=RUNNER_INJURY_LIBRARY.map(d=>{
  const location=termHits(text,d.terms),mechanism=termHits(text,d.mechanisms),features=termHits(text,d.features);
  let score=location.length*5+mechanism.length*2.5+features.length*3;
  if(i.pop&&/strain|rupture|tear|sprain/.test(d.name.toLowerCase()))score+=3;
  if(i.bruising&&/strain|rupture|tear|sprain|stress injury/.test(d.name.toLowerCase()))score+=2;
  if(initialPain>=7&&/strain|rupture|stress injury|sprain/.test(d.name.toLowerCase()))score+=1.5;
  if(walk>=5&&/stress injury|rupture|strain|sprain/.test(d.name.toLowerCase()))score+=1;
  return{...d,score,location,mechanism,features};
 }).filter(x=>x.score>0).sort((a,b)=>b.score-a.score);
 if(!ranked.length){
   const region=(i.bodyRegion||i.location||'lower limb').trim();
   return{name:`Undifferentiated ${region.toLowerCase()} running-injury pattern`,confidence:30,evidence:['insufficient differentiating symptom detail'],alternatives:['Muscle or tendon overload','Joint or bone stress injury','Referred or nerve-related pain'],nominalDays:56,ranked:[],missing:['exact pain location','gradual or sudden onset','activities that reproduce symptoms']};
 }
 const top=ranked[0],second=ranked[1],margin=top.score-(second?.score||0),confidence=Math.round(clamp(42+top.score*3+margin*2,45,92));
 const evidence=[...top.location.map(x=>`location: ${x}`),...top.mechanism.map(x=>`mechanism: ${x}`),...top.features.map(x=>`feature: ${x}`)];
 if(i.pop)evidence.push('pop/snap reported');if(i.bruising)evidence.push('bruising/swelling reported');if(initialPain)evidence.push(`initial pain ${initialPain}/10`);if(walk)evidence.push(`initial walking pain ${walk}/10`);
 const probs=ranked.slice(0,3).map((x,idx)=>({name:x.name,region:x.region,confidence:Math.max(8,Math.round(confidence-(idx===0?0:idx===1?18+margin:30+margin))),urgent:!!x.urgent}));
 const missing=[];if(!i.onset)missing.push('whether onset was sudden or gradual');if(!i.painTriggers)missing.push('which activities reproduce pain');if(!/(front|back|inner|outer|lateral|medial|groin|heel|arch|shin|calf|knee|ankle|foot|hip|buttock)/.test(text))missing.push('a more precise pain location');
 return{name:top.name,confidence,evidence:evidence.length?evidence:['location and symptom pattern'],alternatives:[...(top.alternatives||[]),...probs.slice(1).map(x=>x.name)].filter((x,n,a)=>a.indexOf(x)===n).slice(0,4),nominalDays:top.days,ranked:probs,missing,urgent:!!top.urgent};
}
function criterionState(i,p,stageIndex){const latest=(i.checkIns||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)).at(-1)||{};const tests={
 'Symptoms stable or improving':!latest.nextDayWorse,
 'No new swelling or bruising':!latest.newSwelling,
 'Walking pain ≤3/10':p.walkPain<=3,
 'Walking pain ≤1/10':p.walkPain<=1,
 'Current pain ≤3/10':p.currentPain<=3,
 'Gentle double-leg loading tolerated':!!latest.bridge,
 'Current pain ≤2/10':p.currentPain<=2,
 'Repeated bridge or hinge controlled':!!latest.bridge,
 'No worse the next morning':!latest.nextDayWorse,
 'Walking pain 0/10':p.walkPain===0,
 'Gentle hopping tolerated':!!latest.hop,
 'Pain remains ≤2/10':p.currentPain<=2,
 'No next-morning flare':!latest.nextDayWorse,
 'At least 10 minutes running tolerated':Number(latest.runMinutes)>=10,
 'No altered gait':latest.alteredGait!==true,
 'At least 30 minutes easy running tolerated':Number(latest.runMinutes)>=30,
 'Pain ≤1/10':p.currentPain<=1,
 'Strength and impact confidence restored':!!latest.bridge&&!!latest.hop&&Number(latest.confidence||0)>=8
 };return INJURY_STAGES[stageIndex].criteria.map(label=>({label,met:!!tests[label]}));}
function injuryPrediction(i){
 const checks=(i.checkIns||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)),latest=checks.at(-1)||{};
 const initialPain=Number(i.initialPain)||0,bruising=!!i.bruising,pop=!!i.pop,walkInitially=Number(i.initialWalkPain)||0;
 const diag=workingDiagnosis(i);let nominalTotal=Number(diag.nominalDays)||56;const severityFactor=initialPain>=8||pop||walkInitially>=8?1.18:initialPain>=6||bruising||walkInitially>=5?1.08:initialPain<=2&&walkInitially<=1?.88:1;nominalTotal=Math.round(nominalTotal*severityFactor);
 const currentPain=Number(latest.pain ?? i.currentPain ?? initialPain),walkPain=Number(latest.walkPain ?? i.currentWalkPain ?? walkInitially);
 const elapsed=Math.max(0,Math.floor((today()-dte(i.date))/DAY));
 let stage=0;if(walkPain<=3&&currentPain<=5)stage=1;if(walkPain<=1&&currentPain<=3&&latest.bridge)stage=2;if(currentPain<=2&&walkPain===0&&latest.hop&&!latest.nextDayWorse)stage=3;if(Number(latest.runMinutes)>=10&&currentPain<=2&&!latest.nextDayWorse)stage=4;if(Number(latest.runMinutes)>=30&&currentPain<=1&&!latest.nextDayWorse)stage=5;if(latest.nextDayWorse)stage=Math.max(0,stage-1);
 const stageBase=[8,22,40,58,76,92][stage],within=Math.max(0,Math.min(8,(3-currentPain)*2+(1-walkPain)*2+(latest.bridge?2:0)+(latest.hop?2:0)+(Number(latest.runMinutes)>=10?2:0)-(latest.nextDayWorse?5:0)));
 const completion=Math.round(clamp(stageBase+within,0,98));
 const nominal=Math.round(clamp(elapsed/nominalTotal*100,0,100));
 const delta=completion-nominal;
 let adjustment=0;if(checks.length>=2){adjustment=Math.round(clamp(-delta*.18,-10,14));if(latest.nextDayWorse)adjustment+=4;}
 const total=Math.max(elapsed+7,nominalTotal+adjustment),remaining=Math.max(0,total-elapsed),fullDate=iso(new Date(today().getTime()+remaining*DAY));
 const confidence=checks.length>=7?'High':checks.length>=3?'Moderate':'Low';
 return{nominalTotal,total,elapsed,remaining,stage,fullDate,confidence,currentPain,walkPain,completion,nominal,delta,latest,checks};
}
function comparisonFactors(i,p){let l=p.latest||{},f=[];const add=(name,actual,nominal,status,reason)=>f.push({name,actual,nominal,status,reason});
 add('Pain',`${p.currentPain}/10`,p.elapsed<14?'3–5/10':'0–3/10',p.currentPain<=2?'ahead':p.currentPain>=5?'behind':'on',p.currentPain<=2?'Pain is low enough for progressive loading.':p.currentPain>=5?'Pain remains higher than expected for this point.':'Pain is within the broad expected range.');
 add('Walking',`${p.walkPain}/10 pain`,p.elapsed<10?'Some limitation':'Comfortable',p.walkPain===0?'ahead':p.walkPain>=3?'behind':'on',p.walkPain===0?'Normal walking has returned.':'Walking symptoms still limit progression.');
 add('Strength',l.bridge?'Controlled':'Not demonstrated',p.elapsed<21?'Beginning':'Controlled repeated loading',l.bridge?'ahead':'behind',l.bridge?'A bridge or hinge pattern is tolerated.':'Controlled loading has not yet been demonstrated in a check-in.');
 add('Impact',l.hop?'Tolerated':'Not demonstrated',p.elapsed<28?'Usually not required':'Expected to be emerging',l.hop?'ahead':p.elapsed>=28?'behind':'on',l.hop?'Low-level impact is tolerated.':'Impact tolerance is not yet demonstrated.');
 add('Running',`${Number(l.runMinutes)||0} min`,p.elapsed<28?'Often 0 min':'Some easy exposure',Number(l.runMinutes)>=10?'ahead':p.elapsed>=35?'behind':'on',Number(l.runMinutes)>=10?'Running exposure has started.':'Running tolerance remains a limiting factor.');
 add('Next morning',l.nextDayWorse?'Worse':'Stable', 'Stable',l.nextDayWorse?'behind':'ahead',l.nextDayWorse?'The latest load caused a delayed flare.':'No delayed worsening was reported.');return f;}
function exerciseList(i,p){let list=INJURY_EXERCISES[injuryLocationType(i)]||INJURY_EXERCISES.generic;let exact=list.filter(x=>x.stage===p.stage);if(!exact.length)exact=list.filter(x=>x.stage<=p.stage).slice(-2);return exact;}

function injuryTrajectorySvg(i,p){
 const W=720,H=250,left=46,right=18,top=24,bottom=42,cw=W-left-right,ch=H-top-bottom;
 const horizon=Math.max(7,p.nominalTotal,p.elapsed+p.remaining),x=d=>left+clamp(d/horizon,0,1)*cw,y=v=>top+(100-clamp(v,0,100))/100*ch;
 const nominalPath=`M ${x(0)} ${y(0)} L ${x(p.nominalTotal)} ${y(100)}`;
 const pts=(p.checks||[]).map((c,idx)=>{const temp={...i,checkIns:p.checks.slice(0,idx+1)};const q=injuryPrediction(temp);return{day:Math.max(0,Math.round((dte(c.date)-dte(i.date))/DAY)),score:q.completion};});
 if(!pts.length)pts.push({day:p.elapsed,score:p.completion});
 const observed=pts.map((q,n)=>`${n?'L':'M'} ${x(q.day)} ${y(q.score)}`).join(' ');
 return `<div class="injuryTrajectoryWrap"><svg class="injuryTrajectory" viewBox="0 0 ${W} ${H}" role="img" aria-label="Observed rehabilitation completion compared with nominal recovery"><line x1="${left}" y1="${y(25)}" x2="${W-right}" y2="${y(25)}"/><line x1="${left}" y1="${y(50)}" x2="${W-right}" y2="${y(50)}"/><line x1="${left}" y1="${y(75)}" x2="${W-right}" y2="${y(75)}"/><path class="nominalLine" d="${nominalPath}"/><path class="actualLine" d="${observed}"/>${pts.map(q=>`<circle cx="${x(q.day)}" cy="${y(q.score)}" r="5"/>`).join('')}<text x="${left}" y="${H-12}">Injury</text><text x="${W-right}" y="${H-12}" text-anchor="end">Full unrestricted training</text><text x="${left+4}" y="${top+14}">Completion %</text></svg><div class="injuryTrajectoryLegend"><span class="actual">Observed completion</span><span class="nominal">Nominal recovery</span></div></div>`;
}

function renderInjury(){
 const box=$('injuryList');if(!box)return;const injuries=state.injuries||[],add=$('addInjuryBtn');
 $('injuryIntro').innerHTML='';
 if(add)add.classList.toggle('hidden',injuries.length>0);
 if(!injuries.length){box.innerHTML='<article class="panel injuryEmpty"><h3>No active injury</h3><p class="muted">Record an injury to start criteria-based rehabilitation guidance.</p></article>';return;}
 box.innerHTML=injuries.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(i=>{let p=injuryPrediction(i),st=INJURY_STAGES[p.stage],diag=workingDiagnosis(i),next=Math.min(5,p.stage+1),criteria=criterionState(i,p,next),met=criteria.filter(x=>x.met).length,factors=comparisonFactors(i,p),exercises=exerciseList(i,p),status=p.checks.length<2?'Building trajectory':p.delta>=8?'Ahead of nominal':p.delta<=-8?'Behind nominal':'Near nominal';
 const whyDate=[`The initial ${diag.name.toLowerCase()} assumption uses a nominal ${p.nominalTotal}-day pathway to unrestricted training.`,`You are ${p.elapsed} days after injury and currently in Stage ${p.stage+1}: ${st.name}.`,`Pain is ${p.currentPain}/10 and walking pain ${p.walkPain}/10.`,p.latest.bridge?'Controlled strength loading is demonstrated.':'Controlled strength loading is not yet demonstrated.',p.latest.hop?'Low-level impact is demonstrated.':'Low-level impact is not yet demonstrated.',Number(p.latest.runMinutes)>=10?`${p.latest.runMinutes} minutes of running has been tolerated.`:'Running tolerance has not yet been demonstrated.',p.latest.nextDayWorse?'The latest load caused next-morning worsening, which delays progression.':'No next-morning worsening was reported.'];
 return`<article class="panel injuryCard"><div class="panelHead"><div><h3>${esc(i.location||'Injury')} · ${fmtDate(i.date)}</h3><p class="muted compact">${esc(i.mechanism||'Mechanism not entered')}</p></div><span class="status today">Stage ${p.stage+1}</span></div>
 <div class="injuryKpiGrid"><div class="metric-card"><span>Rehabilitation complete</span><strong class="viz-stat-value rehabCompletionValue ${p.completion>=70?'good':p.completion>=40?'warn':'bad'}">${p.completion}%</strong><small>${status}</small></div><div class="metric-card"><span>Full unrestricted training</span><strong class="viz-stat-value injuryDateValue">${fmtDate(p.fullDate)}</strong><small>${p.remaining} days · ${p.confidence.toLowerCase()} confidence</small></div><div class="metric-card"><span>Current pain</span><strong class="viz-stat-value">${p.currentPain}/10</strong><small>Walking ${p.walkPain}/10</small></div><div class="metric-card"><span>Current stage</span><strong class="viz-stat-value injuryStageValue">${esc(st.name)}</strong><small>${esc(st.goal)}</small></div></div>
 <section class="injurySection"><h4>Clinical working assumption</h4><div class="diagnosisBox"><div><small>MOST LIKELY</small><strong>${esc(diag.name)}</strong><span>${diag.confidence}% pattern confidence</span></div>${diag.urgent?'<div class="note bad"><b>Assessment recommended before rehabilitation progression</b><p class="compact">This symptom pattern can represent a higher-risk tendon or bone injury.</p></div>':''}<p><b>Why:</b> ${diag.evidence.map(esc).join(' · ')||'limited symptom information'}</p>${diag.ranked?.length>1?`<div class="diagnosisRanking">${diag.ranked.map((d,n)=>`<div><i>${n+1}</i><span><b>${esc(d.name)}</b><small>${esc(d.region)}</small></span><strong>${d.confidence}%</strong></div>`).join('')}</div>`:''}${diag.missing?.length?`<div class="diagnosisMissing"><b>Information that would improve confidence</b><p>${diag.missing.map(esc).join(' · ')}</p></div>`:''}<details><summary>Alternatives and diagnostic boundary</summary><p>${diag.alternatives.map(esc).join(' · ')}</p><p class="muted">This is a pattern-based working assumption, not a confirmed medical diagnosis. A physical examination or imaging may change it.</p></details></div></section>
 <section class="injurySection"><div class="injurySectionHead"><div><h4>Why this recovery timeline?</h4><p class="muted compact">Forecast refers to full unrestricted running training, not the first walk–run.</p></div></div><div class="timelineReason">${whyDate.map(x=>`<p>${esc(x)}</p>`).join('')}</div></section>
 <section class="injurySection"><h4>Compared with nominal recovery</h4><div class="nominalSummary"><strong>${p.completion}% observed</strong><span>vs ${p.nominal}% nominal at day ${p.elapsed}</span><b class="${p.delta>=8?'positive':p.delta<=-8?'negative':''}">${p.delta>=0?'+':''}${p.delta} points</b></div><div class="comparisonList">${factors.map(f=>`<div class="comparisonRow ${f.status}"><div><b>${esc(f.name)}</b><small>${esc(f.reason)}</small></div><span>${esc(f.actual)}</span><em>Nominal: ${esc(f.nominal)}</em></div>`).join('')}</div></section>
 <section class="injurySection"><div class="injurySectionHead"><div><h4>Recovery progress</h4><p class="muted compact">Observed rehabilitation completion versus the nominal pathway to unrestricted running.</p></div></div>${injuryTrajectorySvg(i,p)}</section>
 <section class="injurySection"><div class="injurySectionHead"><div><h4>Why you are still in Stage ${p.stage+1}</h4><p class="muted compact">Requirements for Stage ${next+1}: ${INJURY_STAGES[next].name}</p></div><strong>${met}/${criteria.length} met</strong></div><div class="criteriaList">${criteria.map(c=>`<div class="criterion ${c.met?'met':'notMet'}"><i>${c.met?'✓':'○'}</i><span>${esc(c.label)}</span><b>${c.met?'Met':'Not met'}</b></div>`).join('')}</div></section>
 <section class="injurySection"><h4>Today’s focus</h4><div class="todayFocus"><strong>${p.stage<3?'Build the next functional criterion safely':p.stage<5?'Increase running tolerance without a flare':'Restore faster running and full training tolerance'}</strong><p>${criteria.filter(x=>!x.met).length?`The main blockers are: ${criteria.filter(x=>!x.met).map(x=>x.label.toLowerCase()).join(', ')}.`:'All current criteria are met; confirm stability across repeated check-ins before progressing.'}</p></div>${exercises.map(x=>`<details class="exerciseDetail"><summary><span><b>${esc(x.name)}</b><small>${esc(x.dose)} · ${esc(x.purpose)}</small></span><em>Open guide</em></summary><div class="exerciseGuide"><div class="exerciseWhy"><b>Why today?</b><p>${esc(x.why)}</p></div><div><b>How to perform</b><ol>${x.steps.map(y=>`<li>${esc(y)}</li>`).join('')}</ol></div><div class="exerciseRules"><div><b>Pain rule</b><p>Keep pain at 0–2/10. Stop for sharp pain, altered movement, or symptoms that worsen later or next morning.</p></div><div><b>Progress when</b><p>${esc(x.progress)}</p></div></div></div></details>`).join('')}</section>
 <details class="injuryTimelineDetails"><summary>View all rehabilitation stages</summary><div class="injuryTimeline">${INJURY_STAGES.map((x,n)=>`<div class="injuryStage ${n<p.stage?'done':n===p.stage?'active':''}"><i>${n+1}</i><div><b>${esc(x.name)}</b><small>${esc(x.goal)}</small></div></div>`).join('')}</div></details>
 <details><summary>Initial and current symptoms</summary><div class="guideGrid"><div><b>At injury</b><p>Pain ${Number(i.initialPain)||0}/10 · walking pain ${Number(i.initialWalkPain)||0}/10${i.pop?' · pop felt':''}${i.bruising?' · bruising':''}<br>${esc(i.initialSymptoms||'No symptom description')}</p></div><div><b>Latest</b><p>Pain ${p.currentPain}/10 · walking pain ${p.walkPain}/10<br>${esc(p.latest.symptoms||i.currentSymptoms||'No current symptom description')}</p></div></div></details>
 <div class="buttonRow"><button data-injury-check="${i.id}" class="primary">Daily check-in</button><button data-injury-edit="${i.id}">Edit injury</button><button data-injury-delete="${i.id}" class="danger">Delete</button></div></article>`}).join('');
}
function injuryForm(i={}){const regions=['','Hip / pelvis','Groin / inner thigh','Front thigh','Back of thigh / hamstring','Knee','Shin / lower leg','Calf','Achilles / back of ankle','Ankle','Heel / arch','Forefoot / toes'];return`<h3>${i.id?'Edit':'Record'} injury</h3><div class="formGrid"><div class="field"><label>Injury date</label><input id="injDate" type="date" value="${i.date||iso(today())}"></div><div class="field"><label>Body region</label><select id="injRegion">${regions.map(x=>`<option value="${esc(x)}" ${i.bodyRegion===x?'selected':''}>${esc(x||'Select region')}</option>`).join('')}</select></div><div class="field"><label>Exact area / location</label><input id="injLocation" value="${esc(i.location||'')}" placeholder="e.g. outside of right hip"></div><div class="field"><label>Onset</label><select id="injOnset"><option value="">Select onset</option><option value="Sudden" ${i.onset==='Sudden'?'selected':''}>Sudden</option><option value="Gradual" ${i.onset==='Gradual'?'selected':''}>Gradual</option><option value="Unclear" ${i.onset==='Unclear'?'selected':''}>Unclear</option></select></div><div class="field"><label>How did it happen?</label><input id="injMechanism" value="${esc(i.mechanism||'')}" placeholder="e.g. sprint, hill run, mileage increase"></div><div class="field"><label>What reproduces the pain?</label><textarea id="injTriggers" placeholder="e.g. stairs, sitting, squatting, first steps, hopping">${esc(i.painTriggers||'')}</textarea></div><div class="field"><label>Clinician diagnosis (optional)</label><input id="injDiagnosis" value="${esc(i.clinicalDiagnosis||'')}" placeholder="Leave blank for working assumption"></div><div class="field"><label>Pain then 0–10</label><input id="injInitialPain" type="number" min="0" max="10" value="${i.initialPain??''}"></div><div class="field"><label>Walking pain then 0–10</label><input id="injInitialWalk" type="number" min="0" max="10" value="${i.initialWalkPain??''}"></div><div class="field"><label>Signs then</label><label><input id="injPop" type="checkbox" ${i.pop?'checked':''}> Pop / snap</label><label><input id="injBruise" type="checkbox" ${i.bruising?'checked':''}> Bruising or swelling</label></div><div class="field"><label>Symptoms then</label><textarea id="injInitialSymptoms">${esc(i.initialSymptoms||'')}</textarea></div><div class="field"><label>Symptoms now</label><textarea id="injCurrentSymptoms">${esc(i.currentSymptoms||'')}</textarea></div><div class="field"><label>Pain now 0–10</label><input id="injCurrentPain" type="number" min="0" max="10" value="${i.currentPain??''}"></div><div class="field"><label>Walking pain now 0–10</label><input id="injCurrentWalk" type="number" min="0" max="10" value="${i.currentWalkPain??''}"></div></div><div class="note bad"><b>Seek urgent assessment</b> for major trauma, inability to bear weight, marked weakness or deformity, rapidly increasing swelling, numbness, fever, severe night pain, focal bone pain, or calf swelling/breathlessness.</div><button id="saveInjury" class="primary full">Save injury</button>`}
function openInjuryForm(i){$('modalContent').innerHTML=injuryForm(i);$('modal').className='modal';$('saveInjury').onclick=()=>{let obj={...(i||{}),id:i?.id||'inj-'+Date.now(),date:$('injDate').value,bodyRegion:$('injRegion').value,location:$('injLocation').value.trim(),onset:$('injOnset').value,mechanism:$('injMechanism').value.trim(),painTriggers:$('injTriggers').value.trim(),clinicalDiagnosis:$('injDiagnosis').value.trim(),initialPain:Number($('injInitialPain').value),initialWalkPain:Number($('injInitialWalk').value),pop:$('injPop').checked,bruising:$('injBruise').checked,initialSymptoms:$('injInitialSymptoms').value.trim(),currentSymptoms:$('injCurrentSymptoms').value.trim(),currentPain:Number($('injCurrentPain').value),currentWalkPain:Number($('injCurrentWalk').value),checkIns:i?.checkIns||[]};if(!obj.date||!obj.location)return toast('Enter the injury date and location.',true);state.injuries=state.injuries||[];let n=state.injuries.findIndex(x=>x.id===obj.id);n>=0?state.injuries[n]=obj:state.injuries.push(obj);save();$('modal').className='modal hidden';renderInjury();toast('Injury timeline saved.')}}
function openInjuryCheck(i){let prev=(i.checkIns||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)).at(-1)||{};$('modalContent').innerHTML=`<h3>Daily injury check-in</h3><div class="formGrid"><div class="field"><label>Date</label><input id="icDate" type="date" value="${iso(today())}"></div><div class="field"><label>Pain now 0–10</label><input id="icPain" type="number" min="0" max="10" value="${prev.pain??''}"></div><div class="field"><label>Walking pain 0–10</label><input id="icWalk" type="number" min="0" max="10" value="${prev.walkPain??''}"></div><div class="field"><label>Confidence in injured area 0–10</label><input id="icConfidence" type="number" min="0" max="10" value="${prev.confidence??''}"></div><div class="field"><label>Function today</label><label><input id="icBridge" type="checkbox" ${prev.bridge?'checked':''}> Repeated bridge/hinge controlled</label><label><input id="icHop" type="checkbox" ${prev.hop?'checked':''}> Gentle hop/jog-in-place tolerated</label><label><input id="icGait" type="checkbox" ${prev.alteredGait?'checked':''}> Gait or technique altered</label><label><input id="icSwelling" type="checkbox"> New swelling or bruising</label></div><div class="field"><label>Running completed (minutes)</label><input id="icRun" type="number" min="0" value="${prev.runMinutes||0}"></div><div class="field"><label>Worse the next morning?</label><select id="icWorse"><option value="false">No</option><option value="true" ${prev.nextDayWorse?'selected':''}>Yes</option></select></div><div class="field"><label>Symptoms / notes</label><textarea id="icSymptoms"></textarea></div></div><button id="saveCheck" class="primary full">Save and recalculate timeline</button>`;$('modal').className='modal';$('saveCheck').onclick=()=>{let c={date:$('icDate').value,pain:Number($('icPain').value),walkPain:Number($('icWalk').value),confidence:Number($('icConfidence').value)||0,bridge:$('icBridge').checked,hop:$('icHop').checked,alteredGait:$('icGait').checked,newSwelling:$('icSwelling').checked,runMinutes:Number($('icRun').value)||0,nextDayWorse:$('icWorse').value==='true',symptoms:$('icSymptoms').value.trim()};if(!c.date||!Number.isFinite(c.pain)||!Number.isFinite(c.walkPain))return toast('Enter date, pain and walking pain.',true);i.checkIns=i.checkIns||[];let n=i.checkIns.findIndex(x=>x.date===c.date);n>=0?i.checkIns[n]=c:i.checkIns.push(c);save();$('modal').className='modal hidden';renderInjury();toast('Timeline recalculated from today’s feedback.')}}
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.9.0 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.9.0 stable build 9900');
})();