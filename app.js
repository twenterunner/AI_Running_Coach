...
function shoeProgrammeMileageChartHtml(){
 ...
 const allocationHtml=uniqueEvents.length?`<div class="shoeAllocationRationale"><div class="shoeAllocationRationaleHead"><small>WHY THE CURVES CHANGE</small><b>What drives the slope changes?</b><p>Numbered markers on the graph match the explanations below. Blue = rehab plan, teal = training plan, amber = shoe rotation.</p></div>${uniqueEvents.map(e=>`<article class="shoeAllocationEvent driver-${e.driver.toLowerCase().replace(/[^a-z]+/g,'-')}" style="--event-color:${e.markerColor}"><div><span class="shoeEventNumber">${e.markerNo}</span><span class="shoeDriverTag">${esc(e.driver)}</span><time>${esc(shoeChartDate(e.date))}</time></div><b>${esc(e.title)}</b><p>${esc(e.text)}</p></article>`).join('')}</div>`:'';
