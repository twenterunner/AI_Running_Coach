...
const BUILD=10140, SCHEMA=10140, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function openInjuryCheck(i,existing=null){
 ...
 const planForDate=date=>{if(existing?.planSnapshot&&existing.date===date)return existing.planSnapshot;const delta=Math.round((dte(date)-today())/DAY),offset=clamp(delta,0,6);return rehabCalendarDay(i,progress,date,offset)};
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v10.0.14 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v10.0.14 stable build 10140');
})();