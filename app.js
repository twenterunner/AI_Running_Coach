...
const BUILD=9920, SCHEMA=9920, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function diagnosisFamily(key=''){
 ...
 if(/cecs|mortons|radicular|piriformis/.test(key))return'neural';
 ...
 if(/post_tib|navicular|met_|mortons|fhl|mtp|extensor|plantar/.test(key))return'foot';
 if(/ham/.test(key))return'hamstring';
 if(/hip|labral|snapping|gtps/.test(key))return'hip';
 if(/strain|flexor|adductor|quad|sartorius|tfl|gastroc|soleus|pubalgia/.test(key))return'muscle';
 if(/tend|bursa|achilles|ant_tib/.test(key))return'tendon';
 return'generic';
}
const INJURY_REGION_KEYS={
 ...
 'Front thigh':/quad_strain|hip_flexor|femoral_neck/,
 'Back of thigh / hamstring':/ham_strain|prox_ham|piriformis|lumbar_radicular/,
 'Knee':/pfp|patellar_tend|itbs|pes|meniscus|mcl|quad_tend|fat_pad|popliteus/,
 'Shin / lower leg':/mtss|tibial_bsi|cecs|fibular_bsi|ant_tib_tend|soleus/,
 'Calf':/gastroc|soleus|achilles|cecs|fibular_bsi/,
 'Achilles / back of ankle':/achilles|gastroc|soleus|post_tib|peroneal/,
 'Ankle':/ankle_sprain|peroneal|post_tib|ant_tib_tend|achilles|fhl_tend/,
 'Heel / arch':/plantar|post_tib|calcaneal_bsi|achilles_ins|fhl_tend/,
 'Forefoot / toes':/met_bsi|navicular_bsi|sesamoid|mortons|extensor_tend|fhl_tend|mtp_synovitis|fifth_met_bsi/
};
...
function renderInjury(){
 ...
 <div class="injuryPatternSummary"><div><span>Working symptom pattern</span><strong>${esc(diag.name)}</strong><small>${esc(diag.strength)} match · restricted to ${esc(i.bodyRegion||'the recorded body region')}</small></div><span class="injuryFamilyPill">${esc((diag.family||'generic').replace('_',' '))}</span></div>
...
<details class="injuryClinicianStep"><summary>Clinician information (optional)</summary><div class="formGrid"><div class="field"><label>Clinician diagnosis</label><input id="injDiagnosis" value="${esc(i.clinicalDiagnosis||'')}" placeholder="Leave blank for app pattern matching"></div><div class="field"><label>Expected recovery days</label><input id="injClinicianDays" type="number" min="1" value="${i.clinicianExpectedDays??''}" placeholder="Use clinician estimate when supplied"></div></div></details>
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.9.2 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.9.2 stable build 9920');
})();