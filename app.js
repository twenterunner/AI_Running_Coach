...
const BUILD=9320, SCHEMA=9320, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function athleteState(w=currentWeek()){
 ...
 let tolerance='Normal';
 if(Number.isFinite(completion)){if(completion<.70)tolerance='Low';else if(completion<.85)tolerance='Reduced';else if(completion<=1.05)tolerance='Normal';else tolerance='Excess load';}
 const adjustment=nextAfd.factor,direction=adjustment<.995?'Reduce':adjustment>1.005?'Increase':'Maintain',pct=Math.round(Math.abs(adjustment-1)*100),reasons=[];
 if(fitnessTrend!=='Stable')reasons.push(`fitness ${fitnessTrend.toLowerCase()}`);if(readiness!=='Normal')reasons.push(`recovery ${readiness.toLowerCase()}`);if(tolerance!=='Normal')reasons.push(`load completion ${Number.isFinite(completion)?Math.round(completion*100)+'%':tolerance.toLowerCase()}`);if(!reasons.length)reasons.push('fitness, recovery and completed load are balanced');
 const nextStart=weekStart(next),reviewDate=iso(new Date(nextStart.getTime()-DAY));
 return{currentWeek:current,nextWeek:next,fitnessTrend,fitnessIndex:ev.index,fitnessDelta,evidenceConfidence:ev.confidence,readiness,tolerance,completion,adjustment,direction,pct,reasons,applied,nextAfd,pain,hrv,reviewDate,nextStart:iso(nextStart)};
}
...
function factorKpi(afd){
 ...
 return`<div class="kpi factorKpi"><button type="button" class="factorToggle" aria-expanded="false"><label>Weekly adjustment factor</label><strong>${afd.factor.toFixed(2)}</strong><small>Same factor shown in Athlete State · multiplies future plan load · tap for calculation</small></button><div class="factorInlineDetail"><p class="muted compact">Training response combined with the recent Garmin HRV recovery pattern, bounded by ${state.setup.minFactor.toFixed(2)}–${state.setup.maxFactor.toFixed(2)}.</p><div class="adaptiveFormula"><b>Calculation</b><span>${calc}</span></div><div class="adaptiveBreakdown">${afd.items.map(i=>`<div class="adaptiveRow"><b>${i.name}</b><span class="${i.adjustment>0?'positive':i.adjustment<0?'negative':''}">${i.adjustment>0?'+':''}${i.adjustment.toFixed(2)}</span><small>${i.detail}</small></div>`).join('')}</div><p class="muted compact">This factor changes future training load only. It does not directly change race probability.</p></div></div>`
}
...
function renderDashboard(){
 ...
 const blocks=blockNames.map(b=>({name:b.name,start:pctStart(b.startWeek),end:pctEnd(b.endWeek),cls:phaseClass(b.name)}));
 $('raceTimeline').innerHTML=`<div class="timelineBlocks">${blocks.map(b=>`<div class="timelineBlock timeline-${b.cls}" style="left:${b.start}%;width:${b.end-b.start}%"><span>${b.name}</span></div>`).join('')}<i class="timelineProgress" style="width:${pos}%"></i><span class="timelineNow" style="left:${pos}%" aria-label="Current programme position"></span></div><div class="timelineScale"><span>Plan start</span><span>Race day</span></div><div class="timelineMeta"><b>${detailedPhase(engine.cw)} phase · week ${engine.cw} of ${total}</b><span>${Math.max(0,Math.ceil(c.weeksRemaining))} weeks until race</span></div>`;
...
function renderPlan(){
 const ast=athleteState(state.weekView||currentWeek());if($('fitnessEvidence')){
  const recoveryImpact=Math.round((ast.hrv.factor-1)*100),fitnessTarget=Math.round(ast.fitnessDelta*10)/10,completionText=Number.isFinite(ast.completion)?`${Math.round(ast.completion*100)}% of this week's planned km completed`:'No completed-load percentage yet';
  const painAction=!ast.pain.count?'Log pain after runs':ast.pain.max>=5?'Stop quality work and reassess before progressing':ast.pain.max>=3?'Keep all running easy and do not increase load':'No pain-based restriction';
  $('fitnessEvidence').innerHTML=`<div class="stateLinkNote"><b>One system, two outputs</b><p>The Athlete State calculates the same weekly adjustment factor shown on the Coach tab. Fitness evidence changes pace and power targets; recovery, pain and completed load determine how much training is prescribed.</p></div><div class="athleteStateGrid"><div><span>Fitness calibration</span><strong>${fitnessTarget>=0?'+':''}${fitnessTarget.toFixed(1)}%</strong><small>${ast.fitnessTrend} · ${ast.evidenceConfidence}% evidence confidence · applied to future pace and power targets</small></div><div><span>Recovery effect</span><strong>${recoveryImpact?`${recoveryImpact}% load`: '0% load change'}</strong><small>${ast.hrv.ready?`${Math.round(ast.hrv.rolling)} ms recent vs ${Math.round(ast.hrv.baseline)} ms baseline`:'No HRV baseline yet; no HRV penalty applied'}</small></div><div><span>Injury action</span><strong>${ast.pain.count?`Pain max ${ast.pain.max}/10`:'No pain data'}</strong><small>${painAction}</small></div><div><span>Training tolerance</span><strong>${Number.isFinite(ast.completion)?`${Math.round(ast.completion*100)}%`:'Pending'}</strong><small>${completionText}</small></div><div class="stateDecision"><span>Next-week adjustment factor</span><strong>${ast.adjustment.toFixed(2)} · ${ast.direction}${ast.pct?` ${ast.pct}%`:''}</strong><small>${esc(ast.reasons.join(' · '))}</small></div></div><div class="adjustmentSchedule"><b>When it updates</b><p>Current Week ${ast.currentWeek} uses factor ${ast.applied.factor.toFixed(2)}. The next calculation is finalised after ${fmtDate(ast.reviewDate)} and applies from ${fmtDate(ast.nextStart)} to Week ${ast.nextWeek}. Runs, pain and HRV logged before then feed that review; pace and power calibration updates whenever enough evidence crosses the 0.5% threshold.</p></div><details class="stateDetails"><summary>Calculation details</summary><p><b>Coach-tab relationship:</b> the Coach tab's adjustment factor and the value above are the same plan-load multiplier. A factor of 0.92 means approximately 8% less planned load; 1.03 means approximately 3% more. It changes future volume and session load, not established fitness zones.</p></details>`;
 }
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.3.2 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.3.2 stable build 9320');
})();