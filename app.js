...
const BUILD=10160, SCHEMA=10160, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function openInjuryCheck(i,existing=null){
 ...
 const scheduledMeta=plan=>{const walkingItem=(plan.items||[]).find(x=>/^Walk \d+ minutes/i.test(x))||plan.walkingTarget||'';const walkingMatch=String(walkingItem).match(/Walk (\d+) minutes/i);const walkingTargetMinutes=walkingMatch?Number(walkingMatch[1]):null;const exerciseItems=(plan.items||[]).filter(x=>x!==walkingItem&&!/^Walk \d+ minutes/i.test(x));const runningPlanned=['assessment','run'].includes(plan.type);const strengthPlanned=exerciseItems.some(x=>/bridge|strength|hinge|raise|curl|squat|isometric|loading/i.test(x));const impactPlanned=runningPlanned||exerciseItems.some(x=>/hop|jog|impact/i.test(x));return{walkingPlanned:!!walkingItem,walkingItem,walkingTargetMinutes,exercisePlanned:exerciseItems.length>0,exerciseItems,runningPlanned,strengthPlanned,impactPlanned,stretchPlanned:!!walkingItem}};
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v10.0.16 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v10.0.16 stable build 10160');
})();