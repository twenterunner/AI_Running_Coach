...
const BUILD=10150, SCHEMA=10150, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function openInjuryCheck(i,existing=null){
 ...
 const scheduledMeta=plan=>{const walkingItem=(plan.items||[]).find(x=>/^Walk \d+ minutes/i.test(x))||plan.walkingTarget||'';const exerciseItems=(plan.items||[]).filter(x=>x!==walkingItem&&!/^Walk \d+ minutes/i.test(x));const runningPlanned=['assessment','run'].includes(plan.type);const strengthPlanned=exerciseItems.some(x=>/bridge|strength|hinge|raise|curl|squat|isometric|loading/i.test(x));const impactPlanned=runningPlanned||exerciseItems.some(x=>/hop|jog|impact/i.test(x));return{walkingPlanned:!!walkingItem,walkingItem,exercisePlanned:exerciseItems.length>0,exerciseItems,runningPlanned,strengthPlanned,impactPlanned,stretchPlanned:!!walkingItem}};
 ...
 const runStatusField=$('icRunStatus'),runMinutesField=$('icRun'),runPainField=$('icRunPain'),gaitField=$('icGait'),locomotionField=$('icLocomotion'),exerciseField=$('icRehabExercises'),stretchField=$('icStretchGoal'),consistencyBox=$('icConsistency'),dateField=$('icDate');
 ...
 const applyScheduledQuestions=(date,preserve=true)=>{activePlan=planForDate(date);activeScheduled=scheduledMeta(activePlan);$('icScheduledPlan').innerHTML=`<b>Scheduled for ${esc(fmtDate(date))}: ${esc(activePlan.title)}</b><p>${activeScheduled.walkingPlanned?esc(activeScheduled.walkingItem):'No walking target.'}${activeScheduled.exercisePlanned?`<br>Exercises: ${activeScheduled.exerciseItems.map(esc).join(' · ')}`:'<br>No rehabilitation exercises scheduled.'}${activeScheduled.runningPlanned?`<br>Running/impact exposure is scheduled.`:'<br>No run is scheduled.'}</p>`;
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v10.0.15 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v10.0.15 stable build 10150');
})();