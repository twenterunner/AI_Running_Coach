...
function plannedReplacementEvents(){return(state.shoes||[]).filter(s=>s.status!=='retired').map(shoePlannedReplacementEvent).filter(Boolean).sort((a,b)=>a.date.localeCompare(b.date))}
...
function shoeNextPurchasePlan(shoe){
 const f=shoeForecast(shoe),event=shoePlannedReplacementEvent(shoe),replacement=event?.replacement||replacementProfileForShoe(shoe);
...
function shoeProgrammeMileageChartHtml(){
 ...
 const replacementSeries=rawEvents.filter(e=>enabled.has(e.shoe.id)&&e.replacement).map(e=>{const purchaseDate=plannedTrainingPurchaseDate(e),pts=[{date:purchaseDate,km:0,purchase:true,driver:'rotation'}],usageEvents=[];if(purchaseDate<e.date)pts.push({date:e.date,km:0,ready:true,driver:'rotation'});(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>e.date&&p.date<=raceDate).forEach(p=>{if(plannedAssignment(p.id)?.shoeId!==e.shoe.id)return;usageEvents.push({date:p.date,deltaKm:Number(p.distance)||0,type:p.type,driver:'training'})});futureRehabShoeUsage(e.shoe.id,null).filter(x=>x.date>e.date&&x.date<=raceDate).forEach(x=>usageEvents.push({date:x.date,deltaKm:Number(x.distanceKm)||0,type:'Rehab',driver:'rehab'}));usageEvents.sort((a,b)=>a.date.localeCompare(b.date)||(a.driver==='training'?-1:1));let km=0;usageEvents.forEach(u=>{km+=Math.max(0,Number(u.deltaKm)||0);pts.push({...u,km})});pts.sort((a,b)=>a.date.localeCompare(b.date)||Number(a.km)-Number(b.km));for(let i=1;i<pts.length;i++){if(Number(pts[i].km)<Number(pts[i-1].km))pts[i].km=Number(pts[i-1].km)}if(pts.at(-1)?.date!==raceDate)pts.push({date:raceDate,km,driver:'end'});return{event:e,purchaseDate,points:pts,finalKm:km}});
 ...
 replacementSeries.forEach(item=>{const e=item.event;allocationEvents.push({date:e.date,driver:'Shoe rotation',markerKm:Number(e.km)||0,title:`${shoeDisplayName(e.shoe)} → ASICS ${e.replacement?.family||'running shoe'} ${e.replacement?.version||''}`,text:`The current pair reaches its planned replacement range at about ${Math.round(e.km)} km. Subsequent workouts assigned to that rotation slot move to the new physical pair.`})});
...
function shoePurchaseProposals(){
 ...
 plannedReplacementEvents().filter(e=>e.date>=iso(today())&&e.date<=race&&e.replacement).forEach(e=>{const purchaseDate=plannedTrainingPurchaseDate(e);items.push({id:`proposal-training-${e.shoe.id}-${e.date}`,kind:'training',brand:'ASICS',model:e.replacement.family,version:String(e.replacement.version||''),recommendedPurchaseStart:purchaseDate,recommendedPurchaseEnd:e.date,targetFirstUseDate:e.date,targetRaceDayMileage:null,intendedRole:'training',replacesShoeId:e.shoe.id,replacesName:shoeDisplayName(e.shoe),rationale:`Replace ${shoeDisplayName(e.shoe)} before the remaining programme pushes that physical pair beyond its expected service-life range.`})});
