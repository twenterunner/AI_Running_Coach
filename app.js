...
const BUILD=9800, SCHEMA=9800, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function injuryPrediction(i){
 const checks=(i.checkIns||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)),latest=checks.at(-1)||{};
 ...
 const currentPain=Number(latest.pain??i.currentPain??i.initialPain??0),walkPain=Number(latest.walkPain??i.currentWalkPain??i.initialWalkPain??0),score=checkInScore(latest)??clamp(100-currentPain*6-walkPain*4,0,100);
 const expectedScore=clamp(initialScore+(100-initialScore)*Math.min(1,elapsed/base),0,100),delta=Math.round(score-expectedScore);
 let pace=delta>=10?'faster':delta<=-10?'slower':'on-track';
 let stage=0;if(walkPain<=3&&currentPain<=5)stage=1;if(walkPain<=1&&currentPain<=3&&(latest.stairs||latest.strength))stage=2;if(currentPain<=2&&walkPain===0&&latest.hop)stage=3;if((Number(latest.runMinutes)||0)>=10&&currentPain<=2&&!latest.nextDayWorse)stage=4;if((Number(latest.runMinutes)||0)>=30&&currentPain<=1&&!latest.nextDayWorse)stage=5;if(latest.nextDayWorse||latest.swelling)stage=Math.max(0,stage-1);
 ...
 let adjustment=Math.round(-delta*.18);if(latest.nextDayWorse)adjustment+=4;if(trend>=10)adjustment-=2;if(trend<=-8)adjustment+=3;
...
const pages=[['dashboard','Dashboard'],['injury','Injury'],['recovery','Recovery'],['today','Today'],['plan','Plan'],['runs','Runs'],['assessments','Assessments'],['race','Race day'],['settings','Settings']];
