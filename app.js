...
function shoePlannedReplacementEvent(shoe){const f=shoeForecast(shoe),series=shoePlannedMileageSeries(shoe),already=shoeMileage(shoe)>=Number(f.low),cross=series.find((p,i)=>i>0&&Number(p.km)>=Number(f.low));let date=already?iso(today()):(cross?.date||null),method=already?'current mileage':cross?'planned assignments':'forecast';if(!date&&f.lowDate&&state.setup?.raceDate&&f.lowDate<=state.setup.raceDate){date=f.lowDate;method='usage forecast'}if(!date)return null;const replacement=replacementProfileForShoe(shoe);return{shoe,date,km:cross?.km||f.low,replacement,method,range:`${Math.round(f.low)}–${Math.round(f.high)} km`}}
...
function shoeProgrammeMileageChartHtml(){const shoes=(state.shoes||[]).filter(s=>s.status!=='retired'||s.showInHistory!==false);if(!shoes.length)return'<div class="shoeEmpty">Add a shoe to start mileage planning.</div>';const enabled=new Set((state.shoeChartVisible||shoes.map(s=>s.id)).filter(id=>shoes.some(s=>s.id===id)));if(!enabled.size)shoes.forEach(s=>enabled.add(s.id));const raceDate=state.setup?.raceDate||iso(today()),todayStr=iso(today()),programStart=state.setup?.planStart||todayStr,series=shoes.filter(s=>enabled.has(s.id)).map(s=>({shoe:s,actual:shoeActualProgrammeSeries(s,programStart,todayStr),planned:shoePlannedMileageSeries(s)}));const dates=[programStart,todayStr,raceDate,...series.flatMap(x=>x.actual.map(p=>p.date)),...series.flatMap(x=>x.planned.map(p=>p.date))].filter(Boolean).sort(),values=series.flatMap(x=>[...x.actual,...x.planned].map(p=>p.km)).filter(Number.isFinite);if(!values.length)return'<div class="shoeEmpty">No tracked or planned mileage yet.</div>';const minD=dte(dates[0]).getTime(),maxD=Math.max(minD+DAY,dte(dates.at(-1)).getTime()),maxY=Math.max(10,Math.ceil(Math.max(...values)*1.12/50)*50),W=760,H=340,L=52,R=18,T=46,B=58,x=d=>L+(dte(d).getTime()-minD)/(maxD-minD)*(W-L-R),y=v=>T+(maxY-v)/maxY*(H-T-B),palette=['#3dd6c6','#79d69b','#67a7ff','#f1c56a','#b9a3ff','#ff9f9f','#7ee7ff'];let grid='';for(let i=0;i<=4;i++){const v=maxY*i/4,yy=y(v);grid+=`<line x1="${L}" y1="${yy}" x2="${W-R}" y2="${yy}"/><text x="${L-8}" y="${yy+4}" text-anchor="end">${Math.round(v)}</text>`}const paths=series.map((s,i)=>{const c=palette[i%palette.length],actualPts=s.actual.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' '),plannedPts=s.planned.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ');return`${actualPts?`<polyline class="shoeLine shoeActualLine" style="--shoe-color:${c}" points="${actualPts}"/>`:''}${plannedPts?`<polyline class="shoeLine shoePlannedLine" style="--shoe-color:${c}" points="${plannedPts}"/>`:''}`}).join('');const replacementProjection=plannedReplacementEvents().filter(e=>enabled.has(e.shoe.id)&&e.replacement&&e.date<=raceDate).map((e,i)=>{let km=0;const pts=[{date:e.date,km:0}];(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>=e.date&&p.date<=raceDate).slice().sort((a,b)=>a.date.localeCompare(b.date)).forEach(p=>{if(plannedAssignment(p.id)?.shoeId!==e.shoe.id)return;km+=Number(p.distance)||0;pts.push({date:p.date,km})});if(pts.at(-1).date!==raceDate)pts.push({date:raceDate,km});const line=pts.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ');return`<polyline class="shoeLine shoeNewPairLine" points="${line}"/>`}).join('');const raceLifecycle=raceShoePlan(),racePurchaseSeries=raceLifecycle.purchaseNeeded&&raceLifecycle.profile&&raceLifecycle.famSessions?.length?(()=>{let km=0;const first=raceLifecycle.famSessions[0].date,pts=[{date:first,km:0}];raceLifecycle.famSessions.forEach(p=>{km+=Number(p.plannedShoeKm)||0;pts.push({date:p.date,km})});if(pts.at(-1).date!==raceDate)pts.push({date:raceDate,km});return`<polyline class="shoeLine shoeRacePurchaseLine" points="${pts.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ')}"/>`})():'';const todayX=x(todayStr),raceX=x(raceDate),markers=plannedReplacementEvents().filter(e=>enabled.has(e.shoe.id)&&e.date>=dates[0]&&e.date<=dates.at(-1)).map((e,i)=>{const xx=x(e.date),yy=Math.max(T+12,y(Math.min(maxY,e.km))),labelY=16+(i%2)*14;return`<g class="shoeReplacementMarker"><line x1="${xx}" y1="${T}" x2="${xx}" y2="${H-B}"/><circle cx="${xx}" cy="${yy}" r="5"/><text x="${Math.min(W-R-4,Math.max(L+4,xx))}" y="${labelY}" text-anchor="${xx>W*.72?'end':xx<W*.28?'start':'middle'}">Replace ${esc(e.shoe.model)}${e.replacement?` → ${esc(e.replacement.family)} ${esc(e.replacement.version)}`:''}</text></g>`}).join('');const xlabels=[programStart,todayStr,raceDate].filter((v,i,a)=>v&&a.indexOf(v)===i).map(d=>`<text class="xLab" x="${x(d)}" y="${H-18}" text-anchor="middle">${d===todayStr?'Today':d===raceDate?'Race day':dte(d).toLocaleDateString(undefined,{month:'short',year:'2-digit'})}</text>`).join('');const legend=shoes.map((s,i)=>`<label class="shoeLegendToggle"><input type="checkbox" data-shoe-chart-toggle="${esc(s.id)}" ${enabled.has(s.id)?'checked':''}><i style="--shoe-color:${palette[i%palette.length]}"></i><span>${esc(shoeDisplayName(s))}</span></label>`).join('');const events=plannedReplacementEvents();const eventHtml=events.length?`<div class="shoeLifecycleEvents">${events.map(e=>`<article><small>AROUND ${esc(fmtDate(e.date).toUpperCase())}</small><b>Replace ${esc(shoeDisplayName(e.shoe))}</b><span>${esc(e.range)} expected window · ${esc(e.method)}</span>${e.replacement?`<strong>New-pair proposal: ASICS ${esc(e.replacement.family)} ${esc(e.replacement.version)}</strong>`:'<strong>Choose a suitable replacement for the same role.</strong>'}</article>`).join('')}</div>`:'<div class="shoeEmpty compact">No shoe is currently projected to reach its replacement range before race day.</div>';return`<div class="shoeChartWrap shoeProgrammeChart"><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Actual accumulated shoe mileage versus planned mileage through race day"><g class="shoeGrid">${grid}</g><line class="shoeTodayLine" x1="${todayX}" y1="${T}" x2="${todayX}" y2="${H-B}"/><line class="shoeRaceLine" x1="${raceX}" y1="${T}" x2="${raceX}" y2="${H-B}"/>${paths}${replacementProjection}${racePurchaseSeries}${markers}${xlabels}<text class="yTitle" transform="translate(14 ${H/2}) rotate(-90)" text-anchor="middle">Accumulated km</text></svg><div class="shoeChartKey"><span><i class="actual"></i>Actual accumulated mileage</span><span><i class="planned"></i>Planned cumulative mileage</span><span><i class="replace"></i>Approx. replacement point</span><span><i class="newpair"></i>Proposed new-pair mileage</span></div><div class="shoeChartLegend">${legend}</div><p class="muted compact">Solid lines are observed mileage through today. Dashed lines start from each shoe’s current real mileage and add the workouts currently assigned to that shoe through race day. Replacement markers use the first planned threshold crossing where possible, otherwise the existing usage forecast.</p>${eventHtml}${raceLifecycle.purchaseNeeded&&raceLifecycle.profile?`<div class="shoeLifecycleEvents racePurchaseEvent"><article><small>${raceLifecycle.purchaseStart?`BUY AROUND ${esc(fmtDate(raceLifecycle.purchaseStart).toUpperCase())}`:`FUTURE PURCHASE`}</small><b>Proposed race shoe: ASICS ${esc(raceLifecycle.profile.family)} ${esc(raceLifecycle.profile.version)}</b><span>${raceLifecycle.purchaseStart&&raceLifecycle.purchaseEnd?`${esc(fmtDate(raceLifecycle.purchaseStart))} – ${esc(fmtDate(raceLifecycle.purchaseEnd))}`:`Purchase timing remains uncertain`}</span><strong>Projected race-day mileage ~${Math.round(raceLifecycle.projectedNew||0)} km</strong></article></div>`:``}</div>`}
