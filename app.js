...
const BUILD=10030, SCHEMA=10030, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function renderInjury(){
 ...
 box.innerHTML=ordered.map(i=>{
  ...
  const clinicianCard=clinician?`<section class="injuryTopicCard clinicianComparison ${clinician.verdict}"><div class="injurySectionHead"><div><h4>Clinician assessment cross-check</h4><p class="muted compact">The app independently evaluates the symptom pattern rather than accepting the entered diagnosis automatically.</p></div><span class="clinicianVerdict ${clinician.verdict}">${clinician.verdict==='confirms'?'Confirms':clinician.verdict==='partly_agrees'?'Partly agrees':'Contradicts'}</span></div><div class="clinicianCompareGrid"><div><small>Clinician entered</small><b>${esc(clinician.entered)}</b></div><div><small>Independent app assessment</small><b>${esc(diag.name)}</b></div></div><p>${esc(clinician.status)}</p><small>${clinician.reasons.map(esc).join(' · ')}</small></section>`:'';
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v10.0.3 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v10.0.3 stable build 10030');
})();