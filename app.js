...
const BUILD=9880, SCHEMA=9880, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function injuryLocationType(i){let t=((i.location||'')+' '+(i.mechanism||'')+' '+(i.initialSymptoms||'')).toLowerCase();return /hamstring|posterior thigh|back of thigh|ischial|buttock/.test(t)?'hamstring':'generic'}
function workingDiagnosis(i){
 const type=injuryLocationType(i),pain=Number(i.initialPain)||0,walk=Number(i.initialWalkPain)||0,txt=((i.location||'')+' '+(i.mechanism||'')+' '+(i.initialSymptoms||'')).toLowerCase();
 if(type==='hamstring'){
  const proximal=/upper|proximal|ischial|buttock|glute/.test(txt),sprint=/sprint|accelerat|speed|kick/.test(txt);
  let grade=(i.pop||i.bruising||pain>=7||walk>=6)?'moderate (grade II pattern)':'mild-to-moderate (grade I–II pattern)';
  let name=`${proximal?'Proximal ':'Hamstring '}${grade} strain`;
  let evidence=[];if(sprint)evidence.push('high-speed mechanism');if(pain>=5)evidence.push(`initial pain ${pain}/10`);if(i.bruising)evidence.push('bruising/swelling');if(i.pop)evidence.push('pop or snap');if(walk>=3)evidence.push(`walking pain ${walk}/10`);if(proximal)evidence.push('upper hamstring/buttock location');
  let confidence=Math.min(90,55+evidence.length*6);
  return{name,confidence,evidence,alternatives:['More severe partial hamstring tear','Proximal hamstring tendon injury','Referred pain from the lower back or sciatic nerve']};
 }
 return{name:'Lower-limb soft-tissue injury pattern',confidence:45,evidence:['location and symptom pattern entered by the user'],alternatives:['Joint, tendon or bone-related injury','Referred or nerve-related pain','A condition requiring physical examination']};
}
...
function injuryPrediction(i){
 ...
 let nominalTotal=initialPain>=8||pop||walkInitially>=8?70:initialPain>=6||bruising||walkInitially>=5?49:initialPain>=3?35:21;
...
function renderInjury(){
 ...
 <section class="injurySection"><h4>Clinical working assumption</h4><div class="diagnosisBox"><div><small>MOST LIKELY</small><strong>${esc(diag.name)}</strong><span>${diag.confidence}% pattern confidence</span></div><p><b>Why:</b> ${diag.evidence.map(esc).join(' · ')||'limited symptom information'}</p><details><summary>Alternatives and boundary</summary><p>${diag.alternatives.map(esc).join(' · ')}</p><p class="muted">This is a working pattern assumption, not a confirmed medical diagnosis. Examination or imaging may change it.</p></details></div></section>
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.7.0 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.7.0 stable build 9700');
})();