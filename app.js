...
const BUILD=9950, SCHEMA=9950, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
const INJURY_RECOVERY_WINDOWS={
 ...
 fifth_met_bsi:[84,168,270],lumbar_radicular:[28,84,180]
};
...
function injuryPrediction(i){
 ...
 if(Number.isFinite(delta))remaining+=Math.round(clamp(-delta*.20,-14,21));if(latest.nextDayWorse===true)remaining+=7;if(latest.newSwelling===true)remaining+=7;if(latest.alteredGait===true)remaining+=4;if(diag.urgent)remaining=Math.max(remaining,Math.max(14,baselineMin-elapsed));
 remaining=Math.max(7,Math.min(Math.max(14,Math.round(baselineMax*1.25)),remaining));const total=elapsed+remaining,central=new Date(today().getTime()+remaining*DAY),uncertainty=checks.length>=7?Math.max(7,Math.round(nominalTotal*.12)):checks.length>=3?Math.max(10,Math.round(nominalTotal*.18)):Math.max(14,Math.round(nominalTotal*.25)),rangeStartTotal=Math.max(elapsed+7,Math.max(baselineMin,total-uncertainty)),rangeEndTotal=Math.max(rangeStartTotal+7,Math.min(Math.round(baselineMax*1.25),total+uncertainty)),windowStart=iso(new Date(dte(i.date).getTime()+rangeStartTotal*DAY)),windowEnd=iso(new Date(dte(i.date).getTime()+rangeEndTotal*DAY)),confidence=checks.length>=7?'Moderate':checks.length>=3?'Developing':'Low';return{nominalTotal,baselineMin,baselineMax,total,elapsed,remaining,stage,fullDate:iso(central),windowStart,windowEnd,confidence,currentPain,walkPain,completion,nominal,delta,latest,checks,diag,safetyHold:diag.urgent,trendRate};
}
function injuryCompletionForChecks(i,checks,diag,initialPain,walkInitially){
 if(!checks.length)return null;const latest=checks.at(-1)||{},currentPain=nullableNumber(latest.pain)??nullableNumber(i.currentPain)??initialPain,walkPain=nullableNumber(latest.walkPain)??nullableNumber(i.currentWalkPain)??walkInitially;let stage=0;for(let target=1;target<INJURY_STAGES.length;target++){const c=criterionState({...i,checkIns:checks},{currentPain,walkPain},target);if(c.length&&c.every(x=>x.status==='met'))stage=target;else break;}if(diag.urgent)stage=Math.min(stage,1);const stageBase=[5,18,36,56,74,90][stage];let within=0;if(Number.isFinite(currentPain))within+=(3-currentPain)*2;if(Number.isFinite(walkPain))within+=(1-walkPain)*2;if(latest.bridge===true)within+=2;if(latest.hop===true)within+=2;if(Number(latest.runMinutes)>=10)within+=2;if(latest.nextDayWorse===true)within-=5;if(latest.newSwelling===true)within-=4;if(latest.alteredGait===true)within-=3;return Math.round(clamp(stageBase+Math.max(-8,Math.min(10,within)),0,98));
}
...
function renderInjury(){
 ...
 <div class="recoveryScoreWhy ${scoreInfo.status}"><b>${esc(scoreInfo.label)}</b><p>${esc(scoreInfo.text)}</p></div>
 ...
 <section class="injurySection"><div class="injurySectionHead"><div><h4>Daily check-ins</h4><p class="muted compact">Review or correct any current or previous entry.</p></div><strong>${p.checks.length}</strong></div><div class="checkInHistory">${p.checks.length?p.checks.slice().reverse().map(c=>`<button type="button" class="checkInHistoryRow" data-injury-check-edit="${i.id}" data-check-date="${c.date}"><span><b>${fmtDate(c.date)}</b><small>${Number.isFinite(c.pain)?`Pain ${c.pain}/10`:''}${Number.isFinite(c.walkPain)?` · walking ${c.walkPain}/10`:''}${Number.isFinite(c.runMinutes)?` · run ${c.runMinutes} min`:''}</small></span><em>Edit</em></button>`).join(''):'<p class="muted">No check-ins yet.</p>'}</div></section>
 <details class="injuryDisclosure"><summary>Recovery rationale and comparison</summary><div class="timelineReason"><p>The ${esc(diag.name.toLowerCase())} pathway uses a broad baseline window of ${p.baselineMin}–${p.baselineMax} days, with ${p.nominalTotal} days as the central comparison point.</p><p>You are ${p.elapsed} days after injury and currently in Stage ${p.stage+1}: ${esc(st.name)}.</p><p>Blank or unanswered fields are treated as not assessed and do not count toward progression.</p><p><b>${esc(scoreInfo.label)}:</b> ${esc(scoreInfo.text)}</p><p>The displayed date is a window because recovery cannot be predicted to one exact day.</p></div><div class="comparisonList">${factors.map(f=>`<div class="comparisonRow ${f.status}"><div><b>${esc(f.name)}</b><small>${esc(f.reason)}</small></div><span>${esc(f.actual)}</span><em>Nominal: ${esc(f.nominal)}</em></div>`).join('')}</div></details>
...
function openInjuryCheck(i,existing=null){
 const latest=sortedChecks(i).at(-1)||{},prev=existing||latest,editing=!!existing,originalDate=existing?.date||null;
 $('modalContent').innerHTML=`<h3>${editing?'Edit':'Daily'} injury check-in</h3><p class="muted compact">Use “Not assessed” when you did not test something. Updating an earlier entry recalculates every later recovery point and the unrestricted-running estimate.</p><div class="formGrid"><div class="field"><label>Date</label><input id="icDate" type="date" value="${existing?.date||iso(today())}"></div><div class="field"><label>Pain now 0–10</label><input id="icPain" type="number" min="0" max="10" value="${prev.pain??''}"></div><div class="field"><label>Walking pain 0–10</label><input id="icWalk" type="number" min="0" max="10" value="${prev.walkPain??''}"></div><div class="field"><label>Confidence in injured area 0–10</label><input id="icConfidence" type="number" min="0" max="10" value="${prev.confidence??''}"></div>${triSelect('icBridge','Repeated strength loading controlled?',prev.bridge)}${triSelect('icHop','Gentle hop or jog-in-place tolerated?',prev.hop)}${triSelect('icGait','Was gait or running technique altered?',prev.alteredGait)}${triSelect('icSwelling','Any new swelling or bruising?',prev.newSwelling)}<div class="field"><label>Running completed (minutes)</label><input id="icRun" type="number" min="0" value="${prev.runMinutes??''}" placeholder="Leave blank if not assessed"></div>${triSelect('icWorse','Worse the next morning?',prev.nextDayWorse)}<div class="field"><label>Symptoms / notes</label><textarea id="icSymptoms">${esc(prev.symptoms||'')}</textarea></div></div><button id="saveCheck" class="primary full">${editing?'Update check-in':'Save and recalculate timeline'}</button>${editing?'<button id="deleteCheck" class="danger full">Delete this check-in</button>':''}`;
 $('modal').className='modal';$('saveCheck').onclick=()=>{let c={date:$('icDate').value,pain:nullableNumber($('icPain').value),walkPain:nullableNumber($('icWalk').value),confidence:nullableNumber($('icConfidence').value),bridge:readTri('icBridge'),hop:readTri('icHop'),alteredGait:readTri('icGait'),newSwelling:readTri('icSwelling'),runMinutes:nullableNumber($('icRun').value),nextDayWorse:readTri('icWorse'),symptoms:$('icSymptoms').value.trim()};if(!c.date)return toast('Enter the check-in date.',true);if(!Number.isFinite(c.pain)&&!Number.isFinite(c.walkPain)&&!Object.values(c).some(v=>v===true||v===false))return toast('Record at least one observation.',true);i.checkIns=i.checkIns||[];if(editing&&originalDate!==c.date&&i.checkIns.some(x=>x.date===c.date))return toast('A check-in already exists for that date.',true);let n=i.checkIns.findIndex(x=>x.date===(editing?originalDate:c.date));n>=0?i.checkIns[n]=c:i.checkIns.push(c);save();$('modal').className='modal hidden';renderInjury();toast(editing?'Check-in updated and timeline recalculated.':'Timeline recalculated from today’s feedback.')};if(editing)$('deleteCheck').onclick=()=>{if(confirm('Delete this check-in?')){i.checkIns=i.checkIns.filter(x=>x.date!==originalDate);save();$('modal').className='modal hidden';renderInjury();toast('Check-in deleted and timeline recalculated.')}};
}
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.9.5 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.9.5 stable build 9950');
})();