...
function buildRehabCalendar(i,p){
 ...
 for(let offset=0;offset<7;offset++){
  const date=iso(new Date(today().getTime()+offset*DAY)),fresh=rehabCalendarDay(i,p,date,offset),previous=oldMap.get(date),check=checks.get(date),completed=!!check;
  const changed=!!previous&&rehabCalendarSignature(previous)!==rehabCalendarSignature(fresh);
  days.push({...fresh,completed,checkSummary:check?(check.rehabStatus==='completed'?'Rehabilitation completed':check.rehabStatus==='reduced'?'Reduced dose completed':check.rehabStatus==='stopped'?'Stopped because of symptoms':check.runStatus==='completed'?`Run completed${Number.isFinite(check.runMinutes)?` · ${check.runMinutes} min`:''}`:check.runStatus==='not_planned'?'Rest / no run planned':'Check-in recorded'):null,updated:changed});
 }
...
function openInjuryCheck(i,existing=null){
 ...
 <section class="injuryCheckSection"><h4>3. Rehabilitation load</h4><div class="formGrid">${selectField('icRehab','Rehabilitation exercise today',prev.rehabStatus,[['','Not assessed'],['not_planned','Not planned / rest day'],['completed','Completed as prescribed'],['reduced','Completed reduced dose'],['stopped','Stopped because of symptoms']])}${triSelect('icBridge','Strength exercise tolerated with control?',prev.bridge,'Tolerated','Not tolerated')}${triSelect('icHop','Impact test or jog-in-place tolerated?',prev.hop,'Tolerated','Not tolerated')}</div></section>
