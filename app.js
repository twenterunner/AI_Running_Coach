...
const BUILD=9310, SCHEMA=9310, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function athleteState(w=currentWeek()){
 const ev=trainingEvidence(),afd=adaptiveFactorDetails(w),pain=recoveryPainState(),hrv=hrvModel();
 const fitnessTrend=ev.index>100.5?'Improving':ev.index<99.5?'Declining':'Stable';
 ...
 if(afd.status==='calculated'){const load=afd.items.find(x=>x.name==='Completed load');if((load?.adjustment??0)<=-.08)tolerance='Low';else if((load?.adjustment??0)<0)tolerance='Reduced';else if(afd.trainingResponse>1)tolerance='Good';}else if(afd.status==='pending')tolerance='Pending';
 const adjustment=afd.factor,direction=adjustment<.995?'Reduce':adjustment>1.005?'Increase':'Maintain',pct=Math.round(Math.abs(adjustment-1)*100),reasons=[];
 if(fitnessTrend!=='Stable')reasons.push(`fitness ${fitnessTrend.toLowerCase()}`);if(readiness!=='Normal')reasons.push(`recovery ${readiness.toLowerCase()}`);if(tolerance!=='Normal'&&tolerance!=='Pending')reasons.push(`training tolerance ${tolerance.toLowerCase()}`);if(!reasons.length)reasons.push('fitness, recovery and completed load are balanced');
 return{fitnessTrend,fitnessIndex:ev.index,evidenceConfidence:ev.confidence,readiness,tolerance,adjustment,direction,pct,reasons,afd,pain,hrv};
}
...
function renderPlan(){
 const ast=athleteState(state.weekView||currentWeek());if($('fitnessEvidence'))$('fitnessEvidence').innerHTML=`<div class="athleteStateGrid"><div><span>Fitness</span><strong>${ast.fitnessTrend}</strong><small>${ast.evidenceConfidence}% evidence confidence</small></div><div><span>Recovery</span><strong>${ast.readiness}</strong><small>${ast.hrv.ready?`HRV ${esc(ast.hrv.status)}`:'HRV baseline not established'}</small></div><div><span>Injury status</span><strong>${ast.pain.status}</strong><small>${ast.pain.count?`Recent pain max ${ast.pain.max}/10`:'No recent pain entries'}</small></div><div><span>Training tolerance</span><strong>${ast.tolerance}</strong><small>Based on completed load and response</small></div><div class="stateDecision"><span>Next-week adjustment</span><strong>${ast.direction}${ast.pct?` ${ast.pct}%`:''}</strong><small>${esc(ast.reasons.join(' · '))}</small></div></div><details class="stateDetails"><summary>Calibration details</summary><p>Underlying fitness calibration: ${ast.fitnessIndex.toFixed(1)}. It updates pace and power targets, while the weekly adjustment controls volume and session load. These components are combined into the coaching decision above rather than shown as competing headline factors.</p></details>`;
...
function renderTrainingDays(){
 ...
 box.innerHTML=`<div class="note trainingDayNote"><b>Race- and phase-specific sessions</b><p class="muted compact">Choose the days you can train. The engine assigns hills, threshold, race-specific, easy, medium-long, recovery and long-run sessions according to the selected race and current phase.</p></div>`+state.days.map((d,i)=>`<div class="trainingDayRow"><b>${d[0]}</b><label><input data-day="${i}" type="checkbox" ${d[1]?'checked':''}> Train</label><span class="dayRole ${d[0]===longDay?'longRole':''}">${d[1]?(d[0]===longDay?'Long-run anchor':'Adaptive session'):'Rest day'}</span></div>`).join('');
}
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.3.1 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.3.1 stable build 9310');
})();