...
const BUILD=10020, SCHEMA=10020, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function renderInjury(){
 ...
 <details class="injuryDisclosure clinicalReasoningSummary"><summary>Clinical reasoning, likely causes and prevention</summary><div class="clinicalReasoningBody"><div class="injurySectionHead"><div><h4>Clinical reasoning summary</h4><p class="muted compact">Diagnosis rationale, likely contributors, prevention and whether a formal rehabilitation pathway is currently justified.</p></div><strong>${clinicalPlan.selfManagement?'Self-management':'Rehabilitation'}</strong></div><div class="clinicalReasoningGrid"><div><b>Why this pattern?</b><p>${diag.evidence.slice(0,5).map(esc).join(' · ')||'More detail is needed.'}</p></div><div><b>Likely contributors</b><ul>${clinicalPlan.causes.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div><b>Prevention</b><ul>${clinicalPlan.prevention.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div><b>Plan decision</b><p>${clinicalPlan.needsRehab?'A staged rehabilitation plan is appropriate because the current pattern or functional status requires monitored progression.':'A formal rehabilitation pathway may not be necessary if symptoms settle fully within 24–48 hours and no focal tenderness, hopping pain, night pain or neurological symptoms are present. Prevention and load modification remain important.'}</p></div></div></div></details>
 ...
 ${rehabTodayFocusHtml(i,p)}
 ${rehabCalendarHtml(i,p)}
 <section class="injurySection exerciseGuidesSection"><div class="injurySectionHead"><div><h4>Exercise guides</h4><p class="muted compact">Detailed technique guidance for the exercises used in today’s prescription.</p></div></div>${exercises.map(x=>`<details class="exerciseDetail"><summary><span><b>${esc(x.name)}</b><small>${esc(x.dose)} · ${esc(x.purpose)}</small></span><em>Open guide</em></summary><div class="exerciseGuide"><div class="exerciseWhy"><b>Why today?</b><p>${esc(x.why)}</p></div><div><b>How to perform</b><ol>${x.steps.map(y=>`<li>${esc(y)}</li>`).join('')}</ol></div><div class="exerciseRules"><div><b>Pain rule</b><p>${p.safetyHold?'Do not test impact or progress loading until assessed.':'Keep pain at 0–2/10. Stop for sharp pain, altered movement, or symptoms that worsen later or next morning.'}</p></div><div><b>Progress when</b><p>${esc(x.progress)}</p></div></div></div></details>`).join('')}</section>
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v10.0.2 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v10.0.2 stable build 10020');
})();