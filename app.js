...
function renderSettings(){
 ...
 const fieldHtml=d=>{let v=state.setup[d[0]];if(d[2]==='time')v=fmtTime(v);if(d[2]==='percent')v=Math.round(v*100);const type=d[2]==='date'?'date':d[2]==='number'||d[2]==='percent'?'number':'text';const bounds=d[3]!==undefined?`min="${d[3]}" max="${d[4]}" step="${d[3]<1?'.1':'1'}"`:'';return`<div class="field settingsField"><label>${d[1]}</label><div class="settingsInputWrap"><input data-setting="${d[0]}" data-type="${d[2]}" type="${type}" ${bounds} value="${esc(v)}">${d[5]?`<span>${d[5]}</span>`:''}</div></div>`};
...
function shoeRecommendation(plan,{excludeId=null}={}){
 ...
 const ranked=active.map(shoe=>{const p=shoeProfileForShoe(shoe),wear=shoeWearState(shoe),fam=shoeFamiliarity(shoe,plan.type);let weighted=0,total=0,reasons=[];
  ...
  const recent=shoeRecentKm(shoe.id,7),plannedNear=shoePlannedUsage(shoe.id,7),projectedAtWorkout=shoeMileage(shoe)+shoePlannedUsageThrough(shoe.id,plan.date);score-=Math.min(8,recent/12);score-=Math.min(6,plannedNear/18);if(projectedAtWorkout>=wear.high)score-=35;else if(projectedAtWorkout>=wear.low)score-=14;if((p.roles||[]).includes('race')&&!/Race|interval|Threshold|Specific|rehearsal|Fitness/.test(plan.type))score-=10;
...
function ensureShoeAutoAssignments(){const stamp=[state.storageRevision||0,state.setup?.raceDate||'',(state.plan||[]).length,(state.shoes||[]).map(s=>`${s.id}:${s.status}:${shoeMileage(s).toFixed(2)}`).join(','),(state.plannedShoeAssignments||[]).filter(a=>a.source==='user').map(a=>`${a.planId}:${a.shoeId}`).join(',')].join('|');if(shoeAutoAssignmentStamp===stamp)return;shoeAutoAssignments();shoeAutoAssignmentStamp=stamp}
function renderShoes(){const root=$('shoesContent');if(!root)return;reconcileShoeUsage();ensureShoeAutoAssignments();if(!root.closest('.page')?.classList.contains('active'))return;const active=(state.shoes||[]).filter(s=>s.status!=='retired'),next=nextRunningPlan(),rec=next?shoeRecommendationForPlan(next):null;const nextHtml=!active.length?`<div class="shoeEmpty"><b>No shoes configured</b><p>Add your current rotation. The rest of the app remains unchanged until you do.</p></div>`:next&&rec?.best?`<article class="shoeNextCard"><div class="shoeNextWorkout"><span>${planWorkoutPictogram(next.type)}</span><div><small>${fmtDate(next.date)} · ${esc(next.type)}</small><h4>${Number(next.distance).toFixed(1)} km</h4></div><span class="shoeMatchBadge">${esc(rec.best.label)} · ${esc(rec.best.confidence||'Medium')} confidence</span></div><div class="shoeRecommendation"><small>RECOMMENDED</small><h3>${esc(shoeDisplayName(rec.best.shoe))}</h3><p>Best match for the next planned ${esc(next.type.toLowerCase())}.</p><ul>${rec.best.reasons.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>${rec.best.warning?`<div class="shoeWarning">${esc(rec.best.warning)}</div>`:''}${rec.alternative?`<p class="shoeAlternative"><b>Alternative:</b> ${esc(shoeDisplayName(rec.alternative.shoe))}</p>`:''}</div></article>`:`<div class="shoeEmpty">No future running workout is currently available for recommendation.</div>`;
 ...
 root.innerHTML=`<div class="sectionTitle shoesPageTitle"><div><span class="pageEyebrow">SHOE ROTATION</span><h2>Shoes</h2><p>Mileage, workout matching, lifecycle and race-day planning.</p></div><button id="addShoeBtn" class="primary small" type="button">Add running shoe</button></div><section class="shoeSection level1"><div class="shoeSectionHead"><span>01</span><div><small>NEXT RUN</small><h3>What should you run in?</h3></div></div>${nextHtml}</section><section class="shoeSection"><div class="shoeSectionHead"><span>02</span><div><small>ACTIVE ROTATION</small><h3>Your available shoes</h3></div></div><div class="shoeRotationGrid">${active.map(shoeRotationCard).join('')||'<div class="shoeEmpty">No active shoes.</div>'}</div></section><section class="shoeSection"><div class="shoeSectionHead"><span>03</span><div><small>MILEAGE & LIFECYCLE PLAN</small><h3>Actual vs planned shoe mileage</h3><p>Real accumulated mileage versus the planned rotation through race day, including expected replacement points.</p></div></div><article class="shoeChartCard">${shoeMileageChartHtml()}</article></section><section class="shoeSection"><div class="shoeSectionHead"><span>04</span><div><small>REPLACEMENT OUTLOOK</small><h3>When will your shoes need replacing?</h3></div></div><div class="shoeOutlookList">${outlook}</div><p class="muted">Replacement mileage is a planning range, not a scientifically exact retirement point. Wear, terrain and individual use create uncertainty.</p></section><section class="shoeSection"><div class="shoeSectionHead"><span>05</span><div><small>PLANNED ROTATION</small><h3>How will the training plan use your shoes?</h3></div></div>${(active.length||(state.plannedShoePurchases||[]).some(x=>x.status==='planned'))?`<div class="shoePlanTableWrap"><table class="shoePlanTable"><thead><tr><th>Shoe</th><th>Next 7 days</th><th>Next 4 weeks</th><th>Programme</th></tr></thead><tbody>${planned}</tbody></table></div>`:'<div class="shoeEmpty">Add shoes to forecast planned rotation.</div>'}</section><section class="shoeSection"><div class="shoeSectionHead"><span>06</span><div><small>RACE CHOICE TODAY</small><h3>Which owned pair would you race in today?</h3><p>Best existing-shoe choice for common race distances, based on current mileage, suitability and familiarity.</p></div></div>${shoeRaceNowRecommendationsHtml()}</section><section class="shoeSection level1"><div class="shoeSectionHead"><span>07</span><div><small>RACE-DAY SHOE PLAN</small><h3>Will your race shoes be ready?</h3><p>Works backward from race day to balance familiarity, service life and future usage.</p></div></div>${raceDayShoeTargetHtml()}${racePlanHtml()}</section>`;
