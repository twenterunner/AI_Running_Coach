...
function runSummaryHtml(r){
 ...
 return`<section class="runSummaryHero">
   ...
   <div class="runSummaryMetrics"><div><small>DISTANCE</small><br><b>${Number(r.distanceKm).toFixed(2)} km</b>${plan?`<br><span>plan ${Number(plan.distance).toFixed(1)} km</span>`:''}</div><div><small>DURATION</small><br><b>${fmtTime(r.durationSec)}</b><br><span>${pace(m.pace)}</span></div><div><small>HEART RATE</small><br><b>${Number.isFinite(Number(r.avgHr))?Math.round(r.avgHr)+' bpm':'Not available'}</b></div><div><small>POWER</small><br><b>${Number.isFinite(Number(r.avgPower))?Math.round(r.avgPower)+' W':'Not available'}</b></div><div><small>RPE</small><br><b>${Number.isFinite(Number(r.rpe))?Number(r.rpe).toFixed(0)+'/10':'Not entered'}</b></div><div><small>PAIN</small><br><b>${Number.isFinite(Number(r.pain))?Number(r.pain).toFixed(0)+'/10':'Not entered'}</b></div></div>
   ...
   <div class="runDataQuality ${q.missing.length?'partial':'good'}"><b>${quality}</b><br><span>${q.stream?'FIT/CSV record stream can support deeper analysis where relevant.':'No detailed record stream is stored for this session.'}</span></div>
 </section>`;
}
...
function pathwayEvidenceCardHtml(title,d,t){
 ...
 return`<article class="logEvidencePath ${status}">
   ...
   <div class="logEvidenceGaugeRow"><div class="evidenceMiniGauge ${status}" style="--e:${weight}"><strong>${weight}%</strong><br><small>evidence weight</small></div><div class="logEvidenceMetrics"><span><small>RUN SIGNAL</small><br><b>${t.rawSignal>=0?'+':''}${t.rawSignal.toFixed(2)}</b></span><span><small>ACCEPTED CONTRIBUTION</small><br><b>${accepted?signedFactorDelta(t.acceptedContribution):'0.000'}</b></span><span><small>THIS WEEK</small><br><b>${signedFactorDelta(t.weeklyBucket)}</b></span></div></div>
   <div class="logFactorFlow"><div class="factorTile"><small>APPLIED</small><br><b>${t.applied.toFixed(3)}</b><br><span>Current committed factor</span></div><i>→</i><div class="factorTile"><small>PROJECTED NEXT REVIEW</small><br><b>${t.projected.toFixed(3)}</b><br><span>${signedFactorDelta(projectedChange)} vs applied</span></div></div>
