...
function footMechanicsCompatibility(profile,planType='',context='assignment'){
 ...
 const stability=Number(profile?.stability||3),cush=Number(profile?.cushioning||3),comfort=Number(profile?.comfort||3),roles=new Set(profile?.roles||[]);
 const type=String(planType||''),easy=/Easy|Recovery|Steady|Medium-long|Long run|Rehab/i.test(type),quality=/Threshold|interval|VO₂|Fartlek|Progression|Specific|rehearsal|Race-pace|Fitness assessment|Hills/i.test(type),race=/Race Day|^Race$/i.test(type);
 let score=0;
 if(gait==='pronation'){
  if(guided)score+=easy?28:quality?16:race?9:22;
  else{score+=stability>=4?10:stability>=3?4:-6;if(easy&&stability<=2)score-=10;if(quality&&(roles.has('tempo')||roles.has('threshold')||roles.has('intervals')||roles.has('race')))score+=5}
 }else if(gait==='supination'){
  if(guided)score-=context==='purchase'?35:25;
  else score+=12+(cush>=4?7:0)+(comfort>=4?4:0)+(easy&&cush>=4?4:0);
 }else if(gait==='neutral')score+=guided?-7:9;
 else score+=(comfort>=4?2:0)+(stability>=4?1:0);
 if(context==='purchase')score=Math.round(score*1.15);
 if(context==='rehab'){if(gait==='pronation'&&guided)score+=5;if(gait==='supination'&&!guided&&cush>=4)score+=5}
 return clamp(Math.round(score),-40,40)
}
function gaitPurchaseAdjustment(profile){return footMechanicsCompatibility(profile,'','purchase')}
const SHOE_ENGINE_STRATEGIES={
 balanced:{label:'Balanced · recommended',desc:'Use the shoes you already own for the roles they fit best, normally maintain two meaningfully active training pairs, use model-specific life up to 100%, replace only when a genuine lifecycle or programme need exists, and finish Race Day with a suitable familiarised pair.'}
};
function normaliseShoeEngineStrategyKey(){return'balanced'}
function shoeEngineStrategy(){return SHOE_ENGINE_STRATEGIES.balanced}
function shoeEngineStrategyHtml(){return`<div class="shoeApproachControl shoeApproachFixed"><label>Shoe strategy</label><strong>Balanced</strong><small>${esc(SHOE_ENGINE_STRATEGIES.balanced.desc)}</small></div>`}
...
function shoeRecommendationForPlan(plan){
 ...
 if(manual?.source==='user'){
  const chosen=(state.shoes||[]).find(s=>s.id===manual.shoeId&&s.status!=='retired');
  if(chosen){
   const generic=shoeRecommendation(plan),base=generic.ranked.find(x=>x.shoe.id===chosen.id),alt=shoeRecommendation(plan,{excludeId:chosen.id}).best;
   return{best:base||{shoe:chosen,score:50,label:'Selected',reasons:['Manually selected for this workout'],wear:shoeWearState(chosen),familiarity:shoeFamiliarity(chosen,plan.type)},alternative:alt,manual:true,planner:false}
  }
 }
 const life=freshShoeLifecyclePlan(),row=life.assignments.find(a=>a.planId===plan.id&&!a.rehab);
 if(row){
  const pair=life.pairs.find(p=>p.id===row.pairId),shoe=lifecyclePairAsDisplayShoe(pair);
  if(pair&&shoe){
   const generic=pair.owned?shoeRecommendation(plan):null,base=pair.owned?generic?.ranked.find(x=>x.shoe.id===pair.shoe.id):null;
   const reasons=[row.why,`Lifecycle plan: ${shoeEngineStrategy().label}`,pair.owned?`${Math.round(pair.currentKm)} km currently accumulated`:`Future physical pair · first use ${fmtDate(pair.availableDate)}`].filter(Boolean);
   return{best:base||{shoe,score:Math.round(row.fit||lifecycleWorkoutFit(pair.profile,plan)),label:pair.owned?'Lifecycle plan':'Planned future pair',reasons,wear:pair.owned?shoeWearState(pair.shoe):{status:'planned',label:'Planned future pair'},familiarity:pair.owned?shoeFamiliarity(pair.shoe,plan.type):{runs:0,km:0}},alternative:pair.owned?shoeRecommendation(plan,{excludeId:pair.shoe.id}).best:null,manual:false,planner:true,future:!pair.owned,pair,row}
  }
 }
 return shoeRecommendation(plan)
}
...
function raceOptimalWindow(profile,raceDistance){const d=Number(raceDistance)||0,isRace=(profile.roles||[]).includes('race'),plated=Boolean(profile.plated);let min=plated?Math.max(15,Math.min(35,d*.65)):Math.max(20,Math.min(50,d*.8));let max=plated?Math.min(120,Math.max(55,d*2.2)):Math.min(250,Math.max(90,d*4));if(!isRace){min=Math.max(min,30);max=Math.max(max,120)}return{minKm:min,maxKm:max}}
function raceRelevantWorkout(p){return /Race-pace|Specific long run|Race rehearsal|Marathon-specific|Half-marathon-specific|Threshold intervals|Threshold|VO₂max intervals|Fartlek|Long run/.test(String(p?.type||''))}
...
function raceShoePlan(){
 const life=freshShoeLifecyclePlan(),pair=life.racePair;if(!pair)return{status:'no-race',confidence:'Low'};
...
function plannedShoeForRunDraft(run){const planId=run.planId||suggestedPlanId(run),p=(state.plan||[]).find(x=>x.id===planId);if(!p)return null;return plannedAssignment(p.id)?.shoeId||shoeRecommendation(p).best?.shoe.id||null}
...
function planShoeSummaryHtml(plan){if(!plan||plan.type==='Rest'||!(state.shoes||[]).some(s=>s.status!=='retired'))return'';const assignment=plannedAssignment(plan.id),rec=shoeRecommendationForPlan(plan);if(!rec?.best)return'';const label=assignment?.source==='user'?'Selected shoe':rec.future?'Planned future shoe':rec.planner?'Shoe plan':'Proposed shoe';return`<div class="planShoeSummary"><span class="planShoeSummaryIcon">${todayPictogram('shoe')}</span><span><small>${esc(label)}</small><b>${esc(shoeDisplayName(rec.best.shoe))}</b></span></div>`}
function todayShoeRowHtml(plan){if(!plan||(state.shoes||[]).filter(s=>s.status!=='retired').length===0)return'';const rec=shoeRecommendationForPlan(plan);if(!rec.best)return'';return`<div class="todayShoeRow"><span>${todayPictogram('shoe')}</span><div><small>SHOE</small><b>${esc(shoeDisplayName(rec.best.shoe))}</b><p>Recommended for today’s ${esc(plan.type)}</p>${rec.alternative?`<small class="todayShoeAlternative">Alternative · ${esc(shoeDisplayName(rec.alternative.shoe))}</small>`:''}</div></div>`}
function planShoeRowHtml(plan){if(!plan||plan.type==='Rest'||(state.shoes||[]).filter(s=>s.status!=='retired').length===0)return'';const rec=shoeRecommendationForPlan(plan);if(!rec.best)return'';const reasons=(rec.best.reasons||[]).slice(0,4);return`<div class="planShoeRow"><span>${todayPictogram('shoe')}</span><div><small>SHOE</small><b>Recommended: ${esc(shoeDisplayName(rec.best.shoe))}</b>${rec.alternative?`<p>Alternative: ${esc(shoeDisplayName(rec.alternative.shoe))}</p>`:''}<small>${rec.best.label}${rec.manual?' · manually selected':''}</small><details class="planShoeWhy"><summary>Why this shoe?</summary><ul>${reasons.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>${rec.best.warning?`<p>${esc(rec.best.warning)}</p>`:''}</details></div><button type="button" class="secondary small" data-choose-plan-shoe="${esc(plan.id)}">Choose another shoe</button></div>`}
...
function lifecycleStrategyConfig(){
 return{key:'balanced',wearFraction:1.00,minFit:60,purchaseLeadDays:0,minPurchaseGapDays:42,recentUseDivisor:7,versatilityWeight:7,qualityBias:10,protectionBias:9,raceDedicatedBias:0,targetActivePairs:2}
}
function lifecycleRetireKmForProfile(profile,shoe=null){
 const cfg=lifecycleStrategyConfig(),low=Math.max(220,Number(shoe?.replacementRangeKm?.low)||Number(profile?.typicalReplacementLowKm)||600),high=Math.max(low+80,Number(shoe?.replacementRangeKm?.high)||Number(profile?.typicalReplacementHighKm)||850);
 const current=Math.max(0,Number(shoe?shoeMileage(shoe):0)||0),absoluteFloor=.80*high;
 let target=high*cfg.wearFraction;
 const condition=shoe?.condition||shoe?.conditionFeedback||'Feels normal';
 if(condition==='Slightly worn')target=Math.min(target,.92*high);
 if(condition==='Noticeably flat')target=Math.min(target,.85*high);
 if(condition==='Grip deteriorating')target=Math.min(target,.82*high);
 if(condition==='Upper damaged')target=Math.min(target,.80*high);
 if(condition==='Causing discomfort')target=Math.min(target,.80*high);
 target=Math.max(absoluteFloor,target);
 return Math.max(Math.round(current),Math.round(target))
}
...
function lifecycleWorkoutFit(profile,plan){
 if(!profile||!plan)return 0;
 const req=SHOE_REQ[plan.type]||SHOE_REQ.Easy,roles=lifecycleRoles(profile),quality=lifecycleIsQuality(plan.type),easy=lifecycleIsEasy(plan.type),cfg=lifecycleStrategyConfig();
 let weighted=0,total=0;
 const add=(v,w)=>{weighted+=clamp(Number(v)||0,0,100)*w;total+=w};
 add(profile.workoutSuitability?.[plan.type]??shoeProfileWorkoutSuitability(profile,plan.type),4.2);
 add(shoeDistanceScore(profile,Number(plan.distance)||0),1.5);
 add(shoeSurfaceScore(profile,plan),1.6);
 const metric=(key,val)=>{if(req[key])add((Number(val)||0)/5*100,Number(req[key])/6)};
 ['cushioning','protection','responsiveness','efficiency','grip','stability','durability','comfort'].forEach(k=>metric(k,profile[k]));
 let score=total?weighted/total:50;
 const versatility=Math.min(7,roles.size);
 score+=(versatility-3)*cfg.versatilityWeight;
 score+=footMechanicsCompatibility(profile,plan.type,'assignment');
 if(cfg.key==='balanced'){
  if(quality&&(roles.has('tempo')||roles.has('threshold')||roles.has('intervals')||roles.has('race')))score+=cfg.qualityBias;
  if(easy&&(roles.has('daily')||roles.has('easy')||roles.has('recovery')))score+=8;
  if(easy&&(profile.plated||roles.has('race')))score-=18;
 }
 if(cfg.key==='quality'){
  if(quality)score+=(roles.has('threshold')||roles.has('intervals')||roles.has('tempo')||roles.has('race'))?cfg.qualityBias:-20;
  if(easy)score+=(roles.has('daily')||roles.has('easy')||roles.has('recovery'))?16:-22;
  if((profile.plated||roles.has('race'))&&easy)score-=28;
 }
 if(cfg.key==='protection'){
  score+=((profile.cushioning||3)+(profile.protection||3)+(profile.stability||3)+(profile.comfort||3)-12)*cfg.protectionBias/4;
  score+=lifecycleInjuryFootwearAdjustment(profile);
  if(profile.plated||roles.has('race'))score-=quality?8:18;
  if(Number(profile.cushioning||0)<4||Number(profile.protection||0)<4)score-=18;
 }
 return clamp(Math.round(score),0,100)
}
function lifecyclePurchaseScore(profile,plan,role='training'){
 const cfg=lifecycleStrategyConfig(),roles=lifecycleRoles(profile),d=Number(plan?.distance)||Number(state.setup?.raceDistance)||10;
 let score=lifecycleWorkoutFit(profile,plan)+gaitPurchaseAdjustment(profile);
 if(cfg.key==='quality'&&role==='quality')score+=(roles.has('threshold')||roles.has('intervals')||roles.has('tempo'))?30:0;
 if(role==='race')score+=roles.has('race')?28:-35;
 if(cfg.key==='protection')score+=((profile.cushioning||3)+(profile.protection||3)+(profile.stability||3))*4;
 if(cfg.key==='balanced'&&roles.has('daily')&&roles.has('long'))score+=12;
 if(d>Number(profile.preferredDistanceMaxKm||999))score-=35;
 if(d<Number(profile.preferredDistanceMinKm||0))score-=18;
 if(normalizeShoeFamily(profile.family)==='METASPEED RAY'&&d>21.1)score-=25;
 return score
}
...
function lifecycleAcceptableFit(plan){
 const key=lifecycleStrategyConfig().key,quality=lifecycleIsQuality(plan?.type);
 if(key==='quality')return quality?54:44;
 if(key==='protection')return 44;
 return 46
}
...
function lifecycleActiveNonRacePairsForWeek(pairs,week){
 /* v13.7.64: an "active" rotation pair is a physical pair allocated running in this
    programme week. Merely owning a serviceable shoe does not make it an active
    weekly pair; this prevents dormant/retired shoes being counted against the 25% rule. */
 return pairs.filter(pair=>pair.role!=='race'&&pair.assignments.some(a=>!a.rehab&&lifecycleWeekKey(a.date)===week&&Number(a.km)>0));
}
...
function lifecycleEnforceMinWeeklyShare(ctx,fixed,assignments){
 ...
 const weeks=[...new Set((assignments||[]).filter(a=>!a.rehab).map(a=>lifecycleWeekKey(a.date)))].sort();
 for(let outer=0;outer<12;outer++){
  ...
  for(const week of weeks){
   ...
   for(const pair of active){
    ...
    const candidates=(assignments||[]).filter(row=>!row.rehab&&lifecycleWeekKey(row.date)===week&&row.pairId!==pair.id)
      .map(row=>{
       const plan=(fixed||[]).find(p=>p.id===row.planId)||{id:row.planId,date:row.date,type:row.type,distance:row.km,surface:'road'};
...
function lifecycleCompactRotation(ctx,fixed){
 ...
 if(!['balanced','quality','protection'].includes(cfg.key))return;
 const future=pairs.filter(p=>!p.owned&&p.role!=='race').slice().sort((a,b)=>(a.availableDate||'').localeCompare(b.availableDate||''));
 for(const target of future){
  const rows=target.assignments.slice().sort((a,b)=>a.date.localeCompare(b.date));if(!rows.length)continue;
  const virtual=new Map(pairs.map(p=>[p.id,p.km-(p.id===target.id?rows.reduce((n,r)=>n+r.km,0):0)]));
  const choices=[];let feasible=true;
  for(const row of rows){
   const plan=(fixed||[]).find(p=>p.id===row.planId)||{id:row.planId,date:row.date,type:row.type,distance:row.km,surface:'road'};
   const best=pairs.filter(p=>p.id!==target.id&&p.availableDate<=row.date&&(virtual.get(p.id)||0)+row.km<=p.retireKm+1)
    .map(p=>({p,fit:lifecycleWorkoutFit(p.profile,plan),score:lifecyclePairAssignmentScore(p,plan).score+(p.owned?20:0)}))
    .filter(x=>x.fit>=lifecycleAcceptableFit(plan)).sort((a,b)=>b.score-a.score)[0]?.p;
   if(!best){feasible=false;break}
   choices.push([row,best]);virtual.set(best.id,(virtual.get(best.id)||0)+row.km)
  }
  if(!feasible)continue;
  choices.forEach(([row,best])=>lifecycleReassign(row,best,pairs));
  if(!target.assignments.length){
   const pi=purchases.findIndex(p=>p.pairId===target.id);if(pi>=0)purchases.splice(pi,1);
   events.push({date:target.availableDate,type:'purchase-avoided',pairId:target.id,text:`${lifecyclePairLabel(target)} is not purchased because physical pairs already in the rotation have enough acceptable remaining capacity.`})
  }
 }
}
function lifecycleRecentKm(pair,date,days=7){
 return pair.assignments.filter(a=>{const diff=(dte(date)-dte(a.date))/DAY;return diff>=0&&diff<days}).reduce((n,a)=>n+(Number(a.km)||0),0)
}
function lifecyclePairAssignmentScore(pair,plan){
 const cfg=lifecycleStrategyConfig(),fit=lifecycleWorkoutFit(pair.profile,plan),remaining=Math.max(0,pair.retireKm-pair.km),wearRatio=pair.retireKm>0?pair.km/pair.retireKm:0,roles=lifecycleRoles(pair.profile),easy=lifecycleIsEasy(plan.type);
 let score=fit*2.2-Math.min(12,lifecycleRecentKm(pair,plan.date)/Math.max(1,cfg.recentUseDivisor));
 if(cfg.key==='balanced'){score+=wearRatio>=.70?(wearRatio-.70)*70:-wearRatio*3;if(easy&&(pair.profile.plated||roles.has('race')))score-=18}
 if(remaining<(Number(plan.distance)||0))score-=1000;
...
function lifecycleReferencePairForNeed(pairs,plan,role='training'){
 const eligible=pairs.filter(p=>p.availableDate<=plan.date&&(role!=='race'||lifecycleRoles(p.profile).has('race')));
 if(!eligible.length)return null;
 return eligible.map(p=>({p,fit:lifecycleWorkoutFit(p.profile,plan),roles:lifecycleRoles(p.profile)})).sort((a,b)=>{
  const ar=role==='quality'&&(a.roles.has('tempo')||a.roles.has('threshold')||a.roles.has('intervals'))?10:0;
  const br=role==='quality'&&(b.roles.has('tempo')||b.roles.has('threshold')||b.roles.has('intervals'))?10:0;
  return (b.fit+br)-(a.fit+ar)
 })[0]?.p||null
}
function lifecycleContextualPurchaseProfile(plan,role='training',referencePair=null,avoidKeys=new Set()){
 const best=lifecycleChoosePurchaseProfile(plan,role,avoidKeys);
 if(!referencePair)return best;
 const family=normalizeShoeFamily(referencePair.profile?.family||referencePair.shoe?.model||'');
 const same=candidatePurchaseProfiles().filter(p=>normalizeShoeFamily(p.family)===family)
  .filter(p=>role!=='race'||lifecycleRoles(p).has('race'))
  .map(profile=>({profile,score:lifecyclePurchaseScore(profile,plan,role)})).sort((a,b)=>b.score-a.score)[0];
 if(!same)return best;
 const bestScore=best?lifecyclePurchaseScore(best,plan,role):-Infinity;
 const margin=lifecycleStrategyConfig().key==='quality'?8:12;
 return bestScore-same.score>=margin?best:same.profile
}
function lifecycleContextualRaceProfile(racePlan,pairs){
 const candidates=candidatePurchaseProfiles().filter(p=>lifecycleRoles(p).has('race')).map(profile=>({profile,score:lifecyclePurchaseScore(profile,racePlan,'race')})).sort((a,b)=>b.score-a.score);
 const best=candidates[0];if(!best)return null;
 const ownedFamilies=new Set(pairs.filter(p=>p.owned).map(p=>normalizeShoeFamily(p.profile?.family||p.shoe?.model||'')));
 const familiar=candidates.filter(x=>ownedFamilies.has(normalizeShoeFamily(x.profile.family))).sort((a,b)=>b.score-a.score)[0];
 return familiar&&best.score-familiar.score<12?familiar.profile:best.profile
}
function lifecycleCreatePair(ctx,plan,role='training',preferredProfile=null){
 ...
 if(!isRace){
  ...
  if(activeNow.length>=cfg.targetActivePairs){
   const fallback=activeNow.map(p=>({p,...lifecyclePairAssignmentScore(p,plan)})).filter(x=>x.p.km+distance<=x.p.retireKm+1&&x.fit>=lifecycleAcceptableFit(plan)).sort((a,b)=>b.score-a.score)[0]?.p;
 ...
 const reusable=pairs.filter(p=>!p.owned&&p.availableDate<=plan.date&&p.km+distance<=p.retireKm)
  ...
  .map(p=>({p,...lifecyclePairAssignmentScore(p,plan)})).filter(x=>x.fit>=Math.max(28,lifecycleAcceptableFit(plan)-4)).sort((a,b)=>b.score-a.score)[0]?.p;
 ...
 const referencePair=lifecycleReferencePairForNeed(pairs,plan,role);
 const profile=preferredProfile||lifecycleContextualPurchaseProfile(plan,role,referencePair,avoid);if(!profile)return null;
 const recentReusable=recent.map(p=>pairs.find(x=>x.id===p.pairId)).filter(Boolean)
  .filter(p=>!isRace||lifecycleRoles(p.profile).has('race'))
  .map(p=>({p,...lifecyclePairAssignmentScore(p,plan)})).filter(x=>x.p.km+distance<=x.p.retireKm&&x.fit>=Math.max(26,lifecycleAcceptableFit(plan)-6)).sort((a,b)=>b.score-a.score)[0]?.p;
...
function lifecycleAssign(pair,plan,assignments,why,extra={}){
 ...
 pair.km+=km;
 const row={planId:plan.id,date:plan.date,type:plan.type,km,pairId:pair.id,ownedShoeId:pair.owned?pair.shoe.id:null,why,fit:lifecycleWorkoutFit(pair.profile,plan),...extra};
 pair.assignments.push(row);assignments.push(row);return row
}
function lifecycleReassign(row,newPair,pairs){
 const old=pairs.find(p=>p.id===row.pairId);if(old){old.assignments=old.assignments.filter(a=>a!==row);old.km=Math.max(old.currentKm,old.km-row.km)}
 row.pairId=newPair.id;row.ownedShoeId=newPair.owned?newPair.shoe.id:null;newPair.assignments.push(row);newPair.km+=row.km;
}
function lifecycleRaceTargetWindow(profile,raceDistance){
 ...
 if(plated)return{minKm:Math.max(20,Math.min(40,d*.75)),maxKm:Math.min(250,Math.max(70,d*2.3))};
 if(race)return{minKm:Math.max(30,Math.min(60,d)),maxKm:Math.min(250,Math.max(120,d*4))};
 return{minKm:Math.max(40,Math.min(80,d*1.25)),maxKm:250}
}
function lifecycleRaceRelevant(plan){return /Race-pace|Specific|rehearsal|Marathon-specific|Half-marathon-specific|Threshold|interval|VO₂|Fartlek|Long run/i.test(String(plan?.type||''))}
function lifecycleRaceFit(pair,racePlan){
 const fit=lifecycleWorkoutFit(pair.profile,racePlan),window=lifecycleRaceTargetWindow(pair.profile,racePlan.distance),preRaceKm=pair.km;
...
function lifecycleCreateDirectSuccessor(ctx,outgoing,firstPlan){
 const {pairs,purchases,events}=ctx,cfg=lifecycleStrategyConfig();
 if(!outgoing||!firstPlan)return null;
 const ref=outgoing.profile,role=cfg.key==='quality'&&lifecycleIsQuality(firstPlan.type)?'quality':'training';
 const profile=lifecycleContextualPurchaseProfile(firstPlan,role,outgoing,new Set())||outgoing.profile||replacementProfileForShoe(outgoing.shoe||{})||lifecycleChoosePurchaseProfile(firstPlan,role);
 if(!profile)return null;
 const id=`future:${purchases.length+1}:${lifecycleProfileKey(profile)}:successor`,date=firstPlan.date;
 const pair={id,owned:false,shoe:null,profile,currentKm:0,km:0,availableDate:date,purchaseDate:date,retireKm:lifecycleRetireKmForProfile(profile),assignments:[],points:[{date,km:0,purchase:true}],role};
 pairs.push(pair);
 purchases.push({id:`purchase:${id}`,pairId:id,profile,purchaseDate:date,firstUseDate:date,role,reason:`${lifecyclePairLabel(outgoing)} is reaching its planned lifecycle limit. This successor enters during a short handover so the target two-pair rotation is maintained without leaving a one-pair gap.`,replacesPairId:outgoing.id});
 events.push({date,type:'pair-entry',pairId:id,text:`${futureProfileDisplayName(profile)} enters the rotation as the successor to ${lifecyclePairLabel(outgoing)}.`});
 return pair
}
function lifecycleEnsureTwoPairContinuity(ctx,fixed,assignments,manual){
 const {pairs,purchases,events}=ctx,cfg=lifecycleStrategyConfig();if(cfg.targetActivePairs!==2)return;
 const running=fixed.filter(p=>p.type!=='Rest').slice().sort((a,b)=>a.date.localeCompare(b.date));if(!running.length)return;
 const weeks=[...new Set(running.map(p=>lifecycleWeekKey(p.date)))].sort();
 for(let guard=0;guard<10;guard++){
  let changed=false;
  for(const week of weeks){
   const weekRuns=running.filter(p=>lifecycleWeekKey(p.date)===week);if(!weekRuns.length)continue;
   let active=lifecycleActiveNonRacePairsForWeek(pairs,week);
   if(active.length>=2)continue;
   const firstRun=weekRuns.find(p=>!manual.has(p.id))||weekRuns[0],weekStart=dte(week),overlapStart=iso(new Date(weekStart.getTime()-7*DAY));
   const transition=running.filter(p=>p.date>=overlapStart&&p.date<=firstRun.date&&!manual.has(p.id)).slice().sort((a,b)=>a.date.localeCompare(b.date))[0]||firstRun;
   // Prefer bringing forward an already-proposed future training pair rather than inventing another purchase.
   let incoming=pairs.filter(p=>!p.owned&&p.role!=='race'&&p.availableDate>transition.date).sort((a,b)=>a.availableDate.localeCompare(b.availableDate))[0]||null;
   const recentlyOutgoing=pairs.filter(p=>p.role!=='race'&&p.id!==incoming?.id).map(p=>{
    const last=p.assignments.filter(a=>!a.rehab&&a.date<week).slice().sort((a,b)=>a.date.localeCompare(b.date)).at(-1);
    return{p,last,wear:p.retireKm?lifecyclePairKmBeforeDate(p,week)/p.retireKm:0}
   }).filter(x=>x.last).sort((a,b)=>b.wear-a.wear||b.last.date.localeCompare(a.last.date))[0]?.p||active[0]||pairs.find(p=>p.role!=='race');
   if(!incoming)incoming=lifecycleCreateDirectSuccessor(ctx,recentlyOutgoing,transition);
   if(!incoming)continue;
   // Advance purchase/first use into the short handover if this pair had been proposed too late.
   incoming.availableDate=transition.date;incoming.purchaseDate=transition.date;
   const purchase=purchases.find(p=>p.pairId===incoming.id);if(purchase){purchase.purchaseDate=transition.date;purchase.firstUseDate=transition.date;if(!purchase.replacesPairId&&recentlyOutgoing)purchase.replacesPairId=recentlyOutgoing.id}
   let row=assignments.find(a=>a.planId===transition.id&&!a.rehab);
   if(row&&row.pairId!==incoming.id&&lifecyclePairKmBeforeDate(incoming,row.date)+row.km<=incoming.retireKm+1){
    lifecycleReassign(row,incoming,pairs);row.why=`Short handover into ${lifecyclePairLabel(incoming)} before the rotation would otherwise fall to one training pair.`;
   }else{
    const candidate=assignments.filter(a=>!a.rehab&&a.date>=transition.date&&lifecycleWeekKey(a.date)===lifecycleWeekKey(transition.date)&&a.pairId!==incoming.id).find(a=>lifecyclePairKmBeforeDate(incoming,a.date)+a.km<=incoming.retireKm+1);
    if(candidate){lifecycleReassign(candidate,incoming,pairs);candidate.why=`Short handover into ${lifecyclePairLabel(incoming)} before the rotation would otherwise fall to one training pair.`}
   }
   events.push({date:transition.date,type:'pair-entry',pairId:incoming.id,text:`${lifecyclePairLabel(incoming)} enters during a short handover because the projected rotation would otherwise fall to only one active training pair. Two active pairs is the target; three is acceptable during transition.`});
   if(recentlyOutgoing)events.push({date:transition.date,type:'pair-handover',pairId:recentlyOutgoing.id,replacementPairId:incoming.id,text:`${lifecyclePairLabel(incoming)} enters while ${lifecyclePairLabel(recentlyOutgoing)} is still being used. This is a handover, not retirement; the outgoing pair remains active until its actual final planned use.`});
   changed=true;break
  }
  if(!changed)break
 }
}
function lifecycleFuturePairUsageSummary(pair){
 const rows=pair.assignments.slice().sort((a,b)=>a.date.localeCompare(b.date));if(!rows.length)return{rows,first:null,last:null,maxGapDays:Infinity,first28Km:0,first28Count:0};
 const first=rows[0].date,end28=new Date(dte(first).getTime()+28*DAY),first28=rows.filter(a=>dte(a.date)<=end28);
 let maxGapDays=0;for(let i=1;i<rows.length;i++)maxGapDays=Math.max(maxGapDays,(dte(rows[i].date)-dte(rows[i-1].date))/DAY);
 return{rows,first,last:rows.at(-1).date,maxGapDays,first28Km:first28.reduce((n,a)=>n+a.km,0),first28Count:first28.length}
}
function lifecycleBuildDedicatedRacePair(ctx,racePlan,fixed,assignments,manual){
 const {pairs,purchases,events}=ctx,profile=lifecycleContextualRaceProfile(racePlan,pairs);if(!profile)return null;
 const window=lifecycleRaceTargetWindow(profile,racePlan.distance),target=lifecycleRaceTargetKm(window),raceMs=dte(racePlan.date).getTime();
 for(const horizon of [70,84,112]){
  const start=iso(new Date(raceMs-horizon*DAY));
  const candidates=fixed.filter(p=>p.date>=start&&p.date<racePlan.date&&!manual.has(p.id)&&lifecycleWorkoutFit(profile,p)>=58).filter(plan=>{const row=assignments.find(a=>a.planId===plan.id&&!a.rehab);if(!row)return false;const wk=lifecycleWeekKey(plan.date),weekTotal=assignments.filter(a=>!a.rehab&&lifecycleWeekKey(a.date)===wk).reduce((n,a)=>n+(Number(a.km)||0),0),donor=pairs.find(p=>p.id===row.pairId);if(!donor||donor.role==='race')return false;const donorKm=donor.assignments.filter(a=>!a.rehab&&lifecycleWeekKey(a.date)===wk).reduce((n,a)=>n+(Number(a.km)||0),0);return donorKm-row.km>=weekTotal*.25-1e-6}).sort((a,b)=>b.date.localeCompare(a.date));
  let selected=[],km=0,selectedWeeks=new Set();
  const weekTotals=new Map();fixed.filter(p=>p.date>=start&&p.date<racePlan.date).forEach(p=>weekTotals.set(lifecycleWeekKey(p.date),(weekTotals.get(lifecycleWeekKey(p.date))||0)+(Number(p.distance)||0)));
  for(const plan of candidates){
   const d=Math.max(0,Number(plan.distance)||0),wk=lifecycleWeekKey(plan.date);if(!d||km+d>Math.min(250,window.maxKm)||selectedWeeks.has(wk))continue;
   if(d>(weekTotals.get(wk)||d)*.45)continue; // Race specialist never consumes the majority of a training week.
   selected.push(plan);selectedWeeks.add(wk);km+=d;if(km>=target)break
  }
  if(km<window.minKm)continue;
  selected=selected.sort((a,b)=>a.date.localeCompare(b.date));
  const firstUse=selected[0].date,id=`future:${purchases.length+1}:${lifecycleProfileKey(profile)}:race`;
  const racePair={id,owned:false,shoe:null,profile,currentKm:0,km:0,availableDate:firstUse,purchaseDate:firstUse,retireKm:lifecycleRetireKmForProfile(profile),assignments:[],points:[{date:firstUse,km:0,purchase:true}],role:'race'};
  pairs.push(racePair);
  purchases.push({id:`purchase:${id}`,pairId:id,profile,purchaseDate:firstUse,firstUseDate:firstUse,role:'race',reason:'No owned or already-required race-suitable physical pair can finish inside its target Race Day mileage window, so a dedicated pair is introduced when its real familiarisation block starts.',replacesPairId:null});
  const originalPairByPlan=new Map();
  for(const plan of selected){
   const row=assignments.find(a=>a.planId===plan.id);if(!row||row.pairId===racePair.id)continue;originalPairByPlan.set(row.planId,row.pairId);
   if(racePair.km+row.km>Math.min(250,window.maxKm))continue;
   const wk=lifecycleWeekKey(row.date),donor=pairs.find(p=>p.id===row.pairId),weekRows=donor?donor.assignments.filter(a=>!a.rehab&&lifecycleWeekKey(a.date)===wk):[],weekTotal=assignments.filter(a=>!a.rehab&&lifecycleWeekKey(a.date)===wk).reduce((n,a)=>n+(Number(a.km)||0),0),donorAfter=weekRows.reduce((n,a)=>n+(Number(a.km)||0),0)-row.km;
   const nonRaceWeek=pairs.filter(p=>p.role!=='race').map(p=>({p,km:p.assignments.filter(a=>!a.rehab&&lifecycleWeekKey(a.date)===wk).reduce((n,a)=>n+(Number(a.km)||0),0)-(p.id===donor?.id?row.km:0)})).filter(x=>x.km>0);
   if(!donor||weekRows.length<2||donorAfter<weekTotal*.25-1e-6||nonRaceWeek.length<2||nonRaceWeek.some(x=>x.km<weekTotal*.25-1e-6))continue; // never destroy the two-pair/25% training rotation to familiarise race shoes
   lifecycleReassign(row,racePair,pairs);row.why='This existing workout is deliberately assigned to the Race Day pair so it reaches the target familiarisation window without breaking the two-pair training rotation; no extra training is created.';row.raceFamiliarisation=true;
  }
  const summary=lifecycleFuturePairUsageSummary(racePair);
  if(racePair.km<window.minKm||summary.first28Count<2&&summary.first28Km<15){
   racePair.assignments.slice().forEach(row=>{
    const original=pairs.find(p=>p.id===originalPairByPlan.get(row.planId));
    if(original)lifecycleReassign(row,original,pairs)
   });
   pairs.splice(pairs.indexOf(racePair),1);
   const pi=purchases.findIndex(p=>p.pairId===id);if(pi>=0)purchases.splice(pi,1);
   continue
  }
  events.push({date:firstUse,type:'pair-entry',pairId:id,text:`${futureProfileDisplayName(profile)} is bought when its Race Day familiarisation block actually starts.`});
  return lifecycleRaceFit(racePair,racePlan)
 }
 return null
}

function lifecycleTrimPostRetirementAssignments(ctx,fixed,assignments,manual){
 const {pairs,events}=ctx;
 for(let guard=0;guard<8;guard++){
  let changed=false;
  for(const pair of pairs.filter(p=>p.role!=='race')){
   let km=pair.owned?pair.currentKm:0,retiredAt=null;
   const rows=pair.assignments.slice().sort((a,b)=>a.date.localeCompare(b.date));
   for(const row of rows){
    const add=Math.max(0,Number(row.km)||0);if(add<=0)continue;
    if(retiredAt||km+add>pair.retireKm+1e-6){
     const plan=(fixed||[]).find(p=>p.id===row.planId);
     const alternatives=pairs.filter(p=>p.id!==pair.id&&p.role!=='race'&&p.availableDate<=row.date&&lifecyclePairKmBeforeDate(p,row.date)+add<=p.retireKm+1e-6)
       .map(p=>({p,score:plan?lifecyclePairAssignmentScore(p,plan).score:50,fit:plan?lifecycleWorkoutFit(p.profile,plan):50}))
       .filter(x=>!plan||x.fit>=Math.max(24,lifecycleAcceptableFit(plan)-8)).sort((a,b)=>b.score-a.score);
     let alt=alternatives[0]?.p||null;
     if(!alt&&plan&&!manual.has(plan.id))alt=lifecycleCreateDirectSuccessor(ctx,pair,plan);
     if(alt){
      lifecycleReassign(row,alt,pairs);
      row.why=`${lifecyclePairLabel(pair)} has reached its planned retirement boundary; this session moves to ${lifecyclePairLabel(alt)}.`;
      changed=true
     }
     continue
    }
    km+=add;
    if(km>=pair.retireKm-1e-6)retiredAt=row.date
   }
   if(retiredAt)events.push({date:retiredAt,type:'pair-retirement',pairId:pair.id,text:`${lifecyclePairLabel(pair)} reaches its planned retirement boundary here. It is not plotted or assigned beyond this date.`})
  }
  if(!changed)break
 }
}
...
function lifecycleMeaningfulWeeklyFloor(totalKm){
 // A shoe is not considered meaningfully active because of a token recovery jog.
 // Floor is 20% of weekly running, with an absolute 8 km minimum when volume permits.
 return Math.min(totalKm,Math.max(8,.20*Math.max(0,totalKm)));
}
function lifecycleConsolidateTokenUse(ctx,fixed,assignments,manual,racePair){
 const {pairs,events}=ctx,weeks=[...new Set(assignments.filter(a=>!a.rehab).map(a=>lifecycleWeekKey(a.date)))].sort();
 for(const week of weeks){
  const rows=assignments.filter(a=>!a.rehab&&lifecycleWeekKey(a.date)===week),total=rows.reduce((n,a)=>n+(Number(a.km)||0),0);
  if(total<=0)continue;
  const floor=lifecycleMeaningfulWeeklyFloor(total);
  const active=pairs.filter(p=>p.role!=='race'&&p!==racePair&&p.availableDate<=lifecycleWeekEnd(week));
  for(const pair of active){
   const mine=rows.filter(a=>a.pairId===pair.id),km=mine.reduce((n,a)=>n+(Number(a.km)||0),0);
   if(km<=0||km+1e-6>=floor)continue;
   // Do not manufacture mileage merely to keep a pair "active". Move all token use if feasible.
   const moves=[];
   for(const row of mine){
    if(manual?.has(row.planId)){moves.length=0;break}
    const plan=fixed.find(p=>p.id===row.planId);if(!plan){moves.length=0;break}
    const alt=pairs.filter(p=>p.id!==pair.id&&p.role!=='race'&&p.availableDate<=row.date&&lifecyclePairKmBeforeDate(p,row.date)+row.km<=p.retireKm+1)
      .map(p=>({p,fit:lifecycleWorkoutFit(p.profile,plan),score:lifecyclePairAssignmentScore(p,plan).score}))
      .filter(x=>x.fit>=Math.max(24,lifecycleAcceptableFit(plan)-8)).sort((a,b)=>b.score-a.score)[0]?.p;
    if(!alt){moves.length=0;break}
    moves.push([row,alt])
   }
   if(moves.length===mine.length&&moves.length){
    moves.forEach(([row,alt])=>lifecycleReassign(row,alt,pairs));
    events.push({date:week,type:'token-use-consolidated',pairId:pair.id,text:`${lifecyclePairLabel(pair)} is not kept artificially active on only ${km.toFixed(1)} km this week. Its compatible mileage is consolidated into the main rotation.`})
   }
  }
 }
}
function lifecycleFuturePairHasMaterialSpecialistRole(pair,fixed,pairs){
 const rows=(pair.assignments||[]).filter(a=>!a.rehab);
 if(rows.length<2)return false;
 let material=0,qualityKm=0;
 for(const row of rows){
  const plan=(fixed||[]).find(p=>p.id===row.planId);if(!plan||!lifecycleIsQuality(plan.type))continue;
  const own=lifecycleWorkoutFit(pair.profile,plan);
  const bestAlt=pairs.filter(p=>p.id!==pair.id&&p.role!=='race'&&p.availableDate<=row.date&&lifecyclePairKmBeforeDate(p,row.date)+row.km<=p.retireKm+1)
    .map(p=>lifecycleWorkoutFit(p.profile,plan)).sort((a,b)=>b-a)[0]??-Infinity;
  if(own-bestAlt>=8){material++;qualityKm+=Number(row.km)||0}
 }
 return material>=2&&qualityKm>=30
}
function lifecyclePruneLateRedundantPurchases(ctx,fixed,assignments,manual,racePair,raceDate){
 const {pairs,purchases,events}=ctx,cfg=lifecycleStrategyConfig(),raceMs=dte(raceDate).getTime();
 // Work latest-first so an unnecessary late overlap is removed before earlier lifecycle decisions.
 const candidates=pairs.filter(p=>!p.owned&&p.role!=='race'&&p!==racePair).slice().sort((a,b)=>(b.availableDate||'').localeCompare(a.availableDate||''));
 for(const pair of candidates){
  const first=(pair.assignments||[]).filter(a=>Number(a.km)>0).slice().sort((a,b)=>a.date.localeCompare(b.date))[0];if(!first)continue;
  const daysToRace=(raceMs-dte(first.date).getTime())/DAY;
  const plannedKm=(pair.assignments||[]).reduce((n,a)=>n+(Number(a.km)||0),0);
  const fullLife=Math.max(1,Number(pair.lifeKm)||Number(pair.retireKm)||1);
  const late=daysToRace>=0&&daysToRace<=70,lowUse=plannedKm<Math.max(60,.18*fullLife);
  if(!late&&!lowUse)continue;
  if((pair.assignments||[]).some(a=>a.rehab))continue;
  const before=pairs.length;
  const removed=lifecycleRemoveFuturePair(ctx,pair,fixed,assignments,`Purchase removed: ${lifecyclePairLabel(pair)} entered too late or accumulated too little meaningful programme mileage, and every assigned workout can be covered by the remaining rotation.`);
  if(removed&&pairs.length<before)events.push({date:first.date,type:'purchase-consolidated',pairId:pair.id,text:`No purchase is made for ${lifecyclePairLabel(pair)}. The remaining physical rotation covers the same programme demand without creating a low-use pre-race shoe.`})
 }
}
...
function lifecycleRebuildPoints(pair,now,raceDate){
 ...
 // Retirement is reserved for the true strategy lifecycle boundary.
 pair.projectedRetireDate=(eligibleByMileage&&reachesStrategyBoundary)?lastPositiveDate:null;
...
function lifecycleNormalizeEntryEvents(pairs,purchases,events){
 // Purchase/first-use timing is authoritative. No pair may "enter" before it is bought.
 const pairById=new Map(pairs.map(p=>[p.id,p])),purchaseByPair=new Map(purchases.map(p=>[p.pairId,p]));
 const keep=events.filter(e=>e.type!=='pair-entry'&&e.type!=='pair-handover');
 purchases.forEach(p=>{
  const pair=pairById.get(p.pairId);if(!pair)return;
  const date=p.purchaseDate||p.firstUseDate||pair.purchaseDate||pair.availableDate;
  keep.push({date,type:'pair-entry',pairId:pair.id,text:`${lifecyclePairLabel(pair)} is purchased and enters the rotation on its first planned-use date. It cannot receive mileage before this date.`});
  if(p.replacesPairId){
   const outgoing=pairById.get(p.replacesPairId);
   if(outgoing)keep.push({date,type:'pair-handover',pairId:outgoing.id,replacementPairId:pair.id,text:`${lifecyclePairLabel(pair)} enters on ${fmtDate(date)} while ${lifecyclePairLabel(outgoing)} is still in use. This is a handover, not retirement.`})
  }
 });
...
function lifecycleEnforceRetirementSpacing(ctx,fixed,assignments,manual,raceDate,now){
 const {pairs,purchases,events}=ctx,MIN_GAP=56;
 for(let guard=0;guard<12;guard++){
  pairs.forEach(p=>lifecycleRebuildPoints(p,now,raceDate));
  const retiring=pairs.filter(p=>p.role!=='race'&&p.projectedRetireDate).sort((a,b)=>a.projectedRetireDate.localeCompare(b.projectedRetireDate));
  let conflict=null;
  for(let i=1;i<retiring.length;i++){const gap=(dte(retiring[i].projectedRetireDate)-dte(retiring[i-1].projectedRetireDate))/DAY;if(gap<MIN_GAP){conflict={first:retiring[i-1],later:retiring[i]};break}}
  if(!conflict)break;
  const {first,later}=conflict,protectedUntil=iso(new Date(dte(first.projectedRetireDate).getTime()+MIN_GAP*DAY));
  const crossing=(later.assignments||[]).filter(a=>!a.rehab&&a.date>=first.projectedRetireDate&&a.date<protectedUntil).slice().sort((a,b)=>b.date.localeCompare(a.date))[0]
   ||(later.assignments||[]).filter(a=>!a.rehab).slice().sort((a,b)=>b.date.localeCompare(a.date))[0];
  if(!crossing)break;
  const plan=fixed.find(p=>p.id===crossing.planId)||{id:crossing.planId,date:crossing.date,type:crossing.type,distance:crossing.km,surface:'road'};
  let alt=pairs.filter(p=>p.id!==later.id&&p.role!=='race'&&p.availableDate<=crossing.date&&lifecyclePairKmBeforeDate(p,crossing.date)+crossing.km<=p.retireKm+1e-6)
   .map(p=>({p,fit:lifecycleWorkoutFit(p.profile,plan),score:lifecyclePairAssignmentScore(p,plan).score}))
   .filter(x=>x.fit>=Math.max(30,lifecycleAcceptableFit(plan)-6)).sort((a,b)=>b.score-a.score)[0]?.p||null;
  if(!alt){alt=lifecycleCreateDirectSuccessor(ctx,later,plan);if(alt){alt.availableDate=crossing.date;alt.purchaseDate=crossing.date;const pur=purchases.find(p=>p.pairId===alt.id);if(pur){pur.purchaseDate=crossing.date;pur.firstUseDate=crossing.date}}}
  if(!alt)break;
  lifecycleReassign(crossing,alt,pairs);
  crossing.why=`Retirement staggering: ${lifecyclePairLabel(later)} is preserved so two non-race physical pairs are not retired within the same eight-week window; ${lifecyclePairLabel(alt)} is the best compatible shoe for this session.`;
  events.push({date:crossing.date,type:'retirement-stagger',pairId:later.id,replacementPairId:alt.id,text:`Mileage moves from ${lifecyclePairLabel(later)} to ${lifecyclePairLabel(alt)} to keep planned non-race retirements at least eight weeks apart while retaining the best compatible session match.`});
 }
}
function lifecycleNormalizeRetirementEvents(pairs,events){
...
function lifecycleWeekShareFeasible(result,week,total,activePairs){
 const rows=result.assignments.filter(a=>!a.rehab&&lifecycleWeekKey(a.date)===week&&result.pairs.find(p=>p.id===a.pairId)?.role!=='race').slice().sort((a,b)=>a.date.localeCompare(b.date)),target=.25*total;if(rows.length<2)return false;
 const candidates=result.pairs.filter(p=>p.role!=='race'&&p.availableDate<=lifecycleWeekEnd(week)&&lifecyclePairKmBeforeDate(p,week)<p.retireKm-1).slice(0,3);if(candidates.length<2)return false;
 const subsets=[];for(let i=0;i<candidates.length;i++)for(let j=i+1;j<candidates.length;j++)subsets.push([candidates[i],candidates[j]]);if(candidates.length===3)subsets.push(candidates);
 for(const sub of subsets){const sums=new Array(sub.length).fill(0),base=sub.map(p=>lifecyclePairKmBeforeDate(p,week));let ok=false;function rec(i){if(ok)return;if(i===rows.length){if(sums.every(x=>x>=target-1e-6))ok=true;return}const row=rows[i],plan=result.fixed.find(p=>p.id===row.planId)||{type:row.type,distance:row.km,surface:'road'};for(let j=0;j<sub.length;j++){if(base[j]+sums[j]+row.km>sub[j].retireKm+1e-6)continue;if(lifecycleWorkoutFit(sub[j].profile,plan)<Math.max(24,lifecycleAcceptableFit(plan)-10))continue;sums[j]+=row.km;rec(i+1);sums[j]-=row.km}}rec(0);if(ok)return true}
 return false
}
function lifecycleValidatePlan(result){
 const issues=[],cfg=lifecycleStrategyConfig();
 const scheduledRetirements=result.pairs.filter(p=>p.role!=='race'&&p.projectedRetireDate).slice().sort((a,b)=>a.projectedRetireDate.localeCompare(b.projectedRetireDate));
 for(let i=1;i<scheduledRetirements.length;i++){const gap=(dte(scheduledRetirements[i].projectedRetireDate)-dte(scheduledRetirements[i-1].projectedRetireDate))/DAY;if(gap<56)issues.push({code:'retirements-within-eight-weeks',firstPairId:scheduledRetirements[i-1].id,secondPairId:scheduledRetirements[i].id,gapDays:gap})}
 const validationWeeks=[...new Set(result.assignments.filter(a=>!a.rehab).map(a=>lifecycleWeekKey(a.date)))];
 validationWeeks.forEach(week=>{const rows=result.assignments.filter(a=>!a.rehab&&lifecycleWeekKey(a.date)===week),total=rows.reduce((n,a)=>n+(Number(a.km)||0),0),floor=lifecycleMeaningfulWeeklyFloor(total);result.pairs.filter(p=>p.role!=='race'&&p!==result.racePair).forEach(pair=>{const km=rows.filter(a=>a.pairId===pair.id).reduce((n,a)=>n+(Number(a.km)||0),0);if(km>0&&km+1e-6<floor)issues.push({code:'token-weekly-shoe-use',pairId:pair.id,week,km,floor})})});
 result.purchases.forEach(p=>{
  const pair=result.pairs.find(x=>x.id===p.pairId),first=pair?.assignments.slice().sort((a,b)=>a.date.localeCompare(b.date))[0];
  if(!first)issues.push({code:'unused-purchase',pairId:p.pairId});
  else{
   if(first.date<p.purchaseDate)issues.push({code:'use-before-purchase',pairId:p.pairId,purchaseDate:p.purchaseDate,firstUse:first.date});
   if((dte(first.date)-dte(p.purchaseDate))/DAY>Math.max(21,cfg.purchaseLeadDays+7))issues.push({code:'idle-after-purchase',pairId:p.pairId});
  }
 });
 ...
 result.pairs.forEach(pair=>{
  const cutoff=pair.projectedRetireDate||pair.finalPlannedUseDate;
  if(!cutoff)return;
  const later=(pair.points||[]).find(pt=>pt.date>cutoff);
  if(later)issues.push({code:'graph-beyond-retire-date',pairId:pair.id,retireDate:cutoff,pointDate:later.date})
 });
 result.pairs.forEach(pair=>{
  let km=pair.owned?pair.currentKm:0;
 ...
 for(const [week,total] of weeklyTotal.entries()){
  if(total<=0)continue;
  const activeCount=lifecycleActiveNonRacePairsForWeek(result.pairs,week).length;
  if(activeCount<2)issues.push({code:'extended-one-pair-rotation',week,activeCount})
 }
 for(const [week,total] of weeklyTotal.entries()){
  if(total<=0)continue;
  lifecycleActiveNonRacePairsForWeek(result.pairs,week).forEach(pair=>{
   ...
   if(share<.25-1e-6&&lifecycleWeekShareFeasible(result,week,total,lifecycleActiveNonRacePairsForWeek(result.pairs,week)))issues.push({code:'active-non-race-pair-below-25-percent-weekly-share',pairId:pair.id,week,share})
  })
 }
 ...
 result.pairs.filter(p=>p.role!=='race').forEach(pair=>{
  ...
  if(last.date<result.raceDate&&kmAtLast<pair.retireKm-1&&!pair.isProjectedRetired){
 ...
 result.pairs.filter(p=>!p.owned&&p.assignments.length).forEach(pair=>{
  ...
  if(u.maxGapDays>42)issues.push({code:'future-pair-long-idle-gap',pairId:pair.id,maxGapDays:u.maxGapDays});
 });
 ...
 result.purchases.forEach(p=>{
  const pair=result.pairs.find(x=>x.id===p.pairId);if(!pair||pair.role==='race'||pair===result.racePair)return;
  const first=pair.assignments.filter(a=>Number(a.km)>0).slice().sort((a,b)=>a.date.localeCompare(b.date))[0];if(!first)return;
  const days=(dte(result.raceDate)-dte(first.date))/DAY,plannedKm=pair.assignments.reduce((n,a)=>n+(Number(a.km)||0),0),fullLife=Math.max(1,Number(pair.lifeKm)||Number(pair.retireKm)||1);
  if(days>=0&&days<=70&&plannedKm<Math.max(60,.18*fullLife)&&!lifecycleFuturePairHasMaterialSpecialistRole(pair,result.fixed,result.pairs))issues.push({code:'late-low-use-purchase',pairId:pair.id,daysToRace:days,plannedKm})
 });
...
function lifecycleEnumerateWeekAllocation(weekRuns,activePairs,totalOverride=null){
 const cfg=lifecycleStrategyConfig(),movableTotal=weekRuns.reduce((n,p)=>n+Math.max(0,Number(p.distance)||0),0),total=Number(totalOverride)||movableTotal,m=activePairs.length,n=weekRuns.length;
 if(!n||m<1)return null;
 let best=null,choice=new Array(n).fill(0),used=new Array(m).fill(0);
 function rec(i){
  if(i===n){
   const shares=used.map(x=>total?x/total:0),usedCount=shares.filter(x=>x>1e-9).length;
   if(m>=2&&usedCount<2)return;
   // 25% is a hard target whenever session granularity permits it. Final partial-life
   // weeks are handled by choosing the best feasible allocation and retiring at final use.
   const minShare=Math.min(...shares.filter(x=>x>1e-9));
   let score=0,valid=true;
   for(let j=0;j<n;j++){
    const pair=activePairs[choice[j]],plan=weekRuns[j],fit=lifecycleWorkoutFit(pair.profile,plan);
    if(fit<Math.max(24,lifecycleAcceptableFit(plan)-10)){valid=false;break}
    score+=lifecyclePairAssignmentScore(pair,plan).score;
   }
   if(!valid)return;
   const hard25=shares.every(x=>x>=.25-1e-9);
   const objective=(hard25?1000000:0)+score*100+minShare*250;
   if(!best||objective>best.objective)best={choice:choice.slice(),shares,hard25,objective};
   return
  }
  const plan=weekRuns[i],km=Math.max(0,Number(plan.distance)||0);
  for(let j=0;j<m;j++){
   const pair=activePairs[j],prior=lifecyclePairKmBeforeDate(pair,plan.date)+used[j];
   if(prior+km>pair.retireKm+1e-6)continue;
   choice[i]=j;used[j]+=km;rec(i+1);used[j]-=km
  }
 }
 rec(0);return best
}
function lifecycleRepairOnePairWeeks(ctx,fixed,assignments){
 const {pairs,purchases}=ctx,weeks=[...new Set(assignments.filter(a=>!a.rehab).map(a=>lifecycleWeekKey(a.date)))].sort();
 for(const week of weeks){const rows=assignments.filter(a=>!a.rehab&&lifecycleWeekKey(a.date)===week&&pairs.find(p=>p.id===a.pairId)?.role!=='race'),ids=[...new Set(rows.map(r=>r.pairId))];if(rows.length<2||ids.length>=2)continue;const donor=pairs.find(p=>p.id===ids[0]);if(!donor)continue;const first=rows.slice().sort((a,b)=>a.date.localeCompare(b.date))[0],plan=fixed.find(p=>p.id===first.planId)||{id:first.planId,date:first.date,type:first.type,distance:first.km,surface:'road'};let incoming=pairs.find(p=>p.role!=='race'&&p.id!==donor.id&&p.availableDate<=first.date&&rows.some(r=>{const pl=fixed.find(x=>x.id===r.planId)||{type:r.type,distance:r.km,surface:'road'};return lifecyclePairKmBeforeDate(p,r.date)+r.km<=p.retireKm+1e-6&&lifecycleWorkoutFit(p.profile,pl)>=Math.max(24,lifecycleAcceptableFit(pl)-10)}));if(!incoming)incoming=lifecycleCreateDirectSuccessor(ctx,donor,plan);if(!incoming)continue;incoming.availableDate=first.date;incoming.purchaseDate=first.date;const pur=purchases.find(p=>p.pairId===incoming.id);if(pur){pur.purchaseDate=first.date;pur.firstUseDate=first.date}const total=rows.reduce((n,r)=>n+r.km,0),target=.25*total;let moved=0;for(const row of rows.slice().sort((a,b)=>a.km-b.km)){const pl=fixed.find(p=>p.id===row.planId)||{type:row.type,distance:row.km,surface:'road'};if(lifecyclePairKmBeforeDate(incoming,row.date)+row.km>incoming.retireKm+1e-6||lifecycleWorkoutFit(incoming.profile,pl)<Math.max(24,lifecycleAcceptableFit(pl)-10))continue;lifecycleReassign(row,incoming,pairs);moved+=row.km;if(moved>=target-1e-6&&total-moved>=target-1e-6)break}}
}
function lifecycleFinalWeeklyRebalance(ctx,fixed,assignments){
 const {pairs,purchases}=ctx,weeks=[...new Set(assignments.filter(a=>!a.rehab).map(a=>lifecycleWeekKey(a.date)))].sort();
 for(const week of weeks){
  const all=assignments.filter(a=>!a.rehab&&lifecycleWeekKey(a.date)===week),total=all.reduce((n,a)=>n+(Number(a.km)||0),0),rows=all.filter(a=>pairs.find(p=>p.id===a.pairId)?.role!=='race');if(rows.length<2)continue;
  const plans=rows.map(r=>fixed.find(p=>p.id===r.planId)||{id:r.planId,date:r.date,type:r.type,distance:r.km,surface:'road'});
  const originalIds=rows.map(r=>r.pairId);rows.forEach(r=>{const p=pairs.find(x=>x.id===r.pairId);if(p){const i=p.assignments.indexOf(r);if(i>=0)p.assignments.splice(i,1);p.km-=r.km}});
  let candidates=pairs.filter(p=>p.role!=='race'&&p.availableDate<=plans[0].date&&plans.some(pl=>lifecyclePairKmBeforeDate(p,pl.date)+(Number(pl.distance)||0)<=p.retireKm+1)).sort((a,b)=>lifecyclePairAssignmentScore(b,plans[0]).score-lifecyclePairAssignmentScore(a,plans[0]).score).slice(0,3);
  let solutions=[];if(candidates.length>=2){if(candidates.length===2){const sol=lifecycleEnumerateWeekAllocation(plans,candidates,total);if(sol)solutions.push({subset:candidates,sol})}else{const sets=[candidates];for(let i=0;i<3;i++)for(let j=i+1;j<3;j++)sets.push([candidates[i],candidates[j]]);sets.forEach(sub=>{const sol=lifecycleEnumerateWeekAllocation(plans,sub,total);if(sol)solutions.push({subset:sub,sol})})}}
  solutions.sort((a,b)=>(b.sol.hard25-a.sol.hard25)||(b.sol.objective-a.sol.objective));const pick=solutions[0];
  if(pick){rows.forEach((r,i)=>{const np=pick.subset[pick.sol.choice[i]];r.pairId=np.id;r.ownedShoeId=np.owned?np.shoe.id:null;np.assignments.push(r);np.km+=r.km})}
  else{rows.forEach((r,i)=>{const op=pairs.find(p=>p.id===originalIds[i]);if(op){r.pairId=op.id;r.ownedShoeId=op.owned?op.shoe.id:null;op.assignments.push(r);op.km+=r.km}})}
 }
}
function lifecycleEmergencyRacePair(ctx,racePlan,fixed,assignments,manual){
 const {pairs,purchases,events}=ctx,profile=lifecycleContextualRaceProfile(racePlan,pairs)||lifecycleChoosePurchaseProfile(racePlan,'race')||shoeProfileForShoe({brand:'ASICS',model:'SUPERBLAST',version:'3'})||pairs[0]?.profile;if(!profile)return null;const window=lifecycleRaceTargetWindow(profile,racePlan.distance),target=Math.min(window.maxKm,Math.max(window.minKm,lifecycleRaceTargetKm(window))),raceMs=dte(racePlan.date).getTime(),start=iso(new Date(raceMs-112*DAY));
 const eligible=fixed.filter(p=>p.date>=start&&p.date<racePlan.date&&!manual.has(p.id)).map(plan=>{const row=assignments.find(a=>a.planId===plan.id&&!a.rehab),wk=lifecycleWeekKey(plan.date),total=assignments.filter(a=>!a.rehab&&lifecycleWeekKey(a.date)===wk).reduce((n,a)=>n+(Number(a.km)||0),0);return{plan,row,total}}).filter(x=>x.row).sort((a,b)=>a.row.km-b.row.km||a.plan.date.localeCompare(b.plan.date));
 let selected=[],km=0,weeks=new Set();for(const x of eligible){const wk=lifecycleWeekKey(x.plan.date);if(weeks.has(wk)||km+x.row.km>window.maxKm)continue;selected.push(x);weeks.add(wk);km+=x.row.km;if(km>=window.minKm)break}if(km<window.minKm)return null;
 selected.sort((a,b)=>a.plan.date.localeCompare(b.plan.date));const firstUse=selected[0].plan.date,id=`future:${purchases.length+1}:${lifecycleProfileKey(profile)}:race`;const pair={id,owned:false,shoe:null,profile,currentKm:0,km:0,availableDate:firstUse,purchaseDate:firstUse,retireKm:lifecycleRetireKmForProfile(profile),assignments:[],points:[{date:firstUse,km:0,purchase:true}],role:'race'};pairs.push(pair);purchases.push({id:`purchase:${id}`,pairId:id,profile,purchaseDate:firstUse,firstUseDate:firstUse,role:'race',reason:'Dedicated Race Day pair introduced at first meaningful familiarisation use.',replacesPairId:null});selected.forEach(x=>{lifecycleReassign(x.row,pair,pairs);x.row.raceFamiliarisation=true;x.row.why='Race Day familiarisation allocated by the authoritative lifecycle optimiser.'});events.push({date:firstUse,type:'pair-entry',pairId:id,text:`${futureProfileDisplayName(profile)} enters at its first Race Day familiarisation session.`});return lifecycleRaceFit(pair,racePlan)
}
function freshShoeLifecyclePlan(){
 const now=iso(today()),raceDate=state.setup?.raceDate||now,cfg=lifecycleStrategyConfig(),manual=new Map((state.plannedShoeAssignments||[]).filter(a=>a.source==='user').map(a=>[a.planId,a.shoeId]));
 const fixed=(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>=now&&p.date<=raceDate).slice().sort((a,b)=>a.date.localeCompare(b.date));
 const activeInjuryForShoes=(state.injuries||[]).find(x=>x.id===state.activeInjuryPlanId),rehabStamp=activeInjuryForShoes?JSON.stringify({id:activeInjuryForShoes.id,plannedRehabShoes:activeInjuryForShoes.plannedRehabShoes||{},checks:(activeInjuryForShoes.checkIns||[]).map(c=>[c.date,c.walkMinutes,c.runMinutes,c.rehabShoeId,c.pain,c.hop,c.bridge]),updatedAt:activeInjuryForShoes.updatedAt||activeInjuryForShoes.lastUpdated||''}):'';
 const stamp=['v2',state.storageRevision||0,'balanced',runnerFootMechanics(),raceDate,OFFLINE_ASICS_CATALOGUE_VERSION,fixed.map(p=>`${p.id}:${p.date}:${p.type}:${p.distance}`).join(';'),(state.shoes||[]).map(sh=>`${sh.id}:${sh.status}:${shoeMileage(sh).toFixed(2)}:${sh.condition||sh.conditionFeedback||''}`).join(';'),[...manual].join(';'),rehabStamp].join('|');
 if(freshShoePlanCache.stamp===stamp)return freshShoePlanCache.value;
 const pairs=(state.shoes||[]).filter(sh=>sh.status!=='retired').map(sh=>{const profile=shoeProfileForShoe(sh),km=shoeMileage(sh);return{id:`owned:${sh.id}`,owned:true,shoe:sh,profile,currentKm:km,km,availableDate:now,purchaseDate:shoePairStartDate(sh),retireKm:lifecycleRetireKmForProfile(profile,sh),assignments:[],points:[{date:now,km}],role:'owned'}});
 const purchases=[],assignments=[],events=[],ctx={pairs,purchases,events};
 const weeks=[...new Set(fixed.map(p=>lifecycleWeekKey(p.date)))].sort();
 for(const week of weeks){
  const runs=fixed.filter(p=>lifecycleWeekKey(p.date)===week),first=runs[0];if(!runs.length)continue;
  // Pairs with no capacity for any remaining session leave the active forecast before this week.
  let active=pairs.filter(p=>p.role!=='race'&&p.availableDate<=first.date&&runs.some(r=>lifecyclePairKmBeforeDate(p,r.date)+(Number(r.distance)||0)<=p.retireKm+1e-6));
  // Keep the physical rotation compact. Existing owned pairs are honoured; future pairs are only
  // introduced when required. Two is the target, three is accepted during a real handover.
  if(active.length>3)active=active.sort((a,b)=>(b.retireKm-lifecyclePairKmBeforeDate(b,first.date))-(a.retireKm-lifecyclePairKmBeforeDate(a,first.date))).slice(0,3);
  while(active.length<2){
   const ref=active[0]||pairs.filter(p=>p.role!=='race').sort((a,b)=>b.km-a.km)[0]||null;
   const newPair=lifecycleCreateDirectSuccessor(ctx,ref,first);
   if(!newPair||active.includes(newPair))break;newPair.availableDate=first.date;newPair.purchaseDate=first.date;
   const pur=purchases.find(x=>x.pairId===newPair.id);if(pur){pur.purchaseDate=first.date;pur.firstUseDate=first.date}
   active.push(newPair)
  }
  // Try all 2/3-pair subsets and all session allocations; prefer a true >=25% weekly share.
  let candidates=[];const subsets=[];
  if(active.length===2)subsets.push(active);
  else if(active.length>=3){subsets.push(active.slice(0,3));for(let i=0;i<active.length;i++)for(let j=i+1;j<active.length;j++)subsets.push([active[i],active[j]])}
  for(const subset of subsets){const sol=lifecycleEnumerateWeekAllocation(runs,subset);if(sol)candidates.push({subset,sol})}
  candidates.sort((a,b)=>(b.sol.hard25-a.sol.hard25)||(b.sol.objective-a.sol.objective));let picked=candidates[0];
  if(!picked){
   const np=lifecycleCreatePair(ctx,first,'training');if(np){np.availableDate=first.date;np.purchaseDate=first.date;const pur=purchases.find(x=>x.pairId===np.id);if(pur){pur.purchaseDate=first.date;pur.firstUseDate=first.date}active=[...active.slice(-1),np];picked={subset:active,sol:lifecycleEnumerateWeekAllocation(runs,active)}}
  }
  if(!picked?.sol)continue;
  runs.forEach((plan,i)=>{let pair=picked.subset[picked.sol.choice[i]];const mid=manual.get(plan.id);if(mid){const mp=pairs.find(p=>p.owned&&p.shoe.id===mid&&lifecyclePairKmBeforeDate(p,plan.date)+(Number(plan.distance)||0)<=p.retireKm+1);if(mp)pair=mp}lifecycleAssign(pair,plan,assignments,mid?'Manual planned shoe choice retained.':`Authoritative weekly optimiser: ${lifecyclePairLabel(pair)} gives the best feasible combination of workout fit, remaining life and rotation balance${picked.sol.hard25?' while keeping each active training pair at or above 25% of weekly volume':''}.`)});
 }
 // Remove future training pairs that never entered real use; purchase == first use by design.
 for(const p of pairs.filter(x=>!x.owned&&x.role!=='race'&&!x.assignments.length).slice())lifecycleRemoveFuturePair(ctx,p,fixed,assignments,'No purchase: the physical pair never entered meaningful programme use.');
 // Active rehabilitation is allocated through the SAME physical lifecycle, after fixed running demand.
 const injury=(state.injuries||[]).find(x=>x.id===state.activeInjuryPlanId);
 if(injury){const progress=injuryPrediction(injury),rehabEnd=[raceDate,progress?.windowEnd||raceDate].filter(Boolean).sort()[0];for(let d=new Date(dte(now).getTime()+DAY),guard=0;d<=dte(rehabEnd)&&guard<120;d=new Date(d.getTime()+DAY),guard++){const date=iso(d),day=rehabCalendarDay(injury,progress,date,rehabPlanDayIndex(injury,date));if(!rehabDayNeedsShoe(day))continue;const exp=rehabExpectedDistance(day),km=Math.max(0,Number(exp.totalKm)||0);if(km<=0)continue;const rp={id:`rehab-shoe-${injury.id}-${day.date}`,date:day.date,type:'Recovery',distance:km,surface:'road'},explicit=plannedRehabShoeId(injury,day.date);let cand=pairs.filter(p=>p.role!=='race'&&p.availableDate<=date&&lifecyclePairKmBeforeDate(p,date)+km<=p.retireKm+1).map(p=>({p,score:lifecycleWorkoutFit(p.profile,rp)+(p.profile.cushioning||3)*7+(p.profile.protection||3)*7+(p.profile.comfort||3)*6+(p.profile.stability||3)*4+lifecycleInjuryFootwearAdjustment(p.profile)+gaitPurchaseAdjustment(p.profile)})).sort((a,b)=>b.score-a.score);let pair=explicit?pairs.find(p=>p.owned&&p.shoe.id===explicit):cand[0]?.p;if(!pair)continue;const row={planId:rp.id,date,type:'Rehab',km,pairId:pair.id,ownedShoeId:pair.owned?pair.shoe.id:null,why:explicit?'User-selected rehabilitation shoe.':'Same lifecycle optimiser: most protective compatible active pair for the rehabilitation prescription, including injury and pronation context.',fit:lifecycleWorkoutFit(pair.profile,rp),rehab:true,walkMinutes:exp.walkMinutes,runMinutes:exp.runMinutes};pair.assignments.push(row);assignments.push(row)}}
 // Race Day selection. Performance always gets a dedicated specialist; other modes reuse a suitable
 // physical pair only if it can finish inside the target mileage window, otherwise one specialist is added.
 ...
 let raceCandidates=pairs.filter(p=>lifecycleRaceEligiblePair(p,racePlan)).map(p=>lifecycleRaceFit(p,racePlan)).sort((a,b)=>b.score-a.score),raceChoice=null;
 raceChoice=raceCandidates.find(x=>x.preRaceKm>=x.window.minKm&&x.preRaceKm<=Math.min(250,x.window.maxKm))||null;
 if(!raceChoice)raceChoice=lifecycleBuildDedicatedRacePair(ctx,racePlan,fixed,assignments,manual);
 if(!raceChoice)raceChoice=lifecycleEmergencyRacePair(ctx,racePlan,fixed,assignments,manual);
 lifecycleSynchronizeFuturePairTiming(pairs,purchases);
 let finalPairs=pairs.filter(p=>p.owned||p.assignments.length),used=new Set(finalPairs.map(p=>p.id)),finalPurchases=purchases.filter(p=>used.has(p.pairId)).sort((a,b)=>a.purchaseDate.localeCompare(b.purchaseDate));
 lifecycleTrimPostRetirementAssignments({pairs:finalPairs,purchases:finalPurchases,events},fixed,assignments,manual);
 lifecycleFinalWeeklyRebalance({pairs:finalPairs,purchases:finalPurchases,events},fixed,assignments);
 lifecycleTrimPostRetirementAssignments({pairs:finalPairs,purchases:finalPurchases,events},fixed,assignments,manual);
 lifecycleRepairOnePairWeeks({pairs:finalPairs,purchases:finalPurchases,events},fixed,assignments);
 lifecycleSynchronizeFuturePairTiming(finalPairs,finalPurchases);
 finalPairs.forEach(pair=>lifecycleRebuildPoints(pair,now,raceDate));lifecycleNormalizeEntryEvents(finalPairs,finalPurchases,events);lifecycleNormalizeRetirementEvents(finalPairs,events);
 raceCandidates=finalPairs.filter(p=>lifecycleRaceEligiblePair(p,racePlan)).map(p=>lifecycleRaceFit(p,racePlan)).sort((a,b)=>b.score-a.score);raceChoice=raceCandidates.find(x=>x.preRaceKm>=x.window.minKm&&x.preRaceKm<=Math.min(250,x.window.maxKm))||raceChoice;
 if(!raceChoice){const lastCtx={pairs:finalPairs,purchases:finalPurchases,events};raceChoice=lifecycleEmergencyRacePair(lastCtx,racePlan,fixed,assignments,manual);if(raceChoice){finalPairs=lastCtx.pairs.filter(p=>p.owned||p.assignments.length);finalPurchases=lastCtx.purchases.filter(p=>finalPairs.some(x=>x.id===p.pairId));lifecycleSynchronizeFuturePairTiming(finalPairs,finalPurchases);finalPairs.forEach(pair=>lifecycleRebuildPoints(pair,now,raceDate));lifecycleNormalizeEntryEvents(finalPairs,finalPurchases,events);lifecycleNormalizeRetirementEvents(finalPairs,events)}}
 // FINAL ROTATION CONSOLIDATION: do not keep a physical pair alive with token weekly mileage.
 lifecycleConsolidateTokenUse({pairs:finalPairs,purchases:finalPurchases,events},fixed,assignments,manual,raceChoice?.pair||null);
 // FINAL PURCHASE NECESSITY GATE: once a valid Race Day pair exists, remove any late/low-use
 // non-race future pair whose workouts can be absorbed by the remaining rotation.
 lifecyclePruneLateRedundantPurchases({pairs:finalPairs,purchases:finalPurchases,events},fixed,assignments,manual,raceChoice?.pair||null,raceDate);
 lifecycleSynchronizeFuturePairTiming(finalPairs,finalPurchases);
 finalPairs.forEach(pair=>lifecycleRebuildPoints(pair,now,raceDate));
 lifecycleEnforceRetirementSpacing({pairs:finalPairs,purchases:finalPurchases,events},fixed,assignments,manual,raceDate,now);
 lifecycleSynchronizeFuturePairTiming(finalPairs,finalPurchases);
 finalPairs.forEach(pair=>lifecycleRebuildPoints(pair,now,raceDate));
 lifecycleNormalizeEntryEvents(finalPairs,finalPurchases,events);
 lifecycleNormalizeRetirementEvents(finalPairs,events);
 finalPairs=finalPairs.filter(p=>p.owned||p.assignments.length||p===raceChoice?.pair);
 finalPurchases=finalPurchases.filter(p=>finalPairs.some(x=>x.id===p.pairId)).sort((a,b)=>a.purchaseDate.localeCompare(b.purchaseDate));

 const result={strategy:cfg.key,now,raceDate,fixed,pairs:finalPairs,assignments,purchases:finalPurchases,events:events.filter(e=>e.type!=='purchase-avoided'),racePair:raceChoice?.pair||null,raceWindow:raceChoice?.window||null,catalogueSource:'offline',catalogueVersion:OFFLINE_ASICS_CATALOGUE_VERSION,footMechanics:runnerFootMechanics()};
 result.validationIssues=lifecycleValidatePlan(result);freshShoePlanCache={stamp,value:result};return result
}

function freshLifecycleAssignmentForPlan(planId){return freshShoeLifecyclePlan().assignments.find(a=>a.planId===planId)||null}
function freshLifecycleRehabAssignment(injuryId,date){
 const life=freshShoeLifecyclePlan(),row=life.assignments.find(a=>a.rehab&&a.planId===`rehab-shoe-${injuryId}-${date}`);
...
function freshPurchaseForOwnedShoe(shoeId){
 const life=freshShoeLifecyclePlan(),owned=life.pairs.find(p=>p.owned&&p.shoe.id===shoeId);if(!owned)return null;
 const lastUse=owned.assignments.slice().sort((a,b)=>a.date.localeCompare(b.date)).at(-1),future=life.purchases.find(p=>p.role!=='race'&&p.firstUseDate>=(lastUse?.date||life.now));
 return future||null
}
function shoeNextPurchasePlan(shoe){
 const life=freshShoeLifecyclePlan(),owned=life.pairs.find(p=>p.owned&&p.shoe.id===shoe.id),currentKm=shoeMileage(shoe);
...
function shoeGraphRationaleHtml(series,events,raceLifecycle){
 const rows=series.map(item=>{const current=shoeMileage(item.shoe),event=events.find(e=>e.shoe.id===item.shoe.id),f=shoeForecast(item.shoe),start=shoePairStartDate(item.shoe);return`<li><b>${esc(shoeDisplayName(item.shoe))}:</b> this physical pair enters the graph on ${esc(shoeChartDate(start))}. It has ${Math.round(current)} km today.${event?` The current pair is planned to reach its ${Math.round(f.low)}–${Math.round(f.high)} km replacement range on ${esc(shoeChartDate(event.date))}. A proposed training replacement is shown only from its proposed purchase date.${event.optimisation?` The portfolio optimiser moved this replacement from ${esc(shoeChartDate(event.optimisation.originalDate))} to ${esc(shoeChartDate(event.date))} by shifting about ${Math.round(event.optimisation.divertedKm)} km of compatible mileage to ${esc(event.optimisation.absorbedBy)}.`:''}`:' It is not expected to reach its replacement threshold before Race Day.'}</li>`}).join('');
 const race=raceLifecycle?.purchaseNeeded&&raceLifecycle.profile?`<li><b>Race-day pair:</b> a separate ASICS ${esc(raceLifecycle.profile.family)} ${esc(raceLifecycle.profile.version)} is introduced from its proposed purchase window, not from today. It receives only the selected late-programme familiarisation sessions and is targeted at ${Math.round(raceLifecycle.targetWindow?.minKm||0)}–${Math.round(raceLifecycle.targetWindow?.maxKm||0)} km on ${esc(shoeChartDate(state.setup?.raceDate))}.</li>`:raceLifecycle?.preferred&&raceLifecycle.condition?`<li><b>Race-day pair:</b> the current ${esc(shoeDisplayName(raceLifecycle.preferred))} is retained only when it is not scheduled for replacement before Race Day and its full planned mileage remains compatible with the desired ${Math.round(raceLifecycle.condition.opt.minKm)}–${Math.round(raceLifecycle.condition.opt.maxKm)} km window.</li>`:'';
 return`<details class="shoeGraphRationale"><summary>How this physical-pair plan works</summary><div><p>Every curve represents one physical pair. An owned pair appears only from its recorded purchase/tracking start. A future pair appears only from its proposed purchase date. No physical pair is deliberately planned beyond its model-specific service-life threshold: when that threshold is reached, mileage must transfer to a fresh physical pair. Portfolio optimisation may change which compatible shoe receives a workout, but it may never extend a worn pair beyond its replacement threshold. The planner never treats one physical pair as both a worn training shoe and a separate fresh Race Day shoe.</p><ul>${rows}${race}</ul></div></details>`
}

function shoeWeekStart(date){const d=dte(date),shift=(d.getDay()+6)%7;d.setDate(d.getDate()-shift);return iso(d)}
function freshShoeSlopeEvents(life){
 const out=[],seen=new Set();let n=1;
 const add=e=>{const key=`${e.driver}|${e.pairId}|${e.date}`;if(seen.has(key)||out.length>=7)return;seen.add(key);out.push({...e,number:n++})};
 life.events.filter(e=>e.type==='pair-entry').forEach(e=>{const pair=life.pairs.find(p=>p.id===e.pairId);if(pair)add({driver:'Purchase / first use',pairId:pair.id,date:e.date,km:0,label:lifecyclePairLabel(pair),reason:e.text})});
 life.events.filter(e=>e.type==='pair-handover').forEach(e=>{const pair=life.pairs.find(p=>p.id===e.pairId);if(pair)add({driver:'Shoe handover',pairId:pair.id,date:e.date,km:pair.points.filter(p=>p.date<=e.date).at(-1)?.km||0,label:lifecyclePairLabel(pair),reason:e.text})});
 life.events.filter(e=>e.type==='pair-retirement').forEach(e=>{const pair=life.pairs.find(p=>p.id===e.pairId);if(pair)add({driver:'Shoe lifecycle',pairId:pair.id,date:e.date,km:pair.points.filter(p=>p.date<=e.date).at(-1)?.km||pair.km,label:lifecyclePairLabel(pair),reason:e.text})});
 life.events.filter(e=>e.type==='race-preservation').forEach(e=>{const pair=life.pairs.find(p=>p.id===e.pairId);if(pair)add({driver:'Race Day preparation',pairId:pair.id,date:e.date,km:pair.points.filter(p=>p.date<=e.date).at(-1)?.km||0,label:lifecyclePairLabel(pair),reason:e.text})});
 const retireEvents=life.events.filter(e=>e.type==='pair-retirement');
 life.pairs.forEach(pair=>{
  const weeks=new Map();
  pair.assignments.forEach(a=>{const w=shoeWeekStart(a.date),row=weeks.get(w)||{date:w,total:0,rehab:0};row.total+=Number(a.km)||0;if(a.rehab)row.rehab+=Number(a.km)||0;weeks.set(w,row)});
  const arr=[...weeks.values()].sort((a,b)=>a.date.localeCompare(b.date));if(arr.length<4)return;
  for(let i=2;i<=arr.length-2;i++){
   const before=(arr[i-2].total+arr[i-1].total)/2,after=(arr[i].total+arr[i+1].total)/2,delta=after-before,relative=Math.abs(delta)/Math.max(10,before);
   if(Math.abs(delta)<15||relative<.45)continue;
   const nearbyRetire=retireEvents.find(e=>Math.abs(dte(e.date)-dte(arr[i].date))<=14*DAY);
   const rehabBefore=(arr[i-2].rehab+arr[i-1].rehab)/2,rehabAfter=(arr[i].rehab+arr[i+1].rehab)/2,rehabDriven=Math.abs(rehabAfter-rehabBefore)>=8;
   if(nearbyRetire){const outgoing=life.pairs.find(p=>p.id===nearbyRetire.pairId);add({driver:'Shoe lifecycle',pairId:pair.id,date:arr[i].date,km:pair.points.filter(p=>p.date<=arr[i].date).at(-1)?.km||0,label:lifecyclePairLabel(pair),reason:`${lifecyclePairLabel(outgoing)} reaches its planned lifecycle limit around this point. Mileage is therefore redistributed across the remaining/new rotation; the change is caused by shoe lifecycle, not by a change in the running programme.`});}
   else add({driver:rehabDriven?'Rehab plan':'Training plan',pairId:pair.id,date:arr[i].date,km:pair.points.filter(p=>p.date<=arr[i].date).at(-1)?.km||0,label:lifecyclePairLabel(pair),reason:rehabDriven?`Expected rehabilitation shoe exposure changes materially from about ${rehabBefore.toFixed(1)} to ${rehabAfter.toFixed(1)} km/week.`:`This pair's sustained allocation changes from about ${before.toFixed(1)} to ${after.toFixed(1)} km/week because the fixed programme itself changes training demand. Normal weekly variation is ignored.`});
   break
  }
 });
 return out.sort((a,b)=>a.date.localeCompare(b.date)).slice(0,7).map((e,i)=>({...e,number:i+1}))
}
function freshSlopeEventsHtml(events,colorById){
 if(!events.length)return'';
 return`<div class="shoeSlopeEvents"><small>WHY THE SLOPE CHANGES</small><p>Only material allocation changes are marked. Tap a number for the cause.</p>${events.map(e=>`<details style="--event-color:${esc(colorById?.get(e.pairId)||'#58e4df')}"><summary><span class="slopeEventNo">${e.number}</span><span><b>${esc(e.driver)}</b><small>${fmtDate(e.date)} · ${esc(e.label)}</small></span></summary><p>${esc(e.reason)}</p></details>`).join('')}</div>`
}
...
function shoeProgrammeMileageChartHtml(){
 const life=freshShoeLifecyclePlan(),pairs=life.pairs.filter(p=>p.owned||p.assignments.length||p===life.racePair);if(!pairs.length)return'<div class="shoeEmpty">Add a shoe to start mileage planning.</div>';
 ...
 const future=pairs.map(pair=>{
  const col=colorById.get(pair.id),cutoff=pair.projectedRetireDate||raceDate,start=pair.owned?todayStr:(pair.purchaseDate||pair.availableDate||todayStr);
  const raw=(pair.points||[]).filter(pt=>pt.date>=start&&pt.date<=cutoff).sort((a,b)=>a.date.localeCompare(b.date));
  const visible=[];for(const pt of raw){const prev=visible.at(-1);if(prev&&Math.abs(Number(pt.km)-Number(prev.km))<1e-9)continue;visible.push(pt)}
  const cls=pair===life.racePair?'shoeRaceStrategyLine':pair.owned?'shoePlannedLine':'shoeNewPairLine',segments=[];let seg=[];
  for(const pt of visible){const prev=seg.at(-1);if(prev&&(dte(pt.date)-dte(prev.date))/DAY>21){if(seg.length)segments.push(seg);seg=[]}seg.push(pt)}if(seg.length)segments.push(seg);
  return segments.map(part=>{const pts=part.map(pt=>`${x(pt.date).toFixed(1)},${y(pt.km).toFixed(1)}`).join(' ');return part.length>=2?`<polyline class="shoeLine ${cls}" style="--shoe-color:${col}" points="${pts}"/>`:`<circle class="shoeSinglePoint" style="--shoe-color:${col}" cx="${x(part[0].date).toFixed(1)}" cy="${y(part[0].km).toFixed(1)}" r="4"/>`}).join('')
 }).join('');
 const racePairEndpoint=life.racePair?`<g class="shoeRaceEndpoint" style="--shoe-color:${colorById.get(life.racePair.id)}"><circle cx="${x(raceDate)}" cy="${y(life.racePair.km)}" r="5"/><text x="${x(raceDate)-7}" y="${Math.max(T+15,y(life.racePair.km)-10)}" text-anchor="end">${Math.round(life.racePair.km)} km</text></g>`:'';
 ...
 const retirementMarkers=retirePairs.map(pair=>{const date=pair.projectedRetireDate,xx=x(date),last=(pair.points||[]).filter(pt=>pt.date<=date).at(-1),yy=y(last?.km||pair.km||0),col=colorById.get(pair.id);return`<g class="shoeRetireTick" style="--shoe-color:${col}"><line x1="${xx-7}" y1="${yy-7}" x2="${xx+7}" y2="${yy+7}"/><line x1="${xx-7}" y1="${yy+7}" x2="${xx+7}" y2="${yy-7}"/><text x="${xx}" y="${Math.max(T+14,yy-14)}" text-anchor="middle">RETIRE</text></g>`}).join('');
 const historicalRetirementMarkers=actualSeries.filter(item=>item.shoe.status==='retired'&&item.points.length).map(item=>{const last=item.points.at(-1),pair=pairs.find(p=>p.owned&&p.shoe.id===item.shoe.id),col=pair?colorById.get(pair.id):'#8FA9B8',xx=x(last.date),yy=y(last.km);return`<g class="shoeRetireTick" style="--shoe-color:${col}"><line x1="${xx-7}" y1="${yy-7}" x2="${xx+7}" y2="${yy+7}"/><line x1="${xx-7}" y1="${yy+7}" x2="${xx+7}" y2="${yy-7}"/><text x="${xx}" y="${Math.max(T+14,yy-14)}" text-anchor="middle">RETIRE</text></g>`}).join('');
...
function optimiseShoePurchaseProposals(items){const strategy=SHOE_ENGINE_STRATEGIES[state.shoeEngineStrategy]?state.shoeEngineStrategy:'balanced';const sorted=items.slice().sort((a,b)=>String(a.recommendedPurchaseStart||'9999').localeCompare(String(b.recommendedPurchaseStart||'9999'))),kept=[];for(const item of sorted){if(item.kind==='race'){kept.push(item);continue}const t=item.recommendedPurchaseStart?dte(item.recommendedPurchaseStart).getTime():NaN;const duplicate=kept.find(prev=>{if(prev.kind==='race')return false;const pt=prev.recommendedPurchaseStart?dte(prev.recommendedPurchaseStart).getTime():NaN;if(!Number.isFinite(t)||!Number.isFinite(pt)||Math.abs(t-pt)>56*DAY)return false;const overlap=shoeProposalRoleOverlap(item,prev);const sameFamily=normalizeShoeFamily(item.model)===normalizeShoeFamily(prev.model);return sameFamily||overlap>=.67});if(duplicate&&strategy!=='quality'){duplicate.rationale=(duplicate.rationale||'')+' The rotation engine also absorbed a substantially overlapping near-term purchase to avoid buying two shoes for the same job within eight weeks.';continue}kept.push(item)}return kept}
...
function rehabShoeRecommendation(day){const active=(state.shoes||[]).filter(s=>s.status!=='retired');if(!active.length||!rehabDayNeedsShoe(day))return null;const exp=rehabExpectedDistance(day);const ranked=active.map(shoe=>{const p=shoeProfileForShoe(shoe),wear=shoeWearState(shoe),fam=shoeFamiliarity(shoe,exp.runMinutes>0?'Recovery':'Easy');let score=(Number(p.comfort||3)*6)+(Number(p.cushioning||3)*6)+(Number(p.protection||3)*5)+(Number(p.stability||3)*2)+Math.min(12,fam.runs*3)+lifecycleInjuryFootwearAdjustment(p)+gaitPurchaseAdjustment(p);if(wear.status==='approaching')score-=12;if(wear.status==='replace')score-=45;if((p.surfaces||[]).includes('road'))score+=5;if(p.plated)score-=8;return{shoe,profile:p,score,reasons:['injury/rehab compatibility','comfort and cushioning',`${Math.round(wear.km)} km accumulated`],wear,familiarity:fam}}).sort((a,b)=>b.score-a.score);return ranked[0]||null}
function plannedRehabShoeForDay(injury,day){const explicit=plannedRehabShoeId(injury,day.date);if(explicit)return explicit;const planned=freshLifecycleRehabAssignment(injury.id,day.date);return planned?.pair?.owned?planned.pair.shoe.id:(rehabShoeRecommendation(day)?.shoe?.id||'')}
function defaultRehabCheckinShoeId(injury,day,previous={}){
 ...
 // The daily rehabilitation shoe proposal is authoritative for a new check-in.
 // A same-day logged run must not silently replace the proposed rehab shoe.
 const proposed=plannedRehabShoeForDay(injury,day)||'';
 if(proposed&&activeIds.has(proposed))return proposed;
 ...
 return logged&&activeIds.has(logged)?logged:'';
}
function rehabShoePlanHtml(injury,day){
 if(!rehabDayNeedsShoe(day)||!(state.shoes||[]).some(s=>s.status!=='retired'))return'';
 const explicit=plannedRehabShoeId(injury,day.date),selected=plannedRehabShoeForDay(injury,day),selectedShoe=(state.shoes||[]).find(s=>s.id===selected&&s.status!=='retired'),planned=freshLifecycleRehabAssignment(injury.id,day.date),bestLabel=selectedShoe?shoeDisplayName(selectedShoe):(planned?lifecyclePairLabel(planned.pair):''),exp=rehabExpectedDistance(day);
 return`<div class="rehabShoePlan"><span>${todayPictogram('shoe')}</span><div><small>SHOE</small><select data-rehab-shoe-plan="${esc(injury.id)}" data-rehab-shoe-date="${esc(day.date)}">${shoeSelectOptions(selected,true)}</select><p>${bestLabel?`Shoe plan: ${esc(bestLabel)}. `:''}Expected shoe distance ~${exp.totalKm.toFixed(1)} km${exp.runMinutes?` (${exp.walkMinutes} min walk + ${exp.runMinutes} min slow run)`:exp.walkMinutes?` (${exp.walkMinutes} min walk)`:''}. ${explicit?'Your manual/saved shoe choice is retained.':'This same proposed shoe is preselected in the daily check-in.'}</p></div></div>`
}
