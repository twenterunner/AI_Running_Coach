...
const BUILD=9820, SCHEMA=9820, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function checkInScore(c){if(!c)return null;const pain=clamp(Number(c.pain)||0,0,10),walk=clamp(Number(c.walkPain)||0,0,10),stiff=clamp(Number(c.stiffness)||0,0,10);let score=100-pain*5-walk*4-stiff*2;score+=(c.stairs?5:0)+(c.strength?7:0)+(c.hop?8:0)+Math.min(10,(Number(c.runMinutes)||0)/3)+(c.confidence?clamp(Number(c.confidence),0,10):5);if(c.nextDayWorse)score-=15;if(c.swelling)score-=10;return clamp(Math.round(score),0,100)}
function injuryPrediction(i){
 const checks=(i.checkIns||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)),latest=checks.at(-1)||null;
 const elapsed=Math.max(0,Math.floor((today()-dte(i.date))/DAY)),base=initialInjuryDays(i),initialScore=clamp(100-(Number(i.initialPain)||0)*6-(Number(i.initialWalkPain)||0)*4-(i.pop?10:0)-(i.bruising?8:0),5,70);
 ...
 const hasCheckIns=checks.length>0,score=hasCheckIns?checkInScore(latest):null;
 const expectedScore=clamp(initialScore+(100-initialScore)*Math.min(1,elapsed/base),0,100),delta=hasCheckIns?Math.round(score-expectedScore):null;
 let pace=!hasCheckIns?'awaiting':delta>=10?'faster':delta<=-10?'slower':'on-track';
 let stage=0;if(walkPain<=3&&currentPain<=5)stage=1;if(walkPain<=1&&currentPain<=3&&latest&&(latest.stairs||latest.strength))stage=2;if(currentPain<=2&&walkPain===0&&latest?.hop)stage=3;if((Number(latest?.runMinutes)||0)>=10&&currentPain<=2&&!latest?.nextDayWorse)stage=4;if((Number(latest?.runMinutes)||0)>=30&&currentPain<=1&&!latest?.nextDayWorse)stage=5;if(latest&&(latest.nextDayWorse||latest.swelling))stage=Math.max(0,stage-1);
 const scores=checks.map(checkInScore).filter(Number.isFinite),recent=scores.slice(-3),trend=recent.length>=2?recent.at(-1)-recent[0]:0;
 let adjustment=0;if(hasCheckIns){adjustment=Math.round(-delta*.18);if(latest.nextDayWorse)adjustment+=4;if(trend>=10)adjustment-=2;if(trend<=-8)adjustment+=3}
 const total=Math.max(elapsed+1,base+adjustment),remaining=Math.max(0,total-elapsed),startDate=iso(new Date(today().getTime()+remaining*DAY)),confidence=checks.length>=7?'Higher':checks.length>=3?'Moderate':'Low';
 const originalDate=iso(new Date(dte(i.date).getTime()+base*DAY)),dateShift=hasCheckIns?Math.round((dte(startDate)-dte(originalDate))/DAY):null;
 return{base,total,elapsed,remaining,stage,startDate,originalDate,dateShift,confidence,currentPain,walkPain,score,expectedScore,delta,pace,trend,checks,hasCheckIns};
}
function trajectorySvg(i,p){const checks=p.checks,W=700,H=230,left=42,right=18,top=22,bottom=38,cw=W-left-right,ch=H-top-bottom;const days=Math.max(7,p.elapsed+Math.max(1,p.remaining),p.base),x=d=>left+clamp(d/days,0,1)*cw,y=v=>top+(100-clamp(v,0,100))/100*ch;let expected=`M ${x(0)} ${y(clamp(100-(Number(i.initialPain)||0)*6-(Number(i.initialWalkPain)||0)*4,5,70))} L ${x(p.base)} ${y(100)}`;let observed='';const points=checks.map(c=>({day:Math.max(0,Math.round((dte(c.date)-dte(i.date))/DAY)),score:checkInScore(c)})).filter(q=>Number.isFinite(q.score));points.forEach((q,n)=>observed+=(n?' L ':'M ')+x(q.day)+' '+y(q.score));return`<svg class="rehabTrajectory" viewBox="0 0 ${W} ${H}" role="img" aria-label="Observed and expected rehabilitation trajectory"><line x1="${left}" y1="${y(25)}" x2="${W-right}" y2="${y(25)}"/><line x1="${left}" y1="${y(50)}" x2="${W-right}" y2="${y(50)}"/><line x1="${left}" y1="${y(75)}" x2="${W-right}" y2="${y(75)}"/><path class="expectedLine" d="${expected}"/><path class="observedLine" d="${observed}"/>${points.map(q=>`<circle cx="${x(q.day)}" cy="${y(q.score)}" r="5"/>`).join('')}<text x="${left}" y="${H-10}">Injury</text><text x="${W-right}" y="${H-10}" text-anchor="end">Estimated return</text><text x="${left+4}" y="${top+14}">Recovery score</text></svg><div class="trajectoryLegend"><span class="observedKey">Your progress</span><span class="expectedKey">Expected trajectory</span></div>`}
...
function rehabExercises(i,p){const list=(REHAB_EXERCISES[injuryProfile(i)]||REHAB_EXERCISES.generic)[p.stage]||[];return list.map((e,n)=>`<div class="exerciseCard"><div class="exerciseNumber">${n+1}</div><div><b>${esc(e.name)}</b><strong>${esc(e.dose)}</strong><p>${esc(e.cue)}</p></div></div>`).join('')}
function injuryGuidance(i,p){const latest=p.checks.at(-1);let headline,body,action;if(!latest){headline='Complete your first daily check-in';body='No recovery trend is calculated yet. The injury date and current symptoms provide only an initial estimate; real progress starts with your first check-in.';action='Record today’s baseline before progressing exercises.'}else if(latest.nextDayWorse||latest.swelling||p.trend<=-8){headline='Hold or step back today';body='Your latest response suggests the previous load exceeded current tolerance.';action='Repeat the last well-tolerated level, reduce volume by about 25–40%, and reassess tomorrow.'}else if(p.pace==='faster'){headline='Recovering faster than expected';body=`Your recovery score is ${Math.abs(p.delta)} points above the expected trajectory.`;action='Progress only one variable today: repetitions, resistance, impact or running duration—not several together.'}else if(p.pace==='slower'){headline='Recovering slower than expected';body=`Your recovery score is ${Math.abs(p.delta)} points below the expected trajectory.`;action='Keep the current stage, prioritise consistent low-irritability loading, and seek clinical review if the plateau persists or red flags appear.'}else{headline='Recovery is tracking as expected';body='Symptoms and function are close to the original trajectory.';action='Complete today’s exercises and progress only when the next-morning response remains stable.'}return{headline,body,action}}
...
function renderInjury(){
 ...
 $('injuryIntro').innerHTML=`<div class="rehabIntro"><div><b>Adaptive rehabilitation coach</b><p>Today’s exercises, stage criteria and return-to-running estimate update from your daily response. Injury data remains independent from plan generation and race prediction.</p></div><div class="rehabLoop"><span>Check in</span><i>→</i><span>Exercise</span><i>→</i><span>Reassess</span><i>→</i><span>Progress</span></div></div>`;
 box.innerHTML=injuries.length?injuries.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(i=>{const p=injuryPrediction(i),st=INJURY_STAGES[p.stage],g=injuryGuidance(i,p),shift=p.dateShift==null?'Not adjusted yet':p.dateShift===0?'Unchanged':`${Math.abs(p.dateShift)} day${Math.abs(p.dateShift)===1?'':'s'} ${p.dateShift<0?'earlier':'later'}`,paceClass=p.pace==='faster'?'good':p.pace==='slower'?'bad':'warn',trajectoryText=p.pace==='awaiting'?'Awaiting check-in':p.pace==='faster'?'Faster than expected':p.pace==='slower'?'Slower than expected':'On expected trajectory',deltaText=p.delta==null?'No comparison yet':`${p.delta>0?'+':''}${p.delta} points`;return`<article class="panel injuryCard"><div class="panelHead"><div><h3>${esc(i.location||'Injury')} · ${fmtDate(i.date)}</h3><p class="muted compact">${esc(i.mechanism||'Mechanism not entered')}</p></div><span class="status ${p.stage>=3?'completed':p.currentPain>=5?'missed':'today'}">Stage ${p.stage+1}</span></div><div class="rehabHero"><div class="recoveryGauge ${p.hasCheckIns?'':'unscored'}" style="--recovery:${p.score??0}%"><div><strong>${p.score??'—'}</strong><span>${p.hasCheckIns?'recovery score':'awaiting baseline'}</span></div></div><div><small>Current rehabilitation stage</small><h3>${esc(st.name)}</h3><p>${esc(st.goal)}</p><div class="trajectoryBadge ${paceClass}">${trajectoryText} · ${deltaText}</div></div></div>${progressionCriteriaHtml(p)}<div class="injuryForecast three"><div><small>Current return-to-run estimate</small><strong>${p.stage>=3?'Running has started':fmtDate(p.startDate)}</strong><span>${p.remaining} estimated days remaining · ${p.confidence.toLowerCase()} confidence</span></div><div><small>Original estimate</small><strong>${fmtDate(p.originalDate)}</strong><span>${shift}</span></div><div><small>Last 7 days</small><strong>${p.checks.length?p.trend>0?'Improving':p.trend<0?'Declining':'Stable':'Awaiting data'}</strong><span>${weeklyRehabSummary(p)}</span></div></div><div class="coachDirective ${paceClass}"><b>${esc(g.headline)}</b><p>${esc(g.body)}</p><strong>Today: ${esc(g.action)}</strong></div><article class="rehabVisual"><div class="panelHead"><div><h3>Recovery trajectory</h3><p class="muted compact">Observed function and symptom response compared with the original expectation.</p></div></div>${trajectorySvg(i,p)}</article><div class="rehabSectionHead"><div><span>Today</span><h3>Exercise prescription</h3></div><small>Complete only exercises that remain controlled and within the stated symptom limit.</small></div><div class="exerciseGrid">${rehabExercises(i,p)}</div><div class="rehabSectionHead"><div><span>Progression</span><h3>All rehabilitation stages</h3></div><small>Your current stage changes only when its functional criteria are supported by check-in data.</small></div><div class="injuryTimeline visualTimeline">${INJURY_STAGES.map((x,n)=>`<div class="injuryStage ${n<p.stage?'done':n===p.stage?'active':''}"><i>${n<p.stage?'✓':n+1}</i><div><b>${esc(x.name)}</b><small>${esc(x.criteria)}</small></div></div>`).join('')}</div><details><summary>Initial symptoms, latest response and model inputs</summary><div class="guideGrid"><div><b>At injury</b><p>Pain ${Number(i.initialPain)||0}/10 · walking pain ${Number(i.initialWalkPain)||0}/10${i.pop?' · pop felt':''}${i.bruising?' · bruising/swelling':''}<br>${esc(i.initialSymptoms||'No symptom description')}</p></div><div><b>Latest</b><p>Pain ${p.currentPain}/10 · walking pain ${p.walkPain}/10<br>${esc(p.checks.at(-1)?.symptoms||i.currentSymptoms||'No current symptom description')}</p></div></div></details><div class="buttonRow"><button data-injury-check="${i.id}" class="primary">Daily check-in</button><button data-injury-edit="${i.id}">Edit injury</button><button data-injury-delete="${i.id}" class="danger">Delete</button></div></article>`}).join(''):'<article class="panel injuryEmpty"><div class="injuryEmptyIcon">✚</div><h3>Start an adaptive rehabilitation journey</h3><p>Record an injury to receive a staged exercise prescription, daily guidance, progress tracking and an adjustable return-to-running trajectory.</p><button id="emptyAddInjury" class="primary">Record an injury</button></article>';
}
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.8.0 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.8.0 stable build 9800');
})();