...
function shoeProgrammeMileageChartHtml(){
 ...
 const replacementProjection=replacementSeries.map(item=>{const e=item.event,c=palette[Math.max(0,shoes.findIndex(s=>s.id===e.shoe.id))%palette.length],label=`New ${e.replacement.family} ${e.replacement.version}`;return`<polyline class="shoeLine shoeNewPairLine" style="--shoe-color:${c}" points="${item.points.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ')}"/><text class="shoePurchaseLabel" x="${x(item.purchaseDate)+8}" y="${H-B-18}">Buy ${esc(shoeChartDate(item.purchaseDate))}</text><text class="shoeNewPairLabel" x="${raceX-8}" y="${Math.max(T+38,y(item.finalKm)-12)}" text-anchor="end">${esc(label)} · ~${Math.round(item.finalKm)} km</text>`}).join('');
 const raceStrategyPath=raceStrategy?`<polyline class="shoeLine shoeRaceStrategyLine" points="${raceStrategy.points.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ')}"/><circle class="shoeRaceStrategyPoint" cx="${raceX}" cy="${y(raceStrategy.finalKm)}" r="8"/><text class="shoeRaceStrategyLabel" x="${raceX-10}" y="${Math.max(T+32,y(raceStrategy.finalKm)-16)}" text-anchor="end">RACE PAIR · ${esc(raceStrategy.name)} · ~${Math.round(raceStrategy.finalKm)} km</text>${!raceStrategy.owned?`<text class="shoePurchaseLabel race" x="${x(raceStrategy.purchaseDate)+8}" y="${H-B-46}">Buy race pair · ${esc(shoeChartDate(raceStrategy.purchaseDate))}</text>`:''}`:'';
 const markers=rawEvents.filter(e=>enabled.has(e.shoe.id)).map((e,i)=>{const xx=x(e.date),yy=y(Math.min(maxY,e.km)),name=shoeDisplayName(e.shoe).replace(/^ASICS /,'');return`<g class="shoeReplacementMarker"><line x1="${xx}" y1="${T}" x2="${xx}" y2="${H-B}"/><circle cx="${xx}" cy="${yy}" r="7"/><text x="${xx}" y="${T-18-i%2*24}" text-anchor="middle">Replace ${esc(name)} · ${esc(shoeChartDate(e.date))}</text></g>`}).join('');
 ...
 const legend=series.map((s,i)=>`<label class="shoeLegendToggle"><input type="checkbox" data-shoe-chart-toggle="${esc(s.shoe.id)}" checked><span class="shoeLegendSwatch" style="--shoe-color:${palette[i%palette.length]}"></span>${esc(shoeDisplayName(s.shoe))}</label>`).join('');
 ...
 const raceStrategyCard=raceStrategy?`<div class="shoeRaceStrategyCard"><small>RACE-DAY PAIR STRATEGY</small><b>${esc(raceStrategy.name)}</b><span>${raceStrategy.owned?'Current physical pair retained':'New physical pair proposed'} · projected ~${Math.round(raceStrategy.finalKm)} km on ${esc(shoeChartDate(raceDate))}</span><strong>Desired race-day window ${Math.round(raceStrategy.window?.minKm||0)}–${Math.round(raceStrategy.window?.maxKm||0)} km</strong>${!raceStrategy.owned&&raceLifecycle.purchaseStart&&raceLifecycle.purchaseEnd?`<em>Proposed purchase window ${esc(shoeChartDate(raceLifecycle.purchaseStart))} – ${esc(shoeChartDate(raceLifecycle.purchaseEnd))}.</em>`:''}</div>`:'';
 return`<div class="shoeChartWrap shoeProgrammeChart"><div class="shoeChartRaceDate"><small>PHYSICAL-PAIR PLAN THROUGH RACE DAY</small><b>Race day · ${esc(shoeChartDate(raceDate))}</b></div>${raceStrategyCard}<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Physical shoe pairs, purchase dates, planned replacements and race-day pair mileage through race day">${targetBand}<g class="shoeGrid">${grid}</g><line class="shoeTodayLine" x1="${todayX}" y1="${T}" x2="${todayX}" y2="${H-B}"/><line class="shoeRaceLine" x1="${raceX}" y1="${T}" x2="${raceX}" y2="${H-B}"/>${paths}${replacementProjection}${raceStrategyPath}${markers}${xlabels}<text class="yTitle" transform="translate(32 ${H/2}) rotate(-90)" text-anchor="middle">Accumulated km</text></svg><div class="shoeChartKey"><span><i class="actual"></i>Observed current pair</span><span><i class="planned"></i>Planned current-pair mileage</span><span><i class="replace"></i>Planned replacement</span><span><i class="newpair"></i>Future training pair</span><span><i class="raceshoe"></i>Race-day pair</span><span><i class="raceband"></i>Desired race-day window</span></div><div class="shoeChartLegend">${legend}</div>${shoeGraphRationaleHtml(series,rawEvents,raceLifecycle)}${eventHtml}</div>`
}
