...
const BUILD=9350, SCHEMA=9350, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function coachReportHtml(report,compact=false){
 ...
 const ast=report.athleteState,decision=`${ast.adjustment.toFixed(2)} · ${ast.direction}${ast.pct?` ${ast.pct}%`:''}`;
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.3.5 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.3.5 stable build 9350');
})();