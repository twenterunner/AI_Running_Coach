...
const SHOE_ROTATION_PRIORITIES={
 'purchase-spacing':{label:'Purchase spacing priority',short:'Purchase spacing',desc:'Push harder toward the selected purchase-interval target. The engine may accept a larger shoe-suitability compromise on selected quality sessions when lifecycle and Race Day safety remain sound.'},
 'quality-session':{label:'Quality-session priority',short:'Quality-session matching',desc:'Protect the best shoe match for threshold, intervals, race-specific sessions and key long runs. The engine still improves purchase spacing, but accepts a shorter interval sooner rather than materially compromising those sessions.'}
};
function shoeRotationPriority(){const key=state.setup?.shoeRotationPriority;return SHOE_ROTATION_PRIORITIES[key]?key:'purchase-spacing'}
function shoeRotationPriorityProfile(){return SHOE_ROTATION_PRIORITIES[shoeRotationPriority()]}
function shoePurchaseIntervalWeeks(){const n=Math.round(Number(state.setup?.shoePurchaseIntervalWeeks)||12);return clamp(n,4,24)}
function shoePurchaseIntervalDays(){return shoePurchaseIntervalWeeks()*7}
...
function shoeEngineStrategyHtml(){const priority=shoeRotationPriority(),profile=shoeRotationPriorityProfile(),weeks=shoePurchaseIntervalWeeks();return`<div class="shoeApproachControl shoeApproachFixed"><label>Shoe strategy</label><strong>SESSION SUITABILITY</strong><small>${esc(SHOE_ENGINE_STRATEGY.desc)}</small><div class="shoeStrategyControls"><div class="shoePriorityControl"><label for="shoeRotationPriority">Rotation preference</label><select id="shoeRotationPriority"><option value="purchase-spacing" ${priority==='purchase-spacing'?'selected':''}>Purchase spacing priority</option><option value="quality-session" ${priority==='quality-session'?'selected':''}>Quality-session priority</option></select><div class="shoePriorityActive">ACTIVE: ${esc(profile.short.toUpperCase())}</div><small>${esc(profile.desc)}</small></div><div class="shoeIntervalControl"><label for="shoePurchaseIntervalWeeks">Target purchase interval</label><div class="shoeIntervalInput"><select id="shoePurchaseIntervalWeeks"><option value="4" ${weeks===4?'selected':''}>4 weeks</option><option value="8" ${weeks===8?'selected':''}>8 weeks</option><option value="12" ${weeks===12?'selected':''}>12 weeks</option><option value="16" ${weeks===16?'selected':''}>16 weeks</option><option value="20" ${weeks===20?'selected':''}>20 weeks</option><option value="24" ${weeks===24?'selected':''}>24 weeks</option></select></div><small>Soft target between successive planned shoe purchases. The engine may deviate when lifecycle, safe rotation, Race Day readiness or the selected session-priority policy requires it.</small></div></div></div>`}
...
function shoeEngineRaceStartKm(life,pair){
 if(!pair)return 0;
 // Use the final lifecycle ledger first: this is the same source used to build
 // the graph after purchase dates and assignments have been canonicalised.
 const pts=(pair.points||[]).filter(p=>!p.raceDay).slice().sort((a,b)=>String(a.date).localeCompare(String(b.date)));
 const raceDate=life?.raceDate||state.setup?.raceDate||'9999-12-31';
 const eligible=pts.filter(p=>String(p.date)<=String(raceDate));
 if(eligible.length){
  const last=eligible[eligible.length-1];
  const km=Number(last.km);
  if(Number.isFinite(km))return km;
 }
 // Fallback to final assigned mileage + current logged mileage.
...
const SHOE_ENGINE_RULES=Object.freeze({
 ...
 preferredPurchaseGapDays:shoePurchaseIntervalDays(),
 purchaseGapIsSoftTarget:true,
 noArtificialGraphFlats:true
});
function shoeLifecycleConfig(){return{minFit:55,targetActivePairs:2,maxOrdinaryPairs:3,preferredPurchaseGapDays:shoePurchaseIntervalDays(),minPurchaseGapDays:0,purchaseLeadDays:0}}
...
function shoeEnginePurchaseProfile(trigger,sessions,pairs,role='training'){
 const start=dte(trigger.date),end=new Date(start.getTime()+(role==='race'?56:42)*DAY),horizon=sessions.filter(s=>s.date>=trigger.date&&dte(s.date)<=end),candidates=candidatePurchaseProfiles().filter(p=>role!=='race'||lifecycleRoles(p).has('race'));
 ...
 return candidates.map(profile=>{let weighted=0,w=0;for(const session of (horizon.length?horizon:[trigger])){const rehab=Boolean(session.rehab),imp=shoeEngineSessionImportance(session,{rehab}),a=shoeSuitabilityAssessment(profile,session,{rehab,projectedKm:Number(session.distance)||0,retireKm:lifecycleRetireKmForProfile(profile)});weighted+=a.score*imp;w+=imp}let score=weighted/Math.max(1,w);if(!activeFamilies.has(normalizeShoeFamily(profile.family)))score+=2;if(Number(profile.versatility||3)>=4)score+=2;return{profile,score}}).sort((a,b)=>b.score-a.score)[0]?.profile||lifecycleChoosePurchaseProfile(trigger,role)
}
function shoeEngineFutureModelReason(profile,role='training'){
 ...
 else if(roles.has('race'))parts.push('race-capable');
...
function shoeEnginePrepareWeekPortfolio(weekSessions,pairs,allSessions,purchases,events,now){
 ...
 while(active.length<SESSION_SHOE_RULES.targetMinPairs&&allSessions.some(x=>x.date>=first&&Number(x.distance)>0)){
  const np=shoeEngineCreatePair(weekSessions[0],allSessions,pairs,purchases,events,'training','A replacement is required now so at least two actively used serviceable pairs remain in the rotation.');if(!np)break;active=pairs.filter(p=>p.role!=='race'&&shoeEngineIsAvailable(p,first)&&shoeEngineCanReachWeeklyShareAtWeekStart(p,weekSessions,target,first));
 }
 return active
}
...
function shoeEngineRepairPurchaseSpacing(result,allSessions,manual){
 const preferred=Number(shoeLifecycleConfig().preferredPurchaseGapDays||shoePurchaseIntervalDays());if(preferred<=0||!result?.purchases?.length)return;
 // Authoritative final pass: discard any stale spacing metadata before evaluating the final ledger.
 for(const b of result.purchases){b.spacingAdjusted=false;b.spacingReason='';b.spacingOverride=false}
 const shiftDate=(date,days)=>iso(new Date(dte(date).getTime()+days*DAY));
 const gapDays=(a,b)=>Math.round((dte(b)-dte(a))/DAY);
 const snapshot=()=>({
  assignments:result.assignments.map(a=>({...a})),
  pairAssignments:new Map(result.pairs.map(p=>[p.id,(p.assignments||[]).map(a=>({...a}))])),
  pairDates:new Map(result.pairs.map(p=>[p.id,{purchaseDate:p.purchaseDate,availableDate:p.availableDate,plannedEntryDate:p.plannedEntryDate}])),
  purchaseDates:new Map(result.purchases.map(p=>[p.pairId,{purchaseDate:p.purchaseDate,firstUseDate:p.firstUseDate,spacingAdjusted:p.spacingAdjusted,spacingReason:p.spacingReason,spacingOverride:p.spacingOverride}]))
 });
 const restore=snap=>{
  result.assignments.splice(0,result.assignments.length,...snap.assignments.map(a=>({...a})));
  for(const p of result.pairs){
   p.assignments=(snap.pairAssignments.get(p.id)||[]).map(a=>result.assignments.find(x=>x.planId===a.planId&&x.date===a.date)||{...a});
   const d=snap.pairDates.get(p.id)||{};p.purchaseDate=d.purchaseDate;p.availableDate=d.availableDate;p.plannedEntryDate=d.plannedEntryDate;
  }
  for(const b of result.purchases){const d=snap.purchaseDates.get(b.pairId)||{};b.purchaseDate=d.purchaseDate;b.firstUseDate=d.firstUseDate;b.spacingAdjusted=d.spacingAdjusted;b.spacingReason=d.spacingReason;b.spacingOverride=d.spacingOverride}
 };
 const rotationPriority=shoeRotationPriority();
 const isQualityPlan=plan=>shoeEngineSessionImportance(plan,{rehab:Boolean(plan?.rehab)})>=86;
 const moveScoreLoss=(fromPair,toPair,plan,row)=>{
  const from=fromPair?shoeEngineAssessment(fromPair,plan,{rehab:Boolean(row.rehab)}).score:100;
  const to=shoeEngineAssessment(toPair,plan,{rehab:Boolean(row.rehab)}).score;
  return Math.max(0,from-to);
 };
 const moveAllowed=(fromPair,toPair,plan,row)=>{
  const loss=moveScoreLoss(fromPair,toPair,plan,row),quality=isQualityPlan(plan);
  if(rotationPriority==='quality-session'&&quality)return false;
  const limit=rotationPriority==='quality-session'?8:(quality?18:22);
  return loss<=limit;
 };
 const moveCost=(fromPair,toPair,plan,row)=>{
  const imp=shoeEngineSessionImportance(plan,{rehab:Boolean(row.rehab)});
  const loss=moveScoreLoss(fromPair,toPair,plan,row);
  const quality=isQualityPlan(plan);
  const qualityMultiplier=quality?(rotationPriority==='quality-session'?12:1.65):(rotationPriority==='quality-session'?1.35:1);
  return imp*Math.max(1,loss)*qualityMultiplier;
 };
 const tryDelayLater=(prev,buy,minDate)=>{
  const pair=result.pairs.find(p=>p.id===buy.pairId);if(!pair)return false;
  const early=(pair.assignments||[]).filter(a=>a.date<minDate).slice().sort((a,b)=>{
   const pa=allSessions.find(x=>x.id===a.planId),pb=allSessions.find(x=>x.id===b.planId);
   return shoeEngineSessionImportance(pa||a,{rehab:Boolean(a.rehab)})-shoeEngineSessionImportance(pb||b,{rehab:Boolean(b.rehab)})||a.date.localeCompare(b.date)
  });
  for(const row0 of early){
   const row=result.assignments.find(a=>a.planId===row0.planId&&a.date===row0.date);if(!row||row.runnerOverride||manual.has(row.planId))return false;
   const plan=allSessions.find(x=>x.id===row.planId);if(!plan)return false;
   const donor=result.pairs.find(p=>p.id===row.pairId);
   const alternatives=result.pairs.filter(p=>p.id!==pair.id&&shoeEngineIsAvailable(p,row.date)&&shoeEngineCanCover(p,plan,{rehab:Boolean(row.rehab),allowRacePair:false}))
    .map(p=>({p,cost:moveCost(donor,p,plan,row),score:shoeEngineAssessment(p,plan,{rehab:Boolean(row.rehab)}).score}))
    .filter(x=>x.score>=SESSION_SHOE_RULES.safeSuitabilityFloor&&moveAllowed(donor,x.p,plan,row))
    .sort((a,b)=>a.cost-b.cost||b.score-a.score);
   let moved=false;
   for(const alt of alternatives){
    const projected=shoeEngineProjectedKm(alt.p)+Number(row.km||0);if(projected>Number(alt.p.retireKm)+1e-6)continue;
    if(shoeEngineMoveAssignment(row,alt.p,result.pairs)){row.purchaseSpacingCompromise=true;row.why=`Purchase-spacing optimisation: ${lifecyclePairLabel(alt.p)} takes this session so the later purchase can move closer to the selected ${shoePurchaseIntervalWeeks()}-week separation.`;moved=true;break}
   }
   if(!moved)return false;
  }
  const first=(pair.assignments||[]).slice().sort((a,b)=>a.date.localeCompare(b.date))[0];
  if(!first||first.date<minDate)return false;
  buy.purchaseDate=first.date;buy.firstUseDate=first.date;pair.purchaseDate=first.date;pair.availableDate=first.date;pair.plannedEntryDate=first.date;
  return true
 };
 const tryAdvanceEarlier=(purchases,index,buyDate)=>{
  if(index<1)return false;
  const prev=purchases[index-1],prevPair=result.pairs.find(p=>p.id===prev.pairId);if(!prevPair||prevPair.owned)return false;
  const prevDate=prev.purchaseDate||prev.firstUseDate;if(!prevDate)return false;
  const desiredLatest=shiftDate(buyDate,-preferred);
  // Do not create a new purchase-interval conflict with the purchase before this one.
  const prior=index>=2?purchases[index-2]:null,priorDate=prior?(prior.purchaseDate||prior.firstUseDate):null;
  const lowerBound=priorDate?shiftDate(priorDate,preferred):shiftDate(desiredLatest,-28);
  if(lowerBound>desiredLatest)return false;
  const originalDates={purchaseDate:prevPair.purchaseDate,availableDate:prevPair.availableDate,plannedEntryDate:prevPair.plannedEntryDate};
  // Make the earlier pair provisionally available across the candidate window.
  prevPair.purchaseDate=lowerBound;prevPair.availableDate=lowerBound;prevPair.plannedEntryDate=lowerBound;
  const candidates=result.assignments.filter(row=>row.date>=lowerBound&&row.date<prevDate&&row.pairId!==prevPair.id&&!row.runnerOverride&&!manual.has(row.planId))
   .map(row=>{const plan=allSessions.find(x=>x.id===row.planId),donor=result.pairs.find(p=>p.id===row.pairId);if(!plan||!shoeEngineCanCover(prevPair,plan,{rehab:Boolean(row.rehab),allowRacePair:false}))return null;const score=shoeEngineAssessment(prevPair,plan,{rehab:Boolean(row.rehab)}).score;if(score<SESSION_SHOE_RULES.safeSuitabilityFloor||!moveAllowed(donor,prevPair,plan,row))return null;return{row,plan,donor,cost:moveCost(donor,prevPair,plan,row)}})
   .filter(Boolean)
   .sort((a,b)=>{
    // Prefer a first-use date close to the selected purchase-interval boundary, then lower suitability cost.
    const ad=Math.abs(gapDays(a.row.date,desiredLatest)),bd=Math.abs(gapDays(b.row.date,desiredLatest));
    return ad-bd||a.cost-b.cost||a.row.date.localeCompare(b.row.date)
   });
  const anchor=candidates.find(x=>x.row.date<=desiredLatest);
  if(!anchor){prevPair.purchaseDate=originalDates.purchaseDate;prevPair.availableDate=originalDates.availableDate;prevPair.plannedEntryDate=originalDates.plannedEntryDate;return false}
  if(!shoeEngineMoveAssignment(anchor.row,prevPair,result.pairs)){prevPair.purchaseDate=originalDates.purchaseDate;prevPair.availableDate=originalDates.availableDate;prevPair.plannedEntryDate=originalDates.plannedEntryDate;return false}
  anchor.row.purchaseSpacingCompromise=true;anchor.row.why=`Purchase-spacing optimisation: ${lifecyclePairLabel(prevPair)} is introduced earlier on this session to increase the gap to the following planned purchase.`;
  const newFirst=(prevPair.assignments||[]).slice().sort((a,b)=>a.date.localeCompare(b.date))[0];
  if(!newFirst){prevPair.purchaseDate=originalDates.purchaseDate;prevPair.availableDate=originalDates.availableDate;prevPair.plannedEntryDate=originalDates.plannedEntryDate;return false}
  // An advanced purchase must become a real rotation member immediately.
  // From first use until the original need date, target >=25% of each week's shoe mileage
  // whenever that week contains more than one shoe-bearing session.
  const earlySnapshot=snapshot();
  const earlyWeeks=[...new Set(allSessions.filter(s=>s.date>=newFirst.date&&s.date<prevDate&&Number(s.distance||0)>0).map(s=>shoeEngineWeekKey(s.date)))].sort();
  let sustained=true;
  for(const wk of earlyWeeks){
   const weekSessions=allSessions.filter(s=>shoeEngineWeekKey(s.date)===wk&&s.date>=newFirst.date&&s.date<prevDate&&Number(s.distance||0)>0);
   if(!weekSessions.length)continue;
   const totalKm=weekSessions.reduce((n,s)=>n+Number(s.distance||0),0),targetKm=SESSION_SHOE_RULES.weeklyMinimumShare*totalKm;
   let pairKm=(prevPair.assignments||[]).filter(a=>shoeEngineWeekKey(a.date)===wk&&a.date>=newFirst.date&&a.date<prevDate).reduce((n,a)=>n+(Number(a.km)||0),0);
   if(weekSessions.length===1){
    if(pairKm<=0){sustained=false;break}
    continue;
   }
   const weekCandidates=result.assignments.filter(row=>shoeEngineWeekKey(row.date)===wk&&row.date>=newFirst.date&&row.date<prevDate&&row.pairId!==prevPair.id&&!row.runnerOverride&&!manual.has(row.planId))
    .map(row=>{const plan=allSessions.find(x=>x.id===row.planId),donor=result.pairs.find(p=>p.id===row.pairId);if(!plan||!shoeEngineCanCover(prevPair,plan,{rehab:Boolean(row.rehab),allowRacePair:false}))return null;const score=shoeEngineAssessment(prevPair,plan,{rehab:Boolean(row.rehab)}).score;if(score<SESSION_SHOE_RULES.safeSuitabilityFloor||!moveAllowed(donor,prevPair,plan,row))return null;return{row,plan,donor,cost:moveCost(donor,prevPair,plan,row)}})
    .filter(Boolean).sort((a,b)=>a.cost-b.cost||a.row.date.localeCompare(b.row.date));
   for(const x of weekCandidates){
    if(pairKm+1e-6>=targetKm)break;
    if(shoeEngineProjectedKm(prevPair)+Number(x.row.km||0)>Number(prevPair.retireKm)+1e-6)continue;
    if(shoeEngineMoveAssignment(x.row,prevPair,result.pairs)){
     x.row.purchaseSpacingCompromise=true;
     x.row.why=`Purchase-spacing priority: ${lifecyclePairLabel(prevPair)} takes this session so an earlier purchase becomes a genuinely active rotation pair rather than a token early use.`;
     pairKm+=Number(x.row.km)||0;
    }
   }
   if(pairKm+1e-6<targetKm){sustained=false;break}
  }
  if(!sustained){restore(earlySnapshot);return false}
  prev.purchaseDate=newFirst.date;prev.firstUseDate=newFirst.date;prevPair.purchaseDate=newFirst.date;prevPair.availableDate=newFirst.date;prevPair.plannedEntryDate=newFirst.date;
  prev.spacingAdjusted=true;prev.spacingReason=`Purchase brought forward to ${newFirst.date} so the next planned purchase is better separated. The selected rotation policy accepts this earlier date only because the pair can be used as a genuine rotation member from first use onward, targeting at least 25% of weekly shoe mileage rather than receiving token early mileage.`;
  return true
 };
 const purchases=result.purchases.slice().sort((a,b)=>(a.purchaseDate||a.firstUseDate||'').localeCompare(b.purchaseDate||b.firstUseDate||''));
 for(let i=1;i<purchases.length;i++){
  const prev=purchases[i-1],buy=purchases[i],prevDate0=prev.purchaseDate||prev.firstUseDate,buyDate0=buy.purchaseDate||buy.firstUseDate;if(!prevDate0||!buyDate0)continue;
  const initialGap=gapDays(prevDate0,buyDate0);if(initialGap>=preferred)continue;
  if(prev.bootstrapRotation&&buy.bootstrapRotation){buy.spacingOverride=true;buy.spacingReason=`Initial rotation bootstrap: two physical pairs are required immediately because no owned running shoes are available. The selected purchase-interval target starts with the next replacement purchase.`;continue}
  const snap=snapshot();
  const minDate=shiftDate(prevDate0,preferred);
  let solved=tryDelayLater(prev,buy,minDate);
  if(!solved){
   restore(snap);
   // If purchase 2 cannot move far enough, pull purchase 1 forward instead.
   solved=tryAdvanceEarlier(purchases,i,buyDate0);
  }
  // Recalculate the actual gap after either strategy.
  const prevDate=prev.purchaseDate||prev.firstUseDate,buyDate=buy.purchaseDate||buy.firstUseDate,finalGap=prevDate&&buyDate?gapDays(prevDate,buyDate):initialGap;
  if(finalGap>=preferred){
   buy.spacingAdjusted=true;
   buy.spacingReason=`${shoeRotationPriorityProfile().label}: purchase-spacing optimisation increased the separation from ${initialGap} to ${finalGap} days, meeting the selected ~${shoePurchaseIntervalWeeks()}-week target.`;
   buy.spacingOverride=false;
  }else if(finalGap>initialGap){
   buy.spacingAdjusted=true;
   buy.spacingOverride=true;
   buy.spacingReason=`${shoeRotationPriorityProfile().label}: purchase-spacing optimisation increased the separation from ${initialGap} to ${finalGap} days. ${rotationPriority==='quality-session'?'The remaining gap is accepted because key quality/race-specific sessions are protected from purchase-spacing reassignment.':'The full ~${shoePurchaseIntervalDays()}-day target is not feasible without an excessive lifecycle, suitability, runner-override or Race Day compromise.'}`;
  }else{
   restore(snap);
   buy.spacingOverride=true;
   buy.spacingReason=`${shoeRotationPriorityProfile().label}: ${rotationPriority==='quality-session'?'the engine attempted both delaying the later purchase and bringing the earlier purchase forward using only safe lower-value mileage. Key quality/race-specific sessions remained within the 2-point global suitability guard, so any remaining shorter gap is genuinely unavoidable.':'the selected ~${shoePurchaseIntervalWeeks()}-week purchase gap cannot be improved without an excessive lifecycle, sustained-use, session-suitability, runner-override or Race Day compromise.'}`;
  }
 }
}
...
function shoeEngineRaceCandidate(pair,racePlan){if(!shoeEngineIsAvailable(pair,racePlan.date))return null;const a=shoeSuitabilityAssessment(pair.profile,racePlan,{shoe:pair.owned?pair.shoe:null,projectedKm:shoeEngineProjectedKm(pair),retireKm:pair.retireKm}),window=lifecycleRaceTargetWindow(pair.profile,racePlan.distance),km=shoeEngineProjectedKm(pair),remaining=Number(pair.retireKm)-km;return{pair,score:a.score,window,preRaceKm:km,remaining,valid:a.score>=58&&km<SESSION_SHOE_RULES.raceDayMaximumKm&&remaining>=Number(racePlan.distance)+SESSION_SHOE_RULES.minimumRaceLifeAfterStartKm}}
function shoeEngineFamiliariseRacePair(candidate,racePlan,allSessions,pairs,assignments,manual){
 if(!candidate)return null;const {pair,window}=candidate;let km=shoeEngineProjectedKm(pair);if(km>=window.minKm)return shoeEngineRaceCandidate(pair,racePlan);
...
function shoeEngineSelectRacePair(racePlan,allSessions,pairs,assignments,purchases,events,manual){
 ...
 const reserveCandidates=ranked.filter(c=>c.score>=58&&c.preRaceKm>=SESSION_SHOE_RULES.raceDayMaximumKm&&c.remaining>=Number(racePlan.distance)+SESSION_SHOE_RULES.minimumRaceLifeAfterStartKm).sort((a,b)=>(Number(b.pair.owned)-Number(a.pair.owned))||b.score-a.score);
 ...
 const close=iso(new Date(dte(racePlan.date).getTime()-SESSION_SHOE_RULES.raceFamiliarisationLookbackDays*DAY)),late=allSessions.filter(s=>s.date>=close&&s.date<racePlan.date&&!s.rehab&&!manual.has(s.id)).sort((a,b)=>a.date.localeCompare(b.date));
 if(!late.length)return null;
 // Dedicated Race Day planning is transactional. Try late entry first and progressively move earlier.
 // A failed trial is fully rolled back; it can never contaminate the returned lifecycle.
 const racePool=raceOverride?[raceOverride]:candidatePurchaseProfiles().filter(p=>lifecycleRoles(p).has('race'));const raceProfiles=racePool.map(profile=>({profile,score:shoeSuitabilityAssessment(profile,racePlan,{projectedKm:0,retireKm:lifecycleRetireKmForProfile(profile)}).score})).filter(x=>x.score>=SESSION_SHOE_RULES.safeSuitabilityFloor).sort((a,b)=>b.score-a.score).slice(0,6);
 const weeks=[...new Set(late.map(s=>shoeEngineWeekKey(s.date)))].sort();
 const snapshot=()=>JSON.stringify({pairs,assignments,purchases,events});
 const restore=raw=>{const x=JSON.parse(raw);pairs.splice(0,pairs.length,...x.pairs);assignments.splice(0,assignments.length,...x.assignments);purchases.splice(0,purchases.length,...x.purchases);events.splice(0,events.length,...x.events)};
 const base=snapshot();
 for(const entryWeek of weeks.slice().reverse()){
  const first=late.find(s=>shoeEngineWeekKey(s.date)===entryWeek);if(!first)continue;
  for(const rp of raceProfiles){
   restore(base);
   const profile=rp.profile,id=`future:${purchases.length+1}:${lifecycleProfileKey(profile)}:race`,retireKm=lifecycleRetireKmForProfile(profile),pair={id,owned:false,shoe:null,profile,currentKm:0,km:0,plannedEntryDate:first.date,availableDate:first.date,purchaseDate:first.date,retireKm,lifeKm:retireKm,assignments:[],points:[{date:first.date,km:0,purchase:true}],role:'race'};
   pairs.push(pair);purchases.push({id:`purchase:${id}`,pairId:id,profile,purchaseDate:first.date,firstUseDate:first.date,role:'race',reason:'A dedicated Race Day pair is required because no existing physical pair can reach Race Day with suitable race fit, enough familiarity, less than 250 km and comfortable remaining lifecycle.',replacesPairId:null});events.push({date:first.date,type:'pair-entry',pairId:id,text:`${futureProfileDisplayName(profile)} enters at its first required Race Day familiarisation use.`});
   let feasible=true;
   for(const wk of weeks.filter(w=>w>=entryWeek)){
    const sessions=allSessions.filter(s=>shoeEngineWeekKey(s.date)===wk&&s.date>=first.date&&s.date<racePlan.date).sort((a,b)=>a.date.localeCompare(b.date)||b.importance-a.importance);if(!sessions.length)continue;
    const r=shoeEngineWeeklyRebalance(sessions,pairs,assignments,manual,new Set([pair.id]));if(!r.feasible){feasible=false;break}
   }
   if(!feasible)continue;
   const c=shoeEngineRaceCandidate(pair,racePlan);
   if(!c||!c.valid||c.preRaceKm<c.window.minKm||c.preRaceKm>c.window.maxKm)continue;
   for(const row of pair.assignments.filter(a=>a.date>=first.date&&a.date<racePlan.date)){row.raceFamiliarisation=true;row.why=`Race Day familiarisation within the normal active rotation: ${lifecyclePairLabel(pair)} receives at least its required weekly share while the selected 2–3 pair portfolio remains feasible.`}
   return c
  }
 }
 restore(base);return null
}
...
function freshShoeLifecyclePlan(){
 const now=iso(today()),raceDate=state.setup?.raceDate||now,manual=new Map((state.plannedShoeAssignments||[]).filter(a=>a.source==='user').map(a=>[a.planId,a.shoeId])),fixed=(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>=now&&p.date<=raceDate).map(p=>({...p,rehab:false,importance:shoeEngineSessionImportance(p)})).sort((a,b)=>a.date.localeCompare(b.date)),rehab=shoeEngineBuildRehabSessions(now,raceDate),allSessions=[...fixed,...rehab].sort((a,b)=>a.date.localeCompare(b.date)||Number(b.rehab)-Number(a.rehab)),injury=(state.injuries||[]).find(x=>x.id===state.activeInjuryPlanId),rehabStamp=injury?JSON.stringify({id:injury.id,plannedRehabShoes:injury.plannedRehabShoes||{},checks:(injury.checkIns||[]).map(c=>[c.date,c.walkMinutes,c.runMinutes,c.rehabShoeId,c.pain,c.hop,c.bridge]),updatedAt:injury.updatedAt||injury.lastUpdated||''}):'',stamp=['race-first-spacing-v17-quality-dormancy-global-spacing',state.storageRevision||0,runnerFootMechanics(),shoeRotationPriority(),shoePurchaseIntervalWeeks(),JSON.stringify(shoeFuturePairOverrideKeys()),String(state.setup?.shoeRacePairOverride||''),raceDate,OFFLINE_ASICS_CATALOGUE_VERSION,fixed.map(p=>`${p.id}:${p.date}:${p.type}:${p.distance}:${p.surface||''}`).join(';'),(state.shoes||[]).map(sh=>`${sh.id}:${sh.status}:${shoeMileage(sh).toFixed(2)}:${sh.condition||sh.conditionFeedback||''}`).join(';'),[...manual].join(';'),rehabStamp].join('|');
 ...
 // Guaranteed feasible Race Day fallback: if optimisation cannot prepare an existing pair, create the best safe race-capable physical pair.
 if(!raceChoice){const raceOverride=shoeRacePairOverrideProfile(),fallbackPool=raceOverride?[raceOverride]:candidatePurchaseProfiles().filter(p=>lifecycleRoles(p).has('race'));const rp=fallbackPool.map(profile=>({profile,score:shoeSuitabilityAssessment(profile,racePlan,{projectedKm:0,retireKm:lifecycleRetireKmForProfile(profile)}).score})).filter(x=>x.score>=58).sort((a,b)=>b.score-a.score)[0];if(rp){const entry=raceDate,id=`future:${purchases.length+1}:${lifecycleProfileKey(rp.profile)}:race-fallback`,retireKm=lifecycleRetireKmForProfile(rp.profile),pair={id,owned:false,shoe:null,profile:rp.profile,currentKm:0,km:0,availableDate:entry,plannedEntryDate:entry,purchaseDate:entry,retireKm,lifeKm:retireKm,assignments:[],points:[{date:entry,km:0,purchase:true}],role:'race',raceReserved:true};pairs.push(pair);purchases.push({pairId:id,role:'race',purchaseDate:entry,firstUseDate:entry,reason:'A new Race Day physical pair is required only because no owned or already-planned instance of the selected race model can safely cover the race.'});events.push({type:'purchase',date:entry,pairId:id,reason:'No reusable physical Race Day pair available'});raceChoice=shoeEngineRaceCandidate(pair,racePlan)}}
 ...
 // Final preference-specific spacing pass, followed by continuity repair so a new pair cannot sit dormant.
 shoeEngineRepairPurchaseSpacing(postRaceResult,allSessions,manual);
 ...
 shoeEngineRepairActiveRotation(postRaceResult,allSessions,manual,purchases,events,now);
 shoeEngineRepairQualitySessionGuard(postRaceResult,allSessions,manual);
 // Re-run spacing once after final rotation membership is stable; quality-session
 // moves remain protected by the global <=2 guard.
 shoeEngineRepairPurchaseSpacing(postRaceResult,allSessions,manual);
 const result={strategy:'session-suitability',now,raceDate,fixed,allSessions,pairs,assignments,purchases,events,racePair:raceChoice?.pair||null,raceWindow:raceChoice?.window||null,catalogueSource:'offline',catalogueVersion:OFFLINE_ASICS_CATALOGUE_VERSION,footMechanics:runnerFootMechanics(),engine:'race-first-spacing-v17-quality-dormancy-global-spacing'};
 if(result.racePair&&!shoeEngineFinalizeRaceDay(result))result.racePair=null;
 ...
 result.purchases=result.purchases.filter(b=>{const p=result.pairs.find(x=>x.id===b.pairId);return p&&(p.assignments.length||p===result.racePair)}).map(b=>{
  ...
  b.reason=String(b.reason||'')
   ...
   .trim();
  if(b.spacingReason)b.reason=(b.reason?b.reason+' ':'')+b.spacingReason;
  return b
...
function shoeGraphForecastPoints(pair,life){
 ...
 if(pair===life.racePair){
  const raceKm=Math.max(0,Number(state.setup?.raceDistance)||42.195),preRaceKm=km;
  points.push({date:life.raceDate,km:preRaceKm+raceKm,raceDay:true,preRaceKm});
 }
 return points
}
...
function shoeRotationCard(shoe){
 ...
 const purchaseContent=buy.scheduled?`<div class="shoePurchaseCountdownHead"><small>NEXT PAIR</small><b>${esc(successor)}</b></div><div class="shoePurchaseCountdownGrid"><div><small>BUY WHEN NEEDED</small><strong>${esc(shoeChartDate(buy.purchaseDate))}</strong><span>${buy.daysRemaining===0?'Needed now':`${buy.daysRemaining} day${buy.daysRemaining===1?'':'s'} remaining`}</span></div><div><small>UNTIL HANDOVER</small><strong>~${Math.round(buy.kmUntilPurchase)} km more on this pair</strong><span>Projected total ~${Math.round(buy.handoverKm)} km · ${esc(buy.confidence)} confidence</span></div></div><p>Planned handover: <b>${esc(shoeChartDate(buy.handoverDate))}</b> at about <b>${Math.round(buy.handoverKm)} km</b>. The replacement is bought when its first meaningful use starts, not months in advance.</p>`:`<div class="shoePurchaseCountdownHead"><small>NEXT PURCHASE</small><b>No purchase required before Race Day</b></div><div class="shoePurchaseNoNeed"><small>LIKELY SUCCESSOR WHEN EVENTUALLY NEEDED</small><strong>${esc(successor)}</strong><p>The current fixed programme does not require this physical pair to be replaced before Race Day, so no artificial purchase date or mileage countdown is shown.</p></div>`;
 return`<article class="shoeRotationCard ${shoeStatusClass(f.status)}" data-shoe-detail="${esc(shoe.id)}" tabindex="0"><div class="shoeCardHead"><span>${todayPictogram('shoe')}</span><div><small>${esc(shoe.brand||'')}</small><h4>${esc([shoe.model,shoe.version].filter(Boolean).join(' '))}</h4>${shoe.nickname?`<p>${esc(shoe.nickname)}</p>`:''}</div><span class="shoeStatus ${shoeStatusClass(f.status)}">${esc(f.label)}</span></div><div class="shoeBestAt"><small>BEST AT</small><b>${esc(best)}</b></div><div class="shoeCardMetrics"><div><small>CURRENT MILEAGE</small><b>${Math.round(f.km)} km</b></div><div><small>PRIMARY USE</small><b>${esc(SHOE_ROLE_LABELS[primary]||primary)}</b></div><div><small>EXPECTED WINDOW</small><b>${Math.round(f.low)}–${Math.round(f.high)} km</b></div><div><small>EST. REMAINING</small><b>~${Math.round((f.remainingLow+f.remainingHigh)/2)} km</b></div></div><div class="shoePurchaseCountdown">${purchaseContent}</div><div class="shoeCardFooter"><span>Projected replacement period</span><b>${esc(shoeForecastDateText(f))}</b></div></article>`;
}
...
function renderShoes(){const root=$('shoesContent');if(!root)return;reconcileShoeUsage();ensureShoeAutoAssignments();if(!root.closest('.page')?.classList.contains('active'))return;const active=(state.shoes||[]).filter(s=>s.status!=='retired');
 ...
 `;
 const recalcShoeStrategy=message=>{freshShoePlanCache={stamp:null,value:null};shoeAutoAssignmentStamp=null;invalidateShoeAssignmentCache();ensureShoeAutoAssignments();save();renderAll();toast(message)};const prioritySelect=$('shoeRotationPriority');if(prioritySelect)prioritySelect.onchange=()=>{state.setup={...state.setup,shoeRotationPriority:prioritySelect.value};recalcShoeStrategy(`${shoeRotationPriorityProfile().label} applied. Footwear allocation and purchase timing recalculated.`)};const intervalInput=$('shoePurchaseIntervalWeeks');if(intervalInput){intervalInput.onchange=()=>{const weeks=[4,8,12,16,20,24].includes(Number(intervalInput.value))?Number(intervalInput.value):12;state.setup={...state.setup,shoePurchaseIntervalWeeks:weeks};recalcShoeStrategy(`Purchase interval target set to ${weeks} weeks. Footwear allocation and purchase timing recalculated.`)}};
