...
function drawLine(canvas,series,options={}){
 ...
 if(series.some(s=>s.label)){
   ...
   series.forEach(s=>{
     ...
     ctx.fillStyle='#c7def1';ctx.fillText(s.label,x+42,38);
 ...
 if(!vals.length){
   ctx.fillStyle='#a8cfee';ctx.font='600 27px system-ui';ctx.textAlign='center';
 ...
 if(options.labels?.length){
   ...
   ctx.font='19px system-ui';ctx.fillStyle='#a8cfee';ctx.textAlign='center';
...
function drawDonut(canvas,segments,centerLabel='planned',emptyText='No training volume'){ 
 ...
 if(!total){ctx.fillStyle='#a8cfee';ctx.font='600 28px system-ui';ctx.textAlign='center';ctx.fillText(emptyText,W/2,H/2);return}
 let a=-Math.PI/2;
 ...
 ctx.fillStyle='#f7fbff';ctx.font='800 38px system-ui';ctx.textAlign='center';ctx.fillText(Math.round(total)+' km',cx,cy+8);
 ctx.font='20px system-ui';ctx.fillStyle='#a8cfee';ctx.fillText(centerLabel,cx,cy+37);
 ctx.textAlign='left';let y=82;
 segments.forEach(s=>{ctx.fillStyle=s.color;ctx.beginPath();ctx.arc(W*.58,y-6,8,0,Math.PI*2);ctx.fill();ctx.fillStyle='#dceffc';ctx.font='600 22px system-ui';ctx.fillText(s.label,W*.61,y);ctx.fillStyle='#a8cfee';ctx.font='21px system-ui';ctx.fillText(`${s.value.toFixed(1)} km · ${Math.round(s.value/total*100)}%`,W*.61,y+28);y+=65});
}
...
function drawHrvChart(){
 ...
 if(!hist.length){ctx.fillStyle='#a8cfee';ctx.font='600 27px system-ui';ctx.textAlign='center';ctx.fillText('Log previous-night Garmin HRV to start the trend',W/2,H/2);return}
