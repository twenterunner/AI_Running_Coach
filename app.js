...
function gaitPurchaseAdjustment(profile){
 const gait=runnerFootMechanics(),support=String(profile.supportType||'neutral').toLowerCase(),guided=/guided|stability|support/.test(support),neutral=!guided,cush=Number(profile.cushioning||3),comfort=Number(profile.comfort||3);
 if(gait==='pronation')return guided?8:neutral?1:0;
 if(gait==='supination')return neutral?6+(cush>=4?3:0)+(comfort>=4?2:0):guided?-10:0;
 if(gait==='neutral')return neutral?6:guided?-4:0;
 return 0
}
const SHOE_ENGINE_STRATEGIES={
 balanced:{label:'Balanced · recommended',desc:'Target two active training pairs. Use model-specific life up to 100%, keep both pairs meaningfully active, replace an outgoing pair with a short overlap, and choose the best overall compromise between workout fit, protection, familiarity and purchase restraint.'},
 quality:{label:'Performance',desc:'Target two active training pairs plus a dedicated Race Day specialist. Use training pairs up to 90% of model-specific life, keep a specialist quality-session role, and buy replacements only when they immediately enter meaningful use.'},
 protection:{label:'Protection / injury-conscious',desc:'Target two active training pairs. Use them up to 80% of model-specific life, prioritise cushioning, comfort, protection and injury/rehab compatibility, and avoid specialist speed shoes unless they are clearly justified.'}
};function normaliseShoeEngineStrategyKey(key){return key==='fewer'?'balanced':(SHOE_ENGINE_STRATEGIES[key]?key:'balanced')}function shoeEngineStrategy(){const key=normaliseShoeEngineStrategyKey(state.shoeEngineStrategy);return SHOE_ENGINE_STRATEGIES[key]}function shoeEngineStrategyHtml(){const cur=normaliseShoeEngineStrategyKey(state.shoeEngineStrategy);return`<div class="shoeApproachControl"><label for="shoeEngineStrategySelect">Shoe engine approach</label><select id="shoeEngineStrategySelect">${Object.entries(SHOE_ENGINE_STRATEGIES).map(([k,v])=>`<option value="${k}" ${cur===k?'selected':''}>${esc(v.label)}</option>`).join('')}</select><small>${esc(shoeEngineStrategy().desc)}</small></div>`}
...
function shoeRecommendation(plan,{excludeId=null}={}){
 ...
 const ranked=active.map(shoe=>{const p=shoeProfileForShoe(shoe),wear=shoeWearState(shoe),fam=shoeFamiliarity(shoe,plan.type);let weighted=0,total=0,reasons=[];
  ...
  const recent=shoeRecentKm(shoe.id,7),plannedNear=shoePlannedUsage(shoe.id,7),projectedAtWorkout=shoeMileage(shoe)+shoePlannedUsageThrough(shoe.id,plan.date);score-=Math.min(8,recent/12);score-=Math.min(6,plannedNear/18);const strategy=normaliseShoeEngineStrategyKey(state.shoeEngineStrategy),quality=/interval|Threshold|Specific|Race|Fartlek|Progression|Hills|VO₂/i.test(String(plan.type||'')),easyish=/Easy|Recovery|Steady|Medium-long/i.test(String(plan.type||''));
  if(strategy==='protection'){score+=(p.protection||3)*5+(p.cushioning||3)*4+(p.stability||3)*2;score-=Math.max(0,recent-10)*.8;score-=wear.km>=wear.low*.75?18:0}
  else if(strategy==='quality'){if(quality)score+=(p.responsiveness||3)*7+(p.efficiency||3)*5+((p.roles||[]).some(r=>['tempo','intervals','race'].includes(r))?18:0);if(easyish)score+=(p.durability||3)*4+(p.comfort||3)*3-((p.roles||[]).includes('race')?22:0)}
  else{score+=(p.durability||3)*1.5-Math.max(0,recent-18)*.25}
...
function lifecycleStrategyConfig(){
 const key=normaliseShoeEngineStrategyKey(state.shoeEngineStrategy);
 const map={
  balanced:{key,wearFraction:1.00,minFit:60,purchaseLeadDays:0,minPurchaseGapDays:42,recentUseDivisor:7,versatilityWeight:7,qualityBias:10,protectionBias:9,raceDedicatedBias:0,targetActivePairs:2},
  quality:{key,wearFraction:.90,minFit:64,purchaseLeadDays:0,minPurchaseGapDays:42,recentUseDivisor:6,versatilityWeight:2,qualityBias:30,protectionBias:4,raceDedicatedBias:12,targetActivePairs:2},
  protection:{key,wearFraction:.80,minFit:60,purchaseLeadDays:0,minPurchaseGapDays:42,recentUseDivisor:8,versatilityWeight:5,qualityBias:5,protectionBias:26,raceDedicatedBias:-5,targetActivePairs:2}
 };
...
function lifecycleRetireKmForProfile(profile,shoe=null){
 ...
 target=Math.max(Math.min(high,target),Math.min(low,high));
 const condition=shoe?.condition||shoe?.conditionFeedback||'Feels normal';
 if(condition==='Slightly worn')target-=Math.min(70,(high-low)*.22);
 if(['Noticeably flat','Grip deteriorating','Upper damaged'].includes(condition))target=Math.min(target,low);
 if(condition==='Causing discomfort')target=Math.min(target,Math.max(shoeMileage(shoe),low*.8));
...
function lifecycleWorkoutFit(profile,plan){
 ...
 score+=gaitPurchaseAdjustment(profile);
 if(cfg.key==='balanced'){
...
function lifecyclePairAssignmentScore(pair,plan){
 const cfg=lifecycleStrategyConfig(),fit=lifecycleWorkoutFit(pair.profile,plan),remaining=Math.max(0,pair.retireKm-pair.km),wearRatio=pair.retireKm>0?pair.km/pair.retireKm:0,roles=lifecycleRoles(pair.profile),quality=lifecycleIsQuality(plan.type),easy=lifecycleIsEasy(plan.type);
 let score=fit-wearRatio*(cfg.key==='protection'?32:18)-Math.min(14,lifecycleRecentKm(pair,plan.date)/Math.max(1,cfg.recentUseDivisor));
 if(cfg.key==='quality'){
  if(quality)score+=(roles.has('tempo')||roles.has('threshold')||roles.has('intervals')||roles.has('race'))?22:-18;
  if(easy&&(pair.profile.plated||roles.has('race')))score-=28;
 }
 if(cfg.key==='protection')score+=((pair.profile.cushioning||3)+(pair.profile.protection||3)+(pair.profile.comfort||3)-9)*5;
 if(cfg.key==='balanced'&&easy&&(pair.profile.plated||roles.has('race')))score-=18;
 if(remaining<(Number(plan.distance)||0))score-=100;
...
function lifecyclePruneLateRedundantPurchases(ctx,fixed,assignments,manual,racePair,raceDate){
 ...
 for(const pair of candidates){
  ...
  if(cfg.key==='quality'&&lifecycleFuturePairHasMaterialSpecialistRole(pair,fixed,pairs))continue;
...
function lifecycleRebuildPoints(pair,now,raceDate){
 ...
 const noLaterUseBeforeRace=lastPositiveDate<raceDate;
 // A pair may retire at its final planned use once it has consumed >=80% of full model life.
 // It does not need to be artificially fed mileage until the nominal strategy ceiling.
 pair.projectedRetireDate=(eligibleByMileage&&(reachesStrategyBoundary||noLaterUseBeforeRace))?lastPositiveDate:null;
...
function lifecycleEnumerateWeekAllocation(weekRuns,activePairs,totalOverride=null){
 ...
 function rec(i){
  if(i===n){
   ...
   const objective=(hard25?100000:0)+minShare*10000+score;
...
function freshShoeLifecyclePlan(){
 ...
 const stamp=['v2',state.storageRevision||0,state.shoeEngineStrategy||'balanced',runnerFootMechanics(),raceDate,OFFLINE_ASICS_CATALOGUE_VERSION,fixed.map(p=>`${p.id}:${p.date}:${p.type}:${p.distance}`).join(';'),(state.shoes||[]).map(sh=>`${sh.id}:${sh.status}:${shoeMileage(sh).toFixed(2)}:${sh.condition||sh.conditionFeedback||''}`).join(';'),[...manual].join(';'),rehabStamp].join('|');
 ...
 if(cfg.key!=='quality')raceChoice=raceCandidates.find(x=>x.preRaceKm>=x.window.minKm&&x.preRaceKm<=Math.min(250,x.window.maxKm))||null;
 ...
 raceCandidates=finalPairs.filter(p=>lifecycleRaceEligiblePair(p,racePlan)).map(p=>lifecycleRaceFit(p,racePlan)).sort((a,b)=>b.score-a.score);raceChoice=raceCandidates.find(x=>(cfg.key!=='quality'||x.pair.role==='race')&&x.preRaceKm>=x.window.minKm&&x.preRaceKm<=Math.min(250,x.window.maxKm))||raceChoice;
...
function shoeProgrammeMileageChartHtml(){
 ...
 const future=pairs.map(pair=>{const col=colorById.get(pair.id),cutoff=pair.projectedRetireDate||raceDate,start=pair.owned?todayStr:(pair.purchaseDate||pair.availableDate||todayStr),visible=(pair.points||[]).filter(pt=>pt.date>=start&&pt.date<=cutoff),last=visible.at(-1);if(pair===life.racePair&&!pair.projectedRetireDate&&last&&last.date<raceDate)visible.push({date:raceDate,km:last.km,raceEndpoint:true});const pts=visible.map(pt=>`${x(pt.date).toFixed(1)},${y(pt.km).toFixed(1)}`).join(' '),cls=pair===life.racePair?'shoeRaceStrategyLine':pair.owned?'shoePlannedLine':'shoeNewPairLine';return visible.length>=2?`<polyline class="shoeLine ${cls}" style="--shoe-color:${col}" points="${pts}"/>`:visible.length===1?`<circle class="shoeSinglePoint" style="--shoe-color:${col}" cx="${x(visible[0].date).toFixed(1)}" cy="${y(visible[0].km).toFixed(1)}" r="4"/>`:''}).join('');
...
function rehabShoePlanHtml(injury,day){
 ...
 const explicit=plannedRehabShoeId(injury,day.date),planned=freshLifecycleRehabAssignment(injury.id,day.date),selected=explicit||(planned?.pair?.owned?planned.pair.shoe.id:''),bestLabel=planned?lifecyclePairLabel(planned.pair):(rehabShoeRecommendation(day)?shoeDisplayName(rehabShoeRecommendation(day).shoe):''),exp=rehabExpectedDistance(day);
 return`<div class="rehabShoePlan"><span>${todayPictogram('shoe')}</span><div><small>SHOE</small><select data-rehab-shoe-plan="${esc(injury.id)}" data-rehab-shoe-date="${esc(day.date)}">${shoeSelectOptions(selected,true)}</select><p>${bestLabel?`Shoe plan: ${esc(bestLabel)}. `:''}Expected shoe distance ~${exp.totalKm.toFixed(1)} km${exp.runMinutes?` (${exp.walkMinutes} min walk + ${exp.runMinutes} min slow run)`:exp.walkMinutes?` (${exp.walkMinutes} min walk)`:''}. ${explicit?'Your manual shoe choice is retained.':'The same lifecycle proposal is used by the Shoes graph and daily check-in.'}</p></div></div>`
}
...
document.body.addEventListener('change',e=>{if(e.target.id==='shoeEngineStrategySelect'){
 ...
 state.shoeEngineStrategy=normaliseShoeEngineStrategyKey(e.target.value);
 freshShoePlanCache={stamp:null,value:null};
 shoeAutoAssignmentStamp=null;
 shoeAutoAssignments();
 save();
