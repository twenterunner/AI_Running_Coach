...
const BUILD=10070, SCHEMA=10070, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function rehabExecutionMeta(check){
 ...
 const map={
  ...
  completed:{label:'Rehab exercises completed as prescribed',short:'Rehab exercises completed',className:'completed',score:100,assessed:true},
  reduced:{label:'Rehab exercises partially completed',short:'Rehab exercises partially completed',className:'partial',score:65,assessed:true},
  walking_only:{label:'Walking target completed; rehab exercises not completed',short:'Walking target completed only',className:'partial',score:40,assessed:true},
 ...
 return map[status]||{label:'Rehabilitation completion was not answered',short:'Rehab not recorded',className:'unknown',score:null,assessed:false};
}
...
function buildRehabCalendar(i,p){
 ...
 for(let offset=0;offset<7;offset++){
  const date=iso(new Date(today().getTime()+offset*DAY)),fresh=rehabCalendarDay(i,p,date,offset),previous=oldMap.get(date),check=checks.get(date),checkInCompleted=!!check;let execution=rehabExecutionMeta(check);if(!check){execution={label:offset===0?'Today’s rehabilitation completion has not been reported yet':'Rehabilitation completion will be reported after this day',short:offset===0?'Rehab report pending':'Future rehab day',className:'unknown',score:null,assessed:false};}else if(!check.rehabStatus){execution={label:'The questionnaire was completed, but rehabilitation completion was not answered',short:'Rehab not answered',className:'unknown',score:null,assessed:false};}
...
function openInjuryCheck(i,existing=null){
 ...
 <section class="injuryCheckSection"><h4>3. Rehabilitation load</h4><div class="formGrid">${selectField('icRehab','How much of today’s rehabilitation plan did you complete?',prev.rehabStatus,[['','Not assessed'],['not_planned','Planned recovery day / no rehabilitation'],['completed','Rehab exercises completed as prescribed'],['stretch','Rehab exercises completed + optional stretch goal'],['reduced','Rehab exercises partially completed'],['walking_only','Walking target completed only'],['stopped','Started but stopped because of symptoms'],['not_completed','Rehab exercises not completed']])}${triSelect('icBridge','Strength exercise tolerated with control?',prev.bridge,'Tolerated','Not tolerated')}${triSelect('icHop','Impact test or jog-in-place tolerated?',prev.hop,'Tolerated','Not tolerated')}</div></section>
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v10.0.7 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v10.0.7 stable build 10070');
})();