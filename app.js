...
function renderPlan(){
 ...
 if(!state.weekView)state.weekView=currentWeek();let arr=state.plan.filter(p=>p.week===state.weekView),wd=weekData(state.weekView);const ws=weekStart(state.weekView),we=new Date(ws.getTime()+6*DAY);$('weekHeader').innerHTML=`<b>Week ${state.weekView} · ${detailedPhase(state.weekView)}</b><br><span class="muted">${ws.toLocaleDateString(undefined,{weekday:'short',day:'numeric',month:'short'})} – ${we.toLocaleDateString(undefined,{weekday:'short',day:'numeric',month:'short'})} · ${wd.planned.toFixed(1)} km planned · ${wd.actual.toFixed(1)} km completed</span>`;
...
function exerciseImageMarkup(x,family){
 ...
 const visual=img?`<button type="button" class="exerciseImageButton" data-exercise-image="${img}" data-exercise-name="${esc(x.name)}" aria-label="View ${esc(x.name)} image"><img src="${img}" alt="${esc(x.name)} exercise illustration" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><span class="exerciseFallback" style="display:none">${uiIcon('rehab')}</span><em><span aria-hidden="true">↗</span> Enlarge</em></button>`:`<span class="exerciseImageButton noImage"><span class="exerciseFallback">${uiIcon('rehab')}</span></span>`;
 return{visual,muscleChips,img,muscles};
}
function openExerciseImage(name,img,muscles=[]){
 if(!img)return;
 $('modalContent').innerHTML=`<div class="exerciseImageViewer"><div class="exerciseViewerHeading"><small>EXERCISE TECHNIQUE</small><h3>${esc(name)}</h3>${muscles.length?`<div class="exerciseMuscles viewer">${muscles.map(m=>`<i>${esc(m)}</i>`).join('')}</div>`:''}</div><img src="${img}" alt="${esc(name)} exercise technique image"><p class="exerciseViewerHint">Pinch/zoom if your device supports it. Use the visual together with the step-by-step cues, pain rule and progression criteria in the exercise card.</p></div>`;
