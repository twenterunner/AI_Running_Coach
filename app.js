...
const BUILD=10190, SCHEMA=10190, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function rehabCalendarDay(i,p,date,offset){
 ...
 if(safety){title='Protect and arrange assessment';items=['No running, hopping or impact testing','Follow professional guidance for loading'];rationale='The leading pattern or differential contains a higher-risk feature, so app-directed progression is paused.';rule='Do not progress until clinically assessed.';}
  ...
  if(stage===2&&[2,5].includes(cycleDay)){type='impact';title='Impact assessment';items=['Quiet jogging in place — 2 × 20 seconds','Controlled double-leg hops — 2 × 10 repetitions'];running='No running distance planned';rationale='This creates an explicit opportunity to assess low-level impact tolerance before walk–run progression. Two stable assessments are required for progression.';rule='Stop for pain above 2/10, altered landing, guarding or increasing tightness. Record the later and next-morning response.';}
 ...
 return{date,weekday,type,title,items,running,rationale,rule,stage,walkingTarget:walk.walkingTarget,stretchGoal:stretch.text,stretchGoalTitle:stretch.title,stretchGoalWhy:stretch.why,stretchGoalOffered:stretch.offered,bestOutcome,evidence};
}
...
function openInjuryCheck(i,existing=null){
 ...
 const scheduledMeta=plan=>{const walkingItem=(plan.items||[]).find(x=>/^Walk \d+ minutes/i.test(x))||plan.walkingTarget||'';const walkingMatch=String(walkingItem).match(/Walk (\d+) minutes/i);const walkingTargetMinutes=walkingMatch?Number(walkingMatch[1]):null;const exerciseItems=(plan.items||[]).filter(x=>x!==walkingItem&&!/^Walk \d+ minutes/i.test(x));const runningPlanned=['assessment','run'].includes(plan.type);const strengthPlanned=exerciseItems.some(x=>/bridge|strength|hinge|raise|curl|squat|isometric|loading/i.test(x));const impactPlanned=plan.type==='impact'||exerciseItems.some(x=>/hop|jog|impact/i.test(x));return{walkingPlanned:!!walkingItem,walkingItem,walkingTargetMinutes,exercisePlanned:exerciseItems.length>0,exerciseItems,runningPlanned,strengthPlanned,impactPlanned,stretchPlanned:!!walkingItem}};
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v10.0.19 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v10.0.19 stable build 10190');
})();