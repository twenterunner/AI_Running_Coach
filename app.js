...
const BUILD=9360, SCHEMA=9360, STORAGE_KEY='arc_v62_web', MIRROR_KEY='arc_v8500_web', BACKUP_KEY='arc_pre8500_backup';
...
function renderPlan(){
 const ast=athleteState(state.weekView||currentWeek());if($('fitnessEvidence')){
  ...
  const calcTitle=ast.nextAfd.status==='calculated'?`Week ${ast.nextWeek} load calculation`:`Current Week ${ast.currentWeek} applied load calculation`;
...
const brandVersion=document.querySelector('.brand-copy p');if(brandVersion)brandVersion.textContent=`Race-specific adaptive planning • v9.3.6 · build ${BUILD}`;
...
renderAll();
console.info('AI Running Coach v9.3.6 stable build 9360');
})();