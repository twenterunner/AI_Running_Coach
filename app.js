(function (root, factory) {
  ...
  function validateBackup(backup, options = {}) {
    ...
    (backup.injuries || []).forEach(injury=>(injury?.checkIns||[]).forEach(check=>{if(!injury?.id||!check?.date)return;rehabIds.add(safeId(`rehab-${injury.id}-${check.date}-walk`,''));rehabIds.add(safeId(`rehab-${injury.id}-${check.date}-run`,''));}));
...
function shoeProgrammeMileageChartHtml(){
 ...
 const series=shoes.filter(s=>enabled.has(s.id)).map(s=>{const physicalStart=shoePairStartDate(s),chartStart=physicalStart>programStart?physicalStart:programStart,actual=shoeActualProgrammeSeries(s,chartStart,todayStr).filter(p=>p.date>=physicalStart),rawPlanned=shoePlannedMileageSeries(s).filter(p=>p.date>=physicalStart),event=eventByShoe.get(s.id),planned=event?rawPlanned.filter(p=>p.date<=event.date):rawPlanned;return{shoe:s,physicalStart,actual,planned}});
 const raceLifecycle=raceShoePlan(),targetWindow=raceLifecycle?.targetWindow||raceLifecycle?.condition?.opt||null;
 const replacementSeries=rawEvents.filter(e=>enabled.has(e.shoe.id)&&e.replacement).map(e=>{const purchaseDate=plannedTrainingPurchaseDate(e),pts=[{date:purchaseDate,km:0,purchase:true,driver:'rotation'}],usageEvents=[];if(purchaseDate<e.date)pts.push({date:e.date,km:0,ready:true,driver:'rotation'});(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>e.date&&p.date<=raceDate).forEach(p=>{const assigned=plannedAssignment(p.id)?.shoeId;if(assigned!==e.shoe.id&&!e.divertedPlanIds?.includes(p.id))return;usageEvents.push({date:p.date,deltaKm:Number(p.distance)||0,type:p.type,driver:'training',portfolioDiverted:e.divertedPlanIds?.includes(p.id)})});futureRehabShoeUsage(e.shoe.id,null).filter(x=>x.date>e.date&&x.date<=raceDate).forEach(x=>usageEvents.push({date:x.date,deltaKm:Number(x.distanceKm)||0,type:'Rehab',driver:'rehab'}));usageEvents.sort((a,b)=>a.date.localeCompare(b.date)||(a.driver==='training'?-1:1));let km=0;usageEvents.forEach(u=>{km+=Math.max(0,Number(u.deltaKm)||0);pts.push({...u,km})});pts.sort((a,b)=>a.date.localeCompare(b.date)||Number(a.km)-Number(b.km));for(let i=1;i<pts.length;i++){if(Number(pts[i].km)<Number(pts[i-1].km))pts[i].km=Number(pts[i-1].km)}if(pts.at(-1)?.date!==raceDate)pts.push({date:raceDate,km,driver:'end'});return{event:e,purchaseDate,points:pts,finalKm:km}});
 ...
 const ownedLegend=series.map(s=>`<span class="shoeLegendItem"><span class="shoeLegendSwatch" style="--shoe-color:${s.color}"></span><b>OWNED</b> ${esc(shoeDisplayName(s.shoe))}</span>`).join(''),futureLegend=replacementSeries.map(item=>`<span class="shoeLegendItem future"><span class="shoeLegendSwatch dotted" style="--shoe-color:${item.color}"></span><b>FUTURE</b> ${esc(`ASICS ${item.event.replacement?.family||'running shoe'} ${item.event.replacement?.version||''}`)}</span>`).join(''),raceLegend=raceStrategy&&!raceStrategy.owned?`<span class="shoeLegendItem race"><span class="shoeLegendSwatch race" style="--shoe-color:${raceStrategy.color}"></span><b>FUTURE RACE</b> ${esc(raceStrategy.name)}</span>`:'';const legend=ownedLegend+futureLegend+raceLegend;
