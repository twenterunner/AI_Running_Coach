...
function shoeGraphRationaleHtml(series,events,raceLifecycle){const rows=series.map(item=>{const current=shoeMileage(item.shoe),planned=item.planned.at(-1)?.km??current,added=Math.max(0,planned-current),event=events.find(e=>e.shoe.id===item.shoe.id),f=shoeForecast(item.shoe);return`<li><b>${esc(shoeDisplayName(item.shoe))}:</b> ${Math.round(current)} km accumulated today. The current plan adds ~${Math.round(added)} km before this pair is replaced or race day is reached.${event?` The plan-driven replacement is ${esc(shoeChartDate(event.date))}, when the assigned workouts first take it into its expected ${Math.round(f.low)}–${Math.round(f.high)} km replacement range.`:(f.lowDate?` The usage-rate estimate reaches the start of its replacement range around ${esc(shoeChartDate(f.lowDate))}.`:' There is not enough usage evidence for a rate-based replacement estimate.')}</li>`}).join('');const race=raceLifecycle?.preferred&&raceLifecycle.condition?`<li><b>Race-day shoe target:</b> ${esc(shoeDisplayName(raceLifecycle.preferred))} is the intended race shoe. The planner targets ${Math.round(raceLifecycle.condition.opt.minKm)}–${Math.round(raceLifecycle.condition.opt.maxKm)} km on ${esc(shoeChartDate(state.setup?.raceDate))}; the current projection is ~${Math.round(raceLifecycle.condition.projected)} km.</li>`:raceLifecycle?.purchaseNeeded&&raceLifecycle.profile?`<li><b>Race-day shoe target:</b> the planner proposes ASICS ${esc(raceLifecycle.profile.family)} ${esc(raceLifecycle.profile.version)} and targets ${Math.round(raceLifecycle.targetWindow?.minKm||0)}–${Math.round(raceLifecycle.targetWindow?.maxKm||0)} km on ${esc(shoeChartDate(state.setup?.raceDate))}. Planned familiarisation currently projects ~${Math.round(raceLifecycle.projectedNew||0)} km.</li>`:'';return`<details class="shoeGraphRationale" open><summary>How to read this lifecycle plan</summary><div><p><b>Plan-driven replacement</b> is the date on which the actual future workouts assigned to a shoe first push it into its expected replacement range. <b>Usage-rate estimate</b> is a secondary comparison based on the current/planned weekly usage rate. They can differ because the plan deliberately moves kilometres between shoes.</p><p>The original shoe line stops at its plan-driven replacement point. The dotted replacement-pair line then starts at 0 km and carries the later workouts. Therefore an old trainer ending at 600 km does <b>not</b> mean the app intends to race in it at 600 km.</p><ul>${rows}${race}</ul></div></details>`}
function shoeProgrammeMileageChartHtml(){
 ...
 const raceDate=state.setup?.raceDate||iso(today()),todayStr=iso(today()),programStart=state.setup?.planStart||todayStr,rawEvents=plannedReplacementEvents(),eventByShoe=new Map(rawEvents.map(e=>[e.shoe.id,e]));
 ...
 const dates=[programStart,todayStr,raceDate,...series.flatMap(x=>x.actual.map(p=>p.date)),...series.flatMap(x=>x.planned.map(p=>p.date))].filter(Boolean).sort(),values=series.flatMap(x=>[...x.actual,...x.planned].map(p=>p.km)).filter(Number.isFinite);if(targetWindow)values.push(Number(targetWindow.maxKm)||0);if(!values.length)return'<div class="shoeEmpty">No tracked or planned mileage yet.</div>';
 const minD=dte(dates[0]).getTime(),maxD=Math.max(minD+DAY,dte(raceDate).getTime()),maxY=Math.max(10,Math.ceil(Math.max(...values)*1.14/50)*50),W=820,H=470,L=82,R=26,T=118,B=82,x=d=>L+(dte(d).getTime()-minD)/(maxD-minD)*(W-L-R),y=v=>T+(maxY-v)/maxY*(H-T-B),palette=['#3dd6c6','#79d69b','#67a7ff','#f1c56a','#b9a3ff','#ff9f9f','#7ee7ff'];
 let grid='';for(let i=0;i<=4;i++){const v=maxY*i/4,yy=y(v);grid+=`<line x1="${L}" y1="${yy}" x2="${W-R}" y2="${yy}"/><text x="${L-14}" y="${yy+6}" text-anchor="end">${Math.round(v)}</text>`}
 const targetBand=targetWindow?(()=>{const low=Math.max(0,Number(targetWindow.minKm)||0),high=Math.min(maxY,Number(targetWindow.maxKm)||0),yy1=y(high),yy2=y(low),name=raceLifecycle.purchaseNeeded&&raceLifecycle.profile?`ASICS ${raceLifecycle.profile.family} ${raceLifecycle.profile.version}`:raceLifecycle.preferred?shoeDisplayName(raceLifecycle.preferred):'race shoe';return`<g class="shoeRaceTargetBand"><rect x="${L}" y="${yy1}" width="${W-L-R}" height="${Math.max(4,yy2-yy1)}"/><text x="${L+10}" y="${Math.max(T+18,yy1+18)}">Race target · ${esc(name)} · ${Math.round(low)}–${Math.round(high)} km</text></g>`})():'';
 ...
 const events=rawEvents,replacementProjection=events.filter(e=>enabled.has(e.shoe.id)&&e.replacement&&e.date<=raceDate).map(e=>{let km=0;const pts=[{date:e.date,km:0}];(state.plan||[]).filter(p=>p.type!=='Rest'&&p.date>=e.date&&p.date<=raceDate).slice().sort((a,b)=>a.date.localeCompare(b.date)).forEach(p=>{if(plannedAssignment(p.id)?.shoeId!==e.shoe.id)return;km+=Number(p.distance)||0;pts.push({date:p.date,km})});if(pts.at(-1).date!==raceDate)pts.push({date:raceDate,km});const c=palette[Math.max(0,shoes.findIndex(s=>s.id===e.shoe.id))%palette.length],label=e.replacement?`New ${e.replacement.family} ${e.replacement.version}`:'New pair';return`<polyline class="shoeLine shoeNewPairLine" style="--shoe-color:${c}" points="${pts.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ')}"/><text class="shoeNewPairLabel" x="${x(raceDate)-8}" y="${Math.max(T+18,y(km)-8)}" text-anchor="end">${esc(label)} · ~${Math.round(km)} km</text>`}).join('');
 const racePurchaseSeries=raceLifecycle.purchaseNeeded&&raceLifecycle.profile&&raceLifecycle.famSessions?.length?(()=>{let km=0;const first=raceLifecycle.famSessions[0].date,pts=[{date:first,km:0}];raceLifecycle.famSessions.forEach(p=>{km+=Number(p.plannedShoeKm)||0;pts.push({date:p.date,km})});if(pts.at(-1).date!==raceDate)pts.push({date:raceDate,km});return`<polyline class="shoeLine shoeRacePurchaseLine" points="${pts.map(p=>`${x(p.date).toFixed(1)},${y(p.km).toFixed(1)}`).join(' ')}"/><text class="shoeRacePurchaseLabel" x="${x(raceDate)-8}" y="${Math.max(T+22,y(km)-12)}" text-anchor="end">Race shoe · ~${Math.round(km)} km</text>`})():'';
 const todayX=x(todayStr),raceX=x(raceDate),markers=events.filter(e=>enabled.has(e.shoe.id)&&e.date>=dates[0]&&e.date<=raceDate).map((e,i)=>{const xx=x(e.date),yy=Math.max(T+12,y(Math.min(maxY,e.km))),labelY=28+(i%2)*24;return`<g class="shoeReplacementMarker"><line x1="${xx}" y1="${T}" x2="${xx}" y2="${H-B}"/><circle cx="${xx}" cy="${yy}" r="7"/><text x="${Math.min(W-R-4,Math.max(L+4,xx))}" y="${labelY}" text-anchor="${xx>W*.72?'end':xx<W*.28?'start':'middle'}">Plan replacement · ${esc(e.shoe.model)} · ${esc(shoeChartDate(e.date))}</text></g>`}).join('');
 const rateMarkers=series.map(s=>({shoe:s.shoe,forecast:shoeForecast(s.shoe)})).filter(o=>o.forecast.lowDate).map((o,i)=>{const inRange=o.forecast.lowDate<=raceDate,xx=inRange?x(o.forecast.lowDate):raceX,labelY=78+(i%2)*22,anchor=inRange?(xx>W*.72?'end':xx<W*.28?'start':'middle'):'end',label=inRange?`Usage-rate estimate · ${o.shoe.model} · ${shoeChartDate(o.forecast.lowDate)}`:`Usage-rate estimate after race · ${o.shoe.model} · ${shoeChartDate(o.forecast.lowDate)}`;return`<g class="shoeNominalMarker ${inRange?'':'afterRace'}"><line x1="${xx}" y1="${T}" x2="${xx}" y2="${H-B}"/><path d="M ${xx-6} ${T+5} L ${xx+6} ${T+5} L ${xx} ${T+16} Z"/><text x="${Math.min(W-R-4,Math.max(L+4,xx))}" y="${labelY}" text-anchor="${anchor}">${esc(label)}</text></g>`}).join('');
 const xlabels=[programStart,todayStr,raceDate].filter((v,i,a)=>v&&a.indexOf(v)===i).map(d=>`<text class="xLab" x="${x(d)}" y="${H-24}" text-anchor="${d===programStart?'start':d===raceDate?'end':'middle'}">${d===todayStr?'Today · '+shoeChartDate(d):d===raceDate?'Race day · '+shoeChartDate(d):shoeChartDate(d)}</text>`).join('');
 ...
 const eventHtml=events.length?`<div class="shoeLifecycleEvents">${events.map(e=>{const f=shoeForecast(e.shoe);return`<article><small>PLAN-DRIVEN REPLACEMENT · ${esc(shoeChartDate(e.date).toUpperCase())}</small><b>Replace ${esc(shoeDisplayName(e.shoe))}</b><span>The assigned programme first reaches the ${Math.round(f.low)}–${Math.round(f.high)} km expected replacement range here.${f.lowDate?` Usage-rate comparison: ${esc(shoeChartDate(f.lowDate))}.`:''}</span>${e.replacement?`<strong>Proposed new pair: ASICS ${esc(e.replacement.family)} ${esc(e.replacement.version)}</strong>`:'<strong>Choose a suitable replacement for the same role.</strong>'}</article>`}).join('')}</div>`:'<div class="shoeEmpty compact">No shoe is currently projected to enter its replacement range before race day.</div>';
 return`<div class="shoeChartWrap shoeProgrammeChart"><div class="shoeChartRaceDate"><small>PROGRAMME HORIZON</small><b>Race day · ${esc(shoeChartDate(raceDate))}</b></div><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Actual accumulated shoe mileage versus planned mileage through race day">${targetBand}<g class="shoeGrid">${grid}</g><line class="shoeTodayLine" x1="${todayX}" y1="${T}" x2="${todayX}" y2="${H-B}"/><line class="shoeRaceLine" x1="${raceX}" y1="${T}" x2="${raceX}" y2="${H-B}"/>${paths}${replacementProjection}${racePurchaseSeries}${rateMarkers}${markers}${xlabels}<text class="yTitle" transform="translate(22 ${H/2}) rotate(-90)" text-anchor="middle">Accumulated km</text></svg><div class="shoeChartKey"><span><i class="actual"></i>Actual mileage</span><span><i class="planned"></i>Planned mileage until replacement</span><span><i class="replace"></i>Plan-driven replacement</span><span><i class="nominal"></i>Usage-rate estimate</span><span><i class="newpair"></i>Proposed replacement-pair mileage</span><span><i class="raceband"></i>Desired race-shoe mileage window</span></div><div class="shoeChartLegend">${legend}</div>${shoeGraphRationaleHtml(series,events,raceLifecycle)}${eventHtml}${raceLifecycle.purchaseNeeded&&raceLifecycle.profile?`<div class="shoeLifecycleEvents racePurchaseEvent"><article><small>${raceLifecycle.purchaseStart?`BUY AROUND ${esc(shoeChartDate(raceLifecycle.purchaseStart).toUpperCase())}`:`FUTURE PURCHASE`}</small><b>Proposed race shoe: ASICS ${esc(raceLifecycle.profile.family)} ${esc(raceLifecycle.profile.version)}</b><span>${raceLifecycle.purchaseStart&&raceLifecycle.purchaseEnd?`${esc(shoeChartDate(raceLifecycle.purchaseStart))} – ${esc(shoeChartDate(raceLifecycle.purchaseEnd))}`:`Purchase timing remains uncertain`}</span><strong>Projected race-day mileage ~${Math.round(raceLifecycle.projectedNew||0)} km · target ${Math.round(raceLifecycle.targetWindow?.minKm||0)}–${Math.round(raceLifecycle.targetWindow?.maxKm||0)} km</strong></article></div>`:``}</div>`}
